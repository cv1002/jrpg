    ...
    onKey(e) {
      ...
      if (S.storyPage < STORY.length) {
        ...
        if (!S.G.tutDone) {
          ...
          // v22.15 教程行补 [ / ] 音量口径（可发现性·承 v21.30 教程行 E 键口径同步 / v22.12 主音量调节
          // 「功能存在就必须能看到入口」主线）：v22.12 的三端（H 页行/README 上手表/HUD 指示）都齐了，
          // 唯独首次进图的教程行仍只列「M静音」——新玩家第一屏按键清单与全局快捷键表差一格；
          // 现补「 · [ / ] 音量」，与 VOL_STEP/KEY 无 [ ] 冲突一致，纯文字零逻辑零结算。
          boxMsg('💡 教程：WASD移动/Shift奔跑 · Enter/E对话 · Esc菜单 · P存档 · F喝药 · I状态 · J任务 · B图鉴 · C成就 · T旅行 · H帮助 · M静音 · [ / ] 音量', TUTOR_MSG_MS);
        } else {
