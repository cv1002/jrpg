...
// v23.41 文档整理·数值说明·同源口径：README「BGM / 音效」行补「事件专属音效」家族（承 v23.22 SFX.ach /
// v23.33 SFX.craft / v23.40 SFX.flee 同一「事件音效各归其位」主线的口径收口）——README 音效行此前只
...
// v23.50 音效反馈·语义修正：任务交付专属音效（承 v21.3 alert/boss「先闻其声」/ v23.22 SFX.ach /
// v23.33 SFX.craft / v23.40 SFX.flee / v23.43 SFX.crit / v23.46 SFX.transform / v23.47 SFX.charge /
// v23.48 SFX.darkheal / v23.49 SFX.chest 同一「事件音效各归其位」主线的收口，详见 js/audio.js
// SFX.quest 与 js/core.js talkNext 行内注释）——任务交付（core.talkNext act.kind==='reward'）此前金币
// 档与售蘑菇同播 SFX.coin()（880→1320 高音明亮双音）、物品档与战斗胜利同播 SFX.victory()（胜利号角
// 五连）：交任务是「🎁 委托达成」的交付事件，听感却与买东西/打赢仗无从分辨；现补 audio.js SFX.quest()
...
// v23.53 体验打磨·信息透明·纯显示：精英「重击」逐招预判补全（承 v21.47 威胁预警补「重击线」/ v23.44
...
// v23.56 体验打磨·信息透明·反馈不迟到：试炼三连战战报补「获得 N 经验」——grantXp 对试炼各关照常
...
// v23.58 体验打磨·信息透明·纯文字：帮助页「地图指南」潮灯镇行 r[2] 补第十一条支线经办指针
// （承 v22.80 粮田（护粮的委托）同一「地图指南行内指针补全」主线收口：十一条支线里潮灯镇
// 承接 灯长的委托/石壳里的记忆/护粮的委托/夜路的狼嚎/蛇影的药引 五条，而潮灯镇行只列了
// r[1]「灯长/守书记(支线)」与 r[2]「粮田（护粮的委托）」——v23.42 客栈老板娘（夜路的狼嚎）与
// v23.57 酿药师（蛇影的药引）两条镇内讨伐支线的经办人此前在战前知识中枢查无一行：玩家在
// H 页看潮灯镇一行，知道护粮要去粮田（v22.80），却不知道「蛇影的药引」该找谁接、从哪接
// （酿药师 v22.50 起就站在酿造锅旁、v23.57 升格为委托人）；现于 r[2] 末补「 · 酿药师（蛇影的
// 药引）」（委托人名读 NPCS.brewer.name、任务名读 QUESTS.side_snake.name 单一数据源派生，调
// 名字只改 data.js 一处、帮助页自动跟随零裸字面量），行数不变仍 8、r[2] 数不变仍 6、末行基线
// 仍 414 零页长/基线变化，12px estW 307.4→421.2 ≤470 面板预算；纯文字零逻辑零结算零存档
// 零数值变化（v23.42 夜路的狼嚎同理留待后续行宽档位，本版单点收口第十一条）。
// v23.59 音效反馈·听觉信息透明：记忆碎片拾取音效归位（承 v21.3 alert/boss「先闻其声」/
// v23.22-51 「事件音效各归其位」主线收口后的复查补全）——winBattle 强敌首胜掉落「🕯️ 拾起一段
// 记忆」此前全程静默（只报报文），而 v23.49 把宝箱从 SFX.item()（700→900 双音）移出后：
// SFX.item 成为全库无调用点的孤儿音效、唯一仍是「拾取」语义的现场（碎片拾取）反而无声——
// 真结局关键收集落袋与普通胜利同听感；现 battle.js 碎片块补 SFX.item()（音效定义逐字未动）、
// audio.js SFX.item 增 v23.59 注释；零结算零数值零存档，碎片块其余逐字未动。
// v23.60 体验打磨·信息透明·纯文字：帮助页「地图指南」潮灯镇行 r[1] 补第十条支线「夜路的狼嚎」
// 经办地点指针（承 v22.80 粮田（护粮的委托）/ v23.58 酿药师（蛇影的药引）同一「地图指南行内
// 指针补全」主线收口——十一条支线里潮灯镇承接五条，r[1]「灯长/守书记(支线)」点破灯长的委托与
// 石壳里的记忆、r[2]「粮田（护粮的委托）/酿药师（蛇影的药引）」点破两条，唯独 v23.42 客栈老板娘
// 升格的「夜路的狼嚎」（旅馆东侧门外、offer 原文「去潮灯镇旅馆找客栈老板娘」）在战前知识中枢
// 查无一行；现于 r[1]「旅馆」后并注「（狼嚎）」（任务名读 QUESTS.side_wolf.name 单一数据源派生、
// 名字改只改 data.js 一处帮助页自动跟随；r[1] 整行 14px estW 413.4 → ~461.8 ≤470 面板预算，
// 全名「夜路的狼嚎」整行 498.1 越界——v23.58 预留的行宽档位未到，按短指针收口，与 v23.42 注释
// 「留待后续行宽档位」同口径），行数不变仍 8、r[2] 逐字未动、末行基线仍 414 零页长变化；
// 纯文字零逻辑零结算零存档零数值变化。
// v23.61 体验打磨·信息透明·节头第一眼就报进度：状态页（view/menus.js drawStatus）「已学技能：」
// 节头补 N/8 计数（承 v23.16 灯下之声节头 N/37 / v23.19 记忆碎片节头 N/4 / v23.20 支线节头可交付 N /
// v23.21 主线节头 N/5 / v23.34 战斗技能菜单标题「已学 N/7」同一「集合类列表节头第一眼就报进度」
// 主线收口：J 日志四节与战斗技能菜单标题都报计数，唯独 I 状态页「已学技能：」节头仍是无数字裸标签
// ——v23.55 第八招「灯焰长明」后按 I 看「八招学齐了没」节头查无一眼之数（列表就在下方但得逐行数，
// 与 v23.21 主线五徽记同页仍补节头计数同一先例）；现与技能菜单同读 hero.skills.length ·
// Object.keys(SKILL_DATA).length 一份单一数据源（零裸字面量、加/删技能自动跟随），
// 节头直书「已学技能：N/8」；纯显示零结算零存档零数值变化。
// v23.62 体验打磨·信息透明·纯显示：记忆图鉴行补「讨伐支线进度」角标（quests.questKillProg——
// 承 v23.53 决策现场信息透明 / v23.20 支线节头「可交付 N」同一「同一信息在多处决策现场可见」
// 主线收口：图鉴是「该去打哪只」的刷怪中枢，此前与讨伐采集型支线零联动——J 日志每卡有进度、
// 图鉴行查无一行；现图鉴行按 QUESTS 单一数据源派生「 · 📜 支线 N/M」，零新字段零裸字面量、
// 加/删支线改阈值自动跟随，纯显示零结算零存档零数值变化）。
// v23.63 新内容·战斗维度第三枚里程碑：新成就「暴击如雨」（[1]普攻暴击累计 CRIT_GOAL 次，见
// ACH_LIST crit 条目注释）——承 v23.36 以守为攻 / v23.54 蓄势待发「战斗操作」维度的成对收口
// 后的第三枚：[5]防御的奖励动作（趁隙反击）与 [6]蓄力（×CHARGE_MULT 爆发）都有纪念，唯独 [1]
// 攻击的普攻暴击（CRIT_RATE/CRIT_MULT，仅普攻可触发、技能 crit=false）无任何纪念——玩家整局
// 靠「普攻暴击+蓄力」打出爆发，成就一览却查无一声回响；计数由 js/battle.js doAttack 暴击唯一
// 产生点写入 hero.crits（随 snapshotHero 全量快照自动持久化、防御式读取旧档零迁移），阈值
...
// v23.65 新内容·战斗维度第五枚里程碑：新成就「走为上计」（[4]逃跑成功累计 FLEE_GOAL 次，见
// ACH_LIST flee 条目注释）——承 v23.36 以守为攻 / v23.54 蓄势待发 / v23.63 暴击如雨 / v23.64
// 熟能生巧「战斗操作」维度的成对收口后的第五枚：[5]防御的奖励动作（趁隙反击）、[6]蓄力（×
// CHARGE_MULT 爆发）、[1]攻击的普攻暴击（CRIT_RATE/CRIT_MULT）与 [2]技能（SKILL_DATA 八招全表）
// 都有纪念，唯独 [4]逃跑——玩家打不过就走的务实选择（普通怪 FLEE_SUCCESS=60% 概率成功、Boss
// 气场压制不可逃、指令栏 ⛔「别按 4」）——此后仍无任何纪念；计数由 js/battle.js doFlee 逃跑成功
// 唯一产生点写入 hero.flees（随 snapshotHero 全量快照自动持久化、防御式读取旧档零迁移），阈值
...
// v23.70 体验打磨·可发现性·信息透明·纯入口/纯文字：帮助页补四收集页直达——承 v23.68/69 四收集页
// （I 状态 / J 日志 / B 图鉴 / C 成就）互切网格同一「同一功能所有入口口径一致」主线的收口：v23.69 后
// I/J/B/C 四页任意两两互切成网、pause 菜单四页齐，唯独 H 帮助页（战前知识中枢）仍是唯一没有出口的
// 菜单页——且本文件 H 页「操作说明」的 状态/任务日志/记忆图鉴/成就一览 四行（v22.88/v22.90/v23.68/
// v23.69）行内早已描述「按 I/J/B/C 打开对应页与该页互切口径」，玩家在 H 页按这些键却毫无反应（违背
// 「提示讲的键真的可用」契约，承 v22.99 同款）；现补 main.js help.onKey 四分支（i/I→status、j/J→
// journal、b/B→codex、c/C→ach）+ drawHelp 页脚「I/J/B/C 直达 状态·日志·图鉴·成就」+ 本行「操作说明」
// 行内口径 + README H 行 / index.html 常驻帮助条 H 处 五端同口径；纯入口零结算零存档零数值变化
// （onKey 仅新增 else-if 分支，既有 ←/→ 翻页与 h/Esc 关闭逐字未动，KEY 无 i/j/b/c 地图映射零冲突）。
// v23.69 体验打磨·可发现性·信息透明·纯显示：四收集页（I 状态 / J 日志 / B 图鉴 / C 成就）互切网格补口——承
// v23.68 状态页页底 C/B 直达 / v22.99 图鉴·成就页脚 I 直达 / v22.90 状态页↔日志页 I↔J 双向互切同一
// 「同一功能所有入口口径一致」主线的收口：v23.68 后单向/双向链路 I↔J / I↔B / I↔C / B→I / C→I 齐备，
// 唯独 J/B/C 三页彼此之间仍是单向空白——日志里想查图鉴魔物强度/成就进度、图鉴里想查日志支线/成就、
// 成就里想查日志/图鉴，均只能 Esc 回世界再按（「同一功能所有入口口径一致」在收集页网格的最后一角）；
// 现补六条缺失链路（J→B / J→C / B→J / B→C / C→J / C→B）+ 五端口径（menus.js 三页页脚文字 / main.js
// journal·codex·ach.onKey 新分支 / data.js H 页「操作说明」三行 / README 快速上手表 J·B·C 行 /
// index.html 常驻帮助条）同口径同源；纯显示零结算零存档零数值变化（onKey 仅新增 else-if 分支，既有
// I/J/B/C/Esc/方向键分派逐字未动，KEY 无 b/c/j 地图映射零冲突）。
// v23.68 体验打磨·可发现性·信息透明·纯显示：状态页页底补「C 成就 · B 图鉴」直达 + main.js status.onKey
// 补 c/C→ach、b/B→codex 分支（详见 view/menus.js drawStatus 行内注释）——承 v22.99 图鉴/成就页脚
// 「I 状态页」/ v22.90 状态页↔日志页 I↔J 双向互切同一「同一功能所有入口口径一致」主线的收口：
// v21.71 起状态页页底常驻「J 任务日志」、v22.99 起 B/C 页脚「I 状态页」+ I→status 分支，单向链路
// I→J / J→I / B→I / C→I 齐备，唯独状态页本身「I→C / I→B」查无一行；纯显示零结算零存档零数值变化。
// v23.71 体验打磨·可发现性·信息透明·纯入口：J/B/C 三收集页补「H 帮助」直达 + main.js 四页 onKey 补
// h/H→help 分支（详见 view/menus.js 各页脚行内注释）——承 v23.70 帮助页 → 收集页四分支同一「同一功能
// 所有入口口径一致」主线收口：v23.68/69/70 后 I/J/B/C 四收集页两两互切成网、H 页可直达四页，唯独
// 反向「收集页 → H」查无一行（在日志/图鉴/成就里想查操作说明只能 Esc 回世界再按 H）；现补 J/B/C 三页
...
// v23.77 体验打磨·信息透明·纯显示：Esc 暂停菜单头部补「当前所在地」（view/menus.js drawPause）——
// 承 v19.89 状态页「📍 地图名」/ v23.27 快速旅行「当前所在地标注」同一「我在哪」单一数据源口径：
// 暂停菜单是玩家按 Esc 决定「存档/继续/快速旅行/回标题」的第一现场，头部此前只报「名字 · 槽号」——
// 现与 drawStatus 同读 (MAPS[curMap()]||{}).name（加/删地图自动跟随零裸字面量），头部补「 · 📍地图名」，
// 纯显示零结算零存档零数值变化，头部既有「名字 · 槽 N」子串逐字保留（smoke_v2230 槽号行零回归）。
// v23.78 体验打磨·信息透明·纯显示：战斗画面顶部右缘补「📍 所在地」（view/drawBattle.js drawBattle）——
// 承 v23.77 暂停菜单头部「📍 地图名」/ v19.89 状态页「📍 地图名」/ v23.27 快速旅行「当前所在地标注」
...
// 区分背景）；现与 drawStatus/drawPause 同读 (MAPS[curMap()]||{}).name||curMap() 一份源（防御式、
// 加/删/改名地图自动跟随零裸字面量；drawBattle 补 MAPS import 零新增模块依赖），12px 灰字 620
// 右对齐（角标列同 x 位、y=26 空位），纯显示零结算零存档零数值变化。
// v23.79 体验打磨·信息透明·纯显示：阵亡画面战绩行补「📍 阵亡地点」（view/menus.js drawDead）——
// 承 v23.77 暂停菜单头部「📍 地图名」/ v19.89 状态页「📍 地图名」/ v23.27 快速旅行「当前所在地标注」/
// v23.78 战斗画面「📍 所在地」同一「我在哪」单一数据源口径收口：五屏决策现场（状态/快旅/暂停/战斗/阵亡）
// 逐屏核对，唯有阵亡复盘屏查无一行——玩家倒在强敌面前（祭坛/试炼碑/传送门直接切战，死时战场可能在
// 雾语林还是星井矿脉），「B 重整旗鼓 / R 重开 / T 回标题」三选一现场却看不到地点；现与 drawStatus/
...
// v23.83 体验打磨·可发现性·口径一致·纯显示：状态页页底补「 · H 帮助」互切（承 v23.71 四收集页
// 互切网格「页底四直达」主线收口——v23.71 给 图鉴/日志/成就 三页脚补「· H 帮助」并给 main.js
// status.onKey 加 h/H→help 分支，唯独状态页页底因 11px 行宽预算（v23.68 实测右缘 ≈550.3 +
// 「 · H 帮助」≈43 → ≈593 > 570 面板右缘）刻意未并注，H 键可达性只能靠世界教程/index.html 常驻
// 帮助条/pause 菜单三处承载——「功能存在就必须能看到入口」主线在状态页这一格留了窟窿；现按其余
// 三页同式收口：页底改「J 日志 · C 成就 · B 图鉴 · H 帮助」（「J 任务日志」缩短为「J 日志」与
// 图鉴/日志/成就 三页脚同式同词，隔符「   ·  」收紧为「 · 」），H 页「状态」行/index.html 常驻
// 帮助条 I 块/README 上手表 I·J 行同批同口径；与 status.onKey h/H 分支同源（提示讲的键真的可用）；
// 11px 实测（最长支线档 → 右缘 ≈563.9 ≤ 570 面板右缘、预算 460 留 ≈6.1px）；纯显示零结算零存档
// 零数值变化。）
// v23.84 体验打磨·信息透明·纯文字：帮助页「地图指南」星井矿脉行 r[2] 补「 · 拾骨人（未归的
// 矿灯）」经办指针（承 v22.80/v23.58/v23.60「地图指南行内指针补全」主线续篇——十一条支线逐条
// 对经办所在地行：潮灯镇行五条齐备、无字回廊行守名者(支线)在列，唯独星井矿脉行承接的 未归的
// 矿灯/星砂之约 两条在战前知识中枢查无一行；本版单点收口未归的矿灯，见 HELP_PAGES 星井矿脉行
// v23.84 行内注释；委托人名读 NPCS.digger.name、任务名读 QUESTS.side_bone.name 单一数据源派生，
// 12px estW 256.4→370.1 ≤470 面板预算；纯文字零逻辑零结算零存档零数值变化。）
// v23.85 体验打磨·信息透明·纯文字：帮助页「地图指南」星井矿脉行 r[2] 补「 · 星砂之约」经办
// 指针（承 v23.84 拾骨人/v23.58 酿药师/v23.60 旅馆（狼嚎）「地图指南行内指针补全」主线收口——
// v23.84 单点收口未归的矿灯后，星井矿脉行承接的 星砂之约（星砂车夫·矿车区）成为十一条支线里
// 在战前知识中枢唯一查无一行的一支，见 HELP_PAGES 星井矿脉行 v23.85 行内注释；任务名读
// QUESTS.side_cart.name 单一数据源派生，按 v23.60 短指针先例收口（全名「星砂车夫（星砂之约）」
// 12px estW 483.9 越 470 面板预算、短指针 421.6 ≤470），纯文字零逻辑零结算零存档零数值变化。）
// v23.86 体验打磨·信息透明·纯文字：帮助页「地图指南」潮灯镇行 r[2] 补「 · 巡灯人」经办
// 指针（承 v23.84/85 星井矿脉行经办指针收口后「十一条支线逐条对经办地行」审计的最后一处已知
// 缺口——v23.84 注释自述 巡灯人（旧灯卫的名字）「回镇找巡灯人」属潮灯镇行口径（行宽已满另议），
// 十一条支线里唯独 旧灯卫的名字 一支在战前知识中枢查无一行：玩家看 H 页潮灯镇行知道护粮去粮田、
// 蛇影找酿药师、狼嚎在旅馆、石壳找守书记，却不知道「集齐记忆碎片后回镇找巡灯人」的 旧灯卫的名字
// 也在这张图；现按 v23.60/v23.85 短指针先例单点收口：NPC 名读 NPCS.adventurer.name 单一数据源
// 派生（调名只改 data.js 一处、帮助页自动跟随零裸字面量），全名「巡灯人（旧灯卫的名字）」并加
// 12px estW 573.6 越 470 面板预算、仅注 NPC 名——巡灯人即城门动线经办人、任务名由 J 日志
...
// v23.88 体验打磨·可发现性·纯文字：帮助页「操作说明」快速旅行行补面板功能口径（承 v22.90 状态/任务日志
// 两行 I↔J 双向直达 / v23.68-71 四收集页互切同一「操作总清单行内如实标注面板能力」主线）：快速旅行面板
// （drawTravel）实际提供 已到访图瞬移 + 目的地推荐等级（v21.92 红色预警）+ 无泉水/旅店补给提示（v22.32）
// + 已探索 N/4 计数（v23.27）——这些决策信息在 试炼进阶页「快速旅行」行/README T 行/面板本体三端都在，
// 唯独操作说明「快速旅行」行只写裸键「T」（战前知识中枢操作总清单唯一一个无能力口径的行，其余 13 行均带
// 能力说明）；现补「（已到访图瞬移 · 推荐等级/补给标注）」口径（行数不变仍 14、14px estW ≈217.8 ≤470
// 面板预算，与 v21.92/v22.32/v23.27 面板功能同口径），纯文字零逻辑零结算零存档零数值变化（见 HELP_PAGES
// 操作说明快速旅行行 v23.88 行内注释）。
// v23.89 体验打磨·可发现性·纯文字：index.html 常驻帮助条（#help）T 块补「快速旅行」面板功能口径（承
// v23.88 帮助页「操作说明」快速旅行行同款主线收口：v23.88 让战前知识中枢操作总清单摆脱裸键「T」，其
// 零回归面明言「未动 README T 行/index.html 常驻帮助条 T 块」——README T 行（v19.53 起带
// 「已到访地图瞬移」口径、v22.32 补给提醒/v23.75 行者无疆均入行）早已完整，唯独 index.html 画布下方
// 常驻帮助条（v22.19 确立与 README 快速上手表/H 帮助页/首次进图教程行四端口径一致）的 T 块仍是裸键
// 「T 旅行」——玩家在不进 H 页的日常画面里按 T 前不知道这面板能看什么（同条 I/J/B/C/H 五块均带括号
// 能力/直达口径）；现按 H 页同句式补「（已到访图瞬移 · 推荐等级/补给标注）」（与 v23.88 H 页行
// 逐字同口径、纯文字零逻辑零结算零存档零数值变化；index.html 本就是页面壳，见 #help T 块）。
// v23.90 体验打磨·可发现性·纯文字：index.html 常驻帮助条（#help）「喝药」F 块补面板能力口径（承 v23.89
// #help T 块 / v23.88 H 页操作说明快速旅行行同款主线收口：v23.89 让 #help 的 I/J/B/C/H/T 六块全部带括号
// 能力/直达口径后，常驻帮助条键块中唯 F 喝药仍是无括号裸键——F 是全游唯一喝药键（大地图喝药/战斗
// [3] 同入口），其「优先灵药 + 普通 50%HP+8 / 灵药 80%HP+20并回40%MP」语义早在 v22.58/v23.26 H 页
// 「喝药（普通/灵药）」行与 README F 行/战斗指令栏 [3] 预览（POTION_HP_PCT/POTION_HP_FLAT/
...
// v23.92 数值平衡·装备曲线补全（新武器「星铁剑」，详见 WEAPONS 行内注释）：可购剑曲线
// 秘银剑(9/220)→勇者之剑(15/600) 的 +6攻/380金 最大跳档拆为 星铁剑(12/400) 两段步进；
// 商店/状态页/购买反馈全自动跟随，五把既有剑/防具/成就/结算零变化。
// v23.93 文档整理·口径收口（README 快速上手表「阵亡」行补 P 存档口径，详见 README 行内注释）：
// 快速上手表是玩家唯一常备的按键速查——v21.98 胜利画面 P / v22.28 标题页 P / v22.30 暂停菜单 P /
// v22.93 阵亡画面 P 四端落位后，速查表「阵亡」行仍只列「B/R/T」（P 只在系统 bullet v22.93 提及，
// 且 阵亡画面未存档橙行 pauseSaveHint 的「按 P 写入槽 N」与 main.js dead.onKey P→saveGame 同批
// 落位——速查表缺的正是这一键）；现补 `阵亡 \`B/R/T/P\`` 行头与 P 存档口径（与 dead.onKey 逐字同源、
// 与 pauseSaveHint 未存档提示同读一份源），纯文档零逻辑零结算零存档零数值变化。
// v23.94 文档整理·口径收口（帮助页「试炼进阶」重整旗鼓行补 P 存档口径，详见 HELP_PAGES 行内注释）：
// v22.93 让阵亡复盘屏拥有「P 先落盘再决定 B/R/T」的第四键（main.js dead.onKey P→saveGame + 未存档
// 橙行 pauseSaveHint），v23.93 刚把它补进 README 速查表「阵亡」行——而战前知识中枢 H 页「试炼进阶」
// 页的「重整旗鼓」行（v21.14 确立 H 页四页为战前知识中枢）仍只写「B 原地再战；R 重开 · T 回标题」，
// 玩家战前查 H 页准备「要不要死磕强敌」时不知道倒下后还能先 P 存档再决定；现按 v23.88 快速旅行行
// 同句式补「 · P 存档（先落盘再决定）」（与 dead.onKey 逐字同源、与 pauseSaveHint 未存档
// 提示同读一份源；14px 全行 estW 314→466 ≤470 面板预算、行数不变仍 10、r[2] 数不变仍 2、末行基线不变），
// 纯文档零逻辑零结算零存档零数值变化。
// v23.96 数值平衡·装备曲线补全（新防具「精钢甲」，详见下方 ARMORS 行内注释）：可购甲曲线最大跳档
// 锁子甲8/180 → 龙鳞甲13/480（+5防/300金）拆为 精钢甲(10/300) + 龙鳞甲(3/180) 两段——只加一件可购甲、
// shop 清单/▲▼对比/状态页/购买反馈/视窗滚动全读 ARMORS 自动收录自动跟随（v23.95 视窗恰兜住 11 行清单），
// 四件既有甲/全部剑/成就/结算/存档逐字未动，零结算零存档零数值变化。
// v23.95 体验打磨·可发现性·纯显示（商店商品清单视窗滚动，详见 view/menus.js drawShop 行内注释）：
// v23.92 星铁剑入店后商品清单最长 10 行（新手持木剑布衣+持菇： 药水+卖菇+4 剑+3 甲+离开），
// 而 drawShop 面板只有 8 行预算——第 9 行（i=8）已画出 360 高的面板底（410）、第 10 行（i=9）
// 与页脚提示（470）相撞，「离开商店」长期被挤出面板；现仿成就页 achScroll 视窗：固定可视 8 行
// （末行底 394 ≤410 面板预算）、窗口随 shopSel 滑动（无新状态、选中行永不被裁掉），超出时按
// 成就页同款口径补「还有 N 项未在本页显示」提示；纯显示零结算零存档零数值变化。
// v23.97 文档整理·数值说明·同源口径（README「数值速查」补「基础属性」行，详见 README 行内注释）：
// 数值速查表 21 行已覆盖 成长/升级/技能/装备/经济/战斗/掉落/生态/试炼/成就/魔物/技能数值/
// 强敌变身/难度/支线/昼夜——唯独 Lv1 基础属性（baseStats(1) HP 45 · MP 16 · 攻 9 · 防 5）散见
// baseStats 源码与状态页面板，README 维护者速览查无一行；现于「每级成长」行之后补录「基础属性」行
...
// （踩上即开战、无可反悔，级别差却查无一眼之数）；现 ready 态且 S.G.level < RUSH_REC_LV 时于主标签
// 上方追加红色预警行「⚠️ 建议 Lv.N · 你当前 Lv.M · 差 K 级」（全部由 RUSH_REC_LV 与 S.G.level 单一
// 数据源派生，达标/超额零噪音——与快速旅行预警同「未达标才报」口径），纯显示零结算零存档零数值变化，
// RUSH_REC_LV/试炼结算/主标签/通关奖行逐字未动。
...
// 武器成长曲线（单一数据源）：shop.buyWeapon 购买清单 / ▲▼对比 / 状态页面板（baseStats+WEAPONS[hero.weapon].atk
// 唯一公式）全读此表，增删武器零逻辑改动。
// v23.92 数值平衡·装备曲线补全（新武器「星铁剑」）：此前可购剑曲线 木剑2/0 → 铁剑5/80 → 秘银剑9/220 →
// 勇者之剑15/600——秘银剑→勇者之剑一跳 +6 攻/380 金，是毕业装曲线最大跳档（铁剑→秘银剑 4/140、
// 木剑→铁剑 3/80 皆是 3-4 攻小步，唯此一档 6 攻陡升）；Lv8-10 段（矿脉/回廊前，自然收入到 Lv10 ≈900 金）
// 的玩家在 220 金之后只能 600 金一步到位；现补 星铁剑 12/400（星井矿脉沉底的星铁所铸、与星砂同脉），
// 把跳档拆成 3/180 + 3/200 两段，与既有 3/80·4/140 同节奏——只加一把可购剑、shop 自动收录、
// ▲▼对比/状态页/购买反馈自动跟随，五把既有剑逐字未动，零结算零存档零数值变化。
const WEAPONS={
  '木剑':{atk:2,price:0},
  '铁剑':{atk:5,price:80},
  '秘银剑':{atk:9,price:220},
  '星铁剑':{atk:12,price:400},
  '勇者之剑':{atk:15,price:600},
  '圣光之剑':{atk:24,price:0,legend:true},
};

// v23.96 数值平衡·装备曲线补全（防具侧，承 v23.92 星铁剑同款主线）：可购甲曲线 布衣1/0 → 皮甲4/60 →
// 锁子甲8/180 → 龙鳞甲13/480 唯 锁子甲→龙鳞甲 一跳 +5 防/300 金（皮甲→锁子甲 4/120 同剑侧 3-4 小步节奏），
// 是毕业装曲线最大跳档——Lv6-9 段（雾语林后段/矿脉前）的玩家在 180 金之后只能 480 金一步到位；
// 现补 精钢甲 10/300（星井矿脉的沉钢所铸、与星砂车同脉），把跳档拆成 精钢甲(2/120) + 龙鳞甲(3/180)
// 两段，与既有 3/60·4/120 同节奏——只加一件可购甲、shop 自动收录、▲▼对比/状态页/购买反馈自动跟随，
// 四件既有甲逐字未动，零结算零存档零数值变化。
const ARMORS={
  '布衣':{def:1,price:0},
  '皮甲':{def:4,price:60},
  '锁子甲':{def:8,price:180},
  '精钢甲':{def:10,price:300},
...
// v23.63 成就「暴击如雨」阈值（战斗维度第三枚里程碑·单一数据源）：[1]普攻暴击累计 N 次解锁 ——
// 与 ACH_LIST.crit 的 ok/prog/d 同读一份源，调门槛只改本行一处三端自动跟随；数值取中低档
...
// 同「阈值数据化」家族）。
const CAST_GOAL = 30;
// v23.65 成就「走为上计」阈值（战斗维度第五枚里程碑·单一数据源）：[4]逃跑成功累计 N 次解锁 ——
// 与 ACH_LIST.flee 的 ok/prog/d 同读一份源，调门槛只改本行一处三端自动跟随；数值取中低档 10
// （逃跑每次消耗一回合、不像普攻那样每回合可挥——玩家在危险区/低血线战略性撤退，一局约十次
...
const ACH_LIST=[
  ...
  // 暴击如雨（v23.63 新成就·战斗维度第三枚里程碑·承 v23.36 以守为攻 / v23.54 蓄势待发先例）：
  // v23.54 收口「战斗操作」维度时补的是 [6]蓄力，与它并列成对的是 [1]攻击的普攻暴击——12% 概率
  // （CRIT_RATE）触发、×CRIT_MULT 1.8 结算、暴击瞬间有专属上扬音（v23.43）、战报如实报
  // 「（暴击×1.8！）」——玩家整局靠「暴击+蓄力」打出爆发，成就一览却查无一声回响（[5]防御的
  // 反击与 [6]蓄力都有纪念，唯独最常用的 [1]攻击没有）；判定/进度/描述同读 CRIT_GOAL 单一数据源
  // （与 DEFLECT_GOAL/CHARGE_GOAL 同一「阈值数据化」家族——调门槛只改 data.js 一处自动跟随，
  // 绝无第二套口径）；计数读 js/battle.js doAttack 暴击唯一产生点新写入的 hero.crits
  // （随 snapshotHero 全量快照自动持久化），(g.crits||0) 防御式读取——旧档无此字段=0 不误解锁、
  // 零迁移（承 v19.41 seen / v23.36 deflects / v23.54 charges 同款）；无 r 字段纯里程碑（与
  ...
  // 走为上计（v23.65 新成就·战斗维度第五枚里程碑·承 v23.36 以守为攻 / v23.54 蓄势待发 /
  // v23.63 暴击如雨 / v23.64 熟能生巧先例）：v23.64 收口「战斗操作」维度时补的是 [2]技能，与它
  // 并列成对的是 [4]逃跑——普通怪 FLEE_SUCCESS=60% 概率成功、Boss 战被气场压制不可逃（指令栏
  // ⛔「别按 4」）——玩家打不过就走的务实选择，此后仍无任何纪念：防御的反击、蓄力、普攻暴击、
  // 技能都有纪念，唯独最「保命」的一键查无回响；判定/进度/描述同读 FLEE_GOAL 单一数据源（与
  // DEFLECT_GOAL/CHARGE_GOAL/CRIT_GOAL/CAST_GOAL 同一「阈值数据化」家族——调门槛只改 data.js
  // 一处自动跟随，绝无第二套口径）；计数读 js/battle.js doFlee 逃跑成功唯一产生点新写入的
  // hero.flees（随 snapshotHero 全量快照自动持久化），(g.flees||0) 防御式读取——旧档无此字段=0
  // 不误解锁、零迁移（承 v19.41 seen / v23.36 deflects / v23.54 charges / v23.63 crits /
...
const HELP_PAGES=[
  [
    ...
    // v23.68 状态行口径随状态页页底三直达（承 v22.90 同句更新）：v21.71 起页底常驻「J 任务日志」、
    // v23.68 起并注「C 成就 · B 图鉴」（与 menus.js drawStatus 页底 / main.js status.onKey C/B 分支
    // 同口径同源，见 GAME_VERSION 上方 v23.68 注释）；14px estW ≈313 ≤470 面板预算，行数不变仍 14。
    // v23.83 状态行口径随状态页页底四直达收口（承 v23.71 同句更新，见 GAME_VERSION 上方 v23.83 注释）：
    // 「J 任务日志」随页底同批缩短为「J 日志」、并注「 · H 帮助」——与 menus.js drawStatus 页底 /
    // main.js status.onKey h/H 分支同口径同源；14px 实测全行 ≈268.9 ≤470 面板预算，行数不变仍 14。
    ['状态','I（页底 J 日志 · C 成就 · B 图鉴 · H 帮助）'],
    ...
    // v23.69 三行补「四收集页互切」口径（体验打磨·可发现性·纯文字，承 v23.68 状态行三直达 / v22.90
    // I↔J 双向互切同一「同一功能所有入口口径一致」主线，详见 GAME_VERSION 上方 v23.69 注释）：J/B/C
    // 三页页脚互切提示与 main.js 三页 onKey 新分支 v23.69 起同批落位，H 页操作说明作为战前知识中枢
    // 三行同步「I 状态页 · J/B/C 日志/图鉴/成就」相互直达口径（14px estW ≈240/≈252/≈252 ≤470 面板
    // 预算，行数不变仍 14、r[1] 主体与其余 11 行逐字零回归），零裸字面量纯文字零逻辑零结算零存档。
    // v23.71 三行补「H 帮助」互切口径（体验打磨·可发现性·纯文字，承 v23.70 H 页四直达与 v23.68/69
    // 同一「同一功能所有入口口径一致」主线收口，详见 GAME_VERSION 上方 v23.71 注释）：J/B/C 三页页脚
    // 「 · H 帮助」与 main.js 三页/状态页 h/H→help 分支 v23.71 起同批落位，H 页操作说明三行同步
    // 「· H 帮助」口径（14px 实测全行 ≈332.3 ≤470 面板预算、行数不变仍 14、r[1] 主体与其余 11 行逐字
    // 零回归），零裸字面量纯文字零逻辑零结算零存档。
    ['任务日志','J（↑↓ 滚动 · I 状态页 · B 图鉴 · C 成就 · H 帮助）'],
    ['记忆图鉴','B（↑↓ 滚动 · I 状态页 · J 日志 · C 成就 · H 帮助）'],
    ['成就一览','C（↑↓ 滚动 · I 状态页 · J 日志 · B 图鉴 · H 帮助）'],
    // v23.88 快速旅行行补面板功能口径（体验打磨·可发现性·纯文字，承 v22.90 状态/任务日志两行 I↔J 双向
    // 直达 / v23.68-71 四收集页互切同一「操作总清单行内如实标注面板能力」主线）：快速旅行面板（drawTravel）
    // 实际提供 已到访图瞬移 + 每目的地 推荐等级（不达标红色预警「⚠️ 推荐 Lv.N · 你当前 Lv.M · 先补给再战」，
    // v21.92）+ 无泉水/旅店目的地的「出发前请补给」橙行（v22.32）+ 「已探索 N/4」计数（v23.27）——这些
    // 决策信息在 试炼进阶页「快速旅行」行/README T 行/面板本体三端都在，唯独操作说明「快速旅行」行（战前
    // 知识中枢的操作总清单）只写一个裸键「T」——玩家按 T 前不知道这面板能看什么（对比同页其余 13 行
    // 均带能力口径：喝药带恢复量/状态带四直达/战斗带全部指令）；现补「（已到访图瞬移 · 推荐等级/补给标注）」
    // 口径（14px estW ≈217.8 ≤470 面板预算、行数不变仍 14、r[1] 主体与其余 13 行及三页逐字零回归），
    // 与 v21.92/v22.32/v23.27 面板功能同口径；纯文字零逻辑零结算零存档零数值变化。
    ['快速旅行','T（已到访图瞬移 · 推荐等级/补给标注）'],
    // v23.70 「操作说明」行补页内四收集页直达口径（体验打磨·可发现性·纯文字，承 v22.90/v23.68/v23.69
    // 各行互切口径与 main.js help.onKey 四分支 / drawHelp 页脚「I/J/B/C 直达」/README H 行 / index.html
    // 常驻帮助条同口径——H 页按 I/J/B/C 真的能直达对应页（提示讲的键真的可用）；14px 实测
    // （@napi-rs/canvas 14px sans-serif：本行全行 ≈300.8 ≤470 面板预算），行数不变仍 14。
    ['操作说明','H（页内 I/J/B/C 直达 状态·日志·图鉴·成就）'],
  ...
  [
    ...
    // 零存档零数值变化。
    // v23.58 潮灯镇行 r[2] 补「酿药师（蛇影的药引）」指针（体验打磨·信息透明·纯文字，承 v22.80
    // 粮田（护粮的委托）同一「地图指南行内指针补全」主线收口：十一条支线里潮灯镇承接五条（灯长的
    // 委托/石壳里的记忆/护粮的委托/夜路的狼嚎/蛇影的药引），而潮灯镇行只列 r[1]「灯长/守书记(支线)」
    // 与 r[2]「粮田（护粮的委托）」——v23.57 酿药师升格为「蛇影的药引」委托人（v23.42 客栈老板娘
    // 「夜路的狼嚎」同理），经办人却在此前战前知识中枢查无一行：玩家看 H 页潮灯镇一行知道护粮去
    // 粮田，却不知道「蛇影的药引」该找谁接；现于 r[2] 末补「 · 酿药师（蛇影的药引）」（委托人名读
    // NPCS.brewer.name、任务名读 QUESTS.side_snake.name 单一数据源派生，调名字只改 data.js 一处、
    // 帮助页自动跟随零裸字面量），行数不变仍 8、r[2] 数不变仍 6、末行基线仍 414 零页长/基线变化，
    // 12px estW 307.4→421.2 ≤470 面板预算（v23.42 夜路的狼嚎同理留待后续行宽档位，本版单点收口
    // 第十一条）；纯文字零逻辑零结算零存档零数值变化。
    // v23.60 潮灯镇行 r[1]「旅馆」并注「（狼嚎）」指针（体验打磨·信息透明·纯文字，承 v22.80
    // 粮田（护粮的委托）/ v23.58 酿药师（蛇影的药引）同一「地图指南行内指针补全」主线收口：
    // 十一条支线里潮灯镇承接五条（灯长的委托/石壳里的记忆/护粮的委托/夜路的狼嚎/蛇影的药引），
    // r[1]「灯长/守书记(支线)」与 r[2]「粮田/酿药师」已点破四条，唯独 v23.42 客栈老板娘升格的
    // 「夜路的狼嚎」（offer 原文「去潮灯镇旅馆找客栈老板娘」）在战前知识中枢查无一行——玩家看
    // H 页潮灯镇一行知道护粮去粮田、蛇影找酿药师，却不知道狼嚎该去哪接；现于 r[1]「旅馆」后
    // 并注「（狼嚎）」（任务名读 QUESTS.side_wolf.name 单一数据源派生，调名只改 data.js 一处、
    // 帮助页自动跟随零裸字面量——r[1] 整行 14px estW 413.4 → ≈461.8 ≤470 面板预算，全名
    // 「夜路的狼嚎」整行 498.1 越界，v23.58 预留行宽档位未到、按短指针收口），行数不变仍 8、
    // r[2] 逐字未动、末行基线仍 414 零页长/基线变化；纯文字零逻辑零结算零存档零数值变化。
    // v23.86 潮灯镇行 r[2] 补「 · 巡灯人」经办指针（体验打磨·信息透明·纯文字·单一数据源，承
    // v23.84/85 星井矿脉行经办指针收口同一「地图指南行内指针补全」主线：十一条支线逐条对经办地行
    // 审计的最后一处已知缺口——旧灯卫的名字（巡灯人·城门动线）在战前知识中枢查无一行（v23.84
    // 注释「行宽已满另议」档案在案、v23.85 后十一条支线仅此一支未落格）；现于 r[2] 末补「 · 巡灯人」
    // （NPC 名读 NPCS.adventurer.name 单一数据源派生——调名只改 data.js 一处、帮助页自动跟随零裸
    // 字面量，与 v22.79/v23.58 同款派生；全名「巡灯人（旧灯卫的名字）」并加 12px estW 573.6 越 470
    // 面板预算、按 v23.60 旅馆（狼嚎）/v23.85 星砂之约「行宽档位未到按短指针收口」先例仅注 NPC 名——
    // 巡灯人即城门动线经办人、任务名由 J 日志「旧灯卫的名字」where 列（收集记忆碎片 → 回镇找巡灯人）
    // 承载），行数不变仍 8、r[2] 数不变仍 7、末行基线仍 414 零页长/基线变化，12px estW（= @napi-rs/
    // canvas 12px 实测）421.2→462.3 ≤470 面板预算；纯文字零逻辑零结算零存档零数值变化（r[1] 与其余
    // 三图行/三页逐字未动）。）
    ['潮灯镇 Lv.' + MAPS.village.recLv,'商店·旅馆（' + QUESTS.side_wolf.name.slice(-2) + '）·酿造锅·灯长/守书记(支线)·喷泉回血 · 东门→雾语林','水塘灯影 · ' + NPCS.granny.name + '/' + NPCS.lampboat.name + ' · 广场大灯·村井 · 粮田（' + QUESTS.side_grain.name + '） · ' + NPCS.brewer.name + '（' + QUESTS.side_snake.name + '） · ' + NPCS.adventurer.name],
    ...
    // v23.84 星井矿脉行 r[2] 补「 · 拾骨人（未归的矿灯）」经办指针（体验打磨·信息透明·纯文字，
    // 承 v22.80 粮田（护粮的委托）/ v23.58 酿药师（蛇影的药引）/ v23.60 旅馆（狼嚎）同一
    // 「地图指南行内指针补全」主线逐图行核对续篇：十一条支线逐条对经办所在地行——潮灯镇行承接
    // 灯长的委托/石壳里的记忆/护粮的委托/夜路的狼嚎/蛇影的药引 五条指针齐备、无字回廊行
    // 「守名者(支线)」在列、巡灯人（旧灯卫的名字）回镇找巡灯人属潮灯镇行口径（行宽已满另议），
    // 唯独星井矿脉行承接的 未归的矿灯（拾骨人·南走廊北侧凹龛）与 星砂之约（星砂车夫·矿车区）
    // 两条在战前知识中枢查无一行——玩家看矿脉行知道 试炼碑/终焉水晶/无泉水，却不知道「讨伐
    // 骷髅兵的支线该找谁接」；现于 r[2] 末补「 · 拾骨人（未归的矿灯）」（委托人名读
    // NPCS.digger.name、任务名读 QUESTS.side_bone.name 单一数据源派生——调名字只改 data.js
    // 一处、帮助页自动跟随零裸字面量，与 v22.80/v23.58 同款派生），行数不变仍 8、r[2] 数不变
    // 仍 6、末行基线仍 414 零页长/基线变化，12px estW 256.4→370.1 ≤470 面板预算（星砂车夫
    // （星砂之约）并加共 483.9 越界——按 v23.58 单点收口先例本版仅补拾骨人；实测口径与
    // smoke_v2111 estW 同款）；纯文字零逻辑零结算零存档零数值变化（其余三图行与三页逐字未动）。
    // v23.85 星井矿脉行 r[2] 补「 · 星砂之约」经办指针（体验打磨·信息透明·纯文字，承 v23.84
    // 拾骨人（未归的矿灯）同一「地图指南行内指针补全」主线收口：v23.84 单点收口后 星砂之约
    // （星砂车夫·矿车区）成为十一条支线里唯一在战前知识中枢查无一行的一支——玩家看矿脉行知道
    // 试炼碑/终焉水晶/无泉水/拾骨人（未归的矿灯），却不知道「洞窟领主后回矿车区找星砂车夫交
    // 星砂消息」的 星砂之约 也在这张图；现于 r[2] 末补「 · 星砂之约」（任务名读 QUESTS.side_cart.name
    // 单一数据源派生——调任务名只改 data.js 一处、帮助页自动跟随零裸字面量，与 v23.84 同款派生；
    // 全名「星砂车夫（星砂之约）」并加 12px estW 483.9 越 470 面板预算、按 v23.60 旅馆（狼嚎）
    // 「行宽档位未到按短指针收口」先例仅注任务名——星砂车夫即矿车区经办人、任务名已含经办线索，
    // 委托人口径由 J 日志地点列承载），行数不变仍 8、r[2] 数不变仍 7、末行基线仍 414 零页长/
    // 基线变化，12px estW 370.1→421.6 ≤470 面板预算（同 smoke_v2111 estW 口径）；纯文字零逻辑
    // 零结算零存档零数值变化（其余三图行与三页逐字未动）。）
    ['星井矿脉 Lv.' + MAPS.cave.recLv,'更强魔物·迷宫·试炼碑（可问' + NPCS.sentinel.name + '）·中央终焉水晶·双徽记化为门','无泉水/旅店 · 出发前请补给 · 星井（低鸣星蓝/静默灰） · ' + NPCS.digger.name + '（' + QUESTS.side_bone.name + '） · ' + QUESTS.side_cart.name],
  ...
  [
    ...
    // v23.94 重整旗鼓行补 P 存档口径（体验打磨·可发现性·纯文字，承 v23.93 README 速查表「阵亡」行 P
    // 存档口径 / v23.88 快速旅行行同句式同一「功能存在就必须能看到入口」主线，详见 GAME_VERSION 上方
    // v23.94 注释）：v22.93 让阵亡复盘屏补 P 键（main.js dead.onKey P→saveGame、同屏未存档橙行
    // pauseSaveHint「⚠️ 未存档 · 按 P 写入槽 N」）后，战前知识中枢（H 页四页）介绍死屏按键的本行仍只
    // 写「B 原地再战；R 重开 · T 回标题」——与 README 速查表 v23.93 修复前同一缺口形态（P 是死屏唯一
    // 能「先落盘再决定 B/R/T」的键）；现按 v23.88 同句式补「 · P 存档（先落盘再决定）」（与
    // dead.onKey 逐字同源、与 pauseSaveHint 未存档提示同读一份源；14px 全行 estW 314→466 ≤470 面板预算，
    // 行数不变仍 10、r[2] 数不变仍 2、末行基线仍 402 不触页脚 452），纯文字零逻辑零结算零存档零数值变化。
    ['重整旗鼓','被强敌击败后按 B 原地再战；R 重开 · T 回标题 · P 存档（先落盘再决定）'],
