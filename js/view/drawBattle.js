...
export function drawBattle() {
  ...
  if (enemy) {
    ...
    if ((enemy.burn || 0) > 0) {
      // 灼烧每回合扣血数值（信息透明·纯显示）：与 enemyAct 同源 max(DOT_MIN, round(hpMax×BURN_PCT))，一眼看清烧多少
      text(`🔥 灼烧 ${enemy.burn} · 每回合 -${Math.max(DOT_MIN, Math.round(enemy.hpMax * BURN_PCT))}血`, 620, 128, 'bold 12px', '#ff8a2c', 'right');
    }
  ...
  if (!S.battleBusy) {
    ...
    // v23.02 指令栏 [5]防御/[6]蓄力 效果预览（体验打磨·信息透明·纯显示——承 [1]攻击≈N伤/[3]药水恢复量/
    // [4]逃跑成功率同一「指令栏效果透明」家族收口）：防御与蓄力是仅有的两个「选了才见效果」的指令——
    // 防御的减伤/回蓝/反击只在已生效角标（356 行「防御中 · 减伤50%…」）或敌方拟行动预判（409-439）里可见、
    // 蓄力的 ×N 只在已蓄力后（358 行「蓄力中 · 下击/技能×1.5」）可见，首次决策时指令栏本身是黑盒；
    ...
    // 药水恢复量预览（信息透明）：数值与 hero.js takePotion 同读 rules.potionRestore/elixirRestore
    // （单一数据源），结算与预览永远同一份公式，纯显示不改结算
    if (pN > 0 || p2 > 0) {
      const weakP = `🍖+${potionRestore(hero).h}HP`;
      const strongP = p2 > 0 ? `  🧪+${elixirRestore(hero).h}HP/+${elixirRestore(hero).m}MP` : '';
