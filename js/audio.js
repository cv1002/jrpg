...
const SFX = {
  ...
  // 收口）：enemyAI enemyAct 的「暗影回血」招（幽冥魔王 w20 / 洞窟领主 w15 / 终焉之神 w25，hp 低于
  // 40% 才用）此前播放 SFX.heal()（sine 520 单音明亮上挑，与玩家治愈术/喝药/住店/清泉同一音）——
  // 暗影回血是 Boss 的黑暗恢复（🟣 敌方 HP 恢复），听感却与玩家自己的治疗无从分辨：玩家刚用治愈术
  // 或药水回血、紧接着 Boss 就暗影回血，两声同响分不清哪声是自己的奶；现补「暗影低吟」三连（triangle
  // 220→185→147 小三度下行 + 末音 sine 低沉拖尾，与 heal 单音上扬/craft 上行/flee 下行快三步/transform
  // 上涌/block 块响一听即分——先闻其声即知是它在回血而非自己的治疗）；零结算零数值零存档，家族其余
  ...
  // v23.43 SFX.crit / v23.46 SFX.transform / v23.47 SFX.charge / v23.48 SFX.darkheal 同一「事件音效各归
  // 其位」主线收口）：world.onChestStep 开箱（蘑菇/金币/药水三分支）此前与商店购买/任务奖励同播 SFX.coin()
  // （880→1320 高音明亮双音）、与拾取同播 SFX.item()（700→900 双音）——宝箱是冒险探索的「📦 开箱」惊喜
  // 事件，听感却与买东西/交任务/拾取无从分辨（刚在商店买完药、紧接着开箱得金币，两声同响分不清是买是
  // 捡）；现补低暖木质三连（triangle 196→262→392 上挑 + 末音 sine 上扬余韵，与 coin 高音双音/item 双音/
  // select 单音一听即分——先闻其声即知在开箱而非拾取/购买）；零结算零数值零存档，家族其余逐字未动
  // （coin 仍服务商店购买/任务奖励、item 定义保留，玩家端调用点逐字未动）。
  chest() { tone(196, 0.09, 'triangle', 0.12); tone(262, 0.09, 'triangle', 0.11, 0.09); tone(392, 0.2, 'sine', 0.1, 0.18); },
  // v23.50 任务交付专属音效（音效反馈·语义修正——承 v23.22 SFX.ach / v23.33 SFX.craft / v23.40 SFX.flee /
  // v23.43 SFX.crit / v23.46 SFX.transform / v23.47 SFX.charge / v23.48 SFX.darkheal / v23.49 SFX.chest
  // 同一「事件音效各归其位」主线的收口）：core.talkNext 任务交付奖励分支（act.kind==='reward'）此前
  // 金币档播 SFX.coin()（880→1320 高音明亮双音，与售蘑菇同音）、物品档播 SFX.victory()（战斗胜利号角
  // 五连）——交任务是「🎁 委托达成」的交付事件，听感却与买东西/打赢仗无从分辨：刚打赢一场仗回村
  // 交任务，两声同响分不清是凯旋还是交付；现补「交付铃」先抑后扬三连（triangle 659→494 下行四度 +
  // sine 988 上扬余韵，与 coin 双音/victory 号角/ach 上行铃声/craft 气泡一听即分——先闻其声即知是
  // 交任务而非购物/凯旋）；零结算零数值零存档，家族其余逐字未动（coin 仍服务售蘑菇、victory 仍服务
  // 战斗胜利/试炼通关，玩家端调用点逐字未动）。
  quest() { tone(659, 0.08, 'triangle', 0.11); tone(494, 0.08, 'triangle', 0.1, 0.08); tone(988, 0.24, 'sine', 0.1, 0.16); },
