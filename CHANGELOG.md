## v21.37 图鉴「已遭遇」计数化——hero.seen 由布尔升级为计数，图鉴「已遭遇 ✕0」→「已遭遇 ✕N」真实撞见次数（体验打磨·信息透明·口径收尾，承 v19.41 已遭遇揭示 / 图鉴讨伐计数 ✕N 同族：v19.41 起 battle.startBattle 一进战即在 hero.seen 记布尔「撞见过」，图鉴已遭遇行却标写死的「已遭遇 ✕0」——玩家刷某怪撞见 3 次（含逃跑/战败反复撞见）图鉴仍是 ✕0，计数与事实对不上，且与讨伐行 `讨伐 ✕N` 的计数口径不一致）
