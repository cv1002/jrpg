## v21.41 战败画面图鉴入口口径修正——drawDead 普通战败提示「用记忆图鉴(B)」→「再战前先用记忆图鉴看清魔物强度（图鉴在世界画面按 B 打开）」（体验打磨·口径一致·按键与提示对号，承 v21.29-32/v21.38-40「同一功能所有入口口径一致」主线：战败画面普通怪分支的提示告诉玩家「用记忆图鉴(B)查看魔物强度」——但 dead 画面（main.js `dead.onKey`）的 B 从初始提交起就绑定 `retryBoss`：强敌战败=重整旗鼓、普通战败/试炼 rush=无反应（`retryBoss` 无 `_bossRetry` 或 `bossId==='rush'` 时直接 `return false`），记忆图鉴入口只在世界画面（`world.onKey` 的 B → codex）；且 `codex.onKey` 关闭走 `backWorld()`——从 dead 场景打开图鉴再关会错回 world，故正确修法是如实标注入口而非把 B 绑成图鉴。玩家按提示按 B：普通战败时屏幕毫无反应（与 v21.16/v21.33「每个按键都该有反应」同族的「提示引导的键无反应」）——正是 v21.30/v21.38/v21.39 反复修过的那类「提示口径与实际分派对不上」）
