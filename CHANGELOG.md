## v21.4 标题画面存档槽支持 ←/→ 循环切换——全游戏最后一个只认数字键的列表收口（体验打磨·输入一致性，v21.3 战斗音响收口后轮换回「导航一致性」家族——承 v19.48 技能菜单光标 / v20.3 快捷键可发现性：商店列表/快速旅行/暂停菜单/技能菜单/图鉴/成就页全部已支持方向键导航（↑↓ 或 ←→），唯独标题页的存档槽选择只认 1/2/3，方向键在标题页是完全的死键——与「任意列表都能方向键导航」的既有惯例自相矛盾）
