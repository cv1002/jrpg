## v21.7 冒烟套件整修——腐化的版本锚点去硬化 + v19.65 存档冒烟首次挂进 `npm test`（测试基建打磨，v21.6 战斗回看收口后轮换到「回归可靠性」层——v21.6 新增仓库常驻冒烟时暴露出「版本锚点是一次性断言」的通病：锚定 `GAME_VERSION==='vXX.Y'` 的测试在版本继续递增后必然失败，v19.65 的专项冒烟就是活例子）
