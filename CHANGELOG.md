## v21.34 README 升级经验曲线链式口径修正——闭式幂公式与 hero.grantXp 链式结算自 Lv7 起漂移，667 修正为 662（文档·数值口径，承 v19.15 README 酿造配方漂移 / v21.22 宝箱总数注释漂移同一「文档必须与结算逐字同源」主线：README 数值速查宣称「此表据代码实测整理」，但升级经验行写的是闭式幂 `round(20 × 1.42^(lv-1))`——而 hero.grantXp 的真实结算是**链式**逐级 `round(hero.xpNext × XP_GROW)`（XP_INIT=20 起、每级四舍五入后累积），两者自 Lv7→8 起漂移（闭式 164 vs 链式 163）、Lv11→12 闭式 667 vs 链式 662——结算与文档脱节，玩家/维护者按表算出的升级进度与游戏实际对不上）
