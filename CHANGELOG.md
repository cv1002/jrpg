...
## v19.59 帮助页「操作说明」页排版修复——14 行最后两行曾落到画布之外，战斗行拆两行不再溢出右缘（体验打磨·可读性修复，v19.38 时序体系收口后轮换回常规改进的第二十版，本次切「排版修 bug」方向——v19.50/56/57 连续把各处「信息透明」内容加进帮助页/图鉴/旅行列表时，操作说明页的行数已悄悄涨到 14 行，而 drawHelp 的行距仍是写死的 34px：最后两行出现在后台绘制里，玩家在游戏里根本看不到）
