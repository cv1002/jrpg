...
ok('GAME_VERSION 字面量已为 v22.22（旧 v22.21 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.30';") && !dSrc.includes("const GAME_VERSION = 'v22." + "21';"));
...
ok('NPC_SPOTS 总数 28（既有 23 键 + 货郎 1 键，v22.22 随新现实更新）', Object.keys(NPC_SPOTS).length === 28, Object.keys(NPC_SPOTS).length);
...
ok('README tests 树收录 smoke_v2222_peddler 且位于串尾', readme.includes('smoke_v2221_wanderer + smoke_v2222_peddler + smoke_v2223_lampman + smoke_v2224_sifter + smoke_v2225_stonecarver + smoke_v2226_chestprogress + smoke_v2227_minstrel + smoke_v2228_titlesave + smoke_v2229_metall + smoke_v2230_pausewarn（npm test 串跑）'));
ok('README tests 树尾链完整（v2221_wanderer 未被新尾吞并，全链连到 smoke_v2222_peddler）',
  readme.includes('smoke_v2220_hearer + smoke_v2221_wanderer + smoke_v2222_peddler + smoke_v2223_lampman + smoke_v2224_sifter + smoke_v2225_stonecarver + smoke_v2226_chestprogress + smoke_v2227_minstrel + smoke_v2228_titlesave + smoke_v2229_metall + smoke_v2230_pausewarn（npm test 串跑）'));
ok('README 件套口径为一百二十六件套（一百二十五件套清除）',
  readme.includes('冒烟一百二十六件套（一百二十五件套清除）') && !readme.includes('冒烟一百一十七件套（一百一十六件套清' + '除）'));
...
ok('smoke_v2221 的 GAME_VERSION 字面量 pin 已更新为 v22.22', s2221.includes("const GAME_VERSION = 'v22.30';"));
ok('smoke_v2221 的 GAME_VERSION 恒等 pin 已更新为 === v22.22', s2221.includes("GAME_VERSION === 'v22.30'"));
...
ok('smoke_v2221 的 README 树尾 pin 已更新为 + smoke_v2222_peddler',
  s2221.includes('smoke_v2220_hearer + smoke_v2221_wanderer + smoke_v2222_peddler + smoke_v2223_lampman + smoke_v2224_sifter + smoke_v2225_stonecarver + smoke_v2226_chestprogress + smoke_v2227_minstrel + smoke_v2228_titlesave + smoke_v2229_metall + smoke_v2230_pausewarn（npm test 串跑）'));
ok('smoke_v2221 的 package.json 件套计数 pin 已更新为 === 119', s2221.includes('testChain === 126'));
ok('smoke_v2221 的 NPC 总数 pin 已随新现实更新为 24（货郎落位）', s2221.includes('总数 28'));
ok('smoke_v2220 的 GAME_VERSION 字面量 pin 已更新为 v22.22', s2220.includes("const GAME_VERSION = 'v22.30';"));
ok('smoke_v2220 的 GAME_VERSION 恒等 pin 已更新为 === v22.22', s2220.includes("GAME_VERSION === 'v22.30'"));
...
ok('smoke_v2220 的 NPC 总数 pin 已随新现实更新为 24（货郎落位）', s2220.includes('总数 28'));
ok('smoke_v2219 的 GAME_VERSION 字面量 pin 已更新为 v22.22', s2219.includes("const GAME_VERSION = 'v22.30';"));
ok('smoke_v2219 的 README 件套 pin 已随新现实更新为一百二十六件套（一百二十五件套清除）',
  s2219.includes('一百二十六件套（一百二十五件套清除）'));
ok('smoke_v2218 的 GAME_VERSION 字面量 pin 已更新为 v22.22', s2218.includes("const GAME_VERSION = 'v22.30';"));
...
ok('smoke_v2216 的 NPC 总数 pin 已随新现实更新为 24（货郎落位）', s2216.includes('总数 28'));
ok('smoke_v2213 的 NPC 总数 pin 已随新现实更新为 24（货郎落位）', s2213.includes('总数 28'));
ok('smoke_v2211 的 NPC 总数 pin 已随新现实更新为 24（货郎落位）', s2211.includes('总数 28'));
