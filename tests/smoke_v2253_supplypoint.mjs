...
ok('GAME_VERSION 字面量已为 v22.53（旧 v22.52 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.64';") && !dSrc.includes("const GAME_VERSION = 'v22." + "52';"));
...
ok('README 件套口径为一百六十件套（一百五十九件套清除）',
  readme.includes('冒烟一百六十件套（一百五十九件套清除）') && !readme.includes('冒烟一百四十八件套（一百四十七件套清' + '除）'));
...
ok('package.json test 串共 149 件套', testChain === 160, String(testChain));
ok('CHANGELOG 顶部已追加 v22.53 条目', changelog.startsWith('## v22.64'));
...
ok('smoke_v2252 的 GAME_VERSION 字面量 pin 已更新为 v22.53（旧 v22.52 零残留）',
  s2252.includes("const GAME_VERSION = 'v22.64';") && !s2252.includes("const GAME_VERSION = 'v22." + "52';"));
ok('smoke_v2252 的 README 件套 pin 已随新现实更新为一百六十件套（一百五十九件套清除）',
  s2252.includes('一百六十件套（一百五十九件套清除）'));
ok('smoke_v2252 的 package.json 件套计数 pin 已更新为 === 149', s2252.includes('testChain === 160'));
ok('smoke_v2252 的 README 串尾 pin 已随新现实延伸至 smoke_v2253_supplypoint',
  s2252.includes('smoke_v2252_rail + smoke_v2253_supplypoint + smoke_v2254_grainfield + smoke_v2255_steleglow + smoke_v2256_campfire + smoke_v2257_pondglow + smoke_v2258_potionhelp + smoke_v2259_sandpile + smoke_v2260_fountripple + smoke_v2261_rich3 + smoke_v2262_crystal + smoke_v2263_ptime3 + smoke_v2264_hunt3（npm test 串跑）'));
ok('smoke_v2252 的 CHANGELOG pin 已更新为 ## v22.53', s2252.includes("startsWith('## v22.64')"));
const s2143 = fs.readFileSync(path.join(ROOT, 'tests/smoke_v2143_talkekey.mjs'), 'utf8');
ok('v2143-45「件套守护领先一位」哨兵链已推进至 150（一百六十件套（一百五十九件套清除））',
