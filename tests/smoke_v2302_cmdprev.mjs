...
// （冒烟二百零四件套（二百零三件套清除）/串尾/入库 198 份/顶 pin）+ 姊妹件套 pin（smoke_v2301
...
ok('data.js GAME_VERSION 字面量已为 v23.02（旧 v23.01 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.08';") && !dataSrc.includes("const GAME_VERSION = 'v23." + "01';"));
...
ok('README tests 树串尾已延伸至 smoke_v2302_cmdprev（v2301 后接 v2302）',
  readme.includes('smoke_v2301_eco + smoke_v2302_cmdprev + smoke_v2303_rushnum + smoke_v2304_achgoal + smoke_v2305_monnum + smoke_v2306_skillnum + smoke_v2307_bossnum + smoke_v2308_diffnum（npm test 串跑）'));
ok('README 旧串尾零残留（smoke_v2301_eco + smoke_v2302_cmdprev + smoke_v2303_rushnum + smoke_v2304_achgoal + smoke_v2305_monnum + smoke_v2306_skillnum + smoke_v2307_bossnum + smoke_v2308_diffnum（npm test 串跑）不在树尾）',
  !readme.includes('smoke_v2301_eco（npm test 串' + '跑）'));
ok('README 件套口径为二百零四件套（二百零三件套清除）且旧 196 口径零残留',
  readme.includes('冒烟二百零四件套（二百零三件套清除）') && !readme.includes('冒烟一百九十六件套（一百九十五件套清' + '除）'));
...
ok('package.json 串尾为 ... smoke_v2301_eco.mjs && node tests/smoke_v2302_cmdprev.mjs && node tests/smoke_v2303_rushnum.mjs && node tests/smoke_v2304_achgoal.mjs && node tests/smoke_v2305_monnum.mjs && node tests/smoke_v2306_skillnum.mjs && node tests/smoke_v2307_bossnum.mjs && node tests/smoke_v2308_diffnum.mjs"',
  pkg.includes('node tests/smoke_v2300_sidemore.mjs && node tests/smoke_v2301_eco.mjs && node tests/smoke_v2302_cmdprev.mjs && node tests/smoke_v2303_rushnum.mjs && node tests/smoke_v2304_achgoal.mjs && node tests/smoke_v2305_monnum.mjs && node tests/smoke_v2306_skillnum.mjs && node tests/smoke_v2307_bossnum.mjs && node tests/smoke_v2308_diffnum.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 198 件套', testChain === 204, String(testChain));
ok('CHANGELOG 顶部已追加 v23.02 条目', changelog.startsWith('## v23.08 '));
...
ok('smoke_v2301 的 GAME_VERSION 字面量 pin 已更新为 v23.02', s2301.includes("const GAME_VERSION = 'v23.08';"));
ok('smoke_v2301 的 CHANGELOG 顶 pin 已更新为 ## v23.02',
  s2301.includes("startsWith('## v23.08 '"));
ok('smoke_v2301 的件套 pin 已更新为二百零四件套（二百零三件套清除）',
  s2301.includes('二百零四件套（二百零三件套清除）'));
ok('smoke_v2301 的 README 串尾 pin 已延伸至 smoke_v2302_cmdprev',
  s2301.includes('smoke_v2301_eco + smoke_v2302_cmdprev + smoke_v2303_rushnum + smoke_v2304_achgoal + smoke_v2305_monnum + smoke_v2306_skillnum + smoke_v2307_bossnum + smoke_v2308_diffnum（npm test 串跑）'));
ok('smoke_v2301 的 package 串尾 pin 已延伸至 smoke_v2302_cmdprev',
  s2301.includes('node tests/smoke_v2300_sidemore.mjs && node tests/smoke_v2301_eco.mjs && node tests/smoke_v2302_cmdprev.mjs && node tests/smoke_v2303_rushnum.mjs && node tests/smoke_v2304_achgoal.mjs && node tests/smoke_v2305_monnum.mjs && node tests/smoke_v2306_skillnum.mjs && node tests/smoke_v2307_bossnum.mjs && node tests/smoke_v2308_diffnum.mjs"'));
ok('smoke_v2301 的 testChain pin 已更新为 198', s2301.includes('testChain === 204'));
