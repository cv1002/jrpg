...
ok('GAME_VERSION 字面量已为 v22.14（本版独占精确锚点）', GAME_VERSION === 'v22.62', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.14', dataSrc.includes("const GAME_VERSION = 'v22.62';"));
...
ok('README 件套口径为一百五十八件套（一百五十七件套清除）',
  readme.includes('冒烟一百五十八件套（一百五十七件套清除）') && !readme.includes('冒烟一百零三件套（一百零二件套清' + '除）'));
...
for (const nm of ach19) {
  ...
  ok(`${nm} 的 README 成就 pin 已随新现实更新为 42 项双处落位`,
    src.includes('成就一览（全部 46 项进度') && src.includes('**46 项成就**'));
}
