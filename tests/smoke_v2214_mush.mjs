...
ok('GAME_VERSION 字面量已为 v22.14（本版独占精确锚点）', GAME_VERSION === 'v22.23', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.14', dataSrc.includes("const GAME_VERSION = 'v22.23';"));
...
ok('README tests 树收录 smoke_v2214_mush 且位于串尾', readme.includes('smoke_v2214_mush + smoke_v2215_tutorvol + smoke_v2216_picker + smoke_v2217_elixir2 + smoke_v2218_mush2 + smoke_v2219_footkeys + smoke_v2220_hearer + smoke_v2221_wanderer + smoke_v2222_peddler + smoke_v2223_lampman（npm test 串跑）'));
ok('README 件套口径为一百一十九件套（一百一十八件套清除）',
  readme.includes('冒烟一百一十九件套（一百一十八件套清除）') && !readme.includes('冒烟一百零三件套（一百零二件套清' + '除）'));
...
for (const nm of suite110) {
  ...
  ok(`${nm} 的 README 件套 pin 已随新现实更新为一百一十九件套（一百一十八件套清除）`,
    src.includes('一百一十九件套（一百一十八件套清除）'));
}
...
for (const nm of vers34) {
  ...
  ok(`${nm} 的 GAME_VERSION 字面量 pin 已随新现实更新为 v22.14`,
    src.includes("const GAME_VERSION = 'v22.23';"));
}
...
for (const nm of eq19) {
  ...
  ok(`${nm} 的 GAME_VERSION 恒等 pin（===）已随新现实更新为 v22.14`,
    src.includes("GAME_VERSION === 'v22.23'"));
}
...
for (const nm of tail19) {
  ...
  ok(`${nm} 的 README tests 树尾 pin 已随新现实更新（+ smoke_v2214_mush）`,
    src.includes('smoke_v2214_mush + smoke_v2215_tutorvol + smoke_v2216_picker + smoke_v2217_elixir2 + smoke_v2218_mush2 + smoke_v2219_footkeys + smoke_v2220_hearer + smoke_v2221_wanderer + smoke_v2222_peddler + smoke_v2223_lampman（npm test 串跑）'));
}
