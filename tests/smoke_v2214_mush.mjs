...
ok('GAME_VERSION 字面量已为 v22.14（本版独占精确锚点）', GAME_VERSION === 'v22.47', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.14', dataSrc.includes("const GAME_VERSION = 'v22.47';"));
...
ok('README 件套口径为一百四十三件套（一百四十二件套清除）',
  readme.includes('冒烟一百四十三件套（一百四十二件套清除）') && !readme.includes('冒烟一百零三件套（一百零二件套清' + '除）'));
...
for (const nm of ach19) {
  ...
  ok(`${nm} 的 README 成就 pin 已随新现实更新为 42 项双处落位`,
    src.includes('成就一览（全部 45 项进度') && src.includes('**45 项成就**'));
}
