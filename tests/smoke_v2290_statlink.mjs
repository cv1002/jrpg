...
ok('data.js GAME_VERSION 字面量为 v22.90', dataSrc.includes("const GAME_VERSION = 'v23.12';"));
...
ok('GAME_VERSION 导出恒等于 v22.90', GAME_VERSION === 'v23.12', GAME_VERSION);
...
ok('README 件套口径为二百零八件套（二百零七件套清除）且旧 185 口径零残留',
  readme.includes('冒烟二百零八件套（二百零七件套清除）') && !readme.includes('一百八十五件套（一百八十四件套清' + '除）'));
...
ok('package.json test 串共 186 件套', testChain === 208, String(testChain));
ok('CHANGELOG 含 v22.90 条目（顶 pin）', changelog.startsWith('## v23.12 '));
...
ok('smoke_v2289 的 GAME_VERSION 字面量 pin 已更新为 v22.90', s2289.includes("const GAME_VERSION = 'v23.12';"));
ok('smoke_v2289 的 CHANGELOG 顶 pin 已更新为 ## v22.90', s2289.includes("startsWith('## v23.12 '"));
ok('smoke_v2289 的件套 pin 已更新为二百零八件套（二百零七件套清除）', s2289.includes('二百零八件套（二百零七件套清除）'));
...
ok('smoke_v2289 的 testChain pin 已更新为 186', s2289.includes('testChain === 208'));
ok('smoke_v2289 的哨兵 pin 已更新为二百零八件套（二百零七件套清除）', s2289.includes('二百零八件套（二百零七件套清除）'));
ok('smoke_v2288 的 GAME_VERSION 字面量 pin 已更新为 v22.90', s2288.includes("const GAME_VERSION = 'v23.12';"));
ok('smoke_v2288 的 testChain pin 已更新为 186', s2288.includes('testChain === 208'));
ok('smoke_v2288 的 哨兵 pin 已更新为二百零八件套（二百零七件套清除）', s2288.includes('二百零八件套（二百零七件套清除）'));
ok('smoke_v2288 的 状态/任务日志行 pin 已随 v22.90 口径更新',
  s2288.includes("row('状态') === 'I（页底 J 直达日志）'") &&
  s2288.includes("row('任务日志') === 'J（↑↓ 滚动 · I 直达状态页）'"));
ok('smoke_v2287 的 GAME_VERSION 字面量 pin 已更新为 v22.90', s2287.includes("const GAME_VERSION = 'v23.12';"));
ok('smoke_v2287 的 testChain pin 已更新为 186', s2287.includes('testChain === 208'));
