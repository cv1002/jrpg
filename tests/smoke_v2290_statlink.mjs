...
ok('data.js GAME_VERSION 字面量为 v22.90', dataSrc.includes("const GAME_VERSION = 'v23.70';"));
...
ok('data.js 状态行现为 I（页底 J 任务日志 · C 成就 · B 图鉴）（v23.69 收口三直达）', dataSrc.includes("['状态','I（页底 J 任务日志 · C 成就 · B 图鉴）']"));
...
ok('GAME_VERSION 导出恒等于 v22.90', GAME_VERSION === 'v23.70', GAME_VERSION);
...
ok('状态行精确为「I（页底 J 任务日志 · C 成就 · B 图鉴）」', row('状态') === 'I（页底 J 任务日志 · C 成就 · B 图鉴）', row('状态'));
ok('任务日志行精确为「J（↑↓ 滚动 · I 状态页 · B 图鉴 · C 成就）」', row('任务日志') === 'J（↑↓ 滚动 · I 状态页 · B 图鉴 · C 成就）', row('任务日志'));
ok('记忆图鉴行 v23.69 口径（B（↑↓ 滚动 · I 状态页 · J 日志 · C 成就））', row('记忆图鉴') === 'B（↑↓ 滚动 · I 状态页 · J 日志 · C 成就）');
ok('成就一览行 v23.69 口径（C（↑↓ 滚动 · I 状态页 · J 日志 · B 图鉴））', row('成就一览') === 'C（↑↓ 滚动 · I 状态页 · J 日志 · B 图鉴）');
...
ok('运行期：状态行落画「I（页底 J 任务日志 · C 成就 · B 图鉴）」', CAPTURED.some((c) => c.t.includes('状态') && c.t.includes('页底 J 任务日志') && c.t.includes('C 成就') && c.t.includes('B 图鉴')));
ok('运行期：任务日志行落画「J（↑↓ 滚动 · I 状态页 · B 图鉴 · C 成就）」', CAPTURED.some((c) => c.t.includes('任务日志') && c.t.includes('I 状态页')));
ok('运行期：I↔J/B/C 互切口径落画（状态 1 + J/B/C 三行各 1）',
  CAPTURED.filter((c) => c.t.includes('页底 J 任务日志')).length === 1 &&
...
ok('README 件套口径为二百一十二件套（二百一十一件套清除）且旧 185 口径零残留',
  readme.includes('冒烟二百一十二件套（二百一十一件套清除）') && !readme.includes('一百八十五件套（一百八十四件套清' + '除）'));
...
ok('index.html 常驻帮助条 I 处补三直达口径（v23.69）', idx.includes('I</kbd>状态（<kbd>J</kbd>日志 · <kbd>C</kbd>成就 · <kbd>B</kbd>图鉴）'));
ok('index.html J/B/C 滚动口径零回归', idx.includes('J</kbd>任务（<kbd>↑↓</kbd>滚动 · <kbd>I</kbd>/<kbd>B</kbd>/<kbd>C</kbd>直达）') && idx.includes('B</kbd>图鉴（<kbd>↑↓</kbd>滚动 · <kbd>I</kbd>/<kbd>J</kbd>/<kbd>C</kbd>直达）'));
...
ok('package.json test 串共 186 件套', testChain === 212, String(testChain));
ok('CHANGELOG 含 v22.90 条目（顶 pin）', changelog.startsWith('## v23.70 '));
...
ok('smoke_v2289 的 GAME_VERSION 字面量 pin 已更新为 v22.90', s2289.includes("const GAME_VERSION = 'v23.70';"));
ok('smoke_v2289 的 CHANGELOG 顶 pin 已更新为 ## v22.90', s2289.includes("startsWith('## v23.70 '"));
ok('smoke_v2289 的件套 pin 已更新为二百一十二件套（二百一十一件套清除）', s2289.includes('二百一十二件套（二百一十一件套清除）'));
...
ok('smoke_v2289 的 testChain pin 已更新为 186', s2289.includes('testChain === 212'));
ok('smoke_v2289 的哨兵 pin 已更新为二百一十二件套（二百一十一件套清除）', s2289.includes('二百一十二件套（二百一十一件套清除）'));
ok('smoke_v2288 的 GAME_VERSION 字面量 pin 已更新为 v22.90', s2288.includes("const GAME_VERSION = 'v23.70';"));
ok('smoke_v2288 的 testChain pin 已更新为 186', s2288.includes('testChain === 212'));
ok('smoke_v2288 的 哨兵 pin 已更新为二百一十二件套（二百一十一件套清除）', s2288.includes('二百一十二件套（二百一十一件套清除）'));
ok('smoke_v2288 的 状态/任务日志行 pin 已随 v22.90 口径更新',
  s2288.includes("row('状态') === 'I（页底 J 任务日志 · C 成就 · B 图鉴）'") &&
  s2288.includes("row('任务日志') === 'J（↑↓ 滚动 · I 状态页 · B 图鉴 · C 成就）'"));
ok('smoke_v2287 的 GAME_VERSION 字面量 pin 已更新为 v22.90', s2287.includes("const GAME_VERSION = 'v23.70';"));
ok('smoke_v2287 的 testChain pin 已更新为 186', s2287.includes('testChain === 212'));
