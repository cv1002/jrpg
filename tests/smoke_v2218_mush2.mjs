...
ok('GAME_VERSION 字面量已为 v22.18（本版独占精确锚点）', GAME_VERSION === 'v23.13', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.18（旧 v22.17 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.13';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "17';"));
...
ok('README 件套口径为二百零九件套（二百零八件套清除）',
  readme.includes('冒烟二百零九件套（二百零八件套清除）') && !readme.includes('冒烟一百一十三件套（一百一十二件套清' + '除）'));
...
ok('package.json test 串共 114 件套', testChain === 209, String(testChain));
...
ok('smoke_v2217 的 GAME_VERSION 字面量 pin 已更新为 v22.18', s2217.includes("const GAME_VERSION = 'v23.13';"));
ok('smoke_v2217 的 GAME_VERSION 恒等 pin 已更新为 === v22.18', s2217.includes("GAME_VERSION === 'v23.13'"));
ok('smoke_v2217 的 package.json 件套计数 pin 已更新为 === 114', s2217.includes('testChain === 209'));
ok('smoke_v2217 的 README 成就 pin 已随新现实更新为 44 项双处落位',
  s2217.includes('成就一览（全部 60 项进度') && s2217.includes('**60 项成就**'));
...
ok('smoke_v2216 的 GAME_VERSION 字面量 pin 已更新为 v22.18', s2216.includes("const GAME_VERSION = 'v23.13';"));
ok('smoke_v2216 的 package.json 件套计数 pin 已更新为 === 114', s2216.includes('testChain === 209'));
ok('smoke_v2215 的 package.json 件套计数 pin 已更新为 === 114', s2215.includes('testChain === 209'));
ok('smoke_v2215 的 GAME_VERSION 字面量 pin 已更新为 v22.18', s2215.includes("const GAME_VERSION = 'v23.13';"));
ok('smoke_v2214 的 README 成就 pin 已随新现实更新为 44 项双处落位',
  s2214.includes('成就一览（全部 60 项进度') && s2214.includes('**60 项成就**'));
