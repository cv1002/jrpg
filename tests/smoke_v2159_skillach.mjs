...
ok('learnsAt 扫描得 8 招且按领悟级排序（火焰斩/冰霜击/治愈术/雷鸣/陨石术/汲光击/星砂回响/灯焰长明）',
  allSkills.length === 8 &&
  allSkills.join(',') === ['火焰斩', '冰霜击', '治愈术', '雷鸣', '陨石术', '汲光击', '星砂回响', '灯焰长明'].join(','), allSkills.join(','));
ok('八招全部存在于 SKILL_DATA（领悟表与技能数据一致，无悬空条目）',
...
ok('skills 成就描述由 LEARN_AT 派生（「领悟全部 8 个技能」，数量非裸字面量，v21.83/v23.55 随新现实更新）',
...
ok('ok 谓词：八招俱全 → true（顺序无关，v21.83/v23.55 随新现实更新）',
  achSkills.ok({ skills: [...allSkills].reverse() }) === true);
ok('prog 进度：无字段 0/8 · 仅起始 1/8 · 七招 7/8 · 八招 8/8',
...
S.G.skills = [...allSkills]; // 模拟练到 Lv12 领悟全部八招（checkSkills 逐招补学的终态）
applyAchievements();
ok('applyAchievements：八招俱全后真实解锁 skills 落 hero.ach（winBattle 同场胜利即时解锁通路，v21.83/v23.55 随新现实更新）',
  (S.G.ach || []).includes('skills'), (S.G.ach || []).join(','));
