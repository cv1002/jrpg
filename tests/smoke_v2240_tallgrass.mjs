...
// —— 版本锚点 ——
ok('GAME_VERSION 恒等 v22.40', GAME_VERSION === 'v22.41', GAME_VERSION);
const dSrc = fs.readFileSync(path.join(ROOT, 'js/data.js'), 'utf8');
ok('data.js GAME_VERSION 字面量 v22.40 精确', dSrc.includes("const GAME_VERSION = 'v22.41';"));
...
ok('README tests 树收录 smoke_v2240_tallgrass 且位于串尾', readme.includes('smoke_v2240_tallgrass + smoke_v2241_gatearch（npm test 串跑）'));
ok('README 件套口径为一百三十七件套（一百三十六件套清除）',
  readme.includes('冒烟一百三十七件套（一百三十六件套清除）') && !readme.includes('冒烟一百三十五件套（一百三十四件套清' + '除）'));
...
ok('package.json test 串共 136 件套', testChain === 137, String(testChain));
...
ok('smoke_v2239 的 GAME_VERSION 字面量 pin 已更新为 v22.40', s2239.includes("const GAME_VERSION = 'v22.41';"));
ok('smoke_v2239 的 GAME_VERSION 恒等 pin 已更新为 === v22.40', s2239.includes("GAME_VERSION === 'v22.41'"));
ok('smoke_v2239 的 README 串尾 pin 已更新为 + smoke_v2240_tallgrass',
  s2239.includes('smoke_v2240_tallgrass + smoke_v2241_gatearch（npm test 串跑）'));
ok('smoke_v2239 的 package.json 件套计数 pin 已更新为 === 136', s2239.includes('testChain === 137'));
ok('smoke_v2239 的 README 件套口径 pin 已更新为一百三十六件套', s2239.includes('冒烟一百三十七件套（一百三十六件套清除）'));
ok('smoke_v2238 的 GAME_VERSION 字面量 pin 已更新为 v22.40', s2238.includes("const GAME_VERSION = 'v22.41';"));
ok('smoke_v2237 的 GAME_VERSION 字面量 pin 已更新为 v22.40', s2237.includes("const GAME_VERSION = 'v22.41';"));
ok('smoke_v2235 的 GAME_VERSION 字面量 pin 已更新为 v22.40', s2235.includes("const GAME_VERSION = 'v22.41';"));
ok('smoke_v2234 的 GAME_VERSION 恒等 pin 已更新为 === v22.40', s2234.includes("GAME_VERSION === 'v22.41'"));
ok('smoke_v2230 的 GAME_VERSION 恒等 pin 已更新为 === v22.40', s2230.includes("GAME_VERSION === 'v22.41'"));
ok('smoke_v2228/v2229/v2230 的 regex 串尾锚已延伸至 smoke_v2240_tallgrass',
  s2228.includes('smoke_v2239_minercart\\.mjs && node tests\\/smoke_v2240_tallgrass\\.mjs && node tests\\/smoke_v2241_gatearch.mjs"/.test(pkg)') &&
  s2229.includes('smoke_v2239_minercart\\.mjs && node tests\\/smoke_v2240_tallgrass\\.mjs && node tests\\/smoke_v2241_gatearch.mjs"/.test(pkg)') &&
  s2230.includes('smoke_v2239_minercart\\.mjs && node tests\\/smoke_v2240_tallgrass\\.mjs && node tests\\/smoke_v2241_gatearch.mjs"/.test(pkg)'));
ok('smoke_v2230 的 escaped 串尾锚已延伸至 smoke_v2241_gatearch（双转义复查链，任意转义风格容忍）',
  /smoke_v2240_tallgrass[.\\\\]{0,4}mjs\s*&&\s*node tests[\\\\/]{0,4}smoke_v2241_gatearch[.\\\\]{0,4}mjs"/.test(s2230));
ok('smoke_v2226 的 plain 串尾锚已延伸至 smoke_v2240_tallgrass',
  s2228.includes('node tests/smoke_v2239_minercart.mjs && node tests/smoke_v2240_tallgrass.mjs && node tests/smoke_v2241_gatearch.mjs"'));
