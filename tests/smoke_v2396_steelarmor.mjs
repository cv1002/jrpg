...
ok('data.js GAME_VERSION 字面量已为 v23.96（旧 v23.95 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.96';") && !dSrc.includes("const GAME_VERSION = 'v23.9" + "5';"));
...
ok('package.json test 串共 220 件套', testChain === 220, String(testChain));
...
ok('package.json 串尾为 ... smoke_v2395_shopscroll.mjs && node tests/smoke_v2396_steelarmor.mjs"',
  pkg.includes('node tests/smoke_v2395_shopscroll.mjs && node tests/smoke_v2396_steelarmor.mjs"'));
...
ok('README 件套口径为二百二十件套（二百一十九件套清除）且旧 216 口径零残留',
  readme.includes('冒烟二百二十件套（二百一十九件套清除）') && !readme.includes('冒烟二百一十六件套（二百一十五件套清' + '除）'));
...
ok('CHANGELOG 顶部已追加 v23.96 条目（精钢甲）', changelog.startsWith('## v23.96 '));
