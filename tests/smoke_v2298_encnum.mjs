...
// 同步（二百二十二件套（二百二十一件套清除）/串尾/入库 194 份/顶 pin）+ 姊妹件套 pin（smoke_v2297 随新现实
...
ok('data.js GAME_VERSION 字面量已为 v22.98（旧 v22.97 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.98';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "97';"));
...
ok('README 旧串尾零残留（smoke_v2297_chestmid + smoke_v2298_encnum + smoke_v2299_crosslink + smoke_v2300_sidemore + smoke_v2301_eco + smoke_v2302_cmdprev + smoke_v2303_rushnum + smoke_v2304_achgoal + smoke_v2305_monnum + smoke_v2306_skillnum + smoke_v2307_bossnum + smoke_v2308_diffnum + smoke_v2309_questnum + smoke_v2310_diffsum + smoke_v2311_fragprev + smoke_v2312_voltitle + smoke_v2313_talkall + smoke_v2314_voices + smoke_v2315_talkfoot + smoke_v2316_voiceshead + smoke_v2317_pausemap + smoke_v2318_battlemap + smoke_v2319_deadloc + smoke_v2392_stariron + smoke_v2393_deadkey + smoke_v2394_fightback + smoke_v2395_shopscroll + smoke_v2396_steelarmor + smoke_v2397_baserow + smoke_v2398_econrow（npm test 串跑）不在树尾）',
  !readme.includes('smoke_v2297_chestmid（npm test 串' + '跑）'));
ok('README 件套口径为二百二十二件套（二百二十一件套清除）且旧 193 口径零残留',
  readme.includes('冒烟二百二十二件套（二百二十一件套清除）') && !readme.includes('冒烟一百九十三件套（一百九十二件套清' + '除）'));
...
ok('package.json test 串共 194 件套', testChain === 222, String(testChain));
ok('CHANGELOG 顶部已追加 v22.98 条目', changelog.startsWith('## v23.98 '));
...
ok('smoke_v2297 的 GAME_VERSION 字面量 pin 已更新为 v22.98', s2297.includes("const GAME_VERSION = 'v23.98';"));
ok('smoke_v2297 的 CHANGELOG 顶 pin 已更新为 ## v22.98', s2297.includes("startsWith('## v23.98 '"));
ok('smoke_v2297 的件套 pin 已更新为二百二十二件套（二百二十一件套清除）',
  s2297.includes('二百二十二件套（二百二十一件套清除）'));
...
ok('smoke_v2297 的 testChain pin 已更新为 194', s2297.includes('testChain === 222'));
ok('smoke_v2297 的版本锚已推进至 >= 98', s2297.includes('_gv[1] >= 99'));
ok('smoke_v2296 的 GAME_VERSION 字面量 pin 已更新为 v22.98', s2296.includes("const GAME_VERSION = 'v23.98';"));
