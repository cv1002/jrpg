...
// 同步（一百九十五件套（一百九十四件套清除）/串尾/入库 194 份/顶 pin）+ 姊妹件套 pin（smoke_v2297 随新现实
...
ok('GAME_VERSION 格式合法且已越过 v22.97（本版守 v22.98）', !!_gv && (_gv[0] > 22 || (_gv[0] === 22 && _gv[1] >= 98)), GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已为 v22.98（旧 v22.97 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v22.98';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "97';"));
...
ok('README tests 树串尾已延伸至 smoke_v2298_encnum（v2297 后接 v2298）',
  readme.includes('smoke_v2297_chestmid + smoke_v2298_encnum（npm test 串跑）'));
ok('README 旧串尾零残留（smoke_v2297_chestmid + smoke_v2298_encnum（npm test 串跑）不在树尾）',
  !readme.includes('smoke_v2297_chestmid（npm test 串' + '跑）'));
ok('README 件套口径为一百九十四件套（一百九十三件套清除）且旧 193 口径零残留',
  readme.includes('冒烟一百九十四件套（一百九十三件套清除）') && !readme.includes('冒烟一百九十三件套（一百九十二件套清' + '除）'));
...
ok('package.json 串尾为 ... smoke_v2297_chestmid.mjs && node tests/smoke_v2298_encnum.mjs"',
  pkg.includes('node tests/smoke_v2297_chestmid.mjs && node tests/smoke_v2298_encnum.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 194 件套', testChain === 194, String(testChain));
ok('CHANGELOG 顶部已追加 v22.98 条目', changelog.startsWith('## v22.98 '));
...
ok('smoke_v2297 的 GAME_VERSION 字面量 pin 已更新为 v22.98', s2297.includes("const GAME_VERSION = 'v22.98';"));
ok('smoke_v2297 的 CHANGELOG 顶 pin 已更新为 ## v22.98', s2297.includes("startsWith('## v22.98 '"));
ok('smoke_v2297 的件套 pin 已更新为一百九十五件套（一百九十四件套清除）',
  s2297.includes('一百九十五件套（一百九十四件套清除）'));
ok('smoke_v2297 的 README 串尾 pin 已延伸至 smoke_v2298_encnum',
  s2297.includes('smoke_v2297_chestmid + smoke_v2298_encnum（npm test 串跑）'));
ok('smoke_v2297 的 package 串尾 pin 已延伸至 smoke_v2298_encnum',
  s2297.includes('node tests/smoke_v2297_chestmid.mjs && node tests/smoke_v2298_encnum.mjs"'));
ok('smoke_v2297 的 testChain pin 已更新为 194', s2297.includes('testChain === 194'));
ok('smoke_v2297 的版本锚已推进至 >= 98', s2297.includes('_gv[1] >= 98'));
ok('smoke_v2296 的 GAME_VERSION 字面量 pin 已更新为 v22.98', s2296.includes("const GAME_VERSION = 'v22.98';"));
ok('smoke_v2143 哨兵链已推进至一百九十五件套（一百九十四件套清除）',
