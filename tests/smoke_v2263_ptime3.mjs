...
ok('data.js GAME_VERSION 字面量已为 v22.63（旧 v22.62 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v22.64';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "62';"));
...
ok('ACH_LIST 精确总数 47 项（v2229 精确计数 pin 随新现实更新 46→47）', ACH_LIST.length === 48, String(ACH_LIST.length));
...
ok('README tests 树串尾已延伸至 smoke_v2263_ptime3（v2262 后接 v2263）',
  readme.includes('smoke_v2262_crystal + smoke_v2263_ptime3 + smoke_v2264_hunt3（npm test 串跑）'));
ok('README 件套口径为一百六十件套（一百五十九件套清除）且旧 158 口径零残留',
  readme.includes('冒烟一百六十件套（一百五十九件套清除）') && !readme.includes('冒烟一百五十八件套（一百五十七件套清' + '除）'));
...
ok('package.json 已收录 smoke_v2263_ptime3（npm test 串跑第 159 份）',
  pkg.includes('smoke_v2263_ptime3.mjs') && pkg.includes('smoke_v2262_crystal.mjs && node tests/smoke_v2263_ptime3.mjs && node tests/smoke_v2264_hunt3.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 159 件套', testChain === 160, String(testChain));
ok('CHANGELOG 顶为 v22.63 条目', changelog.startsWith('## v22.64 '));
...
ok('smoke_v2262 的 GAME_VERSION 字面量 pin 已更新为 v22.63', s2262.includes("const GAME_VERSION = 'v22.64';"));
ok('smoke_v2262 的 README 件套 pin 已随新现实更新为一百六十件套（一百五十九件套清除）',
  s2262.includes('一百六十件套（一百五十九件套清除）'));
ok('smoke_v2262 的 package.json 件套计数 pin 已更新为 === 159', s2262.includes('testChain === 160'));
ok('smoke_v2262 的 CHANGELOG 顶 pin 已更新为 ## v22.63', s2262.includes("startsWith('## v22.64')"));
ok('smoke_v2262 的 README 串尾 pin 已随新现实延伸至 smoke_v2263_ptime3',
  s2262.includes('smoke_v2262_crystal + smoke_v2263_ptime3 + smoke_v2264_hunt3（npm test 串跑）'));
...
ok('smoke_v2261 的 GAME_VERSION 字面量 pin 已更新为 v22.63', s2261.includes("const GAME_VERSION = 'v22.64';"));
ok('smoke_v2261 的 package.json 串尾 plain pin 已延伸至 smoke_v2263_ptime3',
  s2261.includes('smoke_v2261_rich3.mjs && node tests/smoke_v2262_crystal.mjs && node tests/smoke_v2263_ptime3.mjs && node tests/smoke_v2264_hunt3.mjs"'));
...
ok('smoke_v2260 的 package.json 件套计数 pin 已更新为 === 159', s2260.includes('testChain === 160'));
ok('smoke_v2260 的 CHANGELOG 顶 pin 已更新为 ## v22.63', s2260.includes("startsWith('## v22.64')"));
ok('smoke_v2260 的 README 串尾 pin 已随新现实延伸至 smoke_v2263_ptime3',
  s2260.includes('smoke_v2261_rich3 + smoke_v2262_crystal + smoke_v2263_ptime3 + smoke_v2264_hunt3（npm test 串跑）'));
ok('smoke_v2259 的 GAME_VERSION 字面量 pin 已更新为 v22.63', s2259.includes("const GAME_VERSION = 'v22.64';"));
ok('smoke_v2259 的 README 件套 pin 已随新现实更新为一百六十件套（一百五十九件套清除）',
  s2259.includes('一百六十件套（一百五十九件套清除）'));
ok('smoke_v2259 的 package.json 件套计数 pin 已更新为 === 159', s2259.includes('testChain === 160'));
ok('smoke_v2259 的 CHANGELOG 顶 pin 已更新为 ## v22.63', s2259.includes("startsWith('## v22.64')"));
ok('smoke_v2258 的 GAME_VERSION 字面量 pin 已更新为 v22.63', s2258.includes("const GAME_VERSION = 'v22.64';"));
ok('smoke_v2229 的 ACH_LIST 精确总数 pin 已更新为 === 47（46→47）',
  s2229.includes('ACH_LIST.length === 48') && !s2229.includes('ACH_LIST.length === 4' + '6'));
