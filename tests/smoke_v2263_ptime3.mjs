...
ok('data.js GAME_VERSION 字面量已为 v22.63（旧 v22.62 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.14';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "62';"));
...
ok('ACH_LIST 精确总数 47 项（v2229 精确计数 pin 随新现实更新 46→47）', ACH_LIST.length === 60, String(ACH_LIST.length));
...
ok('README 件套口径为二百一十件套（二百零九件套清除）且旧 158 口径零残留',
  readme.includes('冒烟二百一十件套（二百零九件套清除）') && !readme.includes('冒烟一百五十八件套（一百五十七件套清' + '除）'));
...
ok('package.json test 串共 159 件套', testChain === 210, String(testChain));
ok('CHANGELOG 顶为 v22.63 条目', changelog.startsWith('## v23.14 '));
...
ok('smoke_v2262 的 GAME_VERSION 字面量 pin 已更新为 v22.63', s2262.includes("const GAME_VERSION = 'v23.14';"));
ok('smoke_v2262 的 README 件套 pin 已随新现实更新为二百一十件套（二百零九件套清除）',
  s2262.includes('二百一十件套（二百零九件套清除）'));
ok('smoke_v2262 的 package.json 件套计数 pin 已更新为 === 159', s2262.includes('testChain === 210'));
ok('smoke_v2262 的 CHANGELOG 顶 pin 已更新为 ## v22.63', s2262.includes("startsWith('## v23.14')"));
...
ok('smoke_v2261 的 GAME_VERSION 字面量 pin 已更新为 v22.63', s2261.includes("const GAME_VERSION = 'v23.14';"));
...
ok('smoke_v2260 的 package.json 件套计数 pin 已更新为 === 159', s2260.includes('testChain === 210'));
ok('smoke_v2260 的 CHANGELOG 顶 pin 已更新为 ## v22.63', s2260.includes("startsWith('## v23.14')"));
...
ok('smoke_v2259 的 GAME_VERSION 字面量 pin 已更新为 v22.63', s2259.includes("const GAME_VERSION = 'v23.14';"));
ok('smoke_v2259 的 README 件套 pin 已随新现实更新为二百一十件套（二百零九件套清除）',
  s2259.includes('二百一十件套（二百零九件套清除）'));
ok('smoke_v2259 的 package.json 件套计数 pin 已更新为 === 159', s2259.includes('testChain === 210'));
ok('smoke_v2259 的 CHANGELOG 顶 pin 已更新为 ## v22.63', s2259.includes("startsWith('## v23.14')"));
ok('smoke_v2258 的 GAME_VERSION 字面量 pin 已更新为 v22.63', s2258.includes("const GAME_VERSION = 'v23.14';"));
ok('smoke_v2229 的 ACH_LIST 精确总数 pin 已更新为 === 47（46→47）',
  s2229.includes('ACH_LIST.length === 60') && !s2229.includes('ACH_LIST.length === 4' + '6'));
