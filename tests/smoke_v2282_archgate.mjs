...
ok('data.js GAME_VERSION 字面量为 v22.82', dataSrc.includes("const GAME_VERSION = 'v22.83'"));
...
ok('README tests 树串尾已延伸至 smoke_v2282_archgate（v2281 后接 v2282）',
  readme.includes('smoke_v2281_starwell + smoke_v2282_archgate + smoke_v2283_menuekey（npm test 串跑）'));
ok('README 件套口径为一百七十九件套（一百七十八件套清除）且旧 177 口径零残留',
  readme.includes('冒烟一百七十九件套（一百七十八件套清除）') && !readme.includes('冒烟一百七十七件套（一百七十六件套清' + '除）'));
...
ok('package.json test 串含 smoke_v2282_archgate.mjs 且位于串尾',
  pkg.includes('node tests/smoke_v2281_starwell.mjs && node tests/smoke_v2282_archgate.mjs && node tests/smoke_v2283_menuekey.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 178 件套', testChain === 179, String(testChain));
ok('CHANGELOG 含 v22.82 条目（顶 pin）', changelog.startsWith('## v22.83 '));
...
ok('smoke_v2281 的 GAME_VERSION 字面量 pin 已更新为 v22.82', s2281.includes("const GAME_VERSION = 'v22.83';"));
ok('smoke_v2281 的 CHANGELOG 顶 pin 已更新为 ## v22.82', s2281.includes("startsWith('## v22.83'"));
ok('smoke_v2281 的件套 pin 已更新为一百七十九件套（一百七十八件套清除）', s2281.includes('一百七十九件套（一百七十八件套清除）'));
ok('smoke_v2281 的 README 串尾 pin 已延伸至 smoke_v2282_archgate', s2281.includes('smoke_v2281_starwell + smoke_v2282_archgate + smoke_v2283_menuekey（npm test 串跑）'));
