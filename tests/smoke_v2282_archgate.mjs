...
ok('data.js GAME_VERSION 字面量为 v22.82', dataSrc.includes("const GAME_VERSION = 'v23.78'"));
...
ok('README 件套口径为二百一十四件套（二百一十三件套清除）且旧 177 口径零残留',
  readme.includes('冒烟二百一十四件套（二百一十三件套清除）') && !readme.includes('冒烟一百七十七件套（一百七十六件套清' + '除）'));
...
ok('package.json test 串共 178 件套', testChain === 214, String(testChain));
ok('CHANGELOG 含 v22.82 条目（顶 pin）', changelog.startsWith('## v23.78 '));
...
ok('smoke_v2281 的 GAME_VERSION 字面量 pin 已更新为 v22.82', s2281.includes("const GAME_VERSION = 'v23.78';"));
ok('smoke_v2281 的 CHANGELOG 顶 pin 已更新为 ## v22.82', s2281.includes("startsWith('## v23.78'"));
ok('smoke_v2281 的件套 pin 已更新为二百一十四件套（二百一十三件套清除）', s2281.includes('二百一十四件套（二百一十三件套清除）'));
