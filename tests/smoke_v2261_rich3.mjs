...
ok('data.js GAME_VERSION 字面量已为 v22.61（旧 v22.60 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.72';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "60';"));
...
ok('ACH_LIST 精确总数 46 项（v2229 精确计数 pin 随新现实更新 45→46）', ACH_LIST.length === 68, String(ACH_LIST.length));
...
ok('README 件套口径为二百一十二件套（二百一十一件套清除）且旧 156 口径零残留',
  readme.includes('冒烟二百一十二件套（二百一十一件套清除）') && !readme.includes('冒烟一百五十六件套（一百五十五件套清' + '除）'));
...
ok('package.json test 串共 157 件套', testChain === 212, String(testChain));
ok('CHANGELOG 顶为 v22.61 条目', changelog.startsWith('## v23.72 '));
...
ok('smoke_v2260 的 GAME_VERSION 字面量 pin 已更新为 v22.61', s2260.includes("const GAME_VERSION = 'v23.72';"));
ok('smoke_v2229 的 GAME_VERSION 恒等 pin 已更新为 v22.61（恒等 pin 族随新现实全库更新）', s2229.includes("GAME_VERSION === 'v23.72'"));
ok('smoke_v2260 的 README 件套 pin 已随新现实更新为二百一十二件套（二百一十一件套清除）',
  s2260.includes('二百一十二件套（二百一十一件套清除）'));
ok('smoke_v2260 的 package.json 件套计数 pin 已更新为 === 157', s2260.includes('testChain === 212'));
ok('smoke_v2260 的 CHANGELOG 顶 pin 已更新为 ## v22.61', s2260.includes("startsWith('## v23.72')"));
...
ok('smoke_v2259 的 GAME_VERSION 字面量 pin 已更新为 v22.61', s2259.includes("const GAME_VERSION = 'v23.72';"));
ok('smoke_v2259 的 README 件套 pin 已随新现实更新为二百一十二件套（二百一十一件套清除）',
  s2259.includes('二百一十二件套（二百一十一件套清除）'));
ok('smoke_v2259 的 package.json 件套计数 pin 已更新为 === 157', s2259.includes('testChain === 212'));
ok('smoke_v2259 的 CHANGELOG 顶 pin 已更新为 ## v22.61', s2259.includes("startsWith('## v23.72')"));
...
ok('smoke_v2258 的 GAME_VERSION 字面量 pin 已更新为 v22.61', s2258.includes("const GAME_VERSION = 'v23.72';"));
ok('smoke_v2229 的 ACH_LIST 精确总数 pin 已更新为 === 46（45→46）',
  s2229.includes('ACH_LIST.length === 68') && !s2229.includes('ACH_LIST.length === 4' + '5'));
ok('smoke_v2199 的 README 成就 pin 已随新现实更新为 46 项双处落位',
  s2199.includes("readme.includes('成就一览（全部 68 项进度'") && s2199.includes('**68 项成就**'));
ok('v2143-45「件套守护领先一位」哨兵链已推进至 158（二百一十二件套（二百一十一件套清除））',
