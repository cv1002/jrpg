...
ok('GAME_VERSION 字面量已为 v22.23（旧 v22.22 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.26';") && !dSrc.includes("const GAME_VERSION = 'v22." + "22';"));
...
ok('NPC_SPOTS 总数 27（既有 24 键 + 拾灯人 1 键，v22.23 随新现实更新）', Object.keys(NPC_SPOTS).length === 27, Object.keys(NPC_SPOTS).length);
...
ok('README tests 树收录 smoke_v2223_lampman 且位于串尾', readme.includes('smoke_v2222_peddler + smoke_v2223_lampman + smoke_v2224_sifter + smoke_v2225_stonecarver + smoke_v2226_chestprogress（npm test 串跑）'));
ok('README tests 树尾链完整（v2222_peddler 未被新尾吞并，全链连到 smoke_v2223_lampman）',
  readme.includes('smoke_v2221_wanderer + smoke_v2222_peddler + smoke_v2223_lampman + smoke_v2224_sifter + smoke_v2225_stonecarver + smoke_v2226_chestprogress（npm test 串跑）'));
ok('README 件套口径为一百二十二件套（一百二十一件套清除）',
  readme.includes('冒烟一百二十二件套（一百二十一件套清除）') && !readme.includes('冒烟一百一十八件套（一百一十七件套清' + '除）'));
...
ok('package.json test 串共 119 件套', testChain === 122, String(testChain));
...
ok('smoke_v2222 的 GAME_VERSION 字面量 pin 已更新为 v22.23', s2222.includes("const GAME_VERSION = 'v22.26';"));
ok('smoke_v2222 的 GAME_VERSION 恒等 pin 已更新为 === v22.23', s2222.includes("GAME_VERSION === 'v22.26'"));
...
ok('smoke_v2222 的 package.json 件套计数 pin 已更新为 === 119', s2222.includes('testChain === 122'));
ok('smoke_v2222 的 NPC 总数 pin 已随新现实更新为 25（拾灯人落位）', s2222.includes('总数 27'));
ok('smoke_v2221 的 GAME_VERSION 字面量 pin 已更新为 v22.23', s2221.includes("const GAME_VERSION = 'v22.26';"));
ok('smoke_v2221 的 GAME_VERSION 恒等 pin 已更新为 === v22.23', s2221.includes("GAME_VERSION === 'v22.26'"));
...
ok('smoke_v2221 的 package.json 件套计数 pin 已更新为 === 119', s2221.includes('testChain === 122'));
ok('smoke_v2221 的 NPC 总数 pin 已随新现实更新为 25（拾灯人落位）', s2221.includes('总数 27'));
ok('smoke_v2220 的 GAME_VERSION 字面量 pin 已更新为 v22.23', s2220.includes("const GAME_VERSION = 'v22.26';"));
ok('smoke_v2220 的 GAME_VERSION 恒等 pin 已更新为 === v22.23', s2220.includes("GAME_VERSION === 'v22.26'"));
...
ok('smoke_v2220 的 package.json 件套计数 pin 已更新为 === 119', s2220.includes('testChain === 122'));
ok('smoke_v2220 的 NPC 总数 pin 已随新现实更新为 25（拾灯人落位）', s2220.includes('总数 27'));
ok('smoke_v2219 的 GAME_VERSION 字面量 pin 已更新为 v22.23', s2219.includes("const GAME_VERSION = 'v22.26';"));
ok('smoke_v2219 的 README 件套 pin 已随新现实更新为一百二十二件套（一百二十一件套清除）',
  s2219.includes('一百二十二件套（一百二十一件套清除）'));
ok('smoke_v2218 的 GAME_VERSION 字面量 pin 已更新为 v22.23', s2218.includes("const GAME_VERSION = 'v22.26';"));
ok('smoke_v2218 的 package.json 件套计数 pin 已更新为 === 119', s2218.includes('testChain === 122'));
ok('smoke_v2217 的 README 件套 pin 已随新现实更新为一百二十二件套（一百二十一件套清除）',
  s2217.includes('一百二十二件套（一百二十一件套清除）'));
ok('smoke_v2216 的 NPC 总数 pin 已随新现实更新为 25（拾灯人落位）', s2216.includes('总数 27'));
ok('smoke_v2213 的 NPC 总数 pin 已随新现实更新为 25（拾灯人落位）', s2213.includes('总数 27'));
ok('smoke_v2211 的 NPC 总数 pin 已随新现实更新为 25（拾灯人落位）', s2211.includes('总数 27'));
ok('smoke_v2215 的 package.json 串尾 pin 已更新为 + smoke_v2223_lampman',
  s2215.includes('smoke_v2222_peddler.mjs && node tests/smoke_v2223_lampman.mjs && node tests/smoke_v2224_sifter.mjs && node tests/smoke_v2225_stonecarver.mjs && node tests/smoke_v2226_chestprogress.mjs"'));
