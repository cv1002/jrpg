...
ok('data.js GAME_VERSION 字面量已为 v23.92（旧 v23.91 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.95';") && !dSrc.includes("const GAME_VERSION = 'v23.9" + "1';"));
...
ok('package.json test 串共 216 件套', testChain === 219, String(testChain));
ok('package.json 已收录 smoke_v2392_stariron（npm test 串跑第 216 份）',
  JSON.parse(pkg).scripts.test.includes('smoke_v2392_stariron.mjs'));
ok('package.json 串尾为 ... smoke_v2319_deadloc.mjs && node tests/smoke_v2392_stariron.mjs && node tests/smoke_v2393_deadkey.mjs && node tests/smoke_v2394_fightback.mjs && node tests/smoke_v2395_shopscroll.mjs"',
  pkg.includes('node tests/smoke_v2319_deadloc.mjs && node tests/smoke_v2392_stariron.mjs && node tests/smoke_v2393_deadkey.mjs && node tests/smoke_v2394_fightback.mjs && node tests/smoke_v2395_shopscroll.mjs"'));
ok('README tests 树串尾已延伸至 smoke_v2392_stariron',
  readme.includes('+ smoke_v2319_deadloc + smoke_v2392_stariron + smoke_v2393_deadkey + smoke_v2394_fightback + smoke_v2395_shopscroll（npm test 串跑）'));
...
ok('CHANGELOG 顶部已追加 v23.92 条目（星铁剑）', changelog.startsWith('## v23.95 '));
