...
ok('GAME_VERSION 字面量已为 v21.93', GAME_VERSION === 'v21.94', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v21.93', dataSrc.includes("const GAME_VERSION = 'v21.94';"));
...
ok('ACH_LIST 精确总数 34 项（本版独占精确计数，33→34）', ACH_LIST.length === 34, String(ACH_LIST.length));
...
ok('README tests 树收录 smoke_v2193_hunt100/smoke_v2194_statuscodex 且位于串尾（v2194 随新现实由当版冒烟守护）', readme.includes('smoke_v2192_travelwarn + smoke_v2193_hunt100 + smoke_v2194_statuscodex（npm test 串跑）'));
ok('README 件套口径为九十件套（八十九件套清除）',
  readme.includes('冒烟九十件套（八十九件套清除）') && !readme.includes('冒烟八十八件套（八十七件套清除）'));
...
for (const nm of suite89) {
  ...
  ok(`${nm} 的 README 件套 pin 已随新现实更新为九十件套（八十九件套清除）`,
    src.includes('九十件套（八十九件套清除）'));
}
...
for (const nm of vers) {
  ...
  ok(`${nm} 的 GAME_VERSION 字面量 pin 已随新现实更新为 v21.93`,
    src.includes("const GAME_VERSION = 'v21.94';"));
}
...
for (const nm of achFiles) {
  ...
  ok(`${nm} 的 README 成就 pin 已随新现实更新（33 项正向 pin 零残留，34 项双处落位）`,
    src.includes("readme.includes('成就一览（全部 34 项进度'") &&
