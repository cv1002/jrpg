...
ok('GAME_VERSION 字面量已为 v22.9（本版独占精确锚点）', GAME_VERSION === 'v22.13', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.9', dataSrc.includes("const GAME_VERSION = 'v22.13';"));
...
ok('ACH_LIST 仍精确 41 项（本版零成就变动）', ACH_LIST.length === 41, String(ACH_LIST.length));
...
ok('README tests 树收录 smoke_v2209_achstatus 且位于串尾', readme.includes('smoke_v2209_achstatus + smoke_v2211_lampkid + smoke_v2212_volume + smoke_v2213_teller（npm test 串跑）'));
ok('README 件套口径为一百零九件套（一百零八件套清除）',
  readme.includes('冒烟一百零九件套（一百零八件套清除）') && !readme.includes('冒烟一百零四件套（一百零三件套清' + '除）'));
