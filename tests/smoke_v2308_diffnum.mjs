...
// 零裸字面量）+ README/package.json/CHANGELOG 同步（冒烟二百二十六件套（二百二十五件套清除）/串尾/入库 204
...
ok('data.js GAME_VERSION 字面量已为 v23.08（旧 v23.07 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v24.02';") && !dataSrc.includes("const GAME_VERSION = 'v23." + "07';"));
...
ok('README 件套口径为二百二十六件套（二百二十五件套清除）且旧 203 口径零残留',
  readme.includes('冒烟二百二十六件套（二百二十五件套清除）') && !readme.includes('冒烟二百零三件套（二百零二件套清' + '除）'));
...
ok('package.json test 串共 204 件套', testChain === 226, String(testChain));
ok('CHANGELOG 顶部已追加 v23.08 条目', changelog.startsWith('## v24.02 '));
...
ok('smoke_v2307 的 GAME_VERSION 字面量 pin 已更新为 v23.08', s2307.includes("const GAME_VERSION = 'v24.02';"));
ok('smoke_v2307 的 CHANGELOG 顶 pin 已更新为 ## v23.08',
  s2307.includes("startsWith('## v24.02 "));
ok('smoke_v2307 的件套 pin 已更新为二百二十六件套（二百二十五件套清除）',
  s2307.includes('二百二十六件套（二百二十五件套清除）'));
...
ok('smoke_v2307 的 testChain pin 已更新为 204', s2307.includes('testChain === 226'));
