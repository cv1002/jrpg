...
// 零裸字面量）+ README/package.json/CHANGELOG 同步（冒烟二百零九件套（二百零八件套清除）/串尾/入库 204
...
ok('data.js GAME_VERSION 字面量已为 v23.08（旧 v23.07 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.13';") && !dataSrc.includes("const GAME_VERSION = 'v23." + "07';"));
...
ok('README tests 树串尾已延伸至 smoke_v2308_diffnum（v2307 后接 v2308）',
  readme.includes('smoke_v2306_skillnum + smoke_v2307_bossnum + smoke_v2308_diffnum + smoke_v2309_questnum + smoke_v2310_diffsum + smoke_v2311_fragprev + smoke_v2312_voltitle + smoke_v2313_talkall（npm test 串跑）'));
ok('README 旧串尾零残留（v2307 后无串尾收口）',
  !readme.includes('smoke_v2305_monnum + smoke_v2306_skillnum + smoke_v2307_bossnum（npm test 串' + '跑）'));
ok('README 件套口径为二百零九件套（二百零八件套清除）且旧 203 口径零残留',
  readme.includes('冒烟二百零九件套（二百零八件套清除）') && !readme.includes('冒烟二百零三件套（二百零二件套清' + '除）'));
ok('README 不含哨兵领先一位（二百零九件套（二百零八件套清除））',
...
ok('package.json 串尾为 ... smoke_v2307_bossnum.mjs && node tests/smoke_v2308_diffnum.mjs && node tests/smoke_v2309_questnum.mjs && node tests/smoke_v2310_diffsum.mjs && node tests/smoke_v2311_fragprev.mjs && node tests/smoke_v2312_voltitle.mjs && node tests/smoke_v2313_talkall.mjs"',
  pkg.includes('node tests/smoke_v2307_bossnum.mjs && node tests/smoke_v2308_diffnum.mjs && node tests/smoke_v2309_questnum.mjs && node tests/smoke_v2310_diffsum.mjs && node tests/smoke_v2311_fragprev.mjs && node tests/smoke_v2312_voltitle.mjs && node tests/smoke_v2313_talkall.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 204 件套', testChain === 209, String(testChain));
ok('CHANGELOG 顶部已追加 v23.08 条目', changelog.startsWith('## v23.13 '));
...
ok('smoke_v2307 的 GAME_VERSION 字面量 pin 已更新为 v23.08', s2307.includes("const GAME_VERSION = 'v23.13';"));
ok('smoke_v2307 的 CHANGELOG 顶 pin 已更新为 ## v23.08',
  s2307.includes("startsWith('## v23.13 "));
ok('smoke_v2307 的件套 pin 已更新为二百零九件套（二百零八件套清除）',
  s2307.includes('二百零九件套（二百零八件套清除）'));
ok('smoke_v2307 的 README 串尾 pin 已延伸至 smoke_v2308_diffnum',
  s2307.includes('smoke_v2306_skillnum + smoke_v2307_bossnum + smoke_v2308_diffnum + smoke_v2309_questnum + smoke_v2310_diffsum + smoke_v2311_fragprev + smoke_v2312_voltitle + smoke_v2313_talkall（npm test 串跑）'));
ok('smoke_v2307 的 package 串尾 pin 已延伸至 smoke_v2308_diffnum',
  s2307.includes('node tests/smoke_v2307_bossnum.mjs && node tests/smoke_v2308_diffnum.mjs && node tests/smoke_v2309_questnum.mjs && node tests/smoke_v2310_diffsum.mjs && node tests/smoke_v2311_fragprev.mjs && node tests/smoke_v2312_voltitle.mjs && node tests/smoke_v2313_talkall.mjs"'));
ok('smoke_v2307 的 testChain pin 已更新为 204', s2307.includes('testChain === 209'));
