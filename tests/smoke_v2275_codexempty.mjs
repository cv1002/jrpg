...
ok('data.js GAME_VERSION 字面量已为 v22.75（旧 v22.74 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.97';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "74';"));
...
ok('README 件套口径为二百二十一件套（二百二十件套清除）且旧 170 口径零残留',
  readme.includes('冒烟二百二十一件套（二百二十件套清除）') && !readme.includes('冒烟一百七十件套（一百六十九件套清' + '除）'));
...
ok('package.json test 串共 171 件套', testChain === 221, String(testChain));
ok('CHANGELOG 含 v22.75 条目（顶 pin）', changelog.startsWith('## v23.97 '));

// —— 姊妹件套 pin 随新现实更新 + 旧代 v22.74 pin 零残留 ——
const s2274 = read('smoke_v2274_seen2.mjs');
const s2143 = read('smoke_v2143_talkekey.mjs');
ok('smoke_v2274 的 GAME_VERSION 字面量 pin 已更新为 v22.75', s2274.includes("const GAME_VERSION = 'v23.97';"));
ok('smoke_v2274 的 CHANGELOG 顶 pin 已更新为 ## v22.78', s2274.includes("startsWith('## v23.97'") );
ok('smoke_v2274 的件套 pin 已更新为二百二十一件套（二百二十件套清除）', s2274.includes('二百二十一件套（二百二十件套清除）'));
