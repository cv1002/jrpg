...
ok('data.js GAME_VERSION 字面量已为 v23.95（旧 v23.94 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.95';") && !dSrc.includes("const GAME_VERSION = 'v23.9" + "4';"));
...
// —— 运行期：真实 buildShopList 10 项清单 + fillText 录制逐档验证 ——
{
  ...
  // 最坏情形：新手装（木剑/布衣）+ 5 株蘑菇 → 10 行清单（与 /tmp/jrpg_viz/shot_shop.mjs 实测同档）
  S.G.gold = 9999; S.G.mushrooms = 5; S.G.item = 1;
  const list = buildShopList();
  S.scene = 'shop';
  ok('运行期：最坏情形清单恰 10 项（药水/卖菇/4 剑/3 甲/离开）', list.length === 10, String(list.length));
  ...
  ok('shopSel=0：第 9/10 行（龙鳞甲/离开商店）被视窗裁掉', !has('龙鳞甲') && !has('离开商店'));
  ok('shopSel=0：裁切提示「↓ 还有 2 项未在本页显示」精确落位（y=428）',
    ys('还有 2 项未在本页显示').includes(428));
  ok('shopSel=0：商品行基线最大 394（118+7*38）——零行画面板底 410',
    records.filter((r) => r.y !== 428 && r.y !== 470 && r.y > 384).length === 0,
    JSON.stringify(records.filter((r) => r.y > 384 && r.y !== 428 && r.y !== 470).map((r) => r.y)));

  runAt(7);
  ok('shopSel=7：选中行贴视窗底部仍可见（锁子甲在屏、龙鳞甲仍裁掉）', has('锁子甲') && !has('龙鳞甲'));

  runAt(8);
  ok('shopSel=8：窗口下滑——龙鳞甲入屏、生命药水被裁', has('龙鳞甲') && !has('生命药水'));

  runAt(9);
  ok('shopSel=9（末行）：离开商店入屏且提示翻向「↑ 上方还有 2 项」',
    has('离开商店') && ys('上方还有 2 项').includes(428) && !has('生命药水') && !has('卖菇'));

  // 短清单（≤PAGE 边界档）：直接注入 5 项合成清单（真实对局最少 9 项——木剑恒在清单，≤8 档唯合成可达），
...
ok('package.json test 串共 219 件套', testChain === 219, String(testChain));
...
ok('package.json 串尾为 ... smoke_v2394_fightback.mjs && node tests/smoke_v2395_shopscroll.mjs"',
  pkg.includes('node tests/smoke_v2394_fightback.mjs && node tests/smoke_v2395_shopscroll.mjs"'));
ok('README tests 树串尾已延伸至 smoke_v2395_shopscroll',
  readme.includes('+ smoke_v2394_fightback + smoke_v2395_shopscroll（npm test 串跑）'));
ok('README 件套口径为二百一十九件套（二百一十八件套清除）且旧 215 口径零残留',
  readme.includes('冒烟二百一十九件套（二百一十八件套清除）') && !readme.includes('冒烟二百一十五件套（二百一十四件套清' + '除）'));
...
ok('CHANGELOG 顶部已追加 v23.95 条目（商店视窗滚动）', changelog.startsWith('## v23.95 '));
