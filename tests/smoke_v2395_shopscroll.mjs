...
ok('data.js GAME_VERSION 字面量已为 v23.95（旧 v23.94 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v24.01';") && !dSrc.includes("const GAME_VERSION = 'v23.9" + "4';"));
...
ok('package.json test 串共 219 件套', testChain === 225, String(testChain));
...
ok('package.json 串尾为 ... smoke_v2394_fightback.mjs && node tests/smoke_v2395_shopscroll.mjs && node tests/smoke_v2396_steelarmor.mjs && node tests/smoke_v2397_baserow.mjs && node tests/smoke_v2398_econrow.mjs && node tests/smoke_v2399_dmgformula.mjs && node tests/smoke_v2400_trialwarn.mjs && node tests/smoke_v2401_winbgm.mjs"',
...
ok('README 件套口径为二百二十五件套（二百二十四件套清除）且旧 215 口径零残留',
  readme.includes('冒烟二百二十五件套（二百二十四件套清除）') && !readme.includes('冒烟二百一十五件套（二百一十四件套清' + '除）'));
...
ok('CHANGELOG 顶部已追加 v23.95 条目（商店视窗滚动）', changelog.startsWith('## v24.01 '));
