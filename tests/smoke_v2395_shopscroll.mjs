...
ok('data.js GAME_VERSION 字面量已为 v23.95（旧 v23.94 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.96';") && !dSrc.includes("const GAME_VERSION = 'v23.9" + "4';"));
...
ok('package.json test 串共 219 件套', testChain === 220, String(testChain));
...
ok('package.json 串尾为 ... smoke_v2394_fightback.mjs && node tests/smoke_v2395_shopscroll.mjs && node tests/smoke_v2396_steelarmor.mjs"',
  pkg.includes('node tests/smoke_v2394_fightback.mjs && node tests/smoke_v2395_shopscroll.mjs && node tests/smoke_v2396_steelarmor.mjs"'));
ok('README tests 树串尾已延伸至 smoke_v2395_shopscroll',
  readme.includes('+ smoke_v2394_fightback + smoke_v2395_shopscroll + smoke_v2396_steelarmor（npm test 串跑）'));
ok('README 件套口径为二百二十件套（二百一十九件套清除）且旧 215 口径零残留',
  readme.includes('冒烟二百二十件套（二百一十九件套清除）') && !readme.includes('冒烟二百一十五件套（二百一十四件套清' + '除）'));
...
ok('CHANGELOG 顶部已追加 v23.95 条目（商店视窗滚动）', changelog.startsWith('## v23.96 '));
