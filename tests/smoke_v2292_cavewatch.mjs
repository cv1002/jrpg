...
ok('GAME_VERSION 字面量已为 v22.92（旧 v22.91 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.11';") && !dSrc.includes("const GAME_VERSION = 'v22." + "91';"));
...
ok('README 件套口径为二百零七件套（二百零六件套清除）且旧 187 口径零残留',
  readme.includes('冒烟二百零七件套（二百零六件套清除）') && !readme.includes('冒烟一百八十七件套（一百八十六件套清' + '除）'));
...
ok('package.json test 串共 188 件套', testChain === 207, String(testChain));
ok('CHANGELOG 顶部已追加 v22.92 条目', changelog.startsWith('## v23.11 '));
...
ok('smoke_v2291 的 GAME_VERSION 字面量 pin 已更新为 v22.92（旧 v22.91 零残留）',
  s2291.includes("const GAME_VERSION = 'v23.11';") && !s2291.includes("const GAME_VERSION = 'v22." + "91';"));
ok('smoke_v2291 的 README 件套 pin 已随新现实更新为二百零七件套（二百零六件套清除）',
  s2291.includes('二百零七件套（二百零六件套清除）'));
ok('smoke_v2291 的 package.json 件套计数 pin 已更新为 === 188', s2291.includes('testChain === 207'));
...
ok('smoke_v2291 的 CHANGELOG 顶 pin 已更新为 ## v22.92', s2291.includes("startsWith('## v23.11 '"));
ok('smoke_v2291 的哨兵 pin 已更新为二百零七件套（二百零六件套清除）', s2291.includes('二百零七件套（二百零六件套清除）'));
...
ok('哨兵链 v2143 已含下一版件套口径（二百零七件套（二百零六件套清除））',
  s2143.includes('二百零八件套（二百零七件套清除）') &&
  s2143.includes("!readme.includes('二百零八件套（二百零七件套清除）')"));
