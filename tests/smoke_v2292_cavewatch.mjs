...
ok('GAME_VERSION 格式合法且已越过 v22.91（本版守 v22.92）', !!_gv && (_gv[0] > 22 || (_gv[0] === 22 && _gv[1] >= 98)), GAME_VERSION);
ok('data.js 含 v22.92 注释（守洞人说明）', dSrc.includes('v22.92 新内容·纯风味'));
ok('GAME_VERSION 字面量已为 v22.92（旧 v22.91 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.98';") && !dSrc.includes("const GAME_VERSION = 'v22." + "91';"));
...
ok('README tests 树含 smoke_v2292_cavewatch 串尾且旧串尾零残留（树为历史清单，总数不守恒）',
  treeLine && treeLine.includes('+ smoke_v2292_cavewatch + smoke_v2293_deadsave + smoke_v2294_crystalwatch + smoke_v2295_deadprog + smoke_v2296_endingprog + smoke_v2297_chestmid + smoke_v2298_encnum（npm test 串跑）') &&
  !treeLine.includes('smoke_v2291_lampguide（npm test 串跑）'));
ok('README 件套口径为一百九十四件套（一百九十三件套清除）且旧 187 口径零残留',
  readme.includes('冒烟一百九十四件套（一百九十三件套清除）') && !readme.includes('冒烟一百八十七件套（一百八十六件套清' + '除）'));
...
ok('package.json 串尾为 ... smoke_v2291_lampguide.mjs && node tests/smoke_v2292_cavewatch.mjs && node tests/smoke_v2293_deadsave.mjs && node tests/smoke_v2294_crystalwatch.mjs && node tests/smoke_v2295_deadprog.mjs && node tests/smoke_v2296_endingprog.mjs && node tests/smoke_v2297_chestmid.mjs && node tests/smoke_v2298_encnum.mjs"',
  pkg.includes('node tests/smoke_v2291_lampguide.mjs && node tests/smoke_v2292_cavewatch.mjs && node tests/smoke_v2293_deadsave.mjs && node tests/smoke_v2294_crystalwatch.mjs && node tests/smoke_v2295_deadprog.mjs && node tests/smoke_v2296_endingprog.mjs && node tests/smoke_v2297_chestmid.mjs && node tests/smoke_v2298_encnum.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 188 件套', testChain === 194, String(testChain));
ok('CHANGELOG 顶部已追加 v22.92 条目', changelog.startsWith('## v22.98 '));
...
ok('smoke_v2291 的 GAME_VERSION 字面量 pin 已更新为 v22.92（旧 v22.91 零残留）',
  s2291.includes("const GAME_VERSION = 'v22.98';") && !s2291.includes("const GAME_VERSION = 'v22." + "91';"));
ok('smoke_v2291 的 README 件套 pin 已随新现实更新为一百九十四件套（一百九十三件套清除）',
  s2291.includes('一百九十五件套（一百九十四件套清除）'));
ok('smoke_v2291 的 package.json 件套计数 pin 已更新为 === 188', s2291.includes('testChain === 194'));
ok('smoke_v2291 的 README 串尾 pin 已随新现实延伸至 smoke_v2292_cavewatch（v2287 起尾部）',
  s2291.includes('smoke_v2290_statlink + smoke_v2291_lampguide + smoke_v2292_cavewatch + smoke_v2293_deadsave + smoke_v2294_crystalwatch + smoke_v2295_deadprog + smoke_v2296_endingprog + smoke_v2297_chestmid + smoke_v2298_encnum（npm test 串跑）'));
ok('smoke_v2291 的 package 串尾 pin 已延伸至 smoke_v2292_cavewatch',
  s2291.includes('node tests/smoke_v2290_statlink.mjs && node tests/smoke_v2291_lampguide.mjs && node tests/smoke_v2292_cavewatch.mjs && node tests/smoke_v2293_deadsave.mjs && node tests/smoke_v2294_crystalwatch.mjs && node tests/smoke_v2295_deadprog.mjs && node tests/smoke_v2296_endingprog.mjs && node tests/smoke_v2297_chestmid.mjs && node tests/smoke_v2298_encnum.mjs"' ));
ok('smoke_v2291 的 CHANGELOG 顶 pin 已更新为 ## v22.92', s2291.includes("startsWith('## v22.98 '"));
ok('smoke_v2291 的哨兵 pin 已更新为一百九十四件套（一百九十三件套清除）', s2291.includes('一百九十五件套（一百九十四件套清除）'));
