...
ok('data.js GAME_VERSION 字面量已为 v23.80（旧 v23.79 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.96';") && !dSrc.includes("const GAME_VERSION = 'v23.7" + "9';"));
...
// —— 运行期：drawDead 真实渲染捕获（village / gallery 两档 📍 + 既有行零回归 + 零抛错）——
{
  ...
  try {
    ...
    ok('收集行零回归（🏆 成就 0/76）', cap.includes('🏆 成就 0/76 · 📕 图鉴 0/13 · 📦 宝箱 0/12 · 🕯️ 记忆碎片 0/4'),
      JSON.stringify(cap.filter((t) => t.includes('🏆'))));
...
ok('package.json test 串共 215 件套', testChain === 220, String(testChain));
ok('package.json 已收录 smoke_v2319_deadloc（npm test 串跑第 215 份）',
  JSON.parse(pkg).scripts.test.includes('smoke_v2319_deadloc.mjs'));
ok('package.json 串尾为 ... smoke_v2318_battlemap.mjs && node tests/smoke_v2319_deadloc.mjs && node tests/smoke_v2392_stariron.mjs && node tests/smoke_v2393_deadkey.mjs && node tests/smoke_v2394_fightback.mjs && node tests/smoke_v2395_shopscroll.mjs && node tests/smoke_v2396_steelarmor.mjs"',
  pkg.includes('node tests/smoke_v2318_battlemap.mjs && node tests/smoke_v2319_deadloc.mjs && node tests/smoke_v2392_stariron.mjs && node tests/smoke_v2393_deadkey.mjs && node tests/smoke_v2394_fightback.mjs && node tests/smoke_v2395_shopscroll.mjs && node tests/smoke_v2396_steelarmor.mjs"'));
ok('README tests 树串尾已延伸至 smoke_v2319_deadloc',
  readme.includes('+ smoke_v2316_voiceshead + smoke_v2317_pausemap + smoke_v2318_battlemap + smoke_v2319_deadloc + smoke_v2392_stariron + smoke_v2393_deadkey + smoke_v2394_fightback + smoke_v2395_shopscroll + smoke_v2396_steelarmor（npm test 串跑）'));
ok('README 件套口径为二百二十件套（二百一十九件套清除）且旧 214 口径零残留',
  readme.includes('冒烟二百二十件套（二百一十九件套清除）') && !readme.includes('冒烟二百一十四件套（二百一十三件套清' + '除）'));
...
ok('CHANGELOG 顶部已追加 v23.79 条目（阵亡地点）', changelog.startsWith('## v23.96 '));
...
ok('README 尚无二百二十一件套（二百二十件套清除）前望口径', !readme.includes('二百二十一件套（二百二十件套清除）'));
...
// —— 零回归：ACH_LIST 75 项 / NPCS 37 处不变（本版成就版图并入「身经百战」——v23.80 非显示改动）——
const { ACH_LIST, NPCS } = await import('../js/data.js');
ok('ACH_LIST 精确总数 73（v23.80 身经百战为末项，未动既有 72 项零位移）', ACH_LIST.length === 76, String(ACH_LIST.length));
