...
ok('data.js GAME_VERSION 字面量已为 v23.80（旧 v23.79 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.92';") && !dSrc.includes("const GAME_VERSION = 'v23.7" + "9';"));
...
ok('package.json test 串共 215 件套', testChain === 216, String(testChain));
ok('package.json 已收录 smoke_v2319_deadloc（npm test 串跑第 215 份）',
  JSON.parse(pkg).scripts.test.includes('smoke_v2319_deadloc.mjs'));
ok('package.json 串尾为 ... smoke_v2318_battlemap.mjs && node tests/smoke_v2319_deadloc.mjs && node tests/smoke_v2392_stariron.mjs"',
  pkg.includes('node tests/smoke_v2318_battlemap.mjs && node tests/smoke_v2319_deadloc.mjs && node tests/smoke_v2392_stariron.mjs"'));
ok('README tests 树串尾已延伸至 smoke_v2319_deadloc',
  readme.includes('+ smoke_v2316_voiceshead + smoke_v2317_pausemap + smoke_v2318_battlemap + smoke_v2319_deadloc + smoke_v2392_stariron（npm test 串跑）'));
ok('README 件套口径为二百一十六件套（二百一十五件套清除）且旧 214 口径零残留',
  readme.includes('冒烟二百一十六件套（二百一十五件套清除）') && !readme.includes('冒烟二百一十四件套（二百一十三件套清' + '除）'));
...
ok('CHANGELOG 顶部已追加 v23.79 条目（阵亡地点）', changelog.startsWith('## v23.92 '));
...
ok('README 尚无二百一十七件套（二百一十六件套清除）前望口径', !readme.includes('二百一十七件套（二百一十六件套清除）'));
