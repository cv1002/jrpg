...
ok('GAME_VERSION 字面量已为 v22.29（旧 v22.28 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v22.29';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "28';"));
...
ok('README tests 树收录 smoke_v2229_metall 且位于串尾', readme.includes('smoke_v2228_titlesave + smoke_v2229_metall（npm test 串跑）'));
ok('README 件套口径为一百二十五件套（一百二十四件套清除）',
  readme.includes('冒烟一百二十五件套（一百二十四件套清除）') && !readme.includes('一百二十四件套（一百二十三件套清' + '除）'));
...
ok('package.json 已收录 smoke_v2229_metall（npm test 串跑第 125 份）',
  pkg.includes('smoke_v2229_metall.mjs') && /smoke_v2228_titlesave\.mjs && node tests\/smoke_v2229_metall\.mjs"/.test(pkg));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 125 件套', testChain === 125, String(testChain));
...
ok('smoke_v2228 的 GAME_VERSION 字面量 pin 已更新为 v22.29', s2228.includes("const GAME_VERSION = 'v22.29';"));
ok('smoke_v2228 的 GAME_VERSION 恒等 pin 已更新为 === v22.29', s2228.includes("GAME_VERSION === 'v22.29'"));
ok('smoke_v2228 的 README 件套 pin 已更新为一百二十五件套（一百二十四件套清除）',
  s2228.includes('一百二十五件套（一百二十四件套清除）'));
ok('smoke_v2228 的 README 树尾 pin 已更新为 + smoke_v2229_metall',
  s2228.includes('smoke_v2228_titlesave + smoke_v2229_metall（npm test 串跑）'));
ok('smoke_v2228 的 package.json 件套计数 pin 已更新为 === 125', s2228.includes('testChain === 125'));
ok('smoke_v2227 的 GAME_VERSION 字面量 pin 已更新为 v22.29', s2227.includes("const GAME_VERSION = 'v22.29';"));
...
ok('smoke_v2226 的 README 件套 pin 已更新为一百二十五件套（一百二十四件套清除）',
