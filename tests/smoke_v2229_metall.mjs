...
ok('GAME_VERSION 字面量已为 v22.30（旧 v22.28 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.89';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "28';"));
...
ok('ACH_LIST 精确总数 45 项（本版独占精确计数，44→45）', ACH_LIST.length === 75, String(ACH_LIST.length));
...
ok('README 件套口径为二百一十五件套（二百一十四件套清除）',
  readme.includes('冒烟二百一十五件套（二百一十四件套清除）') && !readme.includes('一百二十四件套（一百二十三件套清' + '除）'));
...
ok('README 成就计数双处口径 45 项（C 行 + 图鉴&成就段，44 项清除）',
  readme.includes('成就一览（全部 75 项进度') && readme.includes('**75 项成就**') && !readme.includes('**44 项成就**'));
...
ok('package.json test 串共 125 件套', testChain === 215, String(testChain));
...
ok('smoke_v2228 的 GAME_VERSION 字面量 pin 已更新为 v22.30', s2228.includes("const GAME_VERSION = 'v23.89';"));
ok('smoke_v2228 的 GAME_VERSION 恒等 pin 已更新为 === v22.30', s2228.includes("GAME_VERSION === 'v23.89'"));
ok('smoke_v2228 的 README 件套 pin 已更新为二百一十五件套（二百一十四件套清除）',
  s2228.includes('二百一十五件套（二百一十四件套清除）'));
...
ok('smoke_v2228 的 package.json 件套计数 pin 已更新为 === 125', s2228.includes('testChain === 215'));
ok('smoke_v2227 的 GAME_VERSION 字面量 pin 已更新为 v22.30', s2227.includes("const GAME_VERSION = 'v23.89';"));
...
ok('smoke_v2226 的 README 件套 pin 已更新为二百一十五件套（二百一十四件套清除）',
