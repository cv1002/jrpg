...
// CHANGELOG 同步（冒烟二百二十一件套（二百二十件套清除）/串尾/入库 205 份/顶 pin）+ 姊妹件套 pin
...
ok('data.js GAME_VERSION 字面量已为 v23.35（旧 v23.33 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.97';") && !dataSrc.includes("const GAME_VERSION = 'v23." + "33';"));
...
ok('README 装备属性行开头逐字（武器 atk 六档·防具 def 五档·BEST_ARMOR 同源派生，v23.96 随新现实更新）',
  readme.includes('| 装备属性 | 武器 atk：木剑 2 / 铁剑 5 / 秘银剑 9 / 星铁剑 12 / 勇者之剑 15 / 圣光之剑 24（`legend`，Boss 首败必得·无售价') &&
  readme.includes('防具 def：布衣 1 / 皮甲 4 / 锁子甲 8 / 精钢甲 10 / 龙鳞甲 13（`BEST_ARMOR` 由 `ARMORS` 防御最大值派生'));
ok('README 装备属性行 补录标记与常量源逐字（v23.37 补录、v23.92 补星铁剑、v23.96 补精钢甲）',
...
ok('README 件套口径为二百二十一件套（二百二十件套清除）且旧 204 口径零残留',
  readme.includes('冒烟二百二十一件套（二百二十件套清除）') && !readme.includes('冒烟二百零四件套（二百零三件套清' + '除）'));
...
ok('package.json test 串共 205 件套', testChain === 221, String(testChain));
ok('CHANGELOG 顶部已追加 v23.35 条目', changelog.startsWith('## v23.97 '));
...
ok('smoke_v2308 的 GAME_VERSION 字面量 pin 已更新为 v23.35', s2308.includes("const GAME_VERSION = 'v23.97';"));
ok('smoke_v2308 的 CHANGELOG 顶 pin 已更新为 ## v23.36',
  s2308.includes("startsWith('## v23.97 "));
ok('smoke_v2308 的件套 pin 已更新为二百二十一件套（二百二十件套清除）',
  s2308.includes('二百二十一件套（二百二十件套清除）'));
...
ok('smoke_v2308 的 testChain pin 已更新为 205', s2308.includes('testChain === 221'));
