...
// CHANGELOG 同步（冒烟二百二十件套（二百一十九件套清除）/串尾/入库 205 份/顶 pin）+ 姊妹件套 pin
...
ok('data.js GAME_VERSION 字面量已为 v23.35（旧 v23.33 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.96';") && !dataSrc.includes("const GAME_VERSION = 'v23." + "33';"));
...
ok('README 件套口径为二百二十件套（二百一十九件套清除）且旧 204 口径零残留',
  readme.includes('冒烟二百二十件套（二百一十九件套清除）') && !readme.includes('冒烟二百零四件套（二百零三件套清' + '除）'));
...
ok('package.json test 串共 205 件套', testChain === 220, String(testChain));
ok('CHANGELOG 顶部已追加 v23.35 条目', changelog.startsWith('## v23.96 '));
...
ok('smoke_v2308 的 GAME_VERSION 字面量 pin 已更新为 v23.35', s2308.includes("const GAME_VERSION = 'v23.96';"));
ok('smoke_v2308 的 CHANGELOG 顶 pin 已更新为 ## v23.36',
  s2308.includes("startsWith('## v23.96 "));
ok('smoke_v2308 的件套 pin 已更新为二百二十件套（二百一十九件套清除）',
  s2308.includes('二百二十件套（二百一十九件套清除）'));
...
ok('smoke_v2308 的 testChain pin 已更新为 205', s2308.includes('testChain === 220'));
ok('smoke_v2308 的版本锚已越过 v23.08 口径（>= 8 对 v23.09 恒真）', s2308.includes('_gv[1] >= 8'));
ok('smoke_v2143 哨兵链已推进至二百二十一件套（二百二十件套清除）',
