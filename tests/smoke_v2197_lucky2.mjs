...
ok('GAME_VERSION 字面量已为 v21.97', GAME_VERSION === 'v22.10', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v21.97', dataSrc.includes("const GAME_VERSION = 'v22.10';"));
...
ok('README tests 树收录 smoke_v2199_rich2 且位于串尾（v2199 随新现实由当版冒烟守护）',
  readme.includes('smoke_v2197_lucky2 + smoke_v2198_winsave + smoke_v2199_rich2 + smoke_v2200_stock + smoke_v2201_fragstatus + smoke_v2202_fragwin + smoke_v2204_brew2 + smoke_v2209_achstatus + smoke_v2210_unsaved（npm test 串跑）'));
ok('README 件套口径为一百零六件套（一百零五件套清除）',
  readme.includes('冒烟一百零六件套（一百零五件套清除）') && !readme.includes('冒烟九十二件套（九十一件套清除）'));
