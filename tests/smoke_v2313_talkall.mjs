...
ok('data.js GAME_VERSION 字面量已为 v23.13（旧 v23.12 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.15';") && !dataSrc.includes("const GAME_VERSION = 'v23.12';"));
...
ok('README tests 树串尾已延伸至 smoke_v2313_talkall（v2312 后接 v2313）',
  readme.includes('+ smoke_v2312_voltitle + smoke_v2313_talkall + smoke_v2314_voices + smoke_v2315_talkfoot（npm test 串跑）'));
ok('README 旧串尾零残留（smoke_v2312_voltitle（npm test 串跑）不在树尾）',
  !readme.includes('smoke_v2312_voltitle（npm test 串跑）'));
ok('README 件套口径为二百一十一件套（二百一十件套清除）且旧 208 口径零残留',
  readme.includes('冒烟二百一十一件套（二百一十件套清除）') && !readme.includes('冒烟二百零八件套（二百零七件套清' + '除）'));
...
ok('package.json test 串共 209 件套', testChain === 211, String(testChain));
ok('CHANGELOG 顶部已追加 v23.13 条目', changelog.startsWith('## v23.15 '));
...
ok('smoke_v2312 的 GAME_VERSION 字面量 pin 已更新为 v23.13', s2312.includes("const GAME_VERSION = 'v23.15';"));
ok('smoke_v2312 的 CHANGELOG 顶 pin 已更新为 ## v23.13',
  s2312.includes("startsWith('## v23.15 "));
ok('smoke_v2312 的件套 pin 已更新为二百一十一件套（二百一十件套清除）', s2312.includes('二百一十一件套（二百一十件套清除）'));
ok('smoke_v2312 的 README 串尾 pin 已延伸至 smoke_v2313_talkall',
  s2312.includes('smoke_v2312_voltitle + smoke_v2313_talkall + smoke_v2314_voices + smoke_v2315_talkfoot（npm test 串跑）'));
...
ok('smoke_v2312 的 testChain pin 已更新为 209', s2312.includes('testChain === 211'));
...
ok('smoke_v2143 哨兵链已推进至二百一十二件套（二百一十一件套清除）', s2143.includes('二百一十二件套（二百一十一件套清除）') && s2143.includes("!readme.includes('二百一十二件套（二百一十一件套清除）')"));
