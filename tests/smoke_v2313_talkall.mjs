...
import { GAME_VERSION, ACH_LIST, NPCS, NPC_SPOTS, DEFLECT_GOAL, CHARGE_GOAL, CRIT_GOAL, CAST_GOAL, FLEE_GOAL, POTION_USE_GOAL, RUSH_CLEAR_GOAL, MAP_POTION_GOAL, INN_REST_GOAL, SPEND_GOAL, TRAVEL_GOAL, STEP_GOAL, TRAVEL_LIST, TY, HELP_PAGES } from '../js/data.js';
...
ok('data.js GAME_VERSION 字面量已为 v23.13（旧 v23.12 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.12';"));
...
ok('ACH_LIST 精确总数 62 项（v23.54 战斗维度第二枚「蓄势待发」入列 61→62）', ACH_LIST.length === 72, String(ACH_LIST.length));
...
ok('data.js GAME_VERSION 已级联 v23.40（旧 v23.38 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.38';"));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.40', dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.35';"));
ok('README 同步（C 行 62 项 / 成就 bullet 62 项·以守为攻 X/15 次 / 成就档位行 DEFLECT_GOAL(15)·共 72 项 / 战斗防御句）',
  readme.includes('全部 72 项进度') && readme.includes('**72 项成就**') && readme.includes('以守为攻 X/15 次（防御反击累计，v23.36）') &&
  readme.includes('DEFLECT_GOAL`(15) 次，v23.36') && readme.includes('共 72 项') && readme.includes('15 次解锁成就「以守为攻」'));
ok('CHANGELOG 顶部已追加 v23.40 条目', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.41（旧 v23.40 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.40';"));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.42（旧 v23.41 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.41';"));
...
ok('CHANGELOG 顶部已追加 v23.42 条目（新支线夜路的狼嚎）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.43（旧 v23.42 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.42';"));
...
ok('CHANGELOG 顶部已追加 v23.43 条目（暴击专属上扬音）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.44（旧 v23.43 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.43';"));
...
ok('CHANGELOG 顶部已追加 v23.44 条目（指令栏 [5]防御 回蓝/反击 预览补全）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.45（旧 v23.44 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.44';"));
...
ok('CHANGELOG 顶部已追加 v23.45 条目（Boss/试炼战专属战斗 BGM）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.46（旧 v23.45 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.45';"));
...
ok('CHANGELOG 顶部已追加 v23.46 条目（真身变身专属音效）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.47（旧 v23.46 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.46';"));
...
ok('CHANGELOG 顶部已追加 v23.47 条目（蓄力专属音效）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.48（旧 v23.47 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.47';"));
...
ok('CHANGELOG 顶部已追加 v23.48 条目（敌方暗影回血专属音效）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.49（旧 v23.48 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.48';"));
...
ok('CHANGELOG 顶部已追加 v23.49 条目（宝箱开启专属音效）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.50（旧 v23.49 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.49';"));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.52（旧 v23.50 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.50';"));
...
ok('CHANGELOG 顶部已追加 v23.51 条目（敌方石甲专属音效）', changelog.startsWith('## v23.78 '));
...
ok('README tests 树串尾已延伸至 smoke_v2313_talkall（v2312 后接 v2313）',
  readme.includes('+ smoke_v2312_voltitle + smoke_v2313_talkall + smoke_v2314_voices + smoke_v2315_talkfoot + smoke_v2316_voiceshead + smoke_v2317_pausemap + smoke_v2318_battlemap（npm test 串跑）'));
ok('README 旧串尾零残留（smoke_v2312_voltitle（npm test 串跑）不在树尾）',
  !readme.includes('smoke_v2312_voltitle（npm test 串跑）'));
ok('README 件套口径为二百一十四件套（二百一十三件套清除）且旧 208 口径零残留',
  readme.includes('冒烟二百一十四件套（二百一十三件套清除）') && !readme.includes('冒烟二百零八件套（二百零七件套清' + '除）'));
ok('README 不含哨兵领先一位（二百一十五件套（二百一十四件套清除））',
  !readme.includes('二百一十五件套（二百一十四件套清除）'));
...
ok('README 数值速查成就档位行含社交档「有口皆碑」与共 72 项',
  readme.includes('社交向单档「有口皆碑」') && readme.includes('共 72 项'));
...
ok('package.json 串尾为 ... smoke_v2313_talkall.mjs && node tests/smoke_v2314_voices.mjs && node tests/smoke_v2315_talkfoot.mjs && node tests/smoke_v2316_voiceshead.mjs && node tests/smoke_v2317_pausemap.mjs && node tests/smoke_v2318_battlemap.mjs"',
  pkg.includes('node tests/smoke_v2312_voltitle.mjs && node tests/smoke_v2313_talkall.mjs && node tests/smoke_v2314_voices.mjs && node tests/smoke_v2315_talkfoot.mjs && node tests/smoke_v2316_voiceshead.mjs && node tests/smoke_v2317_pausemap.mjs && node tests/smoke_v2318_battlemap.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 209 件套', testChain === 214, String(testChain));
ok('CHANGELOG 顶部已追加 v23.41 条目', changelog.startsWith('## v23.78 '));
...
ok('smoke_v2312 的 GAME_VERSION 字面量 pin 已更新为 v23.13', s2312.includes("const GAME_VERSION = 'v23.78';"));
ok('smoke_v2312 的 CHANGELOG 顶 pin 已更新为 ## v23.13',
  s2312.includes("startsWith('## v23.78 "));
ok('smoke_v2312 的件套 pin 已更新为二百一十四件套（二百一十三件套清除）', s2312.includes('二百一十四件套（二百一十三件套清除）'));
ok('smoke_v2312 的 README 串尾 pin 已延伸至 smoke_v2313_talkall',
  s2312.includes('smoke_v2312_voltitle + smoke_v2313_talkall + smoke_v2314_voices + smoke_v2315_talkfoot + smoke_v2316_voiceshead + smoke_v2317_pausemap + smoke_v2318_battlemap（npm test 串跑）'));
ok('smoke_v2312 的 package 串尾 pin 已延伸至 smoke_v2313_talkall',
  s2312.includes('node tests/smoke_v2312_voltitle.mjs && node tests/smoke_v2313_talkall.mjs && node tests/smoke_v2314_voices.mjs && node tests/smoke_v2315_talkfoot.mjs && node tests/smoke_v2316_voiceshead.mjs && node tests/smoke_v2317_pausemap.mjs && node tests/smoke_v2318_battlemap.mjs"'));
ok('smoke_v2312 的 testChain pin 已更新为 209', s2312.includes('testChain === 214'));
ok('smoke_v2297 的 ACH_LIST 精确计数 pin 已更新为 === 60', s2297.includes('ACH_LIST.length === 72'));
ok('smoke_v2229 的 ACH_LIST 精确计数 pin 已更新为 === 60', s2229.includes('ACH_LIST.length === 72'));
ok('smoke_v2143 哨兵链已推进至二百一十五件套（二百一十四件套清除）', s2143.includes('二百一十五件套（二百一十四件套清除）') && s2143.includes("!readme.includes('二百一十五件套（二百一十四件套清除）')"));
...
ok('CHANGELOG 顶部已追加 v23.52 条目（improve-plan 落地状态对齐）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.53（旧 v23.52 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.52';"));
...
ok('CHANGELOG 顶部已追加 v23.53 条目（精英「重击」逐招预判补全）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.54（旧 v23.53 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.53';"));
...
ok('README 同步（C 行 62 项 / 成就 bullet 62 项·蓄势待发 X/15 次 / 成就档位行 CHARGE_GOAL(15)·共 72 项 / 战斗蓄力句）',
  readme.includes('全部 72 项进度') && readme.includes('**72 项成就**') && readme.includes('蓄势待发 X/15 次（蓄力累计，v23.54）') &&
  readme.includes('CHARGE_GOAL`(15) 次，v23.54') && readme.includes('共 72 项') && readme.includes('15 次解锁成就「蓄势待发」'));
ok('CHANGELOG 顶部已追加 v23.54 条目（新成就蓄势待发）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.55（旧 v23.54 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.54';"));
...
ok('CHANGELOG 顶部已追加 v23.55 条目（Lv12 终章新技能灯焰长明）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.56（旧 v23.55 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.55';"));
...
ok('CHANGELOG 顶部已追加 v23.56 条目（试炼战报补经验）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.57（旧 v23.56 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.56';"));
...
ok('CHANGELOG 顶部已追加 v23.57 条目（新支线蛇影的药引）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.58（旧 v23.57 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.57';"));
...
ok('CHANGELOG 顶部已追加 v23.58 条目（潮灯镇行酿药师指针）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.59（旧 v23.58 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.59';"));
...
ok('CHANGELOG 顶部已追加 v23.59 条目（记忆碎片拾取音效归位）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.61（旧 v23.60 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.60';"));
...
ok('CHANGELOG 顶部已追加 v23.61 条目（状态页技能节头计数）', changelog.startsWith('## v23.78 '));
...
// 有进度、图鉴行此前查无一行，现按 quests.questKillProg 单一数据源在行第二行追加「 · 📜 支线 N/M」）——
{
  ...
  ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.62（旧 v23.61 字面量零残留）',
    dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.61';"));
  ...
  ok('CHANGELOG 顶部已追加 v23.62 条目（记忆图鉴行讨伐支线进度角标）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.63（旧 v23.62 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.62';"));
...
ok('README 同步（C 行 63 项 / 成就 bullet 63 项·暴击如雨 X/20 次 / 成就档位行 CRIT_GOAL(20)·共 72 项 / 战斗暴击句）',
  readme.includes('全部 72 项进度') && readme.includes('**72 项成就**') && readme.includes('暴击如雨 X/20 次（普攻暴击累计，v23.63）') &&
  readme.includes('CRIT_GOAL`(20) 次，v23.63') && readme.includes('共 72 项') && readme.includes('20 次解锁成就「暴击如雨」'));
ok('CHANGELOG 顶部已追加 v23.63 条目（新成就暴击如雨）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.64（旧 v23.63 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.63';"));
...
ok('README 同步（C 行 64 项 / 成就 bullet 64 项·熟能生巧 X/30 次 / 成就档位行 CAST_GOAL(30)·共 72 项 / 战斗技能句）',
  readme.includes('全部 72 项进度') && readme.includes('**72 项成就**') && readme.includes('熟能生巧 X/30 次（技能累计释放，v23.64）') &&
  readme.includes('CAST_GOAL`(30) 次，v23.64') && readme.includes('共 72 项') && readme.includes('30 次解锁成就「熟能生巧」'));
ok('CHANGELOG 顶部已追加 v23.64 条目（新成就熟能生巧）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.65（旧 v23.64 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.64';"));
...
ok('README 同步（C 行 65 项 / 成就 bullet 65 项·走为上计 X/10 次 / 成就档位行 FLEE_GOAL(10)·共 72 项 / 战斗逃跑句）',
  readme.includes('全部 72 项进度') && readme.includes('**72 项成就**') && readme.includes('走为上计 X/10 次（逃跑成功累计，v23.65）') &&
  readme.includes('FLEE_GOAL`(10) 次，v23.65') && readme.includes('共 72 项') && readme.includes('10 次解锁成就「走为上计」'));
ok('CHANGELOG 顶部已追加 v23.65 条目（新成就走为上计）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.66（旧 v23.65 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.65';"));
...
ok('README 同步（C 行 66 项 / 成就 bullet 66 项·药到病除 X/15 次 / 成就档位行 POTION_USE_GOAL(15)·共 72 项 / 战斗用药句）',
  readme.includes('全部 72 项进度') && readme.includes('**72 项成就**') && readme.includes('药到病除 X/15 次（战斗用药累计，v23.66）') &&
  readme.includes('POTION_USE_GOAL`(15) 次，v23.66') && readme.includes('共 72 项') && readme.includes('15 次解锁成就「药到病除」'));
ok('CHANGELOG 顶部已追加 v23.66 条目（新成就药到病除）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已随新现实级联为 v23.67（旧 v23.66 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.66';"));
...
ok('README 同步（C 行 68 项 / 成就 bullet 68 项·千锤百炼 X/3 次 / 成就档位行 RUSH_CLEAR_GOAL(3)·共 72 项 / 试炼碑句）',
  readme.includes('全部 72 项进度') && readme.includes('**72 项成就**') && readme.includes('千锤百炼 X/3 次（试炼场累计通关，v23.67）') &&
  readme.includes('RUSH_CLEAR_GOAL`(3) 次，v23.67') && readme.includes('共 72 项') && readme.includes('3 次解锁成就「千锤百炼」'));
ok('CHANGELOG 顶部已追加 v23.67 条目（新成就千锤百炼）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已为 v23.68（旧 v23.67 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.67';"));
...
ok('CHANGELOG 顶部已追加 v23.69 条目（状态页 C/B 直达）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已为 v23.69（旧 v23.68 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.68';"));
...
ok('CHANGELOG 顶部已追加 v23.69 条目（四收集页互切网格）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已为 v23.71（旧 v23.69 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.69';"));
...
ok('CHANGELOG 顶部已追加 v23.70 条目（帮助页补四收集页直达，当前顶 pin v23.71）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已为 v23.71（旧 v23.70 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.70';"));
...
ok('CHANGELOG 顶部已追加 v23.71 条目（收集页补 H 帮助直达）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已为 v23.72（旧 v23.71 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.71';"));
...
ok('ACH_LIST 精确总数 68 项（v23.72 旅中补给首枚入列 67→68）', ACH_LIST.length === 72, String(ACH_LIST.length));
...
ok('README 同步（C 行 68 项 / 成就 bullet 68 项·渴饮甘露 X/10 次 / 成就档位行 MAP_POTION_GOAL(10)·共 72 项 / F 行）',
  readme.includes('全部 72 项进度') && readme.includes('**72 项成就**') &&
  readme.includes('渴饮甘露 X/10 次（大地图按 F 喝药累计，v23.72）') &&
  readme.includes('`MAP_POTION_GOAL`(10) 次，v23.72') && readme.includes('共 72 项') &&
  readme.includes('`MAP_POTION_GOAL`(10) 次解锁成就「渴饮甘露」'));
ok('CHANGELOG 顶部已追加 v23.72 条目（新成就渴饮甘露）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已为 v23.73（旧 v23.72 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.72';"));
...
ok('ACH_LIST 精确总数 69 项（v23.73 旅中休整首枚入列 68→69）', ACH_LIST.length === 72, String(ACH_LIST.length));
...
ok('README 同步（C 行 69 项 / 成就 bullet 69 项·夜宿灯下 X/15 次 / 成就档位行 INN_REST_GOAL(15)·共 72 项 / 经济循环行）',
  readme.includes('全部 72 项进度') && readme.includes('**72 项成就**') &&
  readme.includes('夜宿灯下 X/15 次（旅馆住宿累计，v23.73）') &&
  readme.includes('`INN_REST_GOAL`(15) 次，v23.73') && readme.includes('共 72 项') &&
  readme.includes('次解锁成就「夜宿灯下」') && readme.includes('`INN_REST_GOAL`'));
ok('CHANGELOG 顶部已追加 v23.73 条目（新成就夜宿灯下）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已为 v23.76（旧 v23.73 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.73';"));
...
ok('ACH_LIST 精确总数 70 项（v23.74 经济消费首枚入列 69→70）', ACH_LIST.length === 72, String(ACH_LIST.length));
...
ok('README 同步（C 行 70 项 / 成就 bullet 70 项·一掷千金 X/1000 金 / 成就档位行 SPEND_GOAL(1000)·共 72 项 / 经济循环行）',
  readme.includes('全部 72 项进度') && readme.includes('**72 项成就**') &&
  readme.includes('一掷千金 X/1000 金（累计消费，v23.74）') &&
  readme.includes('`SPEND_GOAL`(1000) 金，v23.74') && readme.includes('共 72 项') &&
  readme.includes('累计消费 1000 金解锁成就「一掷千金」') && readme.includes('`SPEND_GOAL`'));
ok('CHANGELOG 顶部已追加 v23.75 条目（新成就一掷千金）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已为 v23.76（旧 v23.74 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.74';"));
...
ok('ACH_LIST 精确总数 72 项（v23.75 旅行动作首枚入列 70→71）', ACH_LIST.length === 72, String(ACH_LIST.length));
...
ok('README 同步（C 行 72 项 / 成就 bullet 72 项·行者无疆 X/15 次 / T 行 / 成就档位行 TRAVEL_GOAL(15)·共 72 项）',
  readme.includes('全部 72 项进度') && readme.includes('**72 项成就**') &&
  readme.includes('行者无疆 X/15 次（快速旅行累计，v23.75）') &&
  readme.includes('`TRAVEL_GOAL`(15) 次，v23.75') && readme.includes('共 72 项') &&
  readme.includes('`TRAVEL_GOAL`(15) 次解锁成就「行者无疆」') && readme.includes('`TRAVEL_GOAL`'));
ok('CHANGELOG 顶部已追加 v23.75 条目（新成就行者无疆）', changelog.startsWith('## v23.78 '));
...
ok('data.js GAME_VERSION 字面量已为 v23.76（旧 v23.75 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.75';"));
...
ok('ACH_LIST 精确总数 72 项（v23.76 步行维度首枚入列 71→72）', ACH_LIST.length === 72, String(ACH_LIST.length));
...
ok('README 同步（C 行 72 项 / 成就 bullet 72 项·千里之行 X/1000 步 / 成就档位行 STEP_GOAL(1000)·共 72 项）',
  readme.includes('全部 72 项进度') && readme.includes('**72 项成就**') &&
  readme.includes('千里之行 X/1000 步（步行累计，v23.76）') &&
  readme.includes('`STEP_GOAL`(1000) 步，v23.76') && readme.includes('共 72 项') &&
  readme.includes('`STEP_GOAL`'));
ok('CHANGELOG 顶部已追加 v23.76 条目（新成就千里之行）', changelog.startsWith('## v23.78 '));
...
// —— v23.78 守护：暂停菜单「当前所在地」（体验打磨·信息透明·纯显示；源级落位 + 运行期真实 drawPause 头部两档捕获；承 v19.89 状态页 📍/ v23.27 快速旅行当前所在地标注同一「我在哪」口径）——
const menusSrc77 = read('../js/view/menus.js');
ok('data.js 含 v23.78 版本注释（暂停菜单当前所在地说明）', dataSrc.includes('v23.78 体验打磨·信息透明·纯显示'));
ok('data.js GAME_VERSION 字面量已为 v23.78（旧 v23.76 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v23.7" + "6';"));
...
// —— v23.78 级联守护：旧代 v23.76 GAME_VERSION/恒等/顶 pin/件套/testChain/串尾 全库零残留（仅 v23.76 特性标签保留）——
const stale77 = [];
for (const f of allTests) {
  const src = read(f);
