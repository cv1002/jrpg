...
ok('data.js GAME_VERSION 字面量已为 v23.14（旧 v23.13 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.82';") && !dataSrc.includes("const GAME_VERSION = 'v23.13';"));
...
ok('ACH_LIST 含 talkall（v23.13 成就零回归）且总数 60', !!ACH_LIST.find((a) => a.id === 'talkall') && ACH_LIST.length === 74, String(ACH_LIST.length));
...
ok('README tests 树串尾已延伸至 smoke_v2314_voices（v2313 后接 v2314）',
  readme.includes('+ smoke_v2313_talkall + smoke_v2314_voices + smoke_v2315_talkfoot + smoke_v2316_voiceshead + smoke_v2317_pausemap + smoke_v2318_battlemap + smoke_v2319_deadloc（npm test 串跑）'));
ok('README 旧串尾零残留（smoke_v2313_talkall（npm test 串跑）不在树尾）',
  !readme.includes('smoke_v2313_talkall（npm test 串' + '跑）'));
ok('README 件套口径为二百一十五件套（二百一十四件套清除）且旧 209 口径零残留',
  readme.includes('冒烟二百一十五件套（二百一十四件套清除）') && !readme.includes('冒烟二百零九件套（二百零八件套清' + '除）'));
...
ok('package.json 串尾为 ... smoke_v2313_talkall.mjs && node tests/smoke_v2314_voices.mjs && node tests/smoke_v2315_talkfoot.mjs && node tests/smoke_v2316_voiceshead.mjs && node tests/smoke_v2317_pausemap.mjs && node tests/smoke_v2318_battlemap.mjs && node tests/smoke_v2319_deadloc.mjs"',
  pkg.includes('node tests/smoke_v2313_talkall.mjs && node tests/smoke_v2314_voices.mjs && node tests/smoke_v2315_talkfoot.mjs && node tests/smoke_v2316_voiceshead.mjs && node tests/smoke_v2317_pausemap.mjs && node tests/smoke_v2318_battlemap.mjs && node tests/smoke_v2319_deadloc.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 210 件套', testChain === 215, String(testChain));
ok('CHANGELOG 顶部已追加 v23.14 条目', changelog.startsWith('## v23.82 '));
...
ok('smoke_v2313 的 GAME_VERSION 字面量 pin 已更新为 v23.14', s2313.includes("const GAME_VERSION = 'v23.82';"));
ok('smoke_v2313 的 CHANGELOG 顶 pin 已更新为 ## v23.14', s2313.includes("startsWith('## v23.82 "));
ok('smoke_v2313 的件套 pin 已更新为二百一十五件套（二百一十四件套清除）', s2313.includes('二百一十五件套（二百一十四件套清除）'));
ok('smoke_v2313 的 README 串尾 pin 已延伸至 smoke_v2314_voices',
  s2313.includes('smoke_v2313_talkall + smoke_v2314_voices + smoke_v2315_talkfoot + smoke_v2316_voiceshead + smoke_v2317_pausemap + smoke_v2318_battlemap + smoke_v2319_deadloc（npm test 串跑）'));
ok('smoke_v2313 的 package 串尾 pin 已延伸至 smoke_v2314_voices',
  s2313.includes('node tests/smoke_v2313_talkall.mjs && node tests/smoke_v2314_voices.mjs && node tests/smoke_v2315_talkfoot.mjs && node tests/smoke_v2316_voiceshead.mjs && node tests/smoke_v2317_pausemap.mjs && node tests/smoke_v2318_battlemap.mjs && node tests/smoke_v2319_deadloc.mjs"'));
ok('smoke_v2313 的 testChain pin 已更新为 210', s2313.includes('testChain === 215'));
