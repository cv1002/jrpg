...
ok('GAME_VERSION 格式合法且已越过 v21.86', !!_gv && (_gv[0] > 21 || (_gv[0] === 21 && _gv[1] >= 99)), GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v21.87', dataSrc.includes("const GAME_VERSION = 'v23.87';"));
...
ok('drawEnding 页脚文案落位（按 Enter/E 返回标题，v22.85 补 E）', endBlock.includes("按 Enter/E 返回标题"));
...
ok('运行期：页脚文案落位（按 Enter/E 返回标题，v22.85 补 E）', cap.some((t) => t.includes('按 Enter/E 返回标题')));
...
ok('README 件套口径已更新为二百一十五件套（二百一十四件套清除）',
  readme.includes('冒烟二百一十五件套（二百一十四件套清除）') && !readme.includes('冒烟八十二件套（八十一件套清除）'));
