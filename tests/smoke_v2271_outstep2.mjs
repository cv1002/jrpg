...
ok('data.js GAME_VERSION 字面量已为 v22.71（旧 v22.70 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.78';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "70';"));
...
ok('ACH_LIST 精确总数 55 项（v2229 精确计数 pin 随新现实更新 54→55）', ACH_LIST.length === 72, String(ACH_LIST.length));
...
ok('README 件套口径为二百一十四件套（二百一十三件套清除）且旧 166 口径零残留',
  readme.includes('冒烟二百一十四件套（二百一十三件套清除）') && !readme.includes('冒烟一百六十六件套（一百六十五件套清' + '除）'));
...
ok('package.json test 串共 167 件套', testChain === 214, String(testChain));
ok('CHANGELOG 顶为 v22.71 条目', changelog.startsWith('## v23.78 '));
...
ok('smoke_v2269 的 GAME_VERSION 字面量 pin 已更新为 v22.71', s2269.includes("const GAME_VERSION = 'v23.78';"));
ok('smoke_v2269 的 README 件套 pin 已随新现实更新为二百一十四件套（二百一十三件套清除）',
  s2269.includes('二百一十四件套（二百一十三件套清除）'));
ok('smoke_v2269 的 package.json 件套计数 pin 已更新为 === 167', s2269.includes('testChain === 214'));
ok('smoke_v2269 的 CHANGELOG 顶 pin 已更新为 ## v22.72', s2269.includes("startsWith('## v23.78'") || s2269.includes("startsWith('## v22.78 '"));
...
ok('smoke_v2269 的 README 成就 pin 已随新现实更新为 55 项双处', s2269.includes('成就一览（全部 72 项进度') && s2269.includes('**72 项成就**'));
ok('smoke_v2269 的 ACH_LIST 精确计数 pin 已更新为 === 55', s2269.includes('ACH_LIST.length === 72'));
ok('smoke_v2269 的版本锚点已随新现实推进至 >= 71', s2269.includes('_gv[0] === 22 && _gv[1] >= 99'));
ok('smoke_v2268 的 GAME_VERSION 字面量 pin 已更新为 v22.71', s2268.includes("const GAME_VERSION = 'v23.78';"));
ok('smoke_v2268 的 README 件套 pin 已随新现实更新为二百一十四件套（二百一十三件套清除）',
  s2268.includes('二百一十四件套（二百一十三件套清除）'));
ok('smoke_v2268 的 package.json 件套计数 pin 已更新为 === 167', s2268.includes('testChain === 214'));
ok('smoke_v2268 的 CHANGELOG 顶 pin 已更新为 ## v22.72', s2268.includes("startsWith('## v23.78'") || s2268.includes("startsWith('## v22.78 '"));
...
ok('smoke_v2268 的 README 成就 pin 已随新现实更新为 55 项双处', s2268.includes('成就一览（全部 72 项进度') && s2268.includes('**72 项成就**'));
ok('smoke_v2268 的 ACH_LIST 精确计数 pin 已更新为 === 55', s2268.includes('ACH_LIST.length === 72'));
ok('smoke_v2268 的版本锚点已随新现实推进至 >= 71', s2268.includes('_gv[0] === 22 && _gv[1] >= 99'));
ok('smoke_v2267 的 GAME_VERSION 字面量 pin 已更新为 v22.71', s2267.includes("const GAME_VERSION = 'v23.78';"));
ok('smoke_v2267 的 README 件套 pin 已随新现实更新为二百一十四件套（二百一十三件套清除）',
  s2267.includes('二百一十四件套（二百一十三件套清除）'));
ok('smoke_v2267 的 package.json 件套计数 pin 已更新为 === 167', s2267.includes('testChain === 214'));
ok('smoke_v2267 的 CHANGELOG 顶 pin 已更新为 ## v22.72', s2267.includes("startsWith('## v23.78'") || s2267.includes("startsWith('## v22.78 '"));
...
ok('smoke_v2267 的 README 成就 pin 已随新现实更新为 55 项双处', s2267.includes('成就一览（全部 72 项进度') && s2267.includes('**72 项成就**'));
ok('smoke_v2267 的 ACH_LIST 精确计数 pin 已更新为 === 55', s2267.includes('ACH_LIST.length === 72'));
ok('smoke_v2266 的 GAME_VERSION 字面量 pin 已更新为 v22.71', s2266.includes("const GAME_VERSION = 'v23.78';"));
ok('smoke_v2266 的 README 件套 pin 已随新现实更新为二百一十四件套（二百一十三件套清除）',
  s2266.includes('二百一十四件套（二百一十三件套清除）'));
ok('smoke_v2266 的 package.json 件套计数 pin 已更新为 === 167', s2266.includes('testChain === 214'));
ok('smoke_v2266 的 CHANGELOG 顶 pin 已更新为 ## v22.72', s2266.includes("startsWith('## v23.78'") || s2266.includes("startsWith('## v22.78 '"));
...
ok('smoke_v2266 的 README 成就 pin 已随新现实更新为 55 项双处', s2266.includes('成就一览（全部 72 项进度') && s2266.includes('**72 项成就**'));
ok('smoke_v2266 的 ACH_LIST 精确计数 pin 已更新为 === 55', s2266.includes('ACH_LIST.length === 72'));
ok('smoke_v2265 的 GAME_VERSION 字面量 pin 已更新为 v22.71', s2265.includes("const GAME_VERSION = 'v23.78';"));
...
ok('smoke_v2264 的 GAME_VERSION 字面量 pin 已更新为 v22.71', s2264.includes("const GAME_VERSION = 'v23.78';"));
...
ok('smoke_v2263 的 GAME_VERSION 字面量 pin 已更新为 v22.71', s2263.includes("const GAME_VERSION = 'v23.78';"));
ok('smoke_v2263 的 README 件套 pin 已随新现实更新为二百一十四件套（二百一十三件套清除）',
  s2263.includes('二百一十四件套（二百一十三件套清除）'));
ok('smoke_v2262 的 GAME_VERSION 字面量 pin 已更新为 v22.71', s2262.includes("const GAME_VERSION = 'v23.78';"));
ok('smoke_v2261 的 GAME_VERSION 字面量 pin 已更新为 v22.71', s2261.includes("const GAME_VERSION = 'v23.78';"));
...
ok('smoke_v2259 的 GAME_VERSION 字面量 pin 已更新为 v22.71', s2259.includes("const GAME_VERSION = 'v23.78';"));
ok('smoke_v2258 的 GAME_VERSION 字面量 pin 已更新为 v22.71', s2258.includes("const GAME_VERSION = 'v23.78';"));
ok('smoke_v2270（上一版）的 GAME_VERSION 字面量 pin 已随新现实更新为 v22.71', s2270.includes("const GAME_VERSION = 'v23.78';"));
ok('smoke_v2270 的 CHANGELOG 顶 pin 已随新现实更新为 ## v22.72', s2270.includes("startsWith('## v23.78'") || s2270.includes("startsWith('## v22.78 '"));
ok('smoke_v2270 的 README 成就 pin 已随新现实更新为 55 项双处且 ACH === 55',
  s2270.includes('成就一览（全部 72 项进度') && s2270.includes('**72 项成就**') && s2270.includes('ACH_LIST.length === 72'));
ok('smoke_v2270 的哨兵链 pin 已随新现实推进至 168（二百一十四件套（二百一十三件套清除））',
  s2270.includes('二百一十四件套（二百一十三件套清除）'));
