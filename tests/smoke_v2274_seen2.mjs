...
ok('data.js GAME_VERSION 字面量已为 v22.74（旧 v22.73 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.93';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "73';"));
...
ok('README 件套口径为二百一十七件套（二百一十六件套清除）且旧 169 口径零残留',
  readme.includes('冒烟二百一十七件套（二百一十六件套清除）') && !readme.includes('冒烟一百六十九件套（一百六十八件套清' + '除）'));
...
ok('package.json test 串共 170 件套', testChain === 217, String(testChain));
ok('CHANGELOG 含 v22.74 条目（顶 pin）', changelog.startsWith('## v23.93 '));
...
ok('smoke_v2273 的 GAME_VERSION 字面量 pin 已更新为 v22.74', s2273.includes("const GAME_VERSION = 'v23.93';"));
ok('smoke_v2273 的 CHANGELOG 顶 pin 已更新为 ## v22.74', s2273.includes("startsWith('## v23.93'") || s2273.includes("startsWith('## v22.78 '"));
ok('smoke_v2273 的件套 pin 已更新为二百一十七件套（二百一十六件套清除）', s2273.includes('二百一十七件套（二百一十六件套清除）'));
