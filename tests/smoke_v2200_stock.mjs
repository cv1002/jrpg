...
ok('GAME_VERSION 字面量已为 v22.0', GAME_VERSION === 'v23.98', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.0', dataSrc.includes("const GAME_VERSION = 'v23.98';"));
...
ok('README 件套口径为二百二十二件套（二百二十一件套清除）',
  readme.includes('冒烟二百二十二件套（二百二十一件套清除）') && !readme.includes('冒烟九十五件套（九十四件套清' + '除）'));
...
for (const nm of suite96) {
  ...
  ok(`${nm} 的 README 件套 pin 已随新现实更新为二百二十二件套（二百二十一件套清除）`,
    src.includes('二百二十二件套（二百二十一件套清除）'));
}
...
for (const nm of vers) {
  ...
  ok(`${nm} 的 GAME_VERSION 字面量 pin 已随新现实更新为 v22.0`,
    src.includes("const GAME_VERSION = 'v23.98';"));
}
for (const nm of ['smoke_v2199_rich2.mjs', 'smoke_v2198_winsave.mjs', 'smoke_v2197_lucky2.mjs',
  ...
  ok(`${nm} 的 GAME_VERSION 恒等 pin（===）已随新现实更新为 v22.0`,
    src.includes("GAME_VERSION === 'v23.98'"));
}
