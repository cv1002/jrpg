...
ok('GAME_VERSION 字面量已为 v22.0', GAME_VERSION === 'v22.77', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.0', dataSrc.includes("const GAME_VERSION = 'v22.77';"));
...
ok('README 件套口径为一百七十三件套（一百七十二件套清除）',
  readme.includes('冒烟一百七十三件套（一百七十二件套清除）') && !readme.includes('冒烟九十五件套（九十四件套清' + '除）'));
...
for (const nm of suite96) {
  ...
  ok(`${nm} 的 README 件套 pin 已随新现实更新为一百七十三件套（一百七十二件套清除）`,
    src.includes('一百七十三件套（一百七十二件套清除）'));
}
...
for (const nm of vers) {
  ...
  ok(`${nm} 的 GAME_VERSION 字面量 pin 已随新现实更新为 v22.0`,
    src.includes("const GAME_VERSION = 'v22.77';"));
}
for (const nm of ['smoke_v2199_rich2.mjs', 'smoke_v2198_winsave.mjs', 'smoke_v2197_lucky2.mjs',
  ...
  ok(`${nm} 的 GAME_VERSION 恒等 pin（===）已随新现实更新为 v22.0`,
    src.includes("GAME_VERSION === 'v22.77'"));
}
