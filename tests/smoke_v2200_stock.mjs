...
ok('GAME_VERSION 字面量已为 v22.0', GAME_VERSION === 'v22.64', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.0', dataSrc.includes("const GAME_VERSION = 'v22.64';"));
...
ok('README 件套口径为一百六十件套（一百五十九件套清除）',
  readme.includes('冒烟一百六十件套（一百五十九件套清除）') && !readme.includes('冒烟九十五件套（九十四件套清' + '除）'));
...
for (const nm of suite96) {
  ...
  ok(`${nm} 的 README 件套 pin 已随新现实更新为一百六十件套（一百五十九件套清除）`,
    src.includes('一百六十件套（一百五十九件套清除）'));
}
...
for (const nm of vers) {
  ...
  ok(`${nm} 的 GAME_VERSION 字面量 pin 已随新现实更新为 v22.0`,
    src.includes("const GAME_VERSION = 'v22.64';"));
}
for (const nm of ['smoke_v2199_rich2.mjs', 'smoke_v2198_winsave.mjs', 'smoke_v2197_lucky2.mjs',
  ...
  ok(`${nm} 的 GAME_VERSION 恒等 pin（===）已随新现实更新为 v22.0`,
    src.includes("GAME_VERSION === 'v22.64'"));
}
...
for (const nm of achFiles) {
  ...
  ok(`${nm} 的 README 成就 pin 已随新现实更新为 38 项双处落位`,
    src.includes("readme.includes('成就一览（全部 48 项进度'") && src.includes('**48 项成就**'));
}
