...
ok('GAME_VERSION 字面量已为 v22.0', GAME_VERSION === 'v22.1', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.0', dataSrc.includes("const GAME_VERSION = 'v22.1';"));
...
ok('README tests 树收录 smoke_v2200_stock 且位于串尾（v22.0 随新现实由当版冒烟守护）', readme.includes('smoke_v2199_rich2 + smoke_v2200_stock + smoke_v2201_fragstatus（npm test 串跑）'));
ok('README 件套口径为九十七件套（九十六件套清除）',
  readme.includes('冒烟九十七件套（九十六件套清除）') && !readme.includes('冒烟九十五件套（九十四件套清' + '除）'));
...
for (const nm of ['smoke_v2199_rich2.mjs', 'smoke_v2198_winsave.mjs', 'smoke_v2197_lucky2.mjs',
  ...
  ok(`${nm} 的 GAME_VERSION 恒等 pin（===）已随新现实更新为 v22.0`,
    src.includes("GAME_VERSION === 'v22.1'"));
}
