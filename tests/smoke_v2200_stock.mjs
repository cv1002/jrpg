...
ok('GAME_VERSION 字面量已为 v22.0', GAME_VERSION === 'v22.13', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.0', dataSrc.includes("const GAME_VERSION = 'v22.13';"));
ok('data.js 仍保留 v21.99 历史注释（累积注释块，姊妹 pin 不失效）', dataSrc.includes('v21.99 新成就「金玉满堂」'));
ok('data.js 导出 POTIONS_GOAL（export 块落位，与 PLAY_TIME2_GOAL/POTIONS2_GOAL 相邻）', dataSrc.includes('PLAY_TIME_GOAL, PLAY_TIME2_GOAL, POTIONS_GOAL, POTIONS2_GOAL, ELIXIR_STOCK_GOAL, PERFECTION_GOLD'));
...
ok('README tests 树收录 smoke_v2200_stock 且位于串尾（v22.0 随新现实由当版冒烟守护）', readme.includes('smoke_v2199_rich2 + smoke_v2200_stock + smoke_v2201_fragstatus + smoke_v2202_fragwin + smoke_v2204_brew2 + smoke_v2209_achstatus + smoke_v2211_lampkid + smoke_v2212_volume + smoke_v2213_teller（npm test 串跑）'));
ok('README 件套口径为一百零九件套（一百零八件套清除）',
  readme.includes('冒烟一百零九件套（一百零八件套清除）') && !readme.includes('冒烟九十五件套（九十四件套清' + '除）'));
...
for (const nm of achFiles) {
  ...
  ok(`${nm} 的 README 成就 pin 已随新现实更新为 38 项双处落位`,
    src.includes("readme.includes('成就一览（全部 41 项进度'") && src.includes('**41 项成就**'));
}
