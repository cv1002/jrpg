...
ok('GAME_VERSION 字面量已为 v22.21（旧 v22.20 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.71';") && !dSrc.includes("const GAME_VERSION = 'v22." + "20';"));
...
ok('README 件套口径为一百六十七件套（一百六十六件套清除）',
  readme.includes('冒烟一百六十七件套（一百六十六件套清除）') && !readme.includes('冒烟一百一十六件套（一百一十五件套清' + '除）'));
...
ok('package.json test 串共 117 件套', testChain === 167, String(testChain));
...
ok('smoke_v2220 的 GAME_VERSION 字面量 pin 已更新为 v22.21', s2220.includes("const GAME_VERSION = 'v22.71';"));
ok('smoke_v2220 的 GAME_VERSION 恒等 pin 已更新为 === v22.21', s2220.includes("GAME_VERSION === 'v22.71'"));
...
ok('smoke_v2220 的 package.json 件套计数 pin 已更新为 === 117', s2220.includes('testChain === 167'));
ok('smoke_v2220 的 NPC 总数 pin 已随新现实更新为 23（失名的旅人落位）', s2220.includes('总数 29'));
ok('smoke_v2219 的 GAME_VERSION 字面量 pin 已更新为 v22.21', s2219.includes("const GAME_VERSION = 'v22.71';"));
ok('smoke_v2219 的 README 件套 pin 已随新现实更新为一百六十七件套（一百六十六件套清除）',
  s2219.includes('一百六十七件套（一百六十六件套清除）'));
ok('smoke_v2218 的 GAME_VERSION 字面量 pin 已更新为 v22.21', s2218.includes("const GAME_VERSION = 'v22.71';"));
ok('smoke_v2218 的 package.json 件套计数 pin 已更新为 === 117', s2218.includes('testChain === 167'));
