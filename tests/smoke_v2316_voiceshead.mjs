...
ok('data.js GAME_VERSION 字面量已为 v23.16（旧 v23.15 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.77';") && !dataSrc.includes("const GAME_VERSION = 'v23.15';"));
...
ok('README tests 树串尾已延伸至 smoke_v2316_voiceshead（v2315 后接 v2316）',
  readme.includes('+ smoke_v2315_talkfoot + smoke_v2316_voiceshead + smoke_v2317_pausemap（npm test 串跑）'));
ok('README 旧串尾零残留（smoke_v2315_talkfoot（npm test 串' + '跑）不在树尾）',
  !readme.includes('smoke_v2315_talkfoot（npm test 串' + '跑）'));
ok('README 件套口径为二百一十三件套（二百一十二件套清除）且旧 211 口径零残留',
  readme.includes('二百一十三件套（二百一十二件套清除）') && !readme.includes('二百一十一件套（二百一十件套清' + '除）'));
ok('README 不含哨兵领先一位（二百一十四件套（二百一十三件套清' + '除））',
  !readme.includes('二百一十四件套（二百一十三件套清' + '除）'));
...
ok('package.json 已收录 smoke_v2316_voiceshead（npm test 串跑第 213 份）',
  JSON.stringify(JSON.parse(pkg).scripts.test).includes('smoke_v2316_voiceshead.mjs'));
ok('package.json 串尾为 ... smoke_v2315_talkfoot.mjs && node tests/smoke_v2316_voiceshead.mjs && node tests/smoke_v2317_pausemap.mjs"',
  pkg.includes('node tests/smoke_v2315_talkfoot.mjs && node tests/smoke_v2316_voiceshead.mjs && node tests/smoke_v2317_pausemap.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 212 件套', testChain === 213, String(testChain));
ok('CHANGELOG 顶部已追加 v23.16 条目', changelog.startsWith('## v23.77 '));
...
ok('smoke_v2315 的 GAME_VERSION 字面量 pin 已更新为 v23.16', s2315.includes("const GAME_VERSION = 'v23.77';"));
ok('smoke_v2315 的 CHANGELOG 顶 pin 已更新为 ## v23.27', s2315.includes("startsWith('## v23.77 "));
ok('smoke_v2315 的件套 pin 已更新为二百一十三件套（二百一十二件套清除）', s2315.includes('二百一十三件套（二百一十二件套清除）'));
ok('smoke_v2315 的 README 串尾 pin 已延伸至 smoke_v2316_voiceshead',
  s2315.includes('smoke_v2315_talkfoot + smoke_v2316_voiceshead + smoke_v2317_pausemap（npm test 串跑）'));
ok('smoke_v2315 的 package 串尾 pin 已延伸至 smoke_v2316_voiceshead',
  s2315.includes('node tests/smoke_v2315_talkfoot.mjs && node tests/smoke_v2316_voiceshead.mjs && node tests/smoke_v2317_pausemap.mjs"'));
ok('smoke_v2315 的 testChain pin 已更新为 212', s2315.includes('testChain === 213'));
