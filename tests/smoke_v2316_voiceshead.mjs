...
ok('data.js GAME_VERSION 字面量已为 v23.16（旧 v23.15 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.59';") && !dataSrc.includes("const GAME_VERSION = 'v23.15';"));
...
ok('ACH_LIST 含 talkall（v23.13 成就零回归）且总数 60', !!ACH_LIST.find((a) => a.id === 'talkall') && ACH_LIST.length === 62, String(ACH_LIST.length));
...
ok('README tests 树串尾已延伸至 smoke_v2316_voiceshead（v2315 后接 v2316）',
  readme.includes('+ smoke_v2315_talkfoot + smoke_v2316_voiceshead（npm test 串跑）'));
ok('README 旧串尾零残留（smoke_v2315_talkfoot（npm test 串' + '跑）不在树尾）',
  !readme.includes('smoke_v2315_talkfoot（npm test 串' + '跑）'));
ok('README 件套口径为二百一十二件套（二百一十一件套清除）且旧 211 口径零残留',
  readme.includes('二百一十二件套（二百一十一件套清除）') && !readme.includes('二百一十一件套（二百一十件套清' + '除）'));
ok('README 不含哨兵领先一位（二百一十三件套（二百一十二件套清' + '除））',
  !readme.includes('二百一十三件套（二百一十二件套清' + '除）'));
...
ok('package.json 已收录 smoke_v2316_voiceshead（npm test 串跑第 212 份）',
  JSON.stringify(JSON.parse(pkg).scripts.test).includes('smoke_v2316_voiceshead.mjs'));
ok('package.json 串尾为 ... smoke_v2315_talkfoot.mjs && node tests/smoke_v2316_voiceshead.mjs"',
...
ok('CHANGELOG 顶部已追加 v23.16 条目', changelog.startsWith('## v23.59 '));
...
ok('smoke_v2315 的 GAME_VERSION 字面量 pin 已更新为 v23.16', s2315.includes("const GAME_VERSION = 'v23.59';"));
ok('smoke_v2315 的 CHANGELOG 顶 pin 已更新为 ## v23.27', s2315.includes("startsWith('## v23.59 "));
...
ok('smoke_v2315 的 package 串尾 pin 已延伸至 smoke_v2316_voiceshead',
  s2315.includes('node tests/smoke_v2315_talkfoot.mjs && node tests/smoke_v2316_voiceshead.mjs"'));
ok('smoke_v2315 的 testChain pin 已更新为 212', s2315.includes('testChain === 212'));
