...
ok('data.js GAME_VERSION 字面量为 v22.79', dataSrc.includes("const GAME_VERSION = 'v22.81'"));
...
ok('README tests 树串尾已延伸至 smoke_v2279_lampwell（v2278 后接 v2279）',
  readme.includes('smoke_v2277_pondhint + smoke_v2278_mushguide + smoke_v2279_lampwell + smoke_v2280_grainfield + smoke_v2281_starwell（npm test 串跑）'));
ok('README 件套口径为一百七十七件套（一百七十六件套清除）且旧 173 口径零残留',
  readme.includes('冒烟一百七十七件套（一百七十六件套清除）') && !readme.includes('冒烟一百七十四件套（一百七十三件套清' + '除）'));
...
ok('package.json test 串含 smoke_v2278_mushguide.mjs 且位于串尾',
  pkg.includes('node tests/smoke_v2277_pondhint.mjs && node tests/smoke_v2278_mushguide.mjs && node tests/smoke_v2279_lampwell.mjs && node tests/smoke_v2280_grainfield.mjs && node tests/smoke_v2281_starwell.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 174 件套', testChain === 177, String(testChain));
...
ok('smoke_v2276 的 GAME_VERSION 字面量 pin 已更新为 v22.79', s2276.includes("const GAME_VERSION = 'v22.81';"));
ok('smoke_v2276 的 CHANGELOG 顶 pin 已更新为 ## v22.78', s2276.includes("startsWith('## v22.81'"));
ok('smoke_v2276 的件套 pin 已更新为一百七十七件套（一百七十六件套清除）', s2276.includes('一百七十七件套（一百七十六件套清除）'));
ok('smoke_v2276 的 README 串尾 pin 已延伸至 smoke_v2278_mushguide', s2276.includes('smoke_v2277_pondhint + smoke_v2278_mushguide + smoke_v2279_lampwell + smoke_v2280_grainfield + smoke_v2281_starwell（npm test 串跑）'));
