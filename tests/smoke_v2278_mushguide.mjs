...
ok('data.js GAME_VERSION 字面量为 v22.79', dataSrc.includes("const GAME_VERSION = 'v23.97'"));
...
// 其余三行零回归
ok('潮灯镇行 r[1] 逐字零回归（含 v23.60 「旅馆（狼嚎）」指针）',
  String(guide[0][1]).includes('商店·旅馆（狼嚎）·酿造锅·灯长/守书记(支线)·喷泉回血') && String(guide[0][1]).includes('东门→雾语林'));
...
ok('README 件套口径为二百二十一件套（二百二十件套清除）且旧 173 口径零残留',
  readme.includes('冒烟二百二十一件套（二百二十件套清除）') && !readme.includes('冒烟一百七十四件套（一百七十三件套清' + '除）'));
...
ok('package.json test 串共 174 件套', testChain === 221, String(testChain));
...
ok('smoke_v2276 的 GAME_VERSION 字面量 pin 已更新为 v22.79', s2276.includes("const GAME_VERSION = 'v23.97';"));
ok('smoke_v2276 的 CHANGELOG 顶 pin 已更新为 ## v22.78', s2276.includes("startsWith('## v23.97'"));
ok('smoke_v2276 的件套 pin 已更新为二百二十一件套（二百二十件套清除）', s2276.includes('二百二十一件套（二百二十件套清除）'));
