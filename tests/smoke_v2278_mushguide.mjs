...
ok('data.js GAME_VERSION 字面量为 v22.78', dataSrc.includes("const GAME_VERSION = 'v22.78'"));
...
ok('星井矿脉行 r[1]/r[2] 零回归',
  String(guide[2][1]).includes('试炼碑（可问守碑人）') && String(guide[2][2]) === '无泉水/旅店 · 出发前请补给');
ok('无字回廊行 r[1]/r[2] 零回归',
  String(guide[3][1]).includes('名字石碑') && String(guide[3][1]).includes('终焉之神') && String(guide[3][2]) === '无泉水/旅店 · 出发前请补给');
// 其余三页行数零回归
...
ok('README tests 树串尾已延伸至 smoke_v2278_mushguide（v2277 后接 v2278）',
  readme.includes('smoke_v2277_pondhint + smoke_v2278_mushguide（npm test 串跑）'));
ok('README 件套口径为一百七十四件套（一百七十三件套清除）且旧 173 口径零残留',
  readme.includes('冒烟一百七十四件套（一百七十三件套清除）') && !readme.includes('冒烟一百七十三件套（一百七十二件套清' + '除）'));
...
ok('package.json test 串含 smoke_v2278_mushguide.mjs 且位于串尾',
  pkg.includes('node tests/smoke_v2277_pondhint.mjs && node tests/smoke_v2278_mushguide.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 174 件套', testChain === 174, String(testChain));
ok('CHANGELOG 含 v22.78 条目（顶 pin）', changelog.startsWith('## v22.78 '));
...
ok('smoke_v2276 的 GAME_VERSION 字面量 pin 已更新为 v22.78', s2276.includes("const GAME_VERSION = 'v22.78';"));
ok('smoke_v2276 的 CHANGELOG 顶 pin 已更新为 ## v22.78', s2276.includes("startsWith('## v22.78'"));
ok('smoke_v2276 的件套 pin 已更新为一百七十四件套（一百七十三件套清除）', s2276.includes('一百七十四件套（一百七十三件套清除）'));
ok('smoke_v2276 的 README 串尾 pin 已延伸至 smoke_v2278_mushguide', s2276.includes('smoke_v2277_pondhint + smoke_v2278_mushguide（npm test 串跑）'));
ok('smoke_v2260 的 package.json 串尾 pin 已延伸至 smoke_v2278_mushguide', s2260.includes('node tests/smoke_v2278_mushguide.mjs'));
...
for (const f of allTests) {
  ...
  if (src.includes("const GAME_VERSION = 'v22." + "77';") || src.includes("GAME_VERSION === 'v22." + "77'") ||
      src.includes('一百七十三件套（一百七十二件套清' + '除）') || src.includes('testChain === ' + '173') ||
