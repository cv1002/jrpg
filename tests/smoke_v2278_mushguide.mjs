...
ok('data.js GAME_VERSION 字面量为 v22.79', dataSrc.includes("const GAME_VERSION = 'v23.09'"));
...
ok('README 件套口径为二百零五件套（二百零四件套清除）且旧 173 口径零残留',
  readme.includes('冒烟二百零五件套（二百零四件套清除）') && !readme.includes('冒烟一百七十四件套（一百七十三件套清' + '除）'));
...
ok('package.json test 串共 174 件套', testChain === 205, String(testChain));
...
ok('smoke_v2276 的 GAME_VERSION 字面量 pin 已更新为 v22.79', s2276.includes("const GAME_VERSION = 'v23.09';"));
ok('smoke_v2276 的 CHANGELOG 顶 pin 已更新为 ## v22.78', s2276.includes("startsWith('## v23.09'"));
ok('smoke_v2276 的件套 pin 已更新为二百零五件套（二百零四件套清除）', s2276.includes('二百零五件套（二百零四件套清除）'));
