...
ok('GAME_VERSION 字面量已为 v22.45（旧 v22.44 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.80';") && !dSrc.includes("const GAME_VERSION = 'v22." + "44';"));
...
ok('README 件套口径为一百七十六件套（一百七十五件套清除）',
  readme.includes('冒烟一百七十六件套（一百七十五件套清除）') && !readme.includes('冒烟一百四十件套（一百三十九件套清' + '除）'));
...
ok('package.json test 串共 141 件套', testChain === 176, String(testChain));
ok('CHANGELOG 顶部已追加 v22.45 条目', changelog.startsWith('## v22.80'));
...
ok('smoke_v2244 的 GAME_VERSION 字面量 pin 已更新为 v22.45（旧 v22.44 零残留）',
  s2244.includes("const GAME_VERSION = 'v22.80';") && !s2244.includes("const GAME_VERSION = 'v22." + "44';"));
ok('smoke_v2244 的 README 件套 pin 已随新现实更新为一百七十六件套（一百七十五件套清除）',
  s2244.includes('一百七十六件套（一百七十五件套清除）'));
ok('smoke_v2244 的 package.json 件套计数 pin 已更新为 === 141', s2244.includes('testChain === 176'));
