...
ok('GAME_VERSION 字面量已为 v22.45（旧 v22.44 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.50';") && !dSrc.includes("const GAME_VERSION = 'v22." + "44';"));
...
ok('NPC_SPOTS 总数 31（既有 30 键 + 守夜人 1 键，v22.45 随新现实更新）', Object.keys(NPC_SPOTS).length === 33, Object.keys(NPC_SPOTS).length);
...
ok('README tests 树已收录 smoke_v2245_watcher 且位于串尾', readme.includes('smoke_v2244_fulldanger + smoke_v2245_watcher + smoke_v2246_fountgauge + smoke_v2247_villagewell + smoke_v2248_mapguide + smoke_v2249_shopkeep + smoke_v2250_brewer（npm test 串跑）'));
ok('README 件套口径为一百四十六件套（一百四十五件套清除）',
  readme.includes('冒烟一百四十六件套（一百四十五件套清除）') && !readme.includes('冒烟一百四十件套（一百三十九件套清' + '除）'));
...
ok('package.json test 串共 141 件套', testChain === 146, String(testChain));
ok('CHANGELOG 顶部已追加 v22.45 条目', changelog.startsWith('## v22.50'));
...
ok('smoke_v2244 的 GAME_VERSION 字面量 pin 已更新为 v22.45（旧 v22.44 零残留）',
  s2244.includes("const GAME_VERSION = 'v22.50';") && !s2244.includes("const GAME_VERSION = 'v22." + "44';"));
ok('smoke_v2244 的 README 件套 pin 已随新现实更新为一百四十六件套（一百四十五件套清除）',
  s2244.includes('一百四十六件套（一百四十五件套清除）'));
ok('smoke_v2244 的 package.json 件套计数 pin 已更新为 === 141', s2244.includes('testChain === 146'));
ok('smoke_v2244 的 README 串尾 pin 已随新现实延伸至 smoke_v2245_watcher',
  s2244.includes('smoke_v2244_fulldanger + smoke_v2245_watcher + smoke_v2246_fountgauge + smoke_v2247_villagewell + smoke_v2248_mapguide + smoke_v2249_shopkeep + smoke_v2250_brewer（npm test 串跑）'));
ok('smoke_v2244 的 NPC 总数 pin 已随新现实更新为 31（守夜人落位）', s2244.includes('NPC_SPOTS).length === 33'));
// 旧代 v22.44 pin 全库零残留
