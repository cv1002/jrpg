...
ok('GAME_VERSION 字面量已为 v22.25（旧 v22.24 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.71';") && !dSrc.includes("const GAME_VERSION = 'v22." + "24';"));
...
ok('README 件套口径为一百六十七件套（一百六十六件套清除）',
  readme.includes('冒烟一百六十七件套（一百六十六件套清除）') && !readme.includes('冒烟一百二十件套（一百一十九件套清' + '除）'));
...
ok('package.json test 串共 121 件套', testChain === 167, String(testChain));
...
ok('smoke_v2224 的 GAME_VERSION 字面量 pin 已更新为 v22.25', s2224.includes("const GAME_VERSION = 'v22.71';"));
ok('smoke_v2224 的 GAME_VERSION 恒等 pin 已更新为 === v22.25', s2224.includes("GAME_VERSION === 'v22.71'"));
ok('smoke_v2224 的 README 件套 pin 已随新现实更新为一百六十七件套（一百六十六件套清除）',
  s2224.includes('一百六十七件套（一百六十六件套清除）'));
...
ok('smoke_v2224 的 package.json 件套计数 pin 已更新为 === 121', s2224.includes('testChain === 167'));
ok('smoke_v2224 的 NPC 总数 pin 已随新现实更新为 27（刻碑人落位）', s2224.includes('总数 29'));
ok('smoke_v2223 的 GAME_VERSION 字面量 pin 已更新为 v22.25', s2223.includes("const GAME_VERSION = 'v22.71';"));
ok('smoke_v2223 的 GAME_VERSION 恒等 pin 已更新为 === v22.25', s2223.includes("GAME_VERSION === 'v22.71'"));
ok('smoke_v2223 的 README 件套 pin 已随新现实更新为一百六十七件套（一百六十六件套清除）',
  s2223.includes('一百六十七件套（一百六十六件套清除）'));
...
ok('smoke_v2223 的 package.json 件套计数 pin 已更新为 === 121', s2223.includes('testChain === 167'));
ok('smoke_v2223 的 NPC 总数 pin 已随新现实更新为 27（刻碑人落位）', s2223.includes('总数 29'));
ok('smoke_v2222 的 GAME_VERSION 字面量 pin 已更新为 v22.25', s2222.includes("const GAME_VERSION = 'v22.71';"));
ok('smoke_v2222 的 GAME_VERSION 恒等 pin 已更新为 === v22.25', s2222.includes("GAME_VERSION === 'v22.71'"));
ok('smoke_v2222 的 README 件套 pin 已随新现实更新为一百六十七件套（一百六十六件套清除）',
  s2222.includes('一百六十七件套（一百六十六件套清除）'));
ok('smoke_v2222 的 README 树尾 pin 已更新为 + smoke_v2225_stonecarver',
  s2222.includes('smoke_v2222_peddler + smoke_v2223_lampman + smoke_v2224_sifter + smoke_v2225_stonecarver + smoke_v2226_chestprogress + smoke_v2227_minstrel + smoke_v2228_titlesave + smoke_v2229_metall + smoke_v2230_pausewarn + smoke_v2231_smith + smoke_v2232_travelsup + smoke_v2233_nameflavor + smoke_v2234_innkeeper + smoke_v2235_minimapquest + smoke_v2236_villagelamp + smoke_v2237_minimaplegend + smoke_v2238_starwell + smoke_v2240_tallgrass + smoke_v2241_gatearch + smoke_v2242_mushfield + smoke_v2243_encguide + smoke_v2244_fulldanger + smoke_v2245_watcher + smoke_v2246_fountgauge + smoke_v2247_villagewell + smoke_v2248_mapguide + smoke_v2249_shopkeep + smoke_v2250_brewer + smoke_v2251_oathkeep + smoke_v2252_rail + smoke_v2253_supplypoint + smoke_v2254_grainfield + smoke_v2255_steleglow + smoke_v2256_campfire + smoke_v2257_pondglow + smoke_v2258_potionhelp + smoke_v2259_sandpile + smoke_v2260_fountripple + smoke_v2261_rich3 + smoke_v2262_crystal + smoke_v2263_ptime3 + smoke_v2264_hunt3 + smoke_v2265_lucky3 + smoke_v2266_stock3 + smoke_v2267_elixir3 + smoke_v2268_brew3 + smoke_v2269_mush3 + smoke_v2270_outstep + smoke_v2271_outstep2（npm test 串跑）'));
ok('smoke_v2222 的 package.json 件套计数 pin 已更新为 === 121', s2222.includes('testChain === 167'));
ok('smoke_v2222 的 NPC 总数 pin 已随新现实更新为 27（刻碑人落位）', s2222.includes('总数 29'));
ok('smoke_v2221 的 GAME_VERSION 字面量 pin 已更新为 v22.25', s2221.includes("const GAME_VERSION = 'v22.71';"));
ok('smoke_v2221 的 GAME_VERSION 恒等 pin 已更新为 === v22.25', s2221.includes("GAME_VERSION === 'v22.71'"));
ok('smoke_v2221 的 README 件套 pin 已随新现实更新为一百六十七件套（一百六十六件套清除）',
  s2221.includes('一百六十七件套（一百六十六件套清除）'));
...
ok('smoke_v2221 的 package.json 件套计数 pin 已更新为 === 121', s2221.includes('testChain === 167'));
ok('smoke_v2221 的 NPC 总数 pin 已随新现实更新为 27（刻碑人落位）', s2221.includes('总数 29'));
ok('smoke_v2220 的 GAME_VERSION 字面量 pin 已更新为 v22.25', s2220.includes("const GAME_VERSION = 'v22.71';"));
ok('smoke_v2220 的 GAME_VERSION 恒等 pin 已更新为 === v22.25', s2220.includes("GAME_VERSION === 'v22.71'"));
ok('smoke_v2220 的 README 件套 pin 已随新现实更新为一百六十七件套（一百六十六件套清除）',
  s2220.includes('一百六十七件套（一百六十六件套清除）'));
...
ok('smoke_v2220 的 package.json 件套计数 pin 已更新为 === 121', s2220.includes('testChain === 167'));
ok('smoke_v2220 的 NPC 总数 pin 已随新现实更新为 27（刻碑人落位）', s2220.includes('总数 29'));
ok('smoke_v2219 的 GAME_VERSION 字面量 pin 已更新为 v22.25', s2219.includes("const GAME_VERSION = 'v22.71';"));
ok('smoke_v2219 的 README 件套 pin 已随新现实更新为一百六十七件套（一百六十六件套清除）',
  s2219.includes('一百六十七件套（一百六十六件套清除）'));
ok('smoke_v2218 的 GAME_VERSION 字面量 pin 已更新为 v22.25', s2218.includes("const GAME_VERSION = 'v22.71';"));
ok('smoke_v2218 的 package.json 件套计数 pin 已更新为 === 121', s2218.includes('testChain === 167'));
ok('smoke_v2217 的 README 件套 pin 已随新现实更新为一百六十七件套（一百六十六件套清除）',
  s2217.includes('一百六十七件套（一百六十六件套清除）'));
