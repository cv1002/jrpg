...
ok('data.js GAME_VERSION 字面量已为 v23.12（旧 v23.11 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.15';") && !dataSrc.includes("const GAME_VERSION = 'v23.11';"));
...
ok('menus.js 提示行模板主体逐字保留（选槽/←/→/L 读档/R 重开/WASD/Esc/P 存档/M 静音）',
  menuSrc.includes("L 读档 · R 重开新档(连按两次) · WASD 移动 · Esc 菜单 · P 存档 · M 静音 · [ / ] 音量"));
...
const hintOld = '按 1/2/3 或 ←/→ 选择存档槽 · L 读档 · R 重开新档(连按两次) · WASD 移动 · Esc 菜单 · P 存档 · M 静音';
...
ok('README 件套口径为二百一十一件套（二百一十件套清除）且旧 207 口径零残留',
  readme.includes('冒烟二百一十一件套（二百一十件套清除）') && !readme.includes('冒烟二百零七件套（二百零六件套清' + '除）'));
...
ok('package.json test 串共 208 件套', testChain === 211, String(testChain));
ok('CHANGELOG 顶部已追加 v23.12 条目', changelog.startsWith('## v23.15 '));
...
ok('smoke_v2311 的 GAME_VERSION 字面量 pin 已更新为 v23.12', s2311.includes("const GAME_VERSION = 'v23.15';"));
ok('smoke_v2311 的 CHANGELOG 顶 pin 已更新为 ## v23.12',
  s2311.includes("startsWith('## v23.15 "));
ok('smoke_v2311 的件套 pin 已更新为二百一十一件套（二百一十件套清除）',
  s2311.includes('二百一十一件套（二百一十件套清除）'));
ok('smoke_v2311 的 README 串尾 pin 已延伸至 smoke_v2312_voltitle',
  s2311.includes('smoke_v2309_questnum + smoke_v2310_diffsum + smoke_v2311_fragprev + smoke_v2312_voltitle + smoke_v2313_talkall + smoke_v2314_voices + smoke_v2315_talkfoot（npm test 串跑）'));
ok('smoke_v2311 的 package 串尾 pin 已延伸至 smoke_v2312_voltitle',
  s2311.includes('node tests/smoke_v2311_fragprev.mjs && node tests/smoke_v2312_voltitle.mjs && node tests/smoke_v2313_talkall.mjs && node tests/smoke_v2314_voices.mjs && node tests/smoke_v2315_talkfoot.mjs"'));
ok('smoke_v2311 的 testChain pin 已更新为 208', s2311.includes('testChain === 211'));
