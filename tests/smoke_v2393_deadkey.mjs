...
ok('data.js GAME_VERSION 字面量已为 v23.93（旧 v23.92 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v24.00';") && !dSrc.includes("const GAME_VERSION = 'v23.9" + "2';"));
...
ok('package.json test 串共 217 件套', testChain === 224, String(testChain));
...
ok('README tests 树串尾已延伸至 smoke_v2393_deadkey',
  readme.includes('+ smoke_v2319_deadloc + smoke_v2392_stariron + smoke_v2393_deadkey + smoke_v2394_fightback + smoke_v2395_shopscroll + smoke_v2396_steelarmor + smoke_v2397_baserow + smoke_v2398_econrow + smoke_v2399_dmgformula + smoke_v2400_trialwarn（npm test 串跑）'));
ok('README 件套口径为二百二十四件套（二百二十三件套清除）且旧 214 口径零残留',
  readme.includes('冒烟二百二十四件套（二百二十三件套清除）') && !readme.includes('冒烟二百一十四件套（二百一十三件套清' + '除）'));
...
ok('CHANGELOG 顶部已追加 v23.93 条目（阵亡行 P 口径）', changelog.startsWith('## v24.00 '));
