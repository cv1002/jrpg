...
ok('data.js GAME_VERSION 字面量已为 v23.93（旧 v23.92 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.93';") && !dSrc.includes("const GAME_VERSION = 'v23.9" + "2';"));
...
ok('package.json test 串共 217 件套', testChain === 217, String(testChain));
ok('package.json 已收录 smoke_v2393_deadkey（npm test 串跑第 217 份）',
  JSON.parse(pkg).scripts.test.includes('smoke_v2393_deadkey.mjs'));
ok('package.json 串尾为 ... smoke_v2392_stariron.mjs && node tests/smoke_v2393_deadkey.mjs"',
  pkg.includes('node tests/smoke_v2392_stariron.mjs && node tests/smoke_v2393_deadkey.mjs"'));
...
ok('CHANGELOG 顶部已追加 v23.93 条目（阵亡行 P 口径）', changelog.startsWith('## v23.93 '));
