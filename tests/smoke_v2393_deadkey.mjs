...
ok('data.js GAME_VERSION 字面量已为 v23.93（旧 v23.92 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v24.03';") && !dSrc.includes("const GAME_VERSION = 'v23.9" + "2';"));
...
ok('package.json test 串共 217 件套', testChain === 227, String(testChain));
ok('package.json 已收录 smoke_v2393_deadkey（npm test 串跑第 217 份）',
  JSON.parse(pkg).scripts.test.includes('smoke_v2393_deadkey.mjs'));
ok('package.json 串尾为 ... smoke_v2392_stariron.mjs && node tests/smoke_v2393_deadkey.mjs && node tests/smoke_v2394_fightback.mjs && node tests/smoke_v2395_shopscroll.mjs && node tests/smoke_v2396_steelarmor.mjs && node tests/smoke_v2397_baserow.mjs && node tests/smoke_v2398_econrow.mjs && node tests/smoke_v2399_dmgformula.mjs && node tests/smoke_v2400_trialwarn.mjs && node tests/smoke_v2401_winbgm.mjs && node tests/smoke_v2402_firstwin.mjs && node tests/smoke_v2403_diffbattle.mjs"',
...
ok('README tests 树串尾已延伸至 smoke_v2393_deadkey',
  readme.includes('+ smoke_v2319_deadloc + smoke_v2392_stariron + smoke_v2393_deadkey + smoke_v2394_fightback + smoke_v2395_shopscroll + smoke_v2396_steelarmor + smoke_v2397_baserow + smoke_v2398_econrow + smoke_v2399_dmgformula + smoke_v2400_trialwarn + smoke_v2401_winbgm + smoke_v2402_firstwin + smoke_v2403_diffbattle（npm test 串跑）'));
ok('README 件套口径为二百二十七件套（二百二十六件套清除）且旧 214 口径零残留',
  readme.includes('冒烟二百二十七件套（二百二十六件套清除）') && !readme.includes('冒烟二百一十四件套（二百一十三件套清' + '除）'));
...
ok('CHANGELOG 顶部已追加 v23.93 条目（阵亡行 P 口径）', changelog.startsWith('## v24.03 '));
