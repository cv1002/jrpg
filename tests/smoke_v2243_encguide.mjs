...
// 末行（通关之路）基线 366 不触页脚 452。纯文字零逻辑零结算零存档。
// 本冒烟守护：版本锚点、data.js 源级落位（新行 + 既有 7 行逐字零回归 + v22.42/v22.40 注释保留）、
// 宽度预算（estW 口径 ≤470）、页长派生预算（末行基线 366 / 页脚 452）、运行期实证（drawHelp 第 2 页
...
ok('GAME_VERSION 字面量已为 v22.43（旧 v22.42 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.52';") && !dSrc.includes("const GAME_VERSION = 'v22." + "42';"));
...
// 页长派生预算：末行基线 = 80 + (8-1)*34 + 3*16 = 366，不触页脚 452
ok('页长派生：末行基线 366 ≤440（页脚 452 之上留白，v19.59 式预算）',
...
ok('运行期：机制行主行落位（x=100 y=316 14px 普通色）',
  !!at('遇敌槽 / 危险格', 316, '14px sans-serif', '#e8eef1'),
  JSON.stringify(calls.filter((c) => c.t.startsWith('遇敌')).slice(0, 4)));
ok('运行期：机制行次行落位（x=100 y=334 12px 次级灰）',
  !!at('雾语林/矿脉/回廊全图皆危险格', 334, '12px sans-serif', '#7d93a3'));
ok('运行期：通关之路金色收尾仍在末行（x=100 y=366 14px #ffd24a）',
...
ok('package.json test 串共 139 件套（含 smoke_v2243_encguide）', testChain === 148, String(testChain));
...
ok('README tests 树尾已收录 smoke_v2243_encguide（串跑链）',
  readmeSrc.includes('smoke_v2242_mushfield + smoke_v2243_encguide + smoke_v2244_fulldanger + smoke_v2245_watcher + smoke_v2246_fountgauge + smoke_v2247_villagewell + smoke_v2248_mapguide + smoke_v2249_shopkeep + smoke_v2250_brewer + smoke_v2251_oathkeep + smoke_v2252_rail（npm test 串跑）'));
ok('README 件套口径为一百四十八件套（一百四十七件套清除）',
  readmeSrc.includes('冒烟一百四十八件套（一百四十七件套清除）') &&
...
ok('CHANGELOG 顶部已追加 v22.44 条目', changelogSrc.startsWith('## v22.52'));
...
ok('smoke_v2242 的 GAME_VERSION 字面量 pin 已更新为 v22.43（旧 v22.42 零残留）',
  s2242.includes("const GAME_VERSION = 'v22.52';") && !s2242.includes("const GAME_VERSION = 'v22." + "42';"));
...
ok('smoke_v2234 的 README 件套 pin 已随新现实更新为一百四十八件套（一百四十七件套清除）',
  s2234.includes('一百四十八件套（一百四十七件套清除）'));
ok('smoke_v2234 的 package.json 件套计数 pin 已更新为 === 139', s2234.includes('testChain === 148'));
