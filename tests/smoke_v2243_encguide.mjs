...
ok('GAME_VERSION 字面量已为 v22.43（旧 v22.42 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.49';") && !dSrc.includes("const GAME_VERSION = 'v22." + "42';"));
...
ok('package.json test 串共 139 件套（含 smoke_v2243_encguide）', testChain === 145, String(testChain));
...
ok('README tests 树尾已收录 smoke_v2243_encguide（串跑链）',
  readmeSrc.includes('smoke_v2242_mushfield + smoke_v2243_encguide + smoke_v2244_fulldanger + smoke_v2245_watcher + smoke_v2246_fountgauge + smoke_v2247_villagewell + smoke_v2248_mapguide + smoke_v2249_shopkeep（npm test 串跑）'));
ok('README 件套口径为一百四十五件套（一百四十四件套清除）',
  readmeSrc.includes('冒烟一百四十五件套（一百四十四件套清除）') &&
...
ok('CHANGELOG 顶部已追加 v22.44 条目', changelogSrc.startsWith('## v22.49'));
...
ok('smoke_v2242 的 GAME_VERSION 字面量 pin 已更新为 v22.43（旧 v22.42 零残留）',
  s2242.includes("const GAME_VERSION = 'v22.49';") && !s2242.includes("const GAME_VERSION = 'v22." + "42';"));
...
ok('smoke_v2234 的 README 件套 pin 已随新现实更新为一百四十五件套（一百四十四件套清除）',
  s2234.includes('一百四十五件套（一百四十四件套清除）'));
ok('smoke_v2234 的 package.json 件套计数 pin 已更新为 === 139', s2234.includes('testChain === 145'));
