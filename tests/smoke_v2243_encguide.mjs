...
ok('GAME_VERSION 字面量已为 v22.43（旧 v22.42 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.77';") && !dSrc.includes("const GAME_VERSION = 'v22." + "42';"));
...
ok('运行期：机制行主行落位（x=100 y=364 14px 普通色，v22.77 后 348→364）',
  !!at('遇敌槽 / 危险格', 364, '14px sans-serif', '#e8eef1'),
  JSON.stringify(calls.filter((c) => c.t.startsWith('遇敌')).slice(0, 4)));
ok('运行期：机制行次行落位（x=100 y=382 12px 次级灰，v22.77 后 366→382）',
...
ok('运行期：图例两行仍按 264/314 落位（随 v22.53/v22.77 让位下移）',
...
ok('package.json test 串共 139 件套（含 smoke_v2243_encguide）', testChain === 173, String(testChain));
...
ok('README 件套口径为一百七十三件套（一百七十二件套清除）',
  readmeSrc.includes('冒烟一百七十三件套（一百七十二件套清除）') &&
...
ok('CHANGELOG 顶部已追加 v22.44 条目', changelogSrc.startsWith('## v22.77'));
...
ok('smoke_v2242 的 GAME_VERSION 字面量 pin 已更新为 v22.43（旧 v22.42 零残留）',
  s2242.includes("const GAME_VERSION = 'v22.77';") && !s2242.includes("const GAME_VERSION = 'v22." + "42';"));
...
ok('smoke_v2234 的 README 件套 pin 已随新现实更新为一百七十三件套（一百七十二件套清除）',
  s2234.includes('一百七十三件套（一百七十二件套清除）'));
ok('smoke_v2234 的 package.json 件套计数 pin 已更新为 === 139', s2234.includes('testChain === 173'));
