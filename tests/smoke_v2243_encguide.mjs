...
ok('GAME_VERSION 字面量已为 v22.43（旧 v22.42 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.53';") && !dSrc.includes("const GAME_VERSION = 'v22." + "42';"));
...
ok('运行期：机制行主行落位（x=100 y=348 14px 普通色）',
  !!at('遇敌槽 / 危险格', 348, '14px sans-serif', '#e8eef1'),
  JSON.stringify(calls.filter((c) => c.t.startsWith('遇敌')).slice(0, 4)));
ok('运行期：机制行次行落位（x=100 y=366 12px 次级灰）',
  !!at('雾语林/矿脉/回廊全图皆危险格', 366, '12px sans-serif', '#7d93a3'));
...
ok('运行期：图例两行仍按 248/298 落位（随 v22.53 让位下移）',
...
ok('package.json test 串共 139 件套（含 smoke_v2243_encguide）', testChain === 149, String(testChain));
...
ok('README tests 树尾已收录 smoke_v2243_encguide（串跑链）',
  readmeSrc.includes('smoke_v2242_mushfield + smoke_v2243_encguide + smoke_v2244_fulldanger + smoke_v2245_watcher + smoke_v2246_fountgauge + smoke_v2247_villagewell + smoke_v2248_mapguide + smoke_v2249_shopkeep + smoke_v2250_brewer + smoke_v2251_oathkeep + smoke_v2252_rail + smoke_v2253_supplypoint（npm test 串跑）'));
...
ok('smoke_v2242 的 GAME_VERSION 字面量 pin 已更新为 v22.43（旧 v22.42 零残留）',
  s2242.includes("const GAME_VERSION = 'v22.53';") && !s2242.includes("const GAME_VERSION = 'v22." + "42';"));
...
ok('smoke_v2234 的 README 件套 pin 已随新现实更新为一百四十九件套（一百四十八件套清除）',
  s2234.includes('一百四十九件套（一百四十八件套清除）'));
ok('smoke_v2234 的 package.json 件套计数 pin 已更新为 === 139', s2234.includes('testChain === 149'));
