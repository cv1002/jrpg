...
ok('GAME_VERSION 字面量已为 v22.43（旧 v22.42 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.84';") && !dSrc.includes("const GAME_VERSION = 'v22." + "42';"));
...
ok('GAME_VERSION 已至 v23.31', GAME_VERSION === 'v23.84', GAME_VERSION);
...
ok('package.json test 串共 139 件套（含 smoke_v2243_encguide）', testChain === 215, String(testChain));
...
ok('README 件套口径为二百一十五件套（二百一十四件套清除）',
  readmeSrc.includes('冒烟二百一十五件套（二百一十四件套清除）') &&
...
ok('CHANGELOG 顶部已追加 v22.44 条目', changelogSrc.startsWith('## v23.84'));
...
ok('smoke_v2242 的 GAME_VERSION 字面量 pin 已更新为 v22.43（旧 v22.42 零残留）',
  s2242.includes("const GAME_VERSION = 'v23.84';") && !s2242.includes("const GAME_VERSION = 'v22." + "42';"));
...
ok('smoke_v2234 的 README 件套 pin 已随新现实更新为二百一十五件套（二百一十四件套清除）',
  s2234.includes('二百一十五件套（二百一十四件套清除）'));
ok('smoke_v2234 的 package.json 件套计数 pin 已更新为 === 139', s2234.includes('testChain === 215'));
