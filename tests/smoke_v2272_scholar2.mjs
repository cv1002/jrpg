...
ok('data.js GAME_VERSION 字面量已为 v22.72（旧 v22.71 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.66';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "71';"));
...
ok('ACH_LIST 精确总数 56 项（v2229 精确计数 pin 随新现实更新 55→56）', ACH_LIST.length === 66, String(ACH_LIST.length));
...
ok('README 件套口径为二百一十二件套（二百一十一件套清除）且旧 167 口径零残留',
  readme.includes('冒烟二百一十二件套（二百一十一件套清除）') && !readme.includes('冒烟一百六十七件套（一百六十六件套清' + '除）'));
...
ok('package.json test 串共 168 件套', testChain === 212, String(testChain));
ok('CHANGELOG 含 v22.72 条目（顶 pin）', changelog.startsWith('## v23.66 '));

// —— 姊妹件套 pin 随新现实更新 + 旧代 v22.71 pin 零残留 ——
const s2271 = read('smoke_v2271_outstep2.mjs');
const s2229 = read('smoke_v2229_metall.mjs');
ok('smoke_v2271 的 GAME_VERSION 字面量 pin 已更新为 v22.72', s2271.includes("const GAME_VERSION = 'v23.66';"));
ok('smoke_v2271 的 CHANGELOG 顶 pin 已更新为 ## v22.72', s2271.includes("startsWith('## v23.66'") || s2271.includes("startsWith('## v22.78 '"));
ok('smoke_v2271 的件套 pin 已更新为二百一十二件套（二百一十一件套清除）', s2271.includes('二百一十二件套（二百一十一件套清除）'));
...
ok('smoke_v2229 的 ACH_LIST 精确计数 pin 已更新为 === 56', s2229.includes('ACH_LIST.length === 66'));
