...
ok('data.js GAME_VERSION 字面量已为 v22.72（旧 v22.71 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.11';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "71';"));
...
ok('README 件套口径为二百零七件套（二百零六件套清除）且旧 167 口径零残留',
  readme.includes('冒烟二百零七件套（二百零六件套清除）') && !readme.includes('冒烟一百六十七件套（一百六十六件套清' + '除）'));
...
ok('package.json test 串共 168 件套', testChain === 207, String(testChain));
ok('CHANGELOG 含 v22.72 条目（顶 pin）', changelog.startsWith('## v23.11 '));

// —— 姊妹件套 pin 随新现实更新 + 旧代 v22.71 pin 零残留 ——
const s2271 = read('smoke_v2271_outstep2.mjs');
const s2229 = read('smoke_v2229_metall.mjs');
ok('smoke_v2271 的 GAME_VERSION 字面量 pin 已更新为 v22.72', s2271.includes("const GAME_VERSION = 'v23.11';"));
ok('smoke_v2271 的 CHANGELOG 顶 pin 已更新为 ## v22.72', s2271.includes("startsWith('## v23.11'") || s2271.includes("startsWith('## v22.78 '"));
ok('smoke_v2271 的件套 pin 已更新为二百零七件套（二百零六件套清除）', s2271.includes('二百零七件套（二百零六件套清除）'));
