...
ok('GAME_VERSION 格式合法且已越过 v22.71（本版守 v22.79）', !!_gv && (_gv[0] > 22 || (_gv[0] === 22 && _gv[1] >= 80)), GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已为 v22.72（旧 v22.71 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v22.80';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "71';"));
...
ok('README tests 树串尾已延伸至 smoke_v2272_scholar2（v2271 后接 v2272）',
  readme.includes('smoke_v2270_outstep + smoke_v2271_outstep2 + smoke_v2272_scholar2 + smoke_v2273_seen5 + smoke_v2274_seen2 + smoke_v2275_codexempty + smoke_v2276_lampkid + smoke_v2277_pondhint + smoke_v2278_mushguide + smoke_v2279_lampwell + smoke_v2280_grainfield（npm test 串跑）'));
ok('README 件套口径为一百七十六件套（一百七十五件套清除）且旧 167 口径零残留',
  readme.includes('冒烟一百七十六件套（一百七十五件套清除）') && !readme.includes('冒烟一百六十七件套（一百六十六件套清' + '除）'));
...
ok('package.json test 串共 168 件套', testChain === 176, String(testChain));
ok('CHANGELOG 含 v22.72 条目（顶 pin）', changelog.startsWith('## v22.80 '));

// —— 姊妹件套 pin 随新现实更新 + 旧代 v22.71 pin 零残留 ——
const s2271 = read('smoke_v2271_outstep2.mjs');
const s2229 = read('smoke_v2229_metall.mjs');
ok('smoke_v2271 的 GAME_VERSION 字面量 pin 已更新为 v22.72', s2271.includes("const GAME_VERSION = 'v22.80';"));
ok('smoke_v2271 的 CHANGELOG 顶 pin 已更新为 ## v22.72', s2271.includes("startsWith('## v22.80'") || s2271.includes("startsWith('## v22.78 '"));
ok('smoke_v2271 的件套 pin 已更新为一百七十六件套（一百七十五件套清除）', s2271.includes('一百七十六件套（一百七十五件套清除）'));
ok('smoke_v2271 的 README 串尾 pin 已延伸至 smoke_v2272_scholar2', s2271.includes('smoke_v2271_outstep2 + smoke_v2272_scholar2 + smoke_v2273_seen5 + smoke_v2274_seen2 + smoke_v2275_codexempty + smoke_v2276_lampkid + smoke_v2277_pondhint + smoke_v2278_mushguide + smoke_v2279_lampwell + smoke_v2280_grainfield（npm test 串跑）'));
