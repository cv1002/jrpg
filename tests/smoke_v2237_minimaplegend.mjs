...
ok('GAME_VERSION 字面量已为 v22.37（旧 v22.36 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.89';") && !dSrc.includes("const GAME_VERSION = 'v22." + "36';"));
...
ok('页长派生：末行基线 414 ≤440（页脚 452 之上留白，v19.59 式预算）', 80 + (page.length - 1) * 34 + (page.slice(0, -1).reduce((a, r) => a + (r[2] ? 1 : 0), 0)) * 16 === 414);
...
ok('运行期：状态标主行落位（x=100 y=264 14px 普通色，v22.78 后 +16）',
  !!at('小地图·状态标', 264, '14px sans-serif', '#e8eef1'),
  JSON.stringify(calls.filter((c) => c.t.startsWith('小地图')).slice(0, 4)));
ok('运行期：状态标次行落位（x=100 y=282 12px 次级灰，v22.78 后 +16）',
  !!at('遇敌槽红条', 282, '12px sans-serif', '#7d93a3'));
ok('运行期：色标主行落位（x=100 y=314 14px 普通色，v22.78 后 +16）',
  !!at('小地图·色标', 314, '14px sans-serif', '#e8eef1'));
ok('运行期：色标次行落位（x=100 y=332 12px 次级灰，v22.78 后 +16）',
  !!at('设施：商店·金', 332, '12px sans-serif', '#7d93a3'));
...
ok('package.json test 串共 133 件套', testChain === 185, String(testChain));
...
ok('smoke_v2236 的 GAME_VERSION 字面量 pin 已更新为 v22.37', s2236.includes("const GAME_VERSION = 'v22.89';"));
ok('smoke_v2236 的 GAME_VERSION 恒等 pin 已更新为 === v22.37', s2236.includes("GAME_VERSION === 'v22.89'"));
...
ok('smoke_v2236 的 package.json 件套计数 pin 已更新为 === 133', s2236.includes('testChain === 185'));
ok('smoke_v2236 的 README 件套口径 pin 已更新为一百三十三件套', s2236.includes('冒烟一百八十五件套（一百八十四件套清除）'));
ok('smoke_v2235 的 GAME_VERSION 字面量 pin 已更新为 v22.37', s2235.includes("const GAME_VERSION = 'v22.89';"));
ok('smoke_v2234 的 GAME_VERSION 恒等 pin 已更新为 === v22.37', s2234.includes("GAME_VERSION === 'v22.89'"));
...
ok('smoke_v2232 的 package.json 件套计数 pin 已更新为 === 133', s2232.includes('testChain === 185'));
...
ok('smoke_v2230 的 GAME_VERSION 恒等 pin 已更新为 === v22.37', s2230.includes("GAME_VERSION === 'v22.89'"));
ok('smoke_v2229 的 README 件套口径 pin 已更新为一百三十三件套', s2229.includes('冒烟一百八十五件套（一百八十四件套清除）'));
ok('smoke_v2226 的 GAME_VERSION 字面量 pin 已更新为 v22.37', s2226.includes("const GAME_VERSION = 'v22.89';"));
