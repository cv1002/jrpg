...
ok('GAME_VERSION 字面量已为 v22.37（旧 v22.36 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.86';") && !dSrc.includes("const GAME_VERSION = 'v22." + "36';"));
...
ok('页长派生：末行基线 414 ≤440（页脚 452 之上留白，v19.59 式预算）', 80 + (page.length - 1) * 34 + (page.reduce((a, r) => a + (r[2] ? 1 : 0), 0)) * 16 === 414);
...
ok('README 件套口径为一百八十二件套（一百八十一件套清除）',
  readme.includes('冒烟一百八十二件套（一百八十一件套清除）') && !readme.includes('冒烟一百三十二件套（一百三十一件套清' + '除）'));
...
ok('package.json test 串共 133 件套', testChain === 182, String(testChain));
...
ok('smoke_v2236 的 GAME_VERSION 字面量 pin 已更新为 v22.37', s2236.includes("const GAME_VERSION = 'v22.86';"));
ok('smoke_v2236 的 GAME_VERSION 恒等 pin 已更新为 === v22.37', s2236.includes("GAME_VERSION === 'v22.86'"));
...
ok('smoke_v2236 的 package.json 件套计数 pin 已更新为 === 133', s2236.includes('testChain === 182'));
ok('smoke_v2236 的 README 件套口径 pin 已更新为一百三十三件套', s2236.includes('冒烟一百八十二件套（一百八十一件套清除）'));
ok('smoke_v2235 的 GAME_VERSION 字面量 pin 已更新为 v22.37', s2235.includes("const GAME_VERSION = 'v22.86';"));
ok('smoke_v2234 的 GAME_VERSION 恒等 pin 已更新为 === v22.37', s2234.includes("GAME_VERSION === 'v22.86'"));
...
ok('smoke_v2232 的 package.json 件套计数 pin 已更新为 === 133', s2232.includes('testChain === 182'));
...
ok('smoke_v2230 的 GAME_VERSION 恒等 pin 已更新为 === v22.37', s2230.includes("GAME_VERSION === 'v22.86'"));
ok('smoke_v2229 的 README 件套口径 pin 已更新为一百三十三件套', s2229.includes('冒烟一百八十二件套（一百八十一件套清除）'));
ok('smoke_v2226 的 GAME_VERSION 字面量 pin 已更新为 v22.37', s2226.includes("const GAME_VERSION = 'v22.86';"));
