...
ok('GAME_VERSION 字面量已为 v22.37（旧 v22.36 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.74';") && !dSrc.includes("const GAME_VERSION = 'v22." + "36';"));
...
ok('页长派生：末行基线 398 ≤440（页脚 452 之上留白，v19.59 式预算）', 80 + (page.length - 1) * 34 + (page.reduce((a, r) => a + (r[2] ? 1 : 0), 0)) * 16 === 398);
...
ok('运行期：状态标主行落位（x=100 y=248 14px 普通色）',
  !!at('小地图·状态标', 248, '14px sans-serif', '#e8eef1'),
  JSON.stringify(calls.filter((c) => c.t.startsWith('小地图')).slice(0, 4)));
ok('运行期：状态标次行落位（x=100 y=266 12px 次级灰）',
  !!at('遇敌槽红条', 266, '12px sans-serif', '#7d93a3'));
ok('运行期：色标主行落位（x=100 y=298 14px 普通色）',
  !!at('小地图·色标', 298, '14px sans-serif', '#e8eef1'));
ok('运行期：色标次行落位（x=100 y=316 12px 次级灰）',
  !!at('设施：商店·金', 316, '12px sans-serif', '#7d93a3'));
ok('运行期：通关之路金色收尾仍在末行（x=100 y=398 14px #ffd24a）',
  !!at('通关之路', 398, '14px sans-serif', '#ffd24a'));
...
ok('运行期：既有四图 Lv 行仍按 80/114/148/198 落位（星井矿脉主行 148 不移、无字回廊 182→198 让位 r[2]）',
  !!at('潮灯镇 Lv.', 80, '14px sans-serif', '#e8eef1') &&
  !!at('雾语林 Lv.', 114, '14px sans-serif', '#e8eef1') &&
  !!at('星井矿脉 Lv.', 148, '14px sans-serif', '#e8eef1') &&
  !!at('无字回廊 Lv.', 198, '14px sans-serif', '#e8eef1'));

// —— 零回归面：未动任何小地图绘制/标记/数据（色板即证据），未动 NPC/任务/存档 ——
ok('NPC 总数保持 30（零 NPC 变更）', Object.keys(NPC_SPOTS).length === 34, String(Object.keys(NPC_SPOTS).length));
...
ok('README 件套口径为一百七十件套（一百六十九件套清除）',
  readme.includes('冒烟一百七十件套（一百六十九件套清除）') && !readme.includes('冒烟一百三十二件套（一百三十一件套清' + '除）'));
...
ok('package.json test 串共 133 件套', testChain === 170, String(testChain));
...
ok('smoke_v2236 的 GAME_VERSION 字面量 pin 已更新为 v22.37', s2236.includes("const GAME_VERSION = 'v22.74';"));
ok('smoke_v2236 的 GAME_VERSION 恒等 pin 已更新为 === v22.37', s2236.includes("GAME_VERSION === 'v22.74'"));
...
ok('smoke_v2236 的 package.json 件套计数 pin 已更新为 === 133', s2236.includes('testChain === 170'));
ok('smoke_v2236 的 README 件套口径 pin 已更新为一百三十三件套', s2236.includes('冒烟一百七十件套（一百六十九件套清除）'));
ok('smoke_v2235 的 GAME_VERSION 字面量 pin 已更新为 v22.37', s2235.includes("const GAME_VERSION = 'v22.74';"));
ok('smoke_v2234 的 GAME_VERSION 恒等 pin 已更新为 === v22.37', s2234.includes("GAME_VERSION === 'v22.74'"));
...
ok('smoke_v2232 的 package.json 件套计数 pin 已更新为 === 133', s2232.includes('testChain === 170'));
...
ok('smoke_v2230 的 GAME_VERSION 恒等 pin 已更新为 === v22.37', s2230.includes("GAME_VERSION === 'v22.74'"));
ok('smoke_v2229 的 README 件套口径 pin 已更新为一百三十三件套', s2229.includes('冒烟一百七十件套（一百六十九件套清除）'));
ok('smoke_v2226 的 GAME_VERSION 字面量 pin 已更新为 v22.37', s2226.includes("const GAME_VERSION = 'v22.74';"));
