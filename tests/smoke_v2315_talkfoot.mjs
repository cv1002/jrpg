...
ok('data.js GAME_VERSION 字面量已为 v23.15（旧 v23.14 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.15';") && !dataSrc.includes("const GAME_VERSION = 'v23.14';"));
...
ok('README tests 树串尾已延伸至 smoke_v2315_talkfoot（v2314 后接 v2315）',
  readme.includes('+ smoke_v2314_voices + smoke_v2315_talkfoot（npm test 串跑）'));
ok('README 旧串尾零残留（smoke_v2314_voices（npm test 串跑）不在树尾）',
  !readme.includes('smoke_v2314_voices（npm test 串' + '跑）'));
ok('README 件套口径为二百一十一件套（二百一十件套清除）且旧 210 口径零残留',
  readme.includes('冒烟二百一十一件套（二百一十件套清除）') && !readme.includes('冒烟二百一十件套（二百零九件套清' + '除）'));
...
ok('package.json test 串共 211 件套', testChain === 211, String(testChain));
ok('CHANGELOG 顶部已追加 v23.15 条目', changelog.startsWith('## v23.15 '));
...
ok('smoke_v2314 的 GAME_VERSION 字面量 pin 已更新为 v23.15', s2314.includes("const GAME_VERSION = 'v23.15';"));
ok('smoke_v2314 的 CHANGELOG 顶 pin 已更新为 ## v23.15', s2314.includes("startsWith('## v23.15 "));
ok('smoke_v2314 的件套 pin 已更新为二百一十一件套（二百一十件套清除）', s2314.includes('二百一十一件套（二百一十件套清除）'));
ok('smoke_v2314 的 README 串尾 pin 已延伸至 smoke_v2315_talkfoot',
  s2314.includes('smoke_v2314_voices + smoke_v2315_talkfoot（npm test 串跑）'));
...
ok('smoke_v2314 的 testChain pin 已更新为 211', s2314.includes('testChain === 211'));
