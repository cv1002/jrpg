...
ok('GAME_VERSION 字面量已为 v22.47（旧 v22.46 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.61';") && !dSrc.includes("const GAME_VERSION = 'v22." + "46';"));
...
ok('README 件套口径为一百五十七件套（一百五十六件套清除）',
  readme.includes('冒烟一百五十七件套（一百五十六件套清除）') && !readme.includes('冒烟一百四十二件套（一百四十一件套清' + '除）'));
...
ok('CHANGELOG 顶部已追加 v22.47 条目', changelog.startsWith('## v22.61'));
...
ok('smoke_v2246 的 GAME_VERSION 字面量 pin 已更新为 v22.47（旧 v22.46 零残留）',
  s2246.includes("const GAME_VERSION = 'v22.61';") && !s2246.includes("const GAME_VERSION = 'v22." + "46';"));
ok('smoke_v2246 的 README 件套 pin 已随新现实更新为一百五十七件套（一百五十六件套清除）',
  s2246.includes('一百五十七件套（一百五十六件套清除）'));
ok('smoke_v2246 的 package.json 件套计数 pin 已更新为 === 143', s2246.includes('testChain === 157'));
...
ok('v2143-45「件套守护领先一位」哨兵链已推进至 144（一百五十八件套（一百五十七件套清除））',
