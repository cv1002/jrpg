...
// 入列后 ACH_LIST 总数 27→24——本断言守护「bone 新增一项入列」语义（bone 存在且总数不少于 23），
...
// 灯火同心（allquests）自动跟随：支线总数由 QUESTS 派生（6 → 7 → 8 → 9 → 10，v23.42 side_wolf 入列后随新现实更新）
const achAll = ACH_LIST.find((a) => a.id === 'allquests');
ok('灯火同心支线分母自动跟随为 10（side_wolf 入列，无需改 achievement 本体；v23.42 随新现实更新）',
