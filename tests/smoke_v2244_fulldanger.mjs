...
ok('GAME_VERSION 字面量已为 v22.44（旧 v22.43 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.44';") && !dSrc.includes("const GAME_VERSION = 'v22.43';"));
...
ok('package.json test 串共 140 件套（含 smoke_v2244_fulldanger）', testChain === 140, String(testChain));
ok('package.json 串尾已收录 smoke_v2244_fulldanger',
  pkgSrc.includes('smoke_v2243_encguide.mjs && node tests/smoke_v2244_fulldanger.mjs'));
ok('README tests 树尾已收录 smoke_v2244_fulldanger（串跑链）',
  readmeSrc.includes('smoke_v2243_encguide + smoke_v2244_fulldanger（npm test 串跑）'));
...
ok('smoke_v2243 的 GAME_VERSION 字面量 pin 已更新为 v22.44（旧 v22.43 零残留）',
  s2243.includes("const GAME_VERSION = 'v22.44';") && !s2243.includes("const GAME_VERSION = 'v22.43';"));
ok('smoke_v2243 的 README 件套 pin 已随新现实更新为一百四十件套（一百三十九件套清除）',
  s2243.includes('一百四十件套（一百三十九件套清除）'));
ok('smoke_v2243 的 package.json 件套计数 pin 已更新为 === 140', s2243.includes('testChain === 140'));
ok('smoke_v2243 的 README 串尾 pin 已随新现实延伸至 smoke_v2244_fulldanger',
  s2243.includes('smoke_v2243_encguide + smoke_v2244_fulldanger（npm test 串跑）'));
