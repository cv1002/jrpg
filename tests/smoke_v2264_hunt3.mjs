...
// v2263·v2262·v2261·v2260·v2259·v2258 GAME_VERSION v22.64 / v2143-45 哨兵链 161）随新现实更新 +
...
ok('GAME_VERSION 格式合法且已越过 v22.63（本版守 v22.64）', !!_gv && (_gv[0] > 22 || (_gv[0] === 22 && _gv[1] >= 64)), GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已为 v22.64（旧 v22.63 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v22.64';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "63';"));
...
ok('ACH_LIST 精确总数 48 项（v2229 精确计数 pin 随新现实更新 47→48）', ACH_LIST.length === 48, String(ACH_LIST.length));
...
ok('README tests 树串尾已延伸至 smoke_v2264_hunt3（v2263 后接 v2264）',
  readme.includes('smoke_v2263_ptime3 + smoke_v2264_hunt3（npm test 串跑）'));
ok('README 件套口径为一百六十件套（一百五十九件套清除）且旧 159 口径零残留',
  readme.includes('冒烟一百六十件套（一百五十九件套清除）') && !readme.includes('冒烟一百五十九件套（一百五十八件套清' + '除）'));
...
ok('README 成就口径「48 项」双处同步（快速上手表 C 键行 + 图鉴&成就行）',
  readme.includes('成就一览（全部 48 项进度') && readme.includes('**48 项成就**') &&
  !readme.includes('成就一览（全部 47 项进' + '度') && !readme.includes('**47 项成' + '就**'));
ok('package.json 已收录 smoke_v2264_hunt3（npm test 串跑第 160 份）',
  pkg.includes('smoke_v2264_hunt3.mjs') && pkg.includes('smoke_v2263_ptime3.mjs && node tests/smoke_v2264_hunt3.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 160 件套', testChain === 160, String(testChain));
ok('CHANGELOG 顶为 v22.64 条目', changelog.startsWith('## v22.64 '));
...
ok('smoke_v2263 的 GAME_VERSION 字面量 pin 已更新为 v22.64', s2263.includes("const GAME_VERSION = 'v22.64';"));
ok('smoke_v2263 的 README 件套 pin 已随新现实更新为一百六十件套（一百五十九件套清除）',
  s2263.includes('一百六十件套（一百五十九件套清除）'));
ok('smoke_v2263 的 package.json 件套计数 pin 已更新为 === 160', s2263.includes('testChain === 160'));
ok('smoke_v2263 的 CHANGELOG 顶 pin 已更新为 ## v22.64', s2263.includes("startsWith('## v22.64')"));
ok('smoke_v2263 的 README 串尾 pin 已随新现实延伸至 smoke_v2264_hunt3',
  s2263.includes('smoke_v2263_ptime3 + smoke_v2264_hunt3（npm test 串跑）'));
ok('smoke_v2263 的 package.json 串尾 plain pin 已延伸至 smoke_v2264_hunt3',
  s2263.includes('smoke_v2263_ptime3.mjs && node tests/smoke_v2264_hunt3.mjs"'));
ok('smoke_v2262 的 GAME_VERSION 字面量 pin 已更新为 v22.64', s2262.includes("const GAME_VERSION = 'v22.64';"));
ok('smoke_v2262 的 package.json 串尾 plain pin 已延伸至 smoke_v2264_hunt3',
  s2262.includes('smoke_v2262_crystal.mjs && node tests/smoke_v2263_ptime3.mjs && node tests/smoke_v2264_hunt3.mjs"'));
ok('smoke_v2261 的 GAME_VERSION 字面量 pin 已更新为 v22.64', s2261.includes("const GAME_VERSION = 'v22.64';"));
ok('smoke_v2260 的 GAME_VERSION 恒等 pin 族已随新现实全库更新（v22.64 字面量 pin 落位）',
  s2260.includes("const GAME_VERSION = 'v22.64';"));
ok('smoke_v2260 的 README 件套 pin 已随新现实更新为一百六十件套（一百五十九件套清除）',
  s2260.includes('一百六十件套（一百五十九件套清除）'));
ok('smoke_v2260 的 package.json 件套计数 pin 已更新为 === 160', s2260.includes('testChain === 160'));
ok('smoke_v2260 的 CHANGELOG 顶 pin 已更新为 ## v22.64', s2260.includes("startsWith('## v22.64')"));
ok('smoke_v2260 的 README 串尾 pin 已随新现实延伸至 smoke_v2264_hunt3',
  s2260.includes('smoke_v2262_crystal + smoke_v2263_ptime3 + smoke_v2264_hunt3（npm test 串跑）'));
ok('smoke_v2259 的 GAME_VERSION 字面量 pin 已更新为 v22.64', s2259.includes("const GAME_VERSION = 'v22.64';"));
ok('smoke_v2259 的 README 件套 pin 已随新现实更新为一百六十件套（一百五十九件套清除）',
  s2259.includes('一百六十件套（一百五十九件套清除）'));
ok('smoke_v2259 的 package.json 件套计数 pin 已更新为 === 160', s2259.includes('testChain === 160'));
ok('smoke_v2259 的 CHANGELOG 顶 pin 已更新为 ## v22.64', s2259.includes("startsWith('## v22.64')"));
ok('smoke_v2258 的 GAME_VERSION 字面量 pin 已更新为 v22.64', s2258.includes("const GAME_VERSION = 'v22.64';"));
ok('smoke_v2229 的 ACH_LIST 精确总数 pin 已更新为 === 48（47→48）',
  s2229.includes('ACH_LIST.length === 48') && !s2229.includes('ACH_LIST.length === 4' + '7'));
ok('smoke_v2199 的 README 成就 pin 已随新现实更新为 48 项双处落位',
  s2199.includes('成就一览（全部 48 项进度') && s2199.includes('**48 项成就**'));
ok('v2143-45「件套守护领先一位」哨兵链已推进至 161（一百六十件套（一百五十九件套清除））',
  s2143.includes('一百六十一件套（一百六十件套清除）') && s2143.includes("!readme.includes('一百六十一件套')"));
ok('smoke_v2144 的哨兵链 pin 已随新现实推进（!readme 不含一百六十一件套）',
  s2144.includes("!readme.includes('一百六十一件套')"));
ok('smoke_v2145 的哨兵链 pin 已随新现实推进（一百六十件套（一百五十九件套清除））',
  s2145.includes('一百六十一件套（一百六十件套清除）'));
