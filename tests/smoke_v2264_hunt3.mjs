...
ok('data.js GAME_VERSION 字面量已为 v22.64（旧 v22.63 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v22.74';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "63';"));
...
ok('ACH_LIST 精确总数 48 项（v2229 精确计数 pin 随新现实更新 47→48）', ACH_LIST.length === 58, String(ACH_LIST.length));
...
ok('README tests 树串尾已延伸至 smoke_v2264_hunt3（v2263 后接 v2264）',
  readme.includes('smoke_v2263_ptime3 + smoke_v2264_hunt3 + smoke_v2265_lucky3 + smoke_v2266_stock3 + smoke_v2267_elixir3 + smoke_v2268_brew3 + smoke_v2269_mush3 + smoke_v2270_outstep + smoke_v2271_outstep2 + smoke_v2272_scholar2 + smoke_v2273_seen5 + smoke_v2274_seen2（npm test 串跑）'));
ok('README 件套口径为一百七十件套（一百六十九件套清除）且旧 159 口径零残留',
  readme.includes('冒烟一百七十件套（一百六十九件套清除）') && !readme.includes('冒烟一百五十九件套（一百五十八件套清' + '除）'));
...
ok('package.json test 串共 160 件套', testChain === 170, String(testChain));
ok('CHANGELOG 顶为 v22.64 条目', changelog.startsWith('## v22.74 '));
...
ok('smoke_v2263 的 GAME_VERSION 字面量 pin 已更新为 v22.64', s2263.includes("const GAME_VERSION = 'v22.74';"));
ok('smoke_v2263 的 README 件套 pin 已随新现实更新为一百七十件套（一百六十九件套清除）',
  s2263.includes('一百七十件套（一百六十九件套清除）'));
ok('smoke_v2263 的 package.json 件套计数 pin 已更新为 === 160', s2263.includes('testChain === 170'));
ok('smoke_v2263 的 CHANGELOG 顶 pin 已更新为 ## v22.64', s2263.includes("startsWith('## v22.74')"));
...
ok('smoke_v2262 的 GAME_VERSION 字面量 pin 已更新为 v22.64', s2262.includes("const GAME_VERSION = 'v22.74';"));
...
ok('smoke_v2261 的 GAME_VERSION 字面量 pin 已更新为 v22.64', s2261.includes("const GAME_VERSION = 'v22.74';"));
...
ok('smoke_v2260 的 package.json 件套计数 pin 已更新为 === 160', s2260.includes('testChain === 170'));
ok('smoke_v2260 的 CHANGELOG 顶 pin 已更新为 ## v22.64', s2260.includes("startsWith('## v22.74')"));
...
ok('smoke_v2259 的 GAME_VERSION 字面量 pin 已更新为 v22.64', s2259.includes("const GAME_VERSION = 'v22.74';"));
ok('smoke_v2259 的 README 件套 pin 已随新现实更新为一百七十件套（一百六十九件套清除）',
  s2259.includes('一百七十件套（一百六十九件套清除）'));
ok('smoke_v2259 的 package.json 件套计数 pin 已更新为 === 160', s2259.includes('testChain === 170'));
ok('smoke_v2259 的 CHANGELOG 顶 pin 已更新为 ## v22.64', s2259.includes("startsWith('## v22.74')"));
ok('smoke_v2258 的 GAME_VERSION 字面量 pin 已更新为 v22.64', s2258.includes("const GAME_VERSION = 'v22.74';"));
ok('smoke_v2229 的 ACH_LIST 精确总数 pin 已更新为 === 48（47→48）',
  s2229.includes('ACH_LIST.length === 58') && !s2229.includes('ACH_LIST.length === 4' + '7'));
