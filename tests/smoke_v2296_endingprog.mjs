...
ok('data.js GAME_VERSION 字面量为 v22.96', dataSrc.includes("const GAME_VERSION = 'v22.97';"));
...
ok('GAME_VERSION 格式合法且已越过 v22.95（本版守 v22.96）', !!_gv && (_gv[0] > 22 || (_gv[0] === 22 && _gv[1] >= 97)), GAME_VERSION);
...
ok('README 件套口径为一百九十三件套（一百九十二件套清除）且旧 191 口径零残留',
  readme.includes('冒烟一百九十三件套（一百九十二件套清除）') && !readme.includes('冒烟一百九十一件套（一百九十件套清' + '除）'));
...
ok('package.json 串尾为 ... smoke_v2295_deadprog.mjs && node tests/smoke_v2296_endingprog.mjs && node tests/smoke_v2297_chestmid.mjs"',
  pkg.includes('node tests/smoke_v2295_deadprog.mjs && node tests/smoke_v2296_endingprog.mjs && node tests/smoke_v2297_chestmid.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 192 件套', testChain === 193, String(testChain));
ok('CHANGELOG 顶部已追加 v22.96 条目', changelog.startsWith('## v22.97 '));
...
ok('smoke_v2295 的 GAME_VERSION 字面量 pin 已更新为 v22.96', s2295.includes("const GAME_VERSION = 'v22.97';"));
ok('smoke_v2295 的 CHANGELOG 顶 pin 已更新为 ## v22.96', s2295.includes("startsWith('## v22.97 '"));
ok('smoke_v2295 的件套 pin 已更新为一百九十三件套（一百九十二件套清除）', s2295.includes('一百九十三件套（一百九十二件套清除）'));
ok('smoke_v2295 的 README 串尾 pin 已延伸至 smoke_v2296_endingprog',
  s2295.includes('smoke_v2294_crystalwatch + smoke_v2295_deadprog + smoke_v2296_endingprog + smoke_v2297_chestmid（npm test 串跑）'));
ok('smoke_v2295 的 package 串尾 pin 已延伸至 smoke_v2296_endingprog',
  s2295.includes('node tests/smoke_v2295_deadprog.mjs && node tests/smoke_v2296_endingprog.mjs && node tests/smoke_v2297_chestmid.mjs"'));
ok('smoke_v2295 的 testChain pin 已更新为 192', s2295.includes('testChain === 193'));
ok('smoke_v2295 的版本锚已推进至 >= 96', s2295.includes('_gv[1] >= 97'));
ok('smoke_v2294 的 GAME_VERSION 字面量 pin 已更新为 v22.96', s2294.includes("const GAME_VERSION = 'v22.97';"));
ok('smoke_v2294 的 testChain pin 已更新为 192', s2294.includes('testChain === 193'));
