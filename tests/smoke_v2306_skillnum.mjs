...
// 同步（冒烟二百二十二件套（二百二十一件套清除）/串尾/入库 202 份/顶 pin）+ 姊妹件套 pin（smoke_v2305 随新
...
ok('data.js GAME_VERSION 字面量已为 v23.06（旧 v23.05 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.98';") && !dataSrc.includes("const GAME_VERSION = 'v23." + "05';"));
...
ok('README 件套口径为二百二十二件套（二百二十一件套清除）且旧 201 口径零残留',
  readme.includes('冒烟二百二十二件套（二百二十一件套清除）') && !readme.includes('冒烟二百零一件套（二百件套清' + '除）'));
...
ok('package.json test 串共 202 件套', testChain === 222, String(testChain));
ok('CHANGELOG 顶部已追加 v23.06 条目', changelog.startsWith('## v23.98 '));
...
ok('smoke_v2305 的 GAME_VERSION 字面量 pin 已更新为 v23.06', s2305.includes("const GAME_VERSION = 'v23.98';"));
ok('smoke_v2305 的 CHANGELOG 顶 pin 已更新为 ## v23.06',
  s2305.includes("startsWith('## v23.98 "));
ok('smoke_v2305 的件套 pin 已更新为二百二十二件套（二百二十一件套清除）',
  s2305.includes('二百二十二件套（二百二十一件套清除）'));
...
ok('smoke_v2305 的 testChain pin 已更新为 202', s2305.includes('testChain === 222'));
