...
// 同步（冒烟二百零六件套（二百零五件套清除）/串尾/入库 202 份/顶 pin）+ 姊妹件套 pin（smoke_v2305 随新
...
ok('data.js GAME_VERSION 字面量已为 v23.06（旧 v23.05 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.10';") && !dataSrc.includes("const GAME_VERSION = 'v23." + "05';"));
...
ok('README tests 树串尾已延伸至 smoke_v2306_skillnum（v2305 后接 v2306）',
  readme.includes('smoke_v2305_monnum + smoke_v2306_skillnum + smoke_v2307_bossnum + smoke_v2308_diffnum + smoke_v2309_questnum + smoke_v2310_diffsum（npm test 串跑）'));
ok('README 旧串尾零残留（v2305 后无串尾收口）',
  !readme.includes('smoke_v2304_achgoal + smoke_v2305_monnum（npm test 串' + '跑）'));
ok('README 件套口径为二百零六件套（二百零五件套清除）且旧 201 口径零残留',
  readme.includes('冒烟二百零六件套（二百零五件套清除）') && !readme.includes('冒烟二百零一件套（二百件套清' + '除）'));
...
ok('package.json 串尾为 ... smoke_v2305_monnum.mjs && node tests/smoke_v2306_skillnum.mjs && node tests/smoke_v2307_bossnum.mjs && node tests/smoke_v2308_diffnum.mjs && node tests/smoke_v2309_questnum.mjs && node tests/smoke_v2310_diffsum.mjs"',
  pkg.includes('node tests/smoke_v2305_monnum.mjs && node tests/smoke_v2306_skillnum.mjs && node tests/smoke_v2307_bossnum.mjs && node tests/smoke_v2308_diffnum.mjs && node tests/smoke_v2309_questnum.mjs && node tests/smoke_v2310_diffsum.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 202 件套', testChain === 206, String(testChain));
ok('CHANGELOG 顶部已追加 v23.06 条目', changelog.startsWith('## v23.10 '));
...
ok('smoke_v2305 的 GAME_VERSION 字面量 pin 已更新为 v23.06', s2305.includes("const GAME_VERSION = 'v23.10';"));
ok('smoke_v2305 的 CHANGELOG 顶 pin 已更新为 ## v23.06',
  s2305.includes("startsWith('## v23.10 "));
ok('smoke_v2305 的件套 pin 已更新为二百零六件套（二百零五件套清除）',
  s2305.includes('二百零六件套（二百零五件套清除）'));
ok('smoke_v2305 的 README 串尾 pin 已延伸至 smoke_v2306_skillnum',
  s2305.includes('smoke_v2305_monnum + smoke_v2306_skillnum + smoke_v2307_bossnum + smoke_v2308_diffnum + smoke_v2309_questnum + smoke_v2310_diffsum（npm test 串跑）'));
ok('smoke_v2305 的 package 串尾 pin 已延伸至 smoke_v2306_skillnum',
  s2305.includes('node tests/smoke_v2305_monnum.mjs && node tests/smoke_v2306_skillnum.mjs && node tests/smoke_v2307_bossnum.mjs && node tests/smoke_v2308_diffnum.mjs && node tests/smoke_v2309_questnum.mjs && node tests/smoke_v2310_diffsum.mjs"'));
ok('smoke_v2305 的 testChain pin 已更新为 202', s2305.includes('testChain === 206'));
