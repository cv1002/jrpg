...
ok('GAME_VERSION 字面量已为 v22.62（旧 v22.61 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.62';") && !dSrc.includes("const GAME_VERSION = 'v22." + "61';"));
...
ok('README tests 树收录 smoke_v2262_crystal 且位于串尾',
  readme.includes('smoke_v2260_fountripple + smoke_v2261_rich3 + smoke_v2262_crystal（npm test 串跑）'));
ok('README 件套口径为一百五十八件套（一百五十七件套清除）且旧 157 口径零残留',
  readme.includes('冒烟一百五十八件套（一百五十七件套清除）') && !readme.includes('冒烟一百五十七件套（一百五十六件套清' + '除）'));
...
ok('package.json 串尾为 smoke_v2261_rich3.mjs && node tests/smoke_v2262_crystal.mjs"',
  pkg.includes('smoke_v2261_rich3.mjs && node tests/smoke_v2262_crystal.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 158 件套', testChain === 158, String(testChain));
ok('CHANGELOG 顶为 v22.62 条目', changelog.startsWith('## v22.62 '));
...
ok('smoke_v2261 的 GAME_VERSION 字面量 pin 已更新为 v22.62', s2261.includes("const GAME_VERSION = 'v22.62';"));
ok('smoke_v2260 的 GAME_VERSION 恒等 pin 族已随新现实全库更新（v22.62 字面量 pin 落位）',
  s2260.includes("const GAME_VERSION = 'v22.62';"));
ok('smoke_v2260 的 README 件套 pin 已随新现实更新为一百五十八件套（一百五十七件套清除）',
  s2260.includes('一百五十八件套（一百五十七件套清除）'));
ok('smoke_v2260 的 package.json 件套计数 pin 已更新为 === 158', s2260.includes('testChain === 158'));
ok('smoke_v2260 的 CHANGELOG 顶 pin 已更新为 ## v22.62', s2260.includes("startsWith('## v22.62')") || s2260.includes("startsWith('## v22.62 ')"));
ok('smoke_v2260 的 README 串尾 pin 已随新现实延伸至 smoke_v2262_crystal',
  s2260.includes('smoke_v2261_rich3 + smoke_v2262_crystal（npm test 串跑）'));
ok('smoke_v2261 的 package.json 串尾 plain pin 已延伸至 smoke_v2262_crystal',
  s2261.includes('smoke_v2261_rich3.mjs && node tests/smoke_v2262_crystal.mjs"'));
ok('smoke_v2259 的 GAME_VERSION 字面量 pin 已更新为 v22.62', s2259.includes("const GAME_VERSION = 'v22.62';"));
...
ok('smoke_v2144 的哨兵链 pin 已随新现实推进（!readme.includes(\'一百五十九件套\')）',
  s2144.includes("!readme.includes('一百五十九件套')"));
