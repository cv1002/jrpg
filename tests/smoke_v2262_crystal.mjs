...
ok('GAME_VERSION 字面量已为 v22.62（旧 v22.61 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.71';") && !dSrc.includes("const GAME_VERSION = 'v22." + "61';"));
...
ok('README tests 树收录 smoke_v2262_crystal 且位于串尾',
  readme.includes('smoke_v2260_fountripple + smoke_v2261_rich3 + smoke_v2262_crystal + smoke_v2263_ptime3 + smoke_v2264_hunt3 + smoke_v2265_lucky3 + smoke_v2266_stock3 + smoke_v2267_elixir3 + smoke_v2268_brew3 + smoke_v2269_mush3 + smoke_v2270_outstep + smoke_v2271_outstep2（npm test 串跑）'));
ok('README 件套口径为一百六十七件套（一百六十六件套清除）且旧 157 口径零残留',
  readme.includes('冒烟一百六十七件套（一百六十六件套清除）') && !readme.includes('冒烟一百五十七件套（一百五十六件套清' + '除）'));
...
ok('package.json test 串共 158 件套', testChain === 167, String(testChain));
ok('CHANGELOG 顶为 v22.62 条目', changelog.startsWith('## v22.71 '));
...
ok('smoke_v2261 的 GAME_VERSION 字面量 pin 已更新为 v22.62', s2261.includes("const GAME_VERSION = 'v22.71';"));
...
ok('smoke_v2260 的 package.json 件套计数 pin 已更新为 === 158', s2260.includes('testChain === 167'));
ok('smoke_v2260 的 CHANGELOG 顶 pin 已更新为 ## v22.62', s2260.includes("startsWith('## v22.71')") || s2260.includes("startsWith('## v22.71 ')"));
...
ok('smoke_v2259 的 GAME_VERSION 字面量 pin 已更新为 v22.62', s2259.includes("const GAME_VERSION = 'v22.71';"));
ok('v2143-45「件套守护领先一位」哨兵链已推进至 159（一百六十七件套（一百六十六件套清除））',
