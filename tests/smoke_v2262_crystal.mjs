...
ok('GAME_VERSION 字面量已为 v22.62（旧 v22.61 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.12';") && !dSrc.includes("const GAME_VERSION = 'v22." + "61';"));
...
ok('README 件套口径为二百零八件套（二百零七件套清除）且旧 157 口径零残留',
  readme.includes('冒烟二百零八件套（二百零七件套清除）') && !readme.includes('冒烟一百五十七件套（一百五十六件套清' + '除）'));
...
ok('package.json test 串共 158 件套', testChain === 208, String(testChain));
ok('CHANGELOG 顶为 v22.62 条目', changelog.startsWith('## v23.12 '));
...
ok('smoke_v2261 的 GAME_VERSION 字面量 pin 已更新为 v22.62', s2261.includes("const GAME_VERSION = 'v23.12';"));
...
ok('smoke_v2260 的 package.json 件套计数 pin 已更新为 === 158', s2260.includes('testChain === 208'));
ok('smoke_v2260 的 CHANGELOG 顶 pin 已更新为 ## v22.62', s2260.includes("startsWith('## v23.12')") || s2260.includes("startsWith('## v22.78 ')"));
...
ok('smoke_v2259 的 GAME_VERSION 字面量 pin 已更新为 v22.62', s2259.includes("const GAME_VERSION = 'v23.12';"));
ok('v2143-45「件套守护领先一位」哨兵链已推进至 159（二百零八件套（二百零七件套清除））',
