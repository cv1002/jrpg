...
ok('GAME_VERSION 字面量已为 v22.46（旧 v22.45 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.52';") && !dSrc.includes("const GAME_VERSION = 'v22." + "45';"));
...
ok('NPC 总数保持 31（本轮零 NPC 变更）', Object.keys(NPC_SPOTS).length === 34, Object.keys(NPC_SPOTS).length);
...
ok('README tests 树已收录 smoke_v2246_fountgauge 且位于串尾', readme.includes('smoke_v2245_watcher + smoke_v2246_fountgauge + smoke_v2247_villagewell + smoke_v2248_mapguide + smoke_v2249_shopkeep + smoke_v2250_brewer + smoke_v2251_oathkeep + smoke_v2252_rail（npm test 串跑）'));
ok('README 件套口径为一百四十八件套（一百四十七件套清除）',
  readme.includes('冒烟一百四十八件套（一百四十七件套清除）') && !readme.includes('冒烟一百四十二件套（一百四十件套清' + '除）'));
...
ok('package.json test 串共 143 件套', testChain === 148, String(testChain));
ok('CHANGELOG 顶部已追加 v22.46 条目', changelog.startsWith('## v22.52'));
...
ok('smoke_v2245 的 GAME_VERSION 字面量 pin 已更新为 v22.46（旧 v22.45 零残留）',
  s2245.includes("const GAME_VERSION = 'v22.52';") && !s2245.includes("const GAME_VERSION = 'v22." + "45';"));
ok('smoke_v2245 的 README 件套 pin 已随新现实更新为一百四十八件套（一百四十七件套清除）',
  s2245.includes('一百四十八件套（一百四十七件套清除）'));
ok('smoke_v2245 的 package.json 件套计数 pin 已更新为 === 143', s2245.includes('testChain === 148'));
ok('smoke_v2245 的 README 串尾 pin 已随新现实延伸至 smoke_v2246_fountgauge',
  s2245.includes('smoke_v2245_watcher + smoke_v2246_fountgauge + smoke_v2247_villagewell + smoke_v2248_mapguide + smoke_v2249_shopkeep + smoke_v2250_brewer + smoke_v2251_oathkeep + smoke_v2252_rail（npm test 串跑）'));
ok('smoke_v2245 的 NPC 总数 pin 保持 31（本轮零 NPC 变更）', s2245.includes('NPC_SPOTS).length === 34'));
const s2143 = fs.readFileSync(path.join(ROOT, 'tests/smoke_v2143_talkekey.mjs'), 'utf8');
ok('v2143-45「件套守护领先一位」哨兵链已推进至 143（一百四十九件套（一百四十八件套清除））',
  s2143.includes('一百四十九件套（一百四十八件套清除）') && s2143.includes("!readme.includes('一百四十九件套')"));

// —— 旧代 v22.45 pin 全库零残留 ——
const allTests = fs.readdirSync(path.join(ROOT, 'tests')).filter((f) => f.endsWith('.mjs') && f !== 'smoke_v2246_fountgauge.mjs && node tests/smoke_v2247_villagewell.mjs');
const stale = [];
