...
ok('GAME_VERSION 字面量已为 v22.46（旧 v22.45 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.97';") && !dSrc.includes("const GAME_VERSION = 'v22." + "45';"));
...
ok('README 件套口径为二百二十一件套（二百二十件套清除）',
  readme.includes('冒烟二百二十一件套（二百二十件套清除）') && !readme.includes('冒烟一百四十二件套（一百四十件套清' + '除）'));
...
ok('CHANGELOG 顶部已追加 v22.46 条目', changelog.startsWith('## v23.97'));
...
ok('smoke_v2245 的 GAME_VERSION 字面量 pin 已更新为 v22.46（旧 v22.45 零残留）',
  s2245.includes("const GAME_VERSION = 'v23.97';") && !s2245.includes("const GAME_VERSION = 'v22." + "45';"));
ok('smoke_v2245 的 README 件套 pin 已随新现实更新为二百二十一件套（二百二十件套清除）',
  s2245.includes('二百二十一件套（二百二十件套清除）'));
ok('smoke_v2245 的 package.json 件套计数 pin 已更新为 === 143', s2245.includes('testChain === 221'));
...
ok('v2143-45「件套守护领先一位」哨兵链已推进至 143（二百二十一件套（二百二十件套清除））',
