...
ok('GAME_VERSION 字面量已为 v22.46（旧 v22.45 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.89';") && !dSrc.includes("const GAME_VERSION = 'v22." + "45';"));
...
ok('NPC 总数保持 31（本轮零 NPC 变更）', Object.keys(NPC_SPOTS).length === 35, Object.keys(NPC_SPOTS).length);
...
ok('README 件套口径为一百八十五件套（一百八十四件套清除）',
  readme.includes('冒烟一百八十五件套（一百八十四件套清除）') && !readme.includes('冒烟一百四十二件套（一百四十件套清' + '除）'));
...
ok('package.json test 串共 143 件套', testChain === 185, String(testChain));
ok('CHANGELOG 顶部已追加 v22.46 条目', changelog.startsWith('## v22.89'));
...
ok('smoke_v2245 的 GAME_VERSION 字面量 pin 已更新为 v22.46（旧 v22.45 零残留）',
  s2245.includes("const GAME_VERSION = 'v22.89';") && !s2245.includes("const GAME_VERSION = 'v22." + "45';"));
ok('smoke_v2245 的 README 件套 pin 已随新现实更新为一百八十五件套（一百八十四件套清除）',
  s2245.includes('一百八十五件套（一百八十四件套清除）'));
ok('smoke_v2245 的 package.json 件套计数 pin 已更新为 === 143', s2245.includes('testChain === 185'));
...
ok('smoke_v2245 的 NPC 总数 pin 保持 31（本轮零 NPC 变更）', s2245.includes('NPC_SPOTS).length === 35'));
const s2143 = fs.readFileSync(path.join(ROOT, 'tests/smoke_v2143_talkekey.mjs'), 'utf8');
ok('v2143-45「件套守护领先一位」哨兵链已推进至 143（一百八十六件套（一百八十五件套清除））',
