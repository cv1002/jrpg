...
ok('GAME_VERSION 字面量已为 v22.8（本版独占精确锚点）', GAME_VERSION === 'v22.16', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.8', dataSrc.includes("const GAME_VERSION = 'v22.16';"));
...
ok('README tests 树收录 smoke_v2208_elixir 且位于串尾', readme.includes('smoke_v2209_achstatus + smoke_v2211_lampkid + smoke_v2212_volume + smoke_v2213_teller + smoke_v2214_mush + smoke_v2215_tutorvol + smoke_v2216_picker（npm test 串跑）'));
ok('README 件套口径为一百一十二件套（一百一十一件套清除）',
  readme.includes('冒烟一百一十二件套（一百一十一件套清除）') && !readme.includes('冒烟一百零三件套（一百零二件套清' + '除）'));
