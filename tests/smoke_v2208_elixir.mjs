...
ok('GAME_VERSION 字面量已为 v22.8（本版独占精确锚点）', GAME_VERSION === 'v22.12', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.8', dataSrc.includes("const GAME_VERSION = 'v22.12';"));
...
ok('README tests 树收录 smoke_v2208_elixir 且位于串尾', readme.includes('smoke_v2209_achstatus + smoke_v2211_lampkid + smoke_v2212_volume（npm test 串跑）'));
ok('README 件套口径为一百零八件套（一百零七件套清除）',
  readme.includes('冒烟一百零八件套（一百零七件套清除）') && !readme.includes('冒烟一百零三件套（一百零二件套清' + '除）'));
...
for (const nm of ['smoke_v2207_titledel.mjs', 'smoke_v2206_stock2.mjs', 'smoke_v2205_fragtitle.mjs',
  ...
  ok(`${nm} 的 README tests 树尾 pin 已随新现实更新（+ smoke_v2208_elixir）`,
    src.includes('smoke_v2209_achstatus + smoke_v2211_lampkid + smoke_v2212_volume（npm test 串跑）'));
}
