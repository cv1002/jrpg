...
ok('GAME_VERSION 字面量已为 v22.8（本版独占精确锚点）', GAME_VERSION === 'v22.13', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.8', dataSrc.includes("const GAME_VERSION = 'v22.13';"));
ok('data.js 仍保留 v22.7 历史注释（累积注释块，姊妹 pin 不失效）', dataSrc.includes('v22.7 标题页 X 删除存档槽'));
ok('data.js 导出 ELIXIR_STOCK_GOAL（export 块落位，与 POTIONS2_GOAL 相邻）', dataSrc.includes('POTIONS2_GOAL, ELIXIR_STOCK_GOAL, PERFECTION_GOLD'));
...
ok('ACH_LIST 精确总数 41 项（本版独占精确计数，40→41）', ACH_LIST.length === 41, String(ACH_LIST.length));
...
ok('README tests 树收录 smoke_v2208_elixir 且位于串尾', readme.includes('smoke_v2209_achstatus + smoke_v2211_lampkid + smoke_v2212_volume + smoke_v2213_teller（npm test 串跑）'));
ok('README 件套口径为一百零九件套（一百零八件套清除）',
  readme.includes('冒烟一百零九件套（一百零八件套清除）') && !readme.includes('冒烟一百零三件套（一百零二件套清' + '除）'));
ok('README 含 v22.8 守护描述', readme.includes('v22.8 起含新成就「灵药盈囊」'));
ok('README 成就口径「41 项」双处同步（快速上手表 C 键行 + 图鉴&成就行）',
  readme.includes('成就一览（全部 41 项进度') && readme.includes('**41 项成就**') &&
...
for (const nm of suite104) {
  ...
  ok(`${nm} 的 README 件套 pin 已随新现实更新为一百零九件套（一百零八件套清除）`,
    src.includes('一百零九件套（一百零八件套清除）'));
}
...
for (const nm of vers104) {
  ...
  ok(`${nm} 的 GAME_VERSION 字面量 pin 已随新现实更新为 v22.8`,
    src.includes("const GAME_VERSION = 'v22.13';"));
}
for (const nm of ['smoke_v2207_titledel.mjs', 'smoke_v2206_stock2.mjs', 'smoke_v2205_fragtitle.mjs',
  ...
  ok(`${nm} 的 GAME_VERSION 恒等 pin（===）已随新现实更新为 v22.8`,
    src.includes("GAME_VERSION === 'v22.13'"));
}
for (const nm of ['smoke_v2207_titledel.mjs', 'smoke_v2206_stock2.mjs', 'smoke_v2205_fragtitle.mjs',
  'smoke_v2204_brew2.mjs', 'smoke_v2203_fragdead.mjs', 'smoke_v2202_fragwin.mjs', 'smoke_v2201_fragstatus.mjs',
  'smoke_v2200_stock.mjs', 'smoke_v2199_rich2.mjs', 'smoke_v2198_winsave.mjs', 'smoke_v2197_lucky2.mjs',
  ...
  ok(`${nm} 的 README tests 树尾 pin 已随新现实更新（+ smoke_v2208_elixir）`,
    src.includes('smoke_v2209_achstatus + smoke_v2211_lampkid + smoke_v2212_volume + smoke_v2213_teller（npm test 串跑）'));
}
...
for (const nm of achFiles) {
  ...
  ok(`${nm} 的 README 成就 pin 已随新现实更新为 41 项双处落位`,
    src.includes('成就一览（全部 41 项进度') && src.includes('**41 项成就**'));
}
