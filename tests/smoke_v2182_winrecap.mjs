...
ok('GAME_VERSION 格式合法且已越过 v21.81', !!_gv && (_gv[0] > 21 || (_gv[0] === 21 && _gv[1] >= 97)), GAME_VERSION);
...
ok('README 件套口径已更新为一百九十三件套（一百九十二件套清除）',
  readme.includes('冒烟一百九十三件套（一百九十二件套清除）') && !readme.includes('冒烟七十七件套（七十六件套清除）'));
...
for (const [nm, src] of [['smoke_v2181', s2181], ['smoke_v2180', s2180], ['smoke_v2179', s2179], ['smoke_v2178', s2178], ['smoke_v2177', s2177], ['smoke_v2176', s2176]]) {
  ok(`${nm} 的 README 件套 pin 已随新现实更新为一百九十三件套（一百九十二件套清除）`,
    src.includes('一百九十三件套（一百九十二件套清除）') && !src.includes('七十七件套（七十六件套清除）'));
}
