...
ok('GAME_VERSION 格式合法且已越过 v21.81', !!_gv && (_gv[0] > 21 || (_gv[0] === 21 && _gv[1] >= 99)), GAME_VERSION);
...
ok('drawWin 页脚补 P 口径（按 Enter/E 观看尾声 · 按 P 存档 · 按 R 重开新档(连按两次)，v21.98 补 P / v22.85 补 E）',
...
ok('README 件套口径已更新为二百一十四件套（二百一十三件套清除）',
  readme.includes('冒烟二百一十四件套（二百一十三件套清除）') && !readme.includes('冒烟七十七件套（七十六件套清除）'));
...
for (const [nm, src] of [['smoke_v2181', s2181], ['smoke_v2180', s2180], ['smoke_v2179', s2179], ['smoke_v2178', s2178], ['smoke_v2177', s2177], ['smoke_v2176', s2176]]) {
  ok(`${nm} 的 README 件套 pin 已随新现实更新为二百一十四件套（二百一十三件套清除）`,
    src.includes('二百一十四件套（二百一十三件套清除）') && !src.includes('七十七件套（七十六件套清除）'));
}
