...
ok('GAME_VERSION 字面量已为 v22.6（本版独占精确锚点）', GAME_VERSION === 'v22.13', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.6', dataSrc.includes("const GAME_VERSION = 'v22.13';"));
ok('data.js 仍保留 v22.5 历史注释（累积注释块，姊妹 pin 不失效）', dataSrc.includes('v22.5 标题页存档预览补「🕯️ 记忆碎片 N/4」'));
ok('data.js 导出 POTIONS2_GOAL（export 块落位，与 POTIONS_GOAL 相邻）', dataSrc.includes('POTIONS_GOAL, POTIONS2_GOAL, ELIXIR_STOCK_GOAL, PERFECTION_GOLD'));
...
ok('README tests 树收录 smoke_v2206_stock2 且位于串尾', readme.includes('smoke_v2209_achstatus + smoke_v2211_lampkid + smoke_v2212_volume + smoke_v2213_teller（npm test 串跑）'));
ok('README 件套口径为一百零九件套（一百零八件套清除）',
  readme.includes('冒烟一百零九件套（一百零八件套清除）') && !readme.includes('冒烟一百零一件套（一百件套清' + '除）'));
