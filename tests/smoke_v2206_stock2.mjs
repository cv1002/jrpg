...
ok('GAME_VERSION 字面量已为 v22.6（本版独占精确锚点）', GAME_VERSION === 'v22.6', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.6', dataSrc.includes("const GAME_VERSION = 'v22.6';"));
ok('data.js 仍保留 v22.5 历史注释（累积注释块，姊妹 pin 不失效）', dataSrc.includes('v22.5 标题页存档预览补「🕯️ 记忆碎片 N/4」'));
ok('data.js 导出 POTIONS2_GOAL（export 块落位，与 POTIONS_GOAL 相邻）', dataSrc.includes('POTIONS_GOAL, POTIONS2_GOAL, PERFECTION_GOLD'));
...
ok('ACH_LIST 精确总数 40 项（本版独占精确计数，39→40）', ACH_LIST.length === 40, String(ACH_LIST.length));
...
ok('README tests 树收录 smoke_v2206_stock2 且位于串尾', readme.includes('smoke_v2205_fragtitle + smoke_v2206_stock2（npm test 串跑）'));
ok('README 件套口径为一百零二件套（一百零一件套清除）',
  readme.includes('冒烟一百零二件套（一百零一件套清除）') && !readme.includes('冒烟一百零一件套（一百件套清' + '除）'));
...
for (const nm of ['smoke_v2205_fragtitle.mjs', 'smoke_v2204_brew2.mjs', 'smoke_v2203_fragdead.mjs',
  ...
  ok(`${nm} 的 README tests 树尾 pin 已随新现实更新（+ smoke_v2206_stock2）`,
    src.includes('smoke_v2205_fragtitle + smoke_v2206_stock2（npm test 串跑）'));
}
