...
ok('试炼进阶页 10 行（v21.72 记忆碎片行增行；sp=34 档不变）', page3.length === 10, `实际 ${page3.length}`);
...
// —— 页长派生预算：v21.72 起 10 行（记忆碎片行）+ 2 个 r[2]（试炼三连战/终焉之神）——
// 末行（终焉之神）基线 80+9*34+1*16=402（r[2] 只有一行在其之前）、其 r[2] 402+18=420 不触页脚 452 ——
const r2Count = page3.filter((r) => r[2]).length;
ok('试炼进阶页 r[2] 仍 2 个（试炼三连战/终焉之神，v21.72 记忆碎片行无 r[2]）', r2Count === 2, `实际 ${r2Count}`);
ok('末行基线 80+9*34+1*16=402、其 r[2] 402+18=420 ≤440（页脚 452 之前，sp=34 档）', 80 + 9 * 34 + 1 * 16 === 402 && 402 + 18 <= 440);
...
ok('其余三页行数：操作 14 / 地图指南 7（v22.37 起加小地图图例两行）/ 魔物状态 10',
  HELP_PAGES[0].length === 14 && HELP_PAGES[1].length === 8 && HELP_PAGES[2].length === 10);
