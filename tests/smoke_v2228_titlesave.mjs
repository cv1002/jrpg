...
ok('GAME_VERSION 字面量已为 v22.28（旧 v22.27 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.99';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "27';"));
...
// v23.26 口径收口：主提示行已删「WASD 移动」（title.onKey 无 W/S 分支），本行断言随之更新——
// 余下标志（选槽/R 重开两按/L 读档）逐字保留，并反向确认不实 token 已清零（承 v21.18 按键提示如实口径）
ok('警告行之后仍保留主提示行（零回归）', (() => {
...
ok('README 件套口径为二百二十三件套（二百二十二件套清除）',
  readme.includes('冒烟二百二十三件套（二百二十二件套清除）') && !readme.includes('一百二十三件套（一百二十二件套清' + '除）'));
...
ok('smoke_v2227 的 GAME_VERSION 字面量 pin 已更新为 v22.28', s2227.includes("const GAME_VERSION = 'v23.99';"));
ok('smoke_v2227 的 GAME_VERSION 恒等 pin 已更新为 === v22.28', s2227.includes("GAME_VERSION === 'v23.99'"));
ok('smoke_v2227 的 README 件套 pin 已更新为二百二十三件套（二百二十二件套清除）',
  s2227.includes('二百二十三件套（二百二十二件套清除）'));
...
ok('smoke_v2227 的 package.json 件套计数 pin 已更新为 === 124', s2227.includes('testChain === 223'));
ok('smoke_v2226 的 GAME_VERSION 字面量 pin 已更新为 v22.28', s2226.includes("const GAME_VERSION = 'v23.99';"));
