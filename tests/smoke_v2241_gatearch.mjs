...
ok('GAME_VERSION 字面量已为 v22.41（旧 v22.40 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.44';") && !dSrc.includes("const GAME_VERSION = 'v22." + "40';"));
...
ok('契约：NPC 总数保持 30（零 NPC 变更，v22.40 pin 续守）', Object.keys(NPC_SPOTS).length === 30,
...
ok('README tests 树收录 smoke_v2241_gatearch 且位于串尾', readme.includes('smoke_v2240_tallgrass + smoke_v2241_gatearch + smoke_v2242_mushfield + smoke_v2243_encguide + smoke_v2244_fulldanger（npm test 串跑）'));
ok('README 件套口径为一百四十件套（一百三十九件套清除）',
  readme.includes('冒烟一百四十件套（一百三十九件套清除）') && !readme.includes('冒烟一百三十六件套（一百三十五件套清' + '除）'));
...
ok('package.json test 串共 138 件套', testChain === 140, String(testChain));
...
ok('smoke_v2240 的 GAME_VERSION 字面量 pin 已更新为 v22.42', s2240.includes("const GAME_VERSION = 'v22.44';"));
ok('smoke_v2240 的 GAME_VERSION 恒等 pin 已更新为 === v22.42', s2240.includes("GAME_VERSION === 'v22.44'"));
ok('smoke_v2240 的 README 串尾 pin 已更新为 + smoke_v2241_gatearch',
  s2240.includes('smoke_v2240_tallgrass + smoke_v2241_gatearch + smoke_v2242_mushfield + smoke_v2243_encguide + smoke_v2244_fulldanger（npm test 串跑）'));
ok('smoke_v2240 的 package.json 件套计数 pin 已更新为 === 138', s2240.includes('testChain === 140'));
ok('smoke_v2240 的 README 件套口径 pin 已更新为一百三十七件套', s2240.includes('冒烟一百四十件套（一百三十九件套清除）'));
ok('smoke_v2239 的 GAME_VERSION 字面量 pin 已更新为 v22.42', s2239.includes("const GAME_VERSION = 'v22.44';"));
ok('smoke_v2239 的 README 串尾 pin 已更新为 + smoke_v2241_gatearch（NPC 总数/件套 pin 随新现实）',
  s2239.includes('smoke_v2240_tallgrass + smoke_v2241_gatearch + smoke_v2242_mushfield + smoke_v2243_encguide + smoke_v2244_fulldanger（npm test 串跑）') && s2239.includes('testChain === 140'));
ok('smoke_v2238 的 GAME_VERSION 字面量 pin 已更新为 v22.42', s2238.includes("const GAME_VERSION = 'v22.44';"));
ok('smoke_v2235 的 GAME_VERSION 字面量 pin 已更新为 v22.42', s2235.includes("const GAME_VERSION = 'v22.44';"));
ok('smoke_v2235 的 NPC 总数 pin 保持 30（零 NPC 变更）', s2235.includes('NPC_SPOTS).length === 30'));
ok('smoke_v2234 的 GAME_VERSION 恒等 pin 已更新为 === v22.42', s2234.includes("GAME_VERSION === 'v22.44'"));
ok('smoke_v2230 的 GAME_VERSION 恒等 pin 已更新为 === v22.42', s2230.includes("GAME_VERSION === 'v22.44'"));
...
ok('smoke_v2226 的 plain 串尾锚逐字（node tests/smoke_v2240_tallgrass.mjs && node tests/smoke_v2241_gatearch.mjs && node tests/smoke_v2242_mushfield.mjs && node tests/smoke_v2243_encguide.mjs && node tests/smoke_v2244_fulldanger.mjs"）',
  s2226.includes('node tests/smoke_v2240_tallgrass.mjs && node tests/smoke_v2241_gatearch.mjs && node tests/smoke_v2242_mushfield.mjs && node tests/smoke_v2243_encguide.mjs && node tests/smoke_v2244_fulldanger.mjs"'));
ok('smoke_v2228 的 GAME_VERSION 字面量 pin 已更新为 v22.41 且旧代零残留复查仍在（v22.27 风格）',
  s2228.includes("const GAME_VERSION = 'v22.44';") && s2228.includes("'v22.") && s2228.includes("27';"));
