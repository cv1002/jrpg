...
ok('GAME_VERSION 格式合法且已越过 v22.66（本版守 v22.67）', !!_gv && (_gv[0] > 22 || (_gv[0] === 22 && _gv[1] >= 67)), GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已为 v22.66（旧 v22.65 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v22.67';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "65';"));
...
ok('ACH_LIST 精确总数 50 项（v2229 精确计数 pin 随新现实更新 49→50）', ACH_LIST.length === 51, String(ACH_LIST.length));
...
ok('README tests 树串尾已延伸至 smoke_v2266_stock3（v2265 后接 v2266）',
  readme.includes('smoke_v2265_lucky3 + smoke_v2266_stock3 + smoke_v2267_elixir3（npm test 串跑）'));
ok('README 件套口径为一百六十三件套（一百六十二件套清除）且旧 161 口径零残留',
  readme.includes('冒烟一百六十三件套（一百六十二件套清除）') && !readme.includes('冒烟一百六十一件套（一百六十件套清' + '除）'));
...
ok('README 成就口径「50 项」双处同步（快速上手表 C 键行 + 图鉴&成就行）',
  readme.includes('成就一览（全部 51 项进度') && readme.includes('**51 项成就**') &&
  !readme.includes('成就一览（全部 49 项进' + '度') && !readme.includes('**49 项成' + '就**'));
ok('package.json 已收录 smoke_v2266_stock3（npm test 串跑第 162 份）',
  pkg.includes('smoke_v2266_stock3.mjs') && pkg.includes('smoke_v2265_lucky3.mjs && node tests/smoke_v2266_stock3.mjs && node tests/smoke_v2267_elixir3.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 162 件套', testChain === 163, String(testChain));
ok('CHANGELOG 顶为 v22.66 条目', changelog.startsWith('## v22.67 '));
...
ok('smoke_v2265 的 GAME_VERSION 字面量 pin 已更新为 v22.66', s2265.includes("const GAME_VERSION = 'v22.67';"));
ok('smoke_v2265 的 README 件套 pin 已随新现实更新为一百六十三件套（一百六十二件套清除）',
  s2265.includes('一百六十三件套（一百六十二件套清除）'));
ok('smoke_v2265 的 package.json 件套计数 pin 已更新为 === 162', s2265.includes('testChain === 163'));
ok('smoke_v2265 的 CHANGELOG 顶 pin 已更新为 ## v22.66', s2265.includes("startsWith('## v22.67')"));
ok('smoke_v2265 的 README 串尾 pin 已随新现实延伸至 smoke_v2266_stock3',
  s2265.includes('smoke_v2264_hunt3 + smoke_v2265_lucky3 + smoke_v2266_stock3 + smoke_v2267_elixir3（npm test 串跑）'));
ok('smoke_v2265 的 package.json 串尾 plain pin 已延伸至 smoke_v2266_stock3',
  s2265.includes('smoke_v2264_hunt3.mjs && node tests/smoke_v2265_lucky3.mjs && node tests/smoke_v2266_stock3.mjs && node tests/smoke_v2267_elixir3.mjs"'));
ok('smoke_v2265 的 README 成就 pin 已随新现实更新为 50 项双处', s2265.includes('成就一览（全部 51 项进度') && s2265.includes('**51 项成就**'));
ok('smoke_v2264 的 GAME_VERSION 字面量 pin 已更新为 v22.66', s2264.includes("const GAME_VERSION = 'v22.67';"));
ok('smoke_v2264 的 README 件套 pin 已随新现实更新为一百六十三件套（一百六十二件套清除）',
  s2264.includes('一百六十三件套（一百六十二件套清除）'));
ok('smoke_v2264 的 package.json 件套计数 pin 已更新为 === 162', s2264.includes('testChain === 163'));
ok('smoke_v2264 的 CHANGELOG 顶 pin 已更新为 ## v22.66', s2264.includes("startsWith('## v22.67')"));
ok('smoke_v2264 的 README 串尾 pin 已随新现实延伸至 smoke_v2266_stock3',
  s2264.includes('smoke_v2263_ptime3 + smoke_v2264_hunt3 + smoke_v2265_lucky3 + smoke_v2266_stock3 + smoke_v2267_elixir3（npm test 串跑）'));
ok('smoke_v2264 的 package.json 串尾 plain pin 已延伸至 smoke_v2266_stock3',
  s2264.includes('smoke_v2263_ptime3.mjs && node tests/smoke_v2264_hunt3.mjs && node tests/smoke_v2265_lucky3.mjs && node tests/smoke_v2266_stock3.mjs && node tests/smoke_v2267_elixir3.mjs"'));
ok('smoke_v2263 的 GAME_VERSION 字面量 pin 已更新为 v22.66', s2263.includes("const GAME_VERSION = 'v22.67';"));
ok('smoke_v2263 的 README 件套 pin 已随新现实更新为一百六十三件套（一百六十二件套清除）',
  s2263.includes('一百六十三件套（一百六十二件套清除）'));
ok('smoke_v2263 的 package.json 件套计数 pin 已更新为 === 162', s2263.includes('testChain === 163'));
ok('smoke_v2263 的 CHANGELOG 顶 pin 已更新为 ## v22.66', s2263.includes("startsWith('## v22.67')"));
ok('smoke_v2263 的 README 串尾 pin 已随新现实延伸至 smoke_v2266_stock3',
  s2263.includes('smoke_v2263_ptime3 + smoke_v2264_hunt3 + smoke_v2265_lucky3 + smoke_v2266_stock3 + smoke_v2267_elixir3（npm test 串跑）'));
ok('smoke_v2263 的 package.json 串尾 plain pin 已延伸至 smoke_v2266_stock3',
  s2263.includes('smoke_v2263_ptime3.mjs && node tests/smoke_v2264_hunt3.mjs && node tests/smoke_v2265_lucky3.mjs && node tests/smoke_v2266_stock3.mjs && node tests/smoke_v2267_elixir3.mjs"'));
ok('smoke_v2262 的 GAME_VERSION 字面量 pin 已更新为 v22.66', s2262.includes("const GAME_VERSION = 'v22.67';"));
ok('smoke_v2262 的 package.json 串尾 plain pin 已延伸至 smoke_v2266_stock3',
  s2262.includes('smoke_v2262_crystal.mjs && node tests/smoke_v2263_ptime3.mjs && node tests/smoke_v2264_hunt3.mjs && node tests/smoke_v2265_lucky3.mjs && node tests/smoke_v2266_stock3.mjs && node tests/smoke_v2267_elixir3.mjs"'));
ok('smoke_v2261 的 GAME_VERSION 字面量 pin 已更新为 v22.66', s2261.includes("const GAME_VERSION = 'v22.67';"));
ok('smoke_v2260 的 GAME_VERSION 恒等 pin 族已随新现实全库更新（v22.66 字面量 pin 落位）',
  s2260.includes("const GAME_VERSION = 'v22.67';"));
ok('smoke_v2260 的 README 件套 pin 已随新现实更新为一百六十三件套（一百六十二件套清除）',
  s2260.includes('一百六十三件套（一百六十二件套清除）'));
ok('smoke_v2260 的 package.json 件套计数 pin 已更新为 === 162', s2260.includes('testChain === 163'));
ok('smoke_v2260 的 CHANGELOG 顶 pin 已更新为 ## v22.66', s2260.includes("startsWith('## v22.67')"));
ok('smoke_v2260 的 README 串尾 pin 已随新现实延伸至 smoke_v2266_stock3',
  s2260.includes('smoke_v2262_crystal + smoke_v2263_ptime3 + smoke_v2264_hunt3 + smoke_v2265_lucky3 + smoke_v2266_stock3 + smoke_v2267_elixir3（npm test 串跑）'));
ok('smoke_v2259 的 GAME_VERSION 字面量 pin 已更新为 v22.66', s2259.includes("const GAME_VERSION = 'v22.67';"));
ok('smoke_v2259 的 README 件套 pin 已随新现实更新为一百六十三件套（一百六十二件套清除）',
  s2259.includes('一百六十三件套（一百六十二件套清除）'));
ok('smoke_v2259 的 package.json 件套计数 pin 已更新为 === 162', s2259.includes('testChain === 163'));
ok('smoke_v2259 的 CHANGELOG 顶 pin 已更新为 ## v22.66', s2259.includes("startsWith('## v22.67')"));
ok('smoke_v2258 的 GAME_VERSION 字面量 pin 已更新为 v22.66', s2258.includes("const GAME_VERSION = 'v22.67';"));
ok('smoke_v2229 的 ACH_LIST 精确总数 pin 已更新为 === 50（49→50）',
  s2229.includes('ACH_LIST.length === 51') && !s2229.includes('ACH_LIST.length === 4' + '9'));
...
ok('smoke_v2199 的 README 成就 pin 已随新现实更新为 50 项双处落位',
  s2199.includes('成就一览（全部 51 项进度') && s2199.includes('**51 项成就**'));
ok('v2143-45「件套守护领先一位」哨兵链已推进至 163（一百六十三件套（一百六十二件套清除））',
  s2143.includes('一百六十四件套（一百六十三件套清除）') && s2143.includes("!readme.includes('一百六十四件套')"));
ok('smoke_v2144 的哨兵链 pin 已随新现实推进（!readme 不含一百六十四件套）',
  s2144.includes("!readme.includes('一百六十四件套')"));
ok('smoke_v2145 的哨兵链 pin 已随新现实推进（一百六十三件套（一百六十二件套清除））',
  s2145.includes('一百六十四件套（一百六十三件套清除）'));
