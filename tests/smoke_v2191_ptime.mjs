...
ok('GAME_VERSION 格式合法且已越过 v21.90', !!_gv && (_gv[0] > 21 || (_gv[0] === 21 && _gv[1] >= 99)), GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v21.91', dataSrc.includes("const GAME_VERSION = 'v23.72';"));
...
ok('README 件套口径为二百一十二件套（二百一十一件套清除）',
  readme.includes('冒烟二百一十二件套（二百一十一件套清除）') && !readme.includes('冒烟八十七件套（八十六件套清除）'));
...
for (const nm of achFiles) {
  ...
  ok(`${nm} 的 README 成就 pin 已随新现实更新（33 项 pin 零残留，34 项双处落位）`,
    src.includes("readme.includes('成就一览（全部 68 项进度'") && !src.includes('**33 项成就**'));
}
