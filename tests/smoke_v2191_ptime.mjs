...
ok('data.js GAME_VERSION 字面量已更新为 v21.91', dataSrc.includes("const GAME_VERSION = 'v22.66';"));
ok('data.js 导出 PLAY_TIME_GOAL（export 块落位；v22.6 起 PLAY_TIME2_GOAL/POTIONS_GOAL/POTIONS2_GOAL 随其后、v22.63 起 PLAY_TIME3_GOAL 并入同族）', dataSrc.includes(', PLAY_TIME_GOAL, PLAY_TIME2_GOAL, PLAY_TIME3_GOAL, POTIONS_GOAL, POTIONS2_GOAL, POTIONS3_GOAL, ELIXIR_STOCK_GOAL, ELIXIR_STOCK2_GOAL, PERFECTION_GOLD'));
...
ok('README 件套口径为一百六十二件套（一百六十一件套清除）',
  readme.includes('冒烟一百六十二件套（一百六十一件套清除）') && !readme.includes('冒烟八十七件套（八十六件套清除）'));
...
for (const nm of achFiles) {
  ...
  ok(`${nm} 的 README 成就 pin 已随新现实更新（33 项 pin 零残留，34 项双处落位）`,
    src.includes("readme.includes('成就一览（全部 50 项进度'") && !src.includes('**33 项成就**'));
}
