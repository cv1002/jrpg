...
ok('data.js GAME_VERSION 字面量已更新为 v21.91', dataSrc.includes("const GAME_VERSION = 'v22.68';"));
...
ok('README 件套口径为一百六十四件套（一百六十三件套清除）',
  readme.includes('冒烟一百六十四件套（一百六十三件套清除）') && !readme.includes('冒烟八十七件套（八十六件套清除）'));
...
for (const nm of achFiles) {
  ...
  ok(`${nm} 的 README 成就 pin 已随新现实更新（33 项 pin 零残留，34 项双处落位）`,
    src.includes("readme.includes('成就一览（全部 52 项进度'") && !src.includes('**33 项成就**'));
}
