...
ok('GAME_VERSION 字面量已为 v22.55（旧 v22.54 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.66';") && !dSrc.includes("const GAME_VERSION = 'v22." + "54';"));
...
ok('README tests 树收录 smoke_v2255_steleglow 且位于串尾',
  readme.includes('smoke_v2254_grainfield + smoke_v2255_steleglow + smoke_v2256_campfire + smoke_v2257_pondglow + smoke_v2258_potionhelp + smoke_v2259_sandpile + smoke_v2260_fountripple + smoke_v2261_rich3 + smoke_v2262_crystal + smoke_v2263_ptime3 + smoke_v2264_hunt3 + smoke_v2265_lucky3 + smoke_v2266_stock3（npm test 串跑）'));
ok('README 件套口径为一百六十二件套（一百六十一件套清除）',
  readme.includes('冒烟一百六十二件套（一百六十一件套清除）') && !readme.includes('冒烟一百五十件套（一百四十九件套清' + '除）'));
...
ok('package.json test 串共 151 件套', testChain === 162, String(testChain));
...
ok('smoke_v2254 的 GAME_VERSION 字面量 pin 已更新为 v22.55', s2254.includes("const GAME_VERSION = 'v22.66';"));
...
ok('smoke_v2254 的 README 件套 pin 已随新现实更新为一百六十二件套（一百六十一件套清除）',
  s2254.includes('一百六十二件套（一百六十一件套清除）'));
ok('smoke_v2254 的 package.json 件套计数 pin 已更新为 === 151', s2254.includes('testChain === 162'));
ok('smoke_v2254 的 CHANGELOG 顶 pin 已更新为 ## v22.55', s2254.includes("startsWith('## v22.66')"));
ok('smoke_v2253 的 GAME_VERSION 字面量 pin 已更新为 v22.55', s2253.includes("const GAME_VERSION = 'v22.66';"));
ok('smoke_v2253 的 README 串尾 pin 已延伸至 smoke_v2255_steleglow',
  s2253.includes('smoke_v2252_rail + smoke_v2253_supplypoint + smoke_v2254_grainfield + smoke_v2255_steleglow + smoke_v2256_campfire + smoke_v2257_pondglow + smoke_v2258_potionhelp + smoke_v2259_sandpile + smoke_v2260_fountripple + smoke_v2261_rich3 + smoke_v2262_crystal + smoke_v2263_ptime3 + smoke_v2264_hunt3 + smoke_v2265_lucky3 + smoke_v2266_stock3（npm test 串跑）'));
ok('smoke_v2252 的 GAME_VERSION 字面量 pin 已更新为 v22.55', s2252.includes("const GAME_VERSION = 'v22.66';"));
...
ok('v2143-45「件套守护领先一位」哨兵链已推进至 152（一百六十二件套（一百六十一件套清除））',
