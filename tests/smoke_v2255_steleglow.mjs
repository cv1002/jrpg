...
ok('GAME_VERSION 格式合法且已越过 v22.54（本版守 v22.55）', !!_gv && (_gv[0] > 22 || (_gv[0] === 22 && _gv[1] >= 55)), GAME_VERSION);
ok('data.js 含 v22.55 注释（名字石碑回灯温光说明）', dSrc.includes('v22.55 新内容·世界景观·纯显示'));
ok('GAME_VERSION 字面量已为 v22.55（旧 v22.54 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.55';") && !dSrc.includes("const GAME_VERSION = 'v22." + "54';"));
...
ok('README tests 树收录 smoke_v2255_steleglow 且位于串尾',
  readme.includes('smoke_v2254_grainfield + smoke_v2255_steleglow（npm test 串跑）'));
ok('README 件套口径为一百五十一件套（一百五十件套清除）',
  readme.includes('冒烟一百五十一件套（一百五十件套清除）') && !readme.includes('冒烟一百五十件套（一百四十九件套清' + '除）'));
...
ok('package.json test 串共 151 件套', testChain === 151, String(testChain));
...
ok('smoke_v2254 的 GAME_VERSION 字面量 pin 已更新为 v22.55', s2254.includes("const GAME_VERSION = 'v22.55';"));
ok('smoke_v2254 的 README 串尾 pin 已随新现实延伸至 smoke_v2255_steleglow',
  s2254.includes('smoke_v2253_supplypoint + smoke_v2254_grainfield + smoke_v2255_steleglow（npm test 串跑）'));
ok('smoke_v2254 的 README 件套 pin 已随新现实更新为一百五十一件套（一百五十件套清除）',
  s2254.includes('一百五十一件套（一百五十件套清除）'));
ok('smoke_v2254 的 package.json 件套计数 pin 已更新为 === 151', s2254.includes('testChain === 151'));
ok('smoke_v2254 的 CHANGELOG 顶 pin 已更新为 ## v22.55', s2254.includes("startsWith('## v22.55')"));
ok('smoke_v2253 的 GAME_VERSION 字面量 pin 已更新为 v22.55', s2253.includes("const GAME_VERSION = 'v22.55';"));
ok('smoke_v2253 的 README 串尾 pin 已延伸至 smoke_v2255_steleglow',
  s2253.includes('smoke_v2252_rail + smoke_v2253_supplypoint + smoke_v2254_grainfield + smoke_v2255_steleglow（npm test 串跑）'));
ok('smoke_v2252 的 GAME_VERSION 字面量 pin 已更新为 v22.55', s2252.includes("const GAME_VERSION = 'v22.55';"));
ok('smoke_v2252 的 README 串尾 pin 已延伸至 smoke_v2255_steleglow',
  s2252.includes('smoke_v2251_oathkeep + smoke_v2252_rail + smoke_v2253_supplypoint + smoke_v2254_grainfield + smoke_v2255_steleglow（npm test 串跑）'));
ok('smoke_v2242 的 README 串尾 pin 已延伸至 smoke_v2255_steleglow',
  s2242.includes('smoke_v2252_rail + smoke_v2253_supplypoint + smoke_v2254_grainfield + smoke_v2255_steleglow（npm test 串跑）'));
// 串尾锚延伸（任意转义风格：plain `/` / regex `\/` / 双转义 `\\/`）
const archChain = (s) => /smoke_v2253_supplypoint[.\\]{0,4}mjs[\s\S]*?node tests[\\/]{0,4}smoke_v2255_steleglow[.\\]{0,4}mjs/.test(s);
...
const looseChain = (s) => /smoke_v2253_supplypoint[.\\]{0,6}mjs[\s\S]*?node tests[\\/]{0,6}smoke_v2255_steleglow[.\\]{0,6}mjs/.test(s);
ok('smoke_v2239/smoke_v2240 的容错正则尾锚已延伸至 smoke_v2255_steleglow',
  looseChain(s2239) && looseChain(s2240));
ok('v2143-45「件套守护领先一位」哨兵链已推进至 152（一百五十二件套（一百五十一件套清除））',
  s2143.includes('一百五十二件套（一百五十一件套清除）') && s2143.includes("!readme.includes('一百五十二件套')"));
ok('smoke_v2228 的 GAME_VERSION 字面量 pin 已更新为 v22.55 且旧代零残留复查仍在（v22.27 风格）',
  s2228.includes("const GAME_VERSION = 'v22.55';") && s2228.includes("'v22." ) && s2228.includes("27';"));

// 旧代 v22.54 pin 全库零残留（字面量/恒等/件套/串尾/testChain）
const allTests = fs.readdirSync(path.join(ROOT, 'tests')).filter((f) => f.endsWith('.mjs'));
const stale = [];
for (const f of allTests) {
  const src = fs.readFileSync(path.join(ROOT, 'tests', f), 'utf8');
  if (src.includes("const GAME_VERSION = 'v22." + "54';") || src.includes("GAME_VERSION === 'v22." + "54'") ||
      src.includes('一百五十件套（一百四十九件套清' + '除）') || src.includes('testChain === ' + '150') ||
      src.includes('smoke_v2254_grainfield（npm test 串' + '跑）')) stale.push(f);
}
ok('旧代 v22.54 字面量/恒等/件套/串尾/testChain pin 全库零残留（' + allTests.length + ' 件扫描，含本件）', stale.length === 0, stale.join(','));
// 坏链防回归：README 不得出现「smoke_v2254_grainfield + smoke_v2255_steleglow（npm test 串跑）」孤尾
ok('README 串尾无孤尾（smoke_v2254_grainfield 后必须接 smoke_v2255_steleglow）',
  !readme.includes('smoke_v2254_grainfield（npm test 串' + '跑）'));
