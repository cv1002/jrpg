...
ok('data.js GAME_VERSION 字面量已为 v22.73（旧 v22.72 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v24.02';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "72';"));
...
ok('README 件套口径为二百二十六件套（二百二十五件套清除）且旧 168 口径零残留',
  readme.includes('冒烟二百二十六件套（二百二十五件套清除）') && !readme.includes('冒烟一百六十八件套（一百六十七件套清' + '除）'));
...
ok('package.json test 串共 169 件套', testChain === 226, String(testChain));
ok('CHANGELOG 含 v22.73 条目（顶 pin）', changelog.startsWith('## v24.02 '));
...
ok('smoke_v2272 的 GAME_VERSION 字面量 pin 已更新为 v22.73', s2272.includes("const GAME_VERSION = 'v24.02';"));
ok('smoke_v2272 的 CHANGELOG 顶 pin 已更新为 ## v22.73', s2272.includes("startsWith('## v24.02'") || s2272.includes("startsWith('## v22.78 '"));
ok('smoke_v2272 的件套 pin 已更新为二百二十六件套（二百二十五件套清除）', s2272.includes('二百二十六件套（二百二十五件套清除）'));
