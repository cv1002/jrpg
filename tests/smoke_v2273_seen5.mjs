...
ok('GAME_VERSION 格式合法且已越过 v22.72（本版守 v22.76）', !!_gv && (_gv[0] > 22 || (_gv[0] === 22 && _gv[1] >= 76)), GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已为 v22.73（旧 v22.72 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v22.76';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "72';"));
...
ok('README tests 树串尾已延伸至 smoke_v2273_seen5（v2272 后接 v2273）',
  readme.includes('smoke_v2271_outstep2 + smoke_v2272_scholar2 + smoke_v2273_seen5 + smoke_v2274_seen2 + smoke_v2275_codexempty + smoke_v2276_lampkid（npm test 串跑）'));
ok('README 件套口径为一百七十二件套（一百七十一件套清除）且旧 168 口径零残留',
  readme.includes('冒烟一百七十二件套（一百七十一件套清除）') && !readme.includes('冒烟一百六十八件套（一百六十七件套清' + '除）'));
...
ok('package.json 已收录 smoke_v2273_seen5（npm test 串跑第 169 份）',
  pkg.includes('smoke_v2272_scholar2.mjs && node tests/smoke_v2273_seen5.mjs && node tests/smoke_v2274_seen2.mjs && node tests/smoke_v2275_codexempty.mjs && node tests/smoke_v2276_lampkid.mjs'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 169 件套', testChain === 172, String(testChain));
ok('CHANGELOG 含 v22.73 条目（顶 pin）', changelog.startsWith('## v22.76 '));
...
ok('smoke_v2272 的 GAME_VERSION 字面量 pin 已更新为 v22.73', s2272.includes("const GAME_VERSION = 'v22.76';"));
ok('smoke_v2272 的 CHANGELOG 顶 pin 已更新为 ## v22.73', s2272.includes("startsWith('## v22.76')") || s2272.includes("startsWith('## v22.76 ')"));
ok('smoke_v2272 的件套 pin 已更新为一百七十二件套（一百七十一件套清除）', s2272.includes('一百七十二件套（一百七十一件套清除）'));
ok('smoke_v2272 的 README 串尾 pin 已延伸至 smoke_v2273_seen5', s2272.includes('smoke_v2272_scholar2 + smoke_v2273_seen5 + smoke_v2274_seen2 + smoke_v2275_codexempty + smoke_v2276_lampkid（npm test 串跑）'));
ok('smoke_v2272 的 package.json 串尾 plain pin 已延伸至 smoke_v2273_seen5', s2272.includes('smoke_v2272_scholar2.mjs && node tests/smoke_v2273_seen5.mjs && node tests/smoke_v2274_seen2.mjs && node tests/smoke_v2275_codexempty.mjs && node tests/smoke_v2276_lampkid.mjs'));
