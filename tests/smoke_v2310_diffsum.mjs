...
// 逐字零回归两档）+ README/package.json/CHANGELOG 同步（冒烟二百零八件套（二百零七件套清除）/
...
ok('data.js GAME_VERSION 字面量已为 v23.10（旧 v23.09 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.12';") && !dataSrc.includes("const GAME_VERSION = 'v23." + "09';"));
...
ok('README 件套口径为二百零八件套（二百零七件套清除）且旧 205 口径零残留',
  readme.includes('冒烟二百零八件套（二百零七件套清除）') && !readme.includes('冒烟二百零五件套（二百零四件套清' + '除）'));
...
ok('README tests 树串尾已延伸至 smoke_v2310_diffsum（v2309 后接 v2310）',
  readme.includes('smoke_v2309_questnum + smoke_v2310_diffsum + smoke_v2311_fragprev + smoke_v2312_voltitle（npm test 串跑）'));
...
ok('package.json 串尾为 ... smoke_v2309_questnum.mjs && node tests/smoke_v2310_diffsum.mjs && node tests/smoke_v2311_fragprev.mjs && node tests/smoke_v2312_voltitle.mjs"',
  pkg.includes('node tests/smoke_v2309_questnum.mjs && node tests/smoke_v2310_diffsum.mjs && node tests/smoke_v2311_fragprev.mjs && node tests/smoke_v2312_voltitle.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 206 件套', testChain === 208, String(testChain));
ok('CHANGELOG 顶部已追加 v23.10 条目', changelog.startsWith('## v23.12 '));
...
ok('smoke_v2309 的 GAME_VERSION 字面量 pin 已更新为 v23.10', s2309.includes("const GAME_VERSION = 'v23.12';"));
ok('smoke_v2309 的 CHANGELOG 顶 pin 已更新为 ## v23.10',
  s2309.includes("startsWith('## v23.12 "));
ok('smoke_v2309 的件套 pin 已更新为二百零八件套（二百零七件套清除）',
  s2309.includes('二百零八件套（二百零七件套清除）'));
ok('smoke_v2309 的 README 串尾 pin 已延伸至 smoke_v2310_diffsum',
  s2309.includes('smoke_v2309_questnum + smoke_v2310_diffsum + smoke_v2311_fragprev + smoke_v2312_voltitle（npm test 串跑）'));
ok('smoke_v2309 的 package 串尾 pin 已延伸至 smoke_v2310_diffsum',
  s2309.includes('node tests/smoke_v2309_questnum.mjs && node tests/smoke_v2310_diffsum.mjs && node tests/smoke_v2311_fragprev.mjs && node tests/smoke_v2312_voltitle.mjs"'));
ok('smoke_v2309 的 testChain pin 已更新为 206', s2309.includes('testChain === 208'));
