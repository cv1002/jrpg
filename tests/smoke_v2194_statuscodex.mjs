...
ok('GAME_VERSION 字面量已为 v21.94', GAME_VERSION === 'v21.95', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v21.94', dataSrc.includes("const GAME_VERSION = 'v21.95';"));
...
cap = renderStatus(hero, 'village');
ok('运行期：资源行 📕 与 📦/成就/⏱ 同段共存（收集三件套于状态页齐备）',
  cap.some((t) => t.includes('📕:') && t.includes('📦:') && t.includes('成就') && t.includes('⏱️')));

// —— README / package.json / CHANGELOG 同步 ——
ok('README 已同步（tests 树收录 smoke_v2194_statuscodex + 九十件套口径）',
  readme.includes('smoke_v2194_statuscodex') && readme.includes('九十一件套（九十件套清除）'));
