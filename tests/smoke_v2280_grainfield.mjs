...
ok('data.js GAME_VERSION 字面量为 v22.80', dataSrc.includes("const GAME_VERSION = 'v22.80'"));
...
ok('星井矿脉行 r[1]/r[2] 零回归',
  String(guide[2][1]).includes('试炼碑（可问' + NPCS.sentinel.name + '）') && String(guide[2][2]) === '无泉水/旅店 · 出发前请补给');
...
ok('README tests 树串尾已延伸至 smoke_v2280_grainfield（v2279 后接 v2280）',
  readme.includes('smoke_v2279_lampwell + smoke_v2280_grainfield（npm test 串跑）'));
ok('README 件套口径为一百七十六件套（一百七十五件套清除）且旧 175 口径零残留',
  readme.includes('冒烟一百七十六件套（一百七十五件套清除）') && !readme.includes('冒烟一百七十五件套（一百七十四件套清' + '除）'));
...
ok('package.json test 串含 smoke_v2280_grainfield.mjs 且位于串尾',
  pkg.includes('node tests/smoke_v2279_lampwell.mjs && node tests/smoke_v2280_grainfield.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 176 件套', testChain === 176, String(testChain));
ok('CHANGELOG 含 v22.80 条目（顶 pin）', changelog.startsWith('## v22.80 '));
...
ok('smoke_v2279 的 GAME_VERSION 字面量 pin 已更新为 v22.80', s2279.includes("const GAME_VERSION = 'v22.80';"));
ok('smoke_v2279 的 CHANGELOG 顶 pin 已更新为 ## v22.80', s2279.includes("startsWith('## v22.80'"));
ok('smoke_v2279 的件套 pin 已更新为一百七十六件套（一百七十五件套清除）', s2279.includes('一百七十六件套（一百七十五件套清除）'));
ok('smoke_v2279 的 README 串尾 pin 已延伸至 smoke_v2280_grainfield', s2279.includes('smoke_v2279_lampwell + smoke_v2280_grainfield（npm test 串跑）'));
ok('smoke_v2260 的 package.json 串尾 pin 已延伸至 smoke_v2280_grainfield', s2260.includes('node tests/smoke_v2280_grainfield.mjs'));
