...
ok('data.js GAME_VERSION 字面量为 v22.80', dataSrc.includes("const GAME_VERSION = 'v24.00'"));
...
ok('README 件套口径为二百二十四件套（二百二十三件套清除）且旧 175 口径零残留',
  readme.includes('冒烟二百二十四件套（二百二十三件套清除）') && !readme.includes('冒烟一百七十五件套（一百七十四件套清' + '除）'));
...
ok('package.json test 串共 176 件套', testChain === 224, String(testChain));
ok('CHANGELOG 含 v22.80 条目（顶 pin）', changelog.startsWith('## v24.00 '));
...
ok('smoke_v2279 的 GAME_VERSION 字面量 pin 已更新为 v22.80', s2279.includes("const GAME_VERSION = 'v24.00';"));
ok('smoke_v2279 的 CHANGELOG 顶 pin 已更新为 ## v22.80', s2279.includes("startsWith('## v24.00'"));
ok('smoke_v2279 的件套 pin 已更新为二百二十四件套（二百二十三件套清除）', s2279.includes('二百二十四件套（二百二十三件套清除）'));
