...
// 同步（二百零一件套（二百件套清除）/串尾/入库 195 份/顶 pin）+ 姊妹件套 pin（smoke_v2298 随新现实
...
ok('data.js GAME_VERSION 字面量已为 v22.99（旧 v22.98 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.05';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "98';"));
...
ok('README tests 树串尾已延伸至 smoke_v2299_crosslink（v2298 后接 v2299）',
  readme.includes('smoke_v2298_encnum + smoke_v2299_crosslink + smoke_v2300_sidemore + smoke_v2301_eco + smoke_v2302_cmdprev + smoke_v2303_rushnum + smoke_v2304_achgoal + smoke_v2305_monnum（npm test 串跑）'));
ok('README 旧串尾零残留（smoke_v2298_encnum（npm test 串跑）不在树尾）',
  !readme.includes('smoke_v2298_encnum（npm test 串' + '跑）'));
ok('README 件套口径为二百零一件套（二百件套清除）且旧 194 口径零残留',
  readme.includes('冒烟二百零一件套（二百件套清除）') && !readme.includes('冒烟一百九十四件套（一百九十三件套清' + '除）'));
...
ok('package.json 串尾为 ... smoke_v2298_encnum.mjs && node tests/smoke_v2299_crosslink.mjs"',
  pkg.includes('node tests/smoke_v2298_encnum.mjs && node tests/smoke_v2299_crosslink.mjs && node tests/smoke_v2300_sidemore.mjs && node tests/smoke_v2301_eco.mjs && node tests/smoke_v2302_cmdprev.mjs && node tests/smoke_v2303_rushnum.mjs && node tests/smoke_v2304_achgoal.mjs && node tests/smoke_v2305_monnum.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 195 件套', testChain === 201, String(testChain));
ok('CHANGELOG 顶部已追加 v22.99 条目', changelog.startsWith('## v23.05 '));
...
ok('smoke_v2298 的 GAME_VERSION 字面量 pin 已更新为 v22.99', s2298.includes("const GAME_VERSION = 'v23.05';"));
ok('smoke_v2298 的 CHANGELOG 顶 pin 已更新为 ## v22.99',
  s2298.includes("startsWith('## v23.05 '"));
ok('smoke_v2298 的件套 pin 已更新为二百零一件套（二百件套清除）',
  s2298.includes('二百零一件套（二百件套清除）'));
ok('smoke_v2298 的 README 串尾 pin 已延伸至 smoke_v2299_crosslink',
  s2298.includes('smoke_v2298_encnum + smoke_v2299_crosslink + smoke_v2300_sidemore + smoke_v2301_eco + smoke_v2302_cmdprev + smoke_v2303_rushnum + smoke_v2304_achgoal + smoke_v2305_monnum（npm test 串跑）'));
ok('smoke_v2298 的 package 串尾 pin 已延伸至 smoke_v2299_crosslink',
  s2298.includes('node tests/smoke_v2298_encnum.mjs && node tests/smoke_v2299_crosslink.mjs && node tests/smoke_v2300_sidemore.mjs && node tests/smoke_v2301_eco.mjs && node tests/smoke_v2302_cmdprev.mjs && node tests/smoke_v2303_rushnum.mjs && node tests/smoke_v2304_achgoal.mjs && node tests/smoke_v2305_monnum.mjs"'));
ok('smoke_v2298 的 testChain pin 已更新为 195', s2298.includes('testChain === 201'));
