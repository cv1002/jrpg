...
// 同步（二百零九件套（二百零八件套清除）/串尾/入库 195 份/顶 pin）+ 姊妹件套 pin（smoke_v2298 随新现实
...
ok('data.js GAME_VERSION 字面量已为 v22.99（旧 v22.98 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.13';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "98';"));
...
ok('README 件套口径为二百零九件套（二百零八件套清除）且旧 194 口径零残留',
  readme.includes('冒烟二百零九件套（二百零八件套清除）') && !readme.includes('冒烟一百九十四件套（一百九十三件套清' + '除）'));
...
ok('package.json test 串共 195 件套', testChain === 209, String(testChain));
ok('CHANGELOG 顶部已追加 v22.99 条目', changelog.startsWith('## v23.13 '));
...
ok('smoke_v2298 的 GAME_VERSION 字面量 pin 已更新为 v22.99', s2298.includes("const GAME_VERSION = 'v23.13';"));
ok('smoke_v2298 的 CHANGELOG 顶 pin 已更新为 ## v22.99',
  s2298.includes("startsWith('## v23.13 '"));
ok('smoke_v2298 的件套 pin 已更新为二百零九件套（二百零八件套清除）',
  s2298.includes('二百零九件套（二百零八件套清除）'));
...
ok('smoke_v2298 的 testChain pin 已更新为 195', s2298.includes('testChain === 209'));
