...
ok('GAME_VERSION 字面量已为 v22.52（旧 v22.51 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.32';") && !dSrc.includes("const GAME_VERSION = 'v22." + "51';"));
...
ok('README 件套口径为二百一十二件套（二百一十一件套清除）',
  readme.includes('冒烟二百一十二件套（二百一十一件套清除）') &&
...
ok('package.json test 串共 148 件套', testChain === 212, String(testChain));
ok('CHANGELOG 顶部已追加 v22.52 条目', changelog.startsWith('## v23.32'));
...
ok('smoke_v2251 的 GAME_VERSION 字面量 pin 已更新为 v22.52（旧 v22.51 零残留）',
  s2251.includes("const GAME_VERSION = 'v23.32';") && !s2251.includes("const GAME_VERSION = 'v22." + "51';"));
...
ok('smoke_v2251 的 CHANGELOG pin 已更新为 ## v22.52', s2251.includes("startsWith('## v23.32')"));
const s2143 = fs.readFileSync(path.join(ROOT, 'tests/smoke_v2143_talkekey.mjs'), 'utf8');
ok('v2143-45「件套守护领先一位」哨兵链已推进至 149（二百一十二件套（二百一十一件套清除））',
