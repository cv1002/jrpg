...
ok('GAME_VERSION 字面量已为 v22.52（旧 v22.51 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.83';") && !dSrc.includes("const GAME_VERSION = 'v22." + "51';"));
...
ok('README 件套口径为一百七十九件套（一百七十八件套清除）',
  readme.includes('冒烟一百七十九件套（一百七十八件套清除）') &&
...
ok('package.json test 串共 148 件套', testChain === 179, String(testChain));
ok('CHANGELOG 顶部已追加 v22.52 条目', changelog.startsWith('## v22.83'));
...
ok('smoke_v2251 的 GAME_VERSION 字面量 pin 已更新为 v22.52（旧 v22.51 零残留）',
  s2251.includes("const GAME_VERSION = 'v22.83';") && !s2251.includes("const GAME_VERSION = 'v22." + "51';"));
ok('smoke_v2251 的 README 件套 pin 已随新现实更新为一百七十九件套（一百七十八件套清除）',
  s2251.includes('一百七十九件套（一百七十八件套清除）'));
ok('smoke_v2251 的 package.json 件套计数 pin 已更新为 === 148', s2251.includes('testChain === 179'));
ok('smoke_v2251 的 README 串尾 pin 已随新现实延伸至 smoke_v2252_rail',
  s2251.includes('smoke_v2251_oathkeep + smoke_v2252_rail + smoke_v2253_supplypoint + smoke_v2254_grainfield + smoke_v2255_steleglow + smoke_v2256_campfire + smoke_v2257_pondglow + smoke_v2258_potionhelp + smoke_v2259_sandpile + smoke_v2260_fountripple + smoke_v2261_rich3 + smoke_v2262_crystal + smoke_v2263_ptime3 + smoke_v2264_hunt3 + smoke_v2265_lucky3 + smoke_v2266_stock3 + smoke_v2267_elixir3 + smoke_v2268_brew3 + smoke_v2269_mush3 + smoke_v2270_outstep + smoke_v2271_outstep2 + smoke_v2272_scholar2 + smoke_v2273_seen5 + smoke_v2274_seen2 + smoke_v2275_codexempty + smoke_v2276_lampkid + smoke_v2277_pondhint + smoke_v2278_mushguide + smoke_v2279_lampwell + smoke_v2280_grainfield + smoke_v2281_starwell + smoke_v2282_archgate + smoke_v2283_menuekey（npm test 串跑）'));
ok('smoke_v2251 的 CHANGELOG pin 已更新为 ## v22.52', s2251.includes("startsWith('## v22.83')"));
const s2143 = fs.readFileSync(path.join(ROOT, 'tests/smoke_v2143_talkekey.mjs'), 'utf8');
ok('v2143-45「件套守护领先一位」哨兵链已推进至 149（一百八十件套（一百七十九件套清除））',
