...
ok('GAME_VERSION 字面量已为 v22.4（本版独占精确锚点）', GAME_VERSION === 'v22.8', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.4', dataSrc.includes("const GAME_VERSION = 'v22.8';"));
...
ok('README tests 树收录 smoke_v2204_brew2 且位于串尾',
  readme.includes('smoke_v2204_brew2 + smoke_v2207_titledel + smoke_v2208_elixir（npm test 串跑）'));
ok('README 件套口径为一百零四件套（一百零三件套清除）',
  readme.includes('冒烟一百零四件套（一百零三件套清除）') && !readme.includes('冒烟九十九件套（九十八件套清' + '除）'));
...
for (const nm of ['smoke_v2203_fragdead.mjs', 'smoke_v2202_fragwin.mjs', 'smoke_v2201_fragstatus.mjs',
  ...
  ok(`${nm} 的 README tests 树尾 pin 已随新现实更新（+ smoke_v2204_brew2）`,
    src.includes('smoke_v2204_brew2 + smoke_v2207_titledel + smoke_v2208_elixir（npm test 串跑）'));
}
