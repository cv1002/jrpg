...
ok('GAME_VERSION 字面量已为 v22.4（本版独占精确锚点）', GAME_VERSION === 'v22.89', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.4', dataSrc.includes("const GAME_VERSION = 'v22.89';"));
ok('data.js 仍保留 v22.3 历史注释（累积注释块，姊妹 pin 不失效）', dataSrc.includes('v22.3 阵亡画面收集行补「🕯️ 记忆碎片 N/4」'));
ok('data.js 导出 BREW2_GOAL（export 块落位，与 ELIXIR_GOAL 相邻）', dataSrc.includes('ELIXIR_GOAL, BREW2_GOAL, BREW3_GOAL, PLAY_TIME_GOAL'));
...
ok('README 件套口径为一百八十五件套（一百八十四件套清除）',
  readme.includes('冒烟一百八十五件套（一百八十四件套清除）') && !readme.includes('冒烟九十九件套（九十八件套清' + '除）'));
