...
ok('data.js GAME_VERSION 字面量已更新为 v21.86', dSrc.includes("const GAME_VERSION = 'v21.87';"));
...
ok('ACH_LIST 由 30 → 31 项（新增一项，精确总数由本版守护）', ACH_LIST.length === 31, String(ACH_LIST.length));
...
for (const nm of suite82) {
  ...
  ok(`${nm} 的 README 件套 pin 已随新现实更新为八十三件套（八十二件套清除）`,
    src.includes('八十三件套（八十二件套清除）'));
}
for (const nm of ['smoke_v2185_steleclear.mjs', 'smoke_v2184_lvl12.mjs', 'smoke_v2183_mpsip.mjs', 'smoke_v2182_winrecap.mjs', 'smoke_v2181_helpquickcast.mjs', 'smoke_v2179_titlerecap.mjs']) {
  ...
  ok(`${nm} 的 GAME_VERSION 字面量 pin 已随新现实更新为 v21.86`,
    src.includes("const GAME_VERSION = 'v21.87';"));
}
...
for (const nm of achFiles) {
  ...
  ok(`${nm} 的 README 成就 pin 已随新现实更新（30 项 pin 零残留，31 项双处落位）`,
    src.includes("readme.includes('成就一览（全部 31 项进度'") && !src.includes('**30 项成就**'));
}
