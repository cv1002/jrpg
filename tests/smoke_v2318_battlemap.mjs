...
ok('GAME_VERSION 字面量已为 v23.79（旧 v23.77 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.94';") && !dSrc.includes("const GAME_VERSION = 'v23.7" + "7';"));
...
ok('package.json test 串共 214 件套', testChain === 218, String(testChain));
...
ok('package.json 串尾为 ... smoke_v2317_pausemap.mjs && node tests/smoke_v2318_battlemap.mjs && node tests/smoke_v2319_deadloc.mjs && node tests/smoke_v2392_stariron.mjs && node tests/smoke_v2393_deadkey.mjs && node tests/smoke_v2394_fightback.mjs"',
  pkg.includes('node tests/smoke_v2317_pausemap.mjs && node tests/smoke_v2318_battlemap.mjs && node tests/smoke_v2319_deadloc.mjs && node tests/smoke_v2392_stariron.mjs && node tests/smoke_v2393_deadkey.mjs && node tests/smoke_v2394_fightback.mjs"'));
...
ok('README 件套口径为二百一十八件套（二百一十七件套清除）且旧 213 口径零残留',
  readme.includes('冒烟二百一十八件套（二百一十七件套清除）') && !readme.includes('冒烟二百一十三件套（二百一十二件套清' + '除）'));
...
ok('CHANGELOG 顶部已追加 v23.79 条目（战斗画面所在地）', changelog.startsWith('## v23.94 '));
...
ok('ACH_LIST 精确总数 72（v23.76 千里之行为末项，未动）', ACH_LIST.length === 76, String(ACH_LIST.length));
