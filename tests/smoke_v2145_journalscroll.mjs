...
try {
  ...
  // 1) 开局新档：滚动 0 时九条支线 + 主线 + 碎片区全画（v21.45 前 星砂之约/旧灯卫的名字/石壳里的记忆/残焰的安息 被静默裁掉；
  // v21.80 随新现实补 未归的矿灯/护粮的委托 入名单——冒烟钉死此二卡同样全量落画；v23.42 随新现实补 夜路的狼嚎）
  S.journalScroll = 0;
  drawnCalls.length = 0; translated.length = 0;
  drawJournal();
  const names = ['讨回灯芯', '星井之守', '灯长的委托', '星砂之约', '旧灯卫的名字', '雾里的新住客', '石壳里的记忆', '残焰的安息', '未归的矿灯', '护粮的委托', '树精的菌库', '夜路的狼嚎'];
  const absent = names.filter((nm) => !drawnCalls.some((c) => c.t.includes(nm)));
  ok('滚动=0 时日志全量条目落画（5 主线/支线名 + 此前被裁的 星砂之约/旧灯卫的名字/石壳里的记忆/残焰的安息 + v21.80 新入列 未归的矿灯/护粮的委托 + v23.32 新入列 树精的菌库 + v23.42 新入列 夜路的狼嚎）', absent.length === 0, '缺: ' + absent.join(','));
