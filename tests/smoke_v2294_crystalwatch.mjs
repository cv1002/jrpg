...
ok('GAME_VERSION 格式合法且已越过 v22.93（本版守 v22.94）', !!_gv && (_gv[0] > 22 || (_gv[0] === 22 && _gv[1] >= 95)), GAME_VERSION);
ok('data.js 含 v22.94 注释（守晶人说明）', dSrc.includes('v22.94 新内容·纯风味'));
ok('GAME_VERSION 字面量已为 v22.94（旧 v22.93 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.95';") && !dSrc.includes("const GAME_VERSION = 'v22." + "93';"));
...
ok('README tests 树含 smoke_v2294_crystalwatch 串尾且旧串尾零残留（树为历史清单，总数不守恒）',
  treeLine && treeLine.includes('+ smoke_v2293_deadsave + smoke_v2294_crystalwatch + smoke_v2295_deadprog（npm test 串跑）') &&
  !treeLine.includes('smoke_v2292_cavewatch（npm test 串跑）'));
ok('README 件套口径为一百九十一件套（一百九十件套清除）且旧 188 口径零残留',
  readme.includes('冒烟一百九十一件套（一百九十件套清除）') && !readme.includes('冒烟一百八十八件套（一百八十七件套清' + '除）'));
...
ok('package.json 已收录 smoke_v2294_crystalwatch（npm test 串跑第 190 份）',
  JSON.stringify(JSON.parse(pkg).scripts.test).includes('smoke_v2294_crystalwatch.mjs && node tests/smoke_v2295_deadprog.mjs'));
ok('package.json 串尾为 ... smoke_v2293_deadsave.mjs && node tests/smoke_v2294_crystalwatch.mjs && node tests/smoke_v2295_deadprog.mjs"',
  pkg.includes('node tests/smoke_v2293_deadsave.mjs && node tests/smoke_v2294_crystalwatch.mjs && node tests/smoke_v2295_deadprog.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 190 件套', testChain === 191, String(testChain));
ok('CHANGELOG 顶部已追加 v22.94 条目', changelog.startsWith('## v22.95 '));
...
ok('smoke_v2293 的 GAME_VERSION 字面量 pin 已更新为 v22.94（旧 v22.93 零残留）',
  s2293.includes("const GAME_VERSION = 'v22.95';") && !s2293.includes("const GAME_VERSION = 'v22." + "93';"));
ok('smoke_v2293 的 README 件套 pin 已随新现实更新为一百九十一件套（一百九十件套清除）',
  s2293.includes('一百九十二件套（一百九十一件套清除）'));
ok('smoke_v2293 的 package.json 件套计数 pin 已更新为 === 190', s2293.includes('testChain === 191'));
ok('smoke_v2293 的 README 串尾 pin 已随新现实延伸至 smoke_v2294_crystalwatch',
  s2293.includes('smoke_v2292_cavewatch + smoke_v2293_deadsave + smoke_v2294_crystalwatch + smoke_v2295_deadprog（npm test 串跑）'));
ok('smoke_v2293 的 package 串尾 pin 已延伸至 smoke_v2294_crystalwatch',
  s2293.includes('node tests/smoke_v2292_cavewatch.mjs && node tests/smoke_v2293_deadsave.mjs && node tests/smoke_v2294_crystalwatch.mjs && node tests/smoke_v2295_deadprog.mjs"'));
ok('smoke_v2293 的 CHANGELOG 顶 pin 已更新为 ## v22.95', s2293.includes("startsWith('## v22.95 '"));
ok('smoke_v2293 的哨兵 pin 已更新为一百九十二件套（一百九十一件套清除）', s2293.includes('一百九十二件套（一百九十一件套清除）'));
ok('smoke_v2292 的 NPC 总数 pin 已更新为 === 38', s2292.includes('Object.keys(NPC_SPOTS).length === 38'));
ok('smoke_v2292 的版本锚已推进至 >= 94', s2292.includes('_gv[1] >= 95'));

// 旧代 v22.93 pin 全库零残留
const allTests = fs.readdirSync(path.join(ROOT, 'tests')).filter((f) => f.endsWith('.mjs') && f !== 'smoke_v2294_crystalwatch.mjs && node tests/smoke_v2295_deadprog.mjs');
const stale = [];
