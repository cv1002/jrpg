...
ok('data.js GAME_VERSION 字面量为 v22.87', dataSrc.includes("const GAME_VERSION = 'v22.87'"));
...
ok('README tests 树串尾已延伸至 smoke_v2287_trueroute（v2286 后接 v2287）',
  readme.includes('smoke_v2286_titleekey + smoke_v2287_trueroute（npm test 串跑）'));
ok('README 件套口径为一百八十三件套（一百八十二件套清除）且旧 182 口径零残留',
  readme.includes('冒烟一百八十三件套（一百八十二件套清除）') && !readme.includes('冒烟一百八十二件套（一百八十一件套清' + '除）'));
...
ok('package.json test 串含 smoke_v2287_trueroute.mjs 且位于串尾',
  pkg.includes('node tests/smoke_v2286_titleekey.mjs && node tests/smoke_v2287_trueroute.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 183 件套', testChain === 183, String(testChain));
ok('CHANGELOG 含 v22.87 条目（顶 pin）', changelog.startsWith('## v22.87 '));
...
ok('smoke_v2286 的 GAME_VERSION 字面量 pin 已更新为 v22.87', s2286.includes("const GAME_VERSION = 'v22.87';"));
ok('smoke_v2286 的 CHANGELOG 顶 pin 已更新为 ## v22.87', s2286.includes("startsWith('## v22.87 '"));
ok('smoke_v2286 的件套 pin 已更新为一百八十三件套（一百八十二件套清除）', s2286.includes('一百八十三件套（一百八十二件套清除）'));
ok('smoke_v2286 的 README 串尾 pin 已延伸至 smoke_v2287_trueroute',
  s2286.includes('smoke_v2286_titleekey + smoke_v2287_trueroute（npm test 串跑）'));
ok('smoke_v2286 的 package 串尾 pin 已延伸至 smoke_v2287_trueroute',
  s2286.includes('node tests/smoke_v2286_titleekey.mjs && node tests/smoke_v2287_trueroute.mjs"'));
ok('smoke_v2286 的 testChain pin 已更新为 183', s2286.includes('testChain === 183'));
ok('smoke_v2285 的 GAME_VERSION 字面量 pin 已更新为 v22.87', s2285.includes("const GAME_VERSION = 'v22.87';"));
...
ok('哨兵链 v2143 已含下一版件套口径（一百八十四件套（一百八十三件套清除））',
  s2143.includes('一百八十四件套（一百八十三件套清除）') &&
  s2143.includes("!readme.includes('一百八十四件套（一百八十三件套清除）')"));
