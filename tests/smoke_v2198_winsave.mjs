...
ok('GAME_VERSION 字面量已为 v21.98', GAME_VERSION === 'v22.89', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v21.98', dataSrc.includes("const GAME_VERSION = 'v22.89';"));
...
ok('main.js win.onKey Enter→ending 分支零回归（v22.85 起 e/E/Enter 三键同条件逐字保留）',
  winBlock.includes("if (e.key === 'Enter' || e.key === 'e' || e.key === 'E') goto('ending');"));
...
ok('drawWin 页脚补「按 P 存档」（按 Enter/E 观看尾声 · 按 P 存档 · 按 R 重开新档(连按两次)，v22.85 补 E）',
  menusSrc.includes("按 Enter/E 观看尾声 · 按 P 存档 · 按 R 重开新档(连按两次)"));
...
ok('运行期：页脚 P 口径落位（按 Enter/E 观看尾声 · 按 P 存档 · 按 R 重开新档(连按两次)，v22.85 补 E）',
  CAPTURED.some((t) => t.includes('按 Enter/E 观看尾声 · 按 P 存档 · 按 R 重开新档(连按两次)')));
...
ok('README 件套口径为一百八十五件套（一百八十四件套清除）',
  readme.includes('冒烟一百八十五件套（一百八十四件套清除）') && !readme.includes('冒烟九十三件套（九十二件套清' + '除）'));
ok('README 含 v21.98 守护描述', readme.includes('v21.98 起含胜利画面存档入口守护'));
ok('README 快速上手表胜利画面行补 P 存档（Enter/P/R + v21.98 口径）',
  readme.includes('胜利画面 Enter/E/P/R') && readme.includes('v21.98') && readme.includes('回标题按 `L` 读档即可继续'));
