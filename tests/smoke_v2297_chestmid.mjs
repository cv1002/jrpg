...
ok('data.js GAME_VERSION 字面量已为 v22.97（旧 v22.96 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.72';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "96';"));
...
ok('ACH_LIST 精确总数 59 项（v2229 精确计数 pin 随新现实更新 58→59）', ACH_LIST.length === 68, String(ACH_LIST.length));
...
ok('README 件套口径为二百一十二件套（二百一十一件套清除）且旧 192 口径零残留',
  readme.includes('冒烟二百一十二件套（二百一十一件套清除）') && !readme.includes('冒烟一百九十二件套（一百九十一件套清' + '除）'));
...
ok('package.json test 串共 193 件套', testChain === 212, String(testChain));
ok('CHANGELOG 顶部已追加 v22.97 条目', changelog.startsWith('## v23.72 '));
...
ok('smoke_v2296 的 GAME_VERSION 字面量 pin 已更新为 v22.97', s2296.includes("const GAME_VERSION = 'v23.72';"));
ok('smoke_v2296 的 CHANGELOG 顶 pin 已更新为 ## v22.97', s2296.includes("startsWith('## v23.72 '"));
...
ok('smoke_v2296 的 testChain pin 已更新为 193', s2296.includes('testChain === 212'));
ok('smoke_v2296 的版本锚已推进至 >= 97', s2296.includes('_gv[1] >= 99'));
ok('smoke_v2274 的 ACH_LIST 精确计数 pin 已更新为 === 59', s2274.includes('ACH_LIST.length === 68'));
ok('smoke_v2229 的 ACH_LIST 精确计数 pin 已更新为 === 59', s2229.includes('ACH_LIST.length === 68'));
