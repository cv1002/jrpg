...
ok('data.js GAME_VERSION 字面量已为 v22.97（旧 v22.96 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.11';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "96';"));
...
ok('README 件套口径为二百零七件套（二百零六件套清除）且旧 192 口径零残留',
  readme.includes('冒烟二百零七件套（二百零六件套清除）') && !readme.includes('冒烟一百九十二件套（一百九十一件套清' + '除）'));
...
ok('package.json test 串共 193 件套', testChain === 207, String(testChain));
ok('CHANGELOG 顶部已追加 v22.97 条目', changelog.startsWith('## v23.11 '));
...
ok('smoke_v2296 的 GAME_VERSION 字面量 pin 已更新为 v22.97', s2296.includes("const GAME_VERSION = 'v23.11';"));
ok('smoke_v2296 的 CHANGELOG 顶 pin 已更新为 ## v22.97', s2296.includes("startsWith('## v23.11 '"));
ok('smoke_v2296 的件套 pin 已更新为二百零七件套（二百零六件套清除）', s2296.includes('冒烟二百零七件套（二百零六件套清除）'));
...
ok('smoke_v2296 的 testChain pin 已更新为 193', s2296.includes('testChain === 207'));
...
ok('smoke_v2143 哨兵链已推进至二百零七件套（二百零六件套清除）', s2143.includes('二百零八件套（二百零七件套清除）') && s2143.includes("!readme.includes('二百零八件套（二百零七件套清除）')"));
