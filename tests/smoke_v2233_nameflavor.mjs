...
ok('GAME_VERSION 字面量已为 v22.33（旧 v22.32 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.33';") && !dSrc.includes("const GAME_VERSION = 'v22." + "32';"));
...
ok('README tests 树收录 smoke_v2233_nameflavor 且位于串尾', readme.includes('smoke_v2231_smith + smoke_v2232_travelsup + smoke_v2233_nameflavor（npm test 串跑）'));
...
ok('README 件套口径为一百二十九件套（一百二十八件套清除）',
  readme.includes('冒烟一百二十九件套（一百二十八件套清除）') && !readme.includes('冒烟一百二十八件套（一百二十七件套清' + '除）'));
...
ok('package.json test 串共 129 件套', testChain === 129, String(testChain));
...
ok('smoke_v2232 的 GAME_VERSION 字面量 pin 已更新为 v22.33', s2232.includes("const GAME_VERSION = 'v22.33';"));
ok('smoke_v2232 的 GAME_VERSION 恒等 pin 已更新为 === v22.33', s2232.includes("GAME_VERSION === 'v22.33'"));
ok('smoke_v2232 的 README 件套 pin 已随新现实更新为一百二十九件套（一百二十八件套清除）',
  s2232.includes('一百二十九件套（一百二十八件套清除）'));
ok('smoke_v2232 的 README 树尾 pin 已更新为 + smoke_v2232_travelsup + smoke_v2233_nameflavor',
  s2232.includes('smoke_v2231_smith + smoke_v2232_travelsup + smoke_v2233_nameflavor（npm test 串跑）'));
ok('smoke_v2232 的 package.json 件套计数 pin 已更新为 === 129', s2232.includes('testChain === 129'));
ok('smoke_v2231 的 GAME_VERSION 字面量 pin 已更新为 v22.33', s2231.includes("const GAME_VERSION = 'v22.33';"));
...
ok('smoke_v2231 的 package.json 件套计数 pin 已更新为 === 129', s2231.includes('testChain === 129'));
ok('smoke_v2230 的 GAME_VERSION 字面量 pin 已更新为 v22.33', s2230.includes("const GAME_VERSION = 'v22.33';"));
...
ok('smoke_v2230 的 package.json 件套计数 pin 已更新为 === 129', s2230.includes('testChain === 129'));
ok('smoke_v2229 的 GAME_VERSION 字面量 pin 已更新为 v22.33', s2229.includes("const GAME_VERSION = 'v22.33';"));
ok('smoke_v2229 的 GAME_VERSION 恒等 pin 已更新为 === v22.33', s2229.includes("GAME_VERSION === 'v22.33'"));
...
ok('smoke_v2229 的 package.json 件套计数 pin 已更新为 === 129', s2229.includes('testChain === 129'));
