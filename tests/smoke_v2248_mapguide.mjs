...
ok('GAME_VERSION 字面量已为 v22.48（旧 v22.47 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.52';") && !dSrc.includes("const GAME_VERSION = 'v22." + "47';"));
...
ok('README tests 树已收录 smoke_v2248_mapguide 且位于串尾', readme.includes('smoke_v2247_villagewell + smoke_v2248_mapguide + smoke_v2249_shopkeep + smoke_v2250_brewer + smoke_v2251_oathkeep + smoke_v2252_rail（npm test 串跑）'));
ok('README 件套口径为一百四十八件套（一百四十七件套清除）',
  readme.includes('冒烟一百四十八件套（一百四十七件套清除）') && !readme.includes('冒烟一百四十三件套（一百四十二件套清' + '除）'));
...
ok('package.json test 串共 144 件套', testChain === 148, String(testChain));
ok('CHANGELOG 顶部已追加 v22.48 条目', changelog.startsWith('## v22.52'));
...
ok('smoke_v2247 的 GAME_VERSION 字面量 pin 已更新为 v22.48（旧 v22.47 零残留）',
  s2247.includes("const GAME_VERSION = 'v22.52';") && !s2247.includes("const GAME_VERSION = 'v22." + "47';"));
ok('smoke_v2247 的 README 件套 pin 已随新现实更新为一百四十八件套（一百四十七件套清除）',
  s2247.includes('一百四十八件套（一百四十七件套清除）'));
ok('smoke_v2247 的 package.json 件套计数 pin 已更新为 === 144', s2247.includes('testChain === 148'));
ok('smoke_v2247 的 README 串尾 pin 已随新现实延伸至 smoke_v2248_mapguide',
  s2247.includes('smoke_v2247_villagewell + smoke_v2248_mapguide + smoke_v2249_shopkeep + smoke_v2250_brewer + smoke_v2251_oathkeep + smoke_v2252_rail（npm test 串跑）'));
...
ok('v2143-45「件套守护领先一位」哨兵链已推进至 145（一百四十九件套（一百四十八件套清除））',
