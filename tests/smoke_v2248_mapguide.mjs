...
ok('GAME_VERSION 字面量已为 v22.48（旧 v22.47 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.55';") && !dSrc.includes("const GAME_VERSION = 'v22." + "47';"));
...
ok('README tests 树已收录 smoke_v2248_mapguide 且位于串尾', readme.includes('smoke_v2247_villagewell + smoke_v2248_mapguide + smoke_v2249_shopkeep + smoke_v2250_brewer + smoke_v2251_oathkeep + smoke_v2252_rail + smoke_v2253_supplypoint + smoke_v2254_grainfield + smoke_v2255_steleglow（npm test 串跑）'));
...
ok('package.json test 串共 144 件套', testChain === 151, String(testChain));
ok('CHANGELOG 顶部已追加 v22.48 条目', changelog.startsWith('## v22.55'));
...
ok('smoke_v2247 的 GAME_VERSION 字面量 pin 已更新为 v22.48（旧 v22.47 零残留）',
  s2247.includes("const GAME_VERSION = 'v22.55';") && !s2247.includes("const GAME_VERSION = 'v22." + "47';"));
ok('smoke_v2247 的 README 件套 pin 已随新现实更新为一百五十一件套（一百五十件套清除）',
  s2247.includes('一百五十一件套（一百五十件套清除）'));
ok('smoke_v2247 的 package.json 件套计数 pin 已更新为 === 144', s2247.includes('testChain === 151'));
ok('smoke_v2247 的 README 串尾 pin 已随新现实延伸至 smoke_v2248_mapguide',
  s2247.includes('smoke_v2247_villagewell + smoke_v2248_mapguide + smoke_v2249_shopkeep + smoke_v2250_brewer + smoke_v2251_oathkeep + smoke_v2252_rail + smoke_v2253_supplypoint + smoke_v2254_grainfield + smoke_v2255_steleglow（npm test 串跑）'));
...
ok('v2143-45「件套守护领先一位」哨兵链已推进至 145（一百五十二件套（一百五十一件套清除））',
