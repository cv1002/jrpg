...
ok('GAME_VERSION 字面量已为 v22.48（旧 v22.47 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.13';") && !dSrc.includes("const GAME_VERSION = 'v22." + "47';"));
...
ok('package.json test 串共 144 件套', testChain === 209, String(testChain));
ok('CHANGELOG 顶部已追加 v22.48 条目', changelog.startsWith('## v23.13'));
...
ok('smoke_v2247 的 GAME_VERSION 字面量 pin 已更新为 v22.48（旧 v22.47 零残留）',
  s2247.includes("const GAME_VERSION = 'v23.13';") && !s2247.includes("const GAME_VERSION = 'v22." + "47';"));
ok('smoke_v2247 的 README 件套 pin 已随新现实更新为二百零九件套（二百零八件套清除）',
  s2247.includes('二百零九件套（二百零八件套清除）'));
ok('smoke_v2247 的 package.json 件套计数 pin 已更新为 === 144', s2247.includes('testChain === 209'));
...
ok('v2143-45「件套守护领先一位」哨兵链已推进至 145（二百零九件套（二百零八件套清除））',
