...
ok('GAME_VERSION 字面量已为 v22.48（旧 v22.47 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.76';") && !dSrc.includes("const GAME_VERSION = 'v22." + "47';"));
...
ok('package.json test 串共 144 件套', testChain === 172, String(testChain));
ok('CHANGELOG 顶部已追加 v22.48 条目', changelog.startsWith('## v22.76'));
...
ok('smoke_v2247 的 GAME_VERSION 字面量 pin 已更新为 v22.48（旧 v22.47 零残留）',
  s2247.includes("const GAME_VERSION = 'v22.76';") && !s2247.includes("const GAME_VERSION = 'v22." + "47';"));
ok('smoke_v2247 的 README 件套 pin 已随新现实更新为一百七十二件套（一百七十一件套清除）',
  s2247.includes('一百七十二件套（一百七十一件套清除）'));
ok('smoke_v2247 的 package.json 件套计数 pin 已更新为 === 144', s2247.includes('testChain === 172'));
...
ok('v2143-45「件套守护领先一位」哨兵链已推进至 145（一百七十三件套（一百七十二件套清除））',
