...
ok('GAME_VERSION 字面量已为 v22.17（本版独占精确锚点）', GAME_VERSION === 'v22.17', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.17（旧 v22.16 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v22.17';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "16';"));
...
ok('README tests 树收录 smoke_v2216_picker + smoke_v2217_elixir2 且位于串尾', readme.includes('smoke_v2216_picker + smoke_v2217_elixir2（npm test 串跑）'));
ok('README 件套口径为一百一十三件套（一百一十二件套清除）',
  readme.includes('冒烟一百一十三件套（一百一十二件套清除）') && !readme.includes('冒烟一百一十二件套（一百一十一件套清' + '除）'));
...
ok('README 成就口径「43 项」双处同步（快速上手表 C 键行 + 图鉴&成就行）',
  readme.includes('成就一览（全部 43 项进度') && readme.includes('**43 项成就**') &&
...
ok('package.json test 串共 113 件套', testChain === 113, String(testChain));
...
ok('smoke_v2216 的 README 件套 pin 已随新现实更新为一百一十三件套（一百一十二件套清除）',
  s2216.includes('一百一十三件套（一百一十二件套清除）'));
ok('smoke_v2216 的 README 树尾 pin 已更新为 + smoke_v2217_elixir2', s2216.includes('smoke_v2216_picker + smoke_v2217_elixir2（npm test 串跑）'));
ok('smoke_v2216 的 GAME_VERSION 字面量 pin 已更新为 v22.17', s2216.includes("const GAME_VERSION = 'v22.17';"));
ok('smoke_v2216 的 GAME_VERSION 恒等 pin 已更新为 === v22.17', s2216.includes("GAME_VERSION === 'v22.17'"));
ok('smoke_v2216 的 package.json 件套计数 pin 已更新为 === 113', s2216.includes('testChain === 113'));
ok('smoke_v2215 的 GAME_VERSION 字面量 pin 已更新为 v22.17', s2215.includes("const GAME_VERSION = 'v22.17';"));
ok('smoke_v2215 的 package.json 件套计数 pin 已更新为 === 113', s2215.includes('testChain === 113'));
ok('smoke_v2214 的 README 成就 pin 已随新现实更新为 43 项双处落位',
  s2214.includes('成就一览（全部 43 项进度') && s2214.includes('**43 项成就**'));
...
ok('smoke_v2213 的 README 树尾 pin 已更新为 + smoke_v2217_elixir2', s2213.includes('smoke_v2216_picker + smoke_v2217_elixir2（npm test 串跑）'));
