...
ok('GAME_VERSION 字面量已为 v22.38（旧 v22.37 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.86';") && !dSrc.includes("const GAME_VERSION = 'v22." + "37';"));
...
ok('页长派生：末行基线 414 ≤440（行数 8 不变、r[2] 数 3→6（v22.53 3→5、v22.78 5→6），页脚 452 之上留白）',
...
ok('README 件套口径为一百八十二件套（一百八十一件套清除）',
  readme.includes('冒烟一百八十二件套（一百八十一件套清除）') && !readme.includes('冒烟一百三十三件套（一百三十二件套清' + '除）'));
...
ok('package.json test 串共 134 件套', testChain === 182, String(testChain));
...
ok('smoke_v2237 的 GAME_VERSION 字面量 pin 已更新为 v22.38', s2237.includes("const GAME_VERSION = 'v22.86';"));
ok('smoke_v2237 的 GAME_VERSION 恒等 pin 已更新为 === v22.38', s2237.includes("GAME_VERSION === 'v22.86'"));
...
ok('smoke_v2237 的 package.json 件套计数 pin 已更新为 === 134', s2237.includes('testChain === 182'));
ok('smoke_v2237 的 README 件套口径 pin 已更新为一百三十四件套', s2237.includes('冒烟一百八十二件套（一百八十一件套清除）'));
ok('smoke_v2237 的图例正文 pin 已随星井更新（状态标 r[2] 含星井）',
  s2237.includes('星井 低鸣星蓝/静默灰'));
ok('smoke_v2236 的 GAME_VERSION 字面量 pin 已更新为 v22.38', s2236.includes("const GAME_VERSION = 'v22.86';"));
...
ok('smoke_v2236 的件套/恒等 pin 已更新（===134/===v22.38）',
  s2236.includes('testChain === 182') && s2236.includes("GAME_VERSION === 'v22.86'"));
ok('smoke_v2235 的 GAME_VERSION 字面量 pin 已更新为 v22.38', s2235.includes("const GAME_VERSION = 'v22.86';"));
ok('smoke_v2234 的 GAME_VERSION 恒等 pin 已更新为 === v22.38', s2234.includes("GAME_VERSION === 'v22.86'"));
...
ok('smoke_v2232 的 package.json 件套计数 pin 已更新为 === 134', s2232.includes('testChain === 182'));
ok('smoke_v2230 的 GAME_VERSION 恒等 pin 已更新为 === v22.38', s2230.includes("GAME_VERSION === 'v22.86'"));
