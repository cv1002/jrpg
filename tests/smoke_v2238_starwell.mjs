...
ok('GAME_VERSION 字面量已为 v22.38（旧 v22.37 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.74';") && !dSrc.includes("const GAME_VERSION = 'v22." + "37';"));
...
ok('契约：NPC 总数保持 30（零 NPC 变更，v22.37 pin 续守）', Object.keys(NPC_SPOTS).length === 34,
...
ok('页长派生：末行基线 398 ≤440（行数 8 不变、r[2] 数 3→5，页脚 452 之上留白）',
  80 + (page.length - 1) * 34 + (page.reduce((a, r) => a + (r[2] ? 1 : 0), 0)) * 16 === 398);

// —— 运行期实证：drawHelp 第 2 页渲染捕获（新 r[2] 落位 y=266，既有行随 v22.53 让位下移）——
...
ok('运行期：状态标次行含星井落位（x=100 y=266 12px 次级灰）',
  calls.some((c) => c.t === '遇敌槽红条 · 大灯 熄冷灰/亮暖金（与画布大灯同档） · 星井 低鸣星蓝/静默灰 · 无委托NPC 米色' &&
    c.y === 266 && c.font === '12px sans-serif' && c.fill === '#7d93a3'));
ok('运行期：状态标主行/色标两行/通关之路/页脚基线零位移（248/298/316/398/452）',
  calls.some((c) => c.t.startsWith('小地图·状态标') && c.y === 248) &&
  calls.some((c) => c.t.startsWith('小地图·色标') && c.y === 298) &&
  calls.some((c) => c.t.startsWith('设施：商店·金') && c.y === 316) &&
  calls.some((c) => c.t.startsWith('通关之路') && c.y === 398) &&
...
ok('README 件套口径为一百七十件套（一百六十九件套清除）',
  readme.includes('冒烟一百七十件套（一百六十九件套清除）') && !readme.includes('冒烟一百三十三件套（一百三十二件套清' + '除）'));
...
ok('package.json test 串共 134 件套', testChain === 170, String(testChain));
...
ok('smoke_v2237 的 GAME_VERSION 字面量 pin 已更新为 v22.38', s2237.includes("const GAME_VERSION = 'v22.74';"));
ok('smoke_v2237 的 GAME_VERSION 恒等 pin 已更新为 === v22.38', s2237.includes("GAME_VERSION === 'v22.74'"));
...
ok('smoke_v2237 的 package.json 件套计数 pin 已更新为 === 134', s2237.includes('testChain === 170'));
ok('smoke_v2237 的 README 件套口径 pin 已更新为一百三十四件套', s2237.includes('冒烟一百七十件套（一百六十九件套清除）'));
ok('smoke_v2237 的图例正文 pin 已随星井更新（状态标 r[2] 含星井）',
  s2237.includes('星井 低鸣星蓝/静默灰'));
ok('smoke_v2236 的 GAME_VERSION 字面量 pin 已更新为 v22.38', s2236.includes("const GAME_VERSION = 'v22.74';"));
...
ok('smoke_v2236 的件套/恒等 pin 已更新（===134/===v22.38）',
  s2236.includes('testChain === 170') && s2236.includes("GAME_VERSION === 'v22.74'"));
ok('smoke_v2235 的 GAME_VERSION 字面量 pin 已更新为 v22.38', s2235.includes("const GAME_VERSION = 'v22.74';"));
ok('smoke_v2234 的 GAME_VERSION 恒等 pin 已更新为 === v22.38', s2234.includes("GAME_VERSION === 'v22.74'"));
...
ok('smoke_v2232 的 package.json 件套计数 pin 已更新为 === 134', s2232.includes('testChain === 170'));
ok('smoke_v2230 的 GAME_VERSION 恒等 pin 已更新为 === v22.38', s2230.includes("GAME_VERSION === 'v22.74'"));
