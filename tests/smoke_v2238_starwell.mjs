...
ok('GAME_VERSION 字面量已为 v22.38（旧 v22.37 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.47';") && !dSrc.includes("const GAME_VERSION = 'v22." + "37';"));
...
ok('静默档零星砂浮光（#cfeaff 仅出现在 hum 分支内；v22.39 星砂车再增注释+绘制各 1 处 → 全文件 4 处；v22.47 村井再增注释+绘制各 1 处 → 全文件 6 处）', wSrc.split('#cfeaff').length === 7);
...
ok('契约：NPC 总数保持 30（零 NPC 变更，v22.37 pin 续守）', Object.keys(NPC_SPOTS).length === 31,
...
ok('页长派生：末行基线仍 366 ≤440（行数/r[2] 数均不变，页脚 452 之上留白）',
  80 + (page.length - 1) * 34 + (page.reduce((a, r) => a + (r[2] ? 1 : 0), 0)) * 16 === 366);
...
ok('运行期：状态标主行/色标两行/通关之路/页脚基线零位移（216/266/284/316/452）',
  ...
  calls.some((c) => c.t.startsWith('通关之路') && c.y === 366) &&
...
ok('README tests 树收录 smoke_v2238_starwell 且位于串尾', readme.includes('smoke_v2237_minimaplegend + smoke_v2238_starwell + smoke_v2240_tallgrass + smoke_v2241_gatearch + smoke_v2242_mushfield + smoke_v2243_encguide + smoke_v2244_fulldanger + smoke_v2245_watcher + smoke_v2246_fountgauge + smoke_v2247_villagewell（npm test 串跑）'));
ok('README 件套口径为一百四十三件套（一百四十二件套清除）',
  readme.includes('冒烟一百四十三件套（一百四十二件套清除）') && !readme.includes('冒烟一百三十三件套（一百三十二件套清' + '除）'));
...
ok('package.json test 串共 134 件套', testChain === 143, String(testChain));
...
ok('smoke_v2237 的 GAME_VERSION 字面量 pin 已更新为 v22.38', s2237.includes("const GAME_VERSION = 'v22.47';"));
ok('smoke_v2237 的 GAME_VERSION 恒等 pin 已更新为 === v22.38', s2237.includes("GAME_VERSION === 'v22.47'"));
ok('smoke_v2237 的 README 串尾 pin 已更新为 + smoke_v2238_starwell',
  s2237.includes('smoke_v2236_villagelamp + smoke_v2237_minimaplegend + smoke_v2238_starwell + smoke_v2240_tallgrass + smoke_v2241_gatearch + smoke_v2242_mushfield + smoke_v2243_encguide + smoke_v2244_fulldanger + smoke_v2245_watcher + smoke_v2246_fountgauge + smoke_v2247_villagewell（npm test 串跑）'));
ok('smoke_v2237 的 package.json 件套计数 pin 已更新为 === 134', s2237.includes('testChain === 143'));
ok('smoke_v2237 的 README 件套口径 pin 已更新为一百三十四件套', s2237.includes('冒烟一百四十三件套（一百四十二件套清除）'));
ok('smoke_v2237 的图例正文 pin 已随星井更新（状态标 r[2] 含星井）',
  s2237.includes('星井 低鸣星蓝/静默灰'));
ok('smoke_v2236 的 GAME_VERSION 字面量 pin 已更新为 v22.38', s2236.includes("const GAME_VERSION = 'v22.47';"));
ok('smoke_v2236 的 README 串尾 pin 已更新为 + smoke_v2238_starwell',
  s2236.includes('smoke_v2235_minimapquest + smoke_v2236_villagelamp + smoke_v2237_minimaplegend + smoke_v2238_starwell + smoke_v2240_tallgrass + smoke_v2241_gatearch + smoke_v2242_mushfield + smoke_v2243_encguide + smoke_v2244_fulldanger + smoke_v2245_watcher + smoke_v2246_fountgauge + smoke_v2247_villagewell（npm test 串跑）'));
ok('smoke_v2236 的件套/恒等 pin 已更新（===134/===v22.38）',
  s2236.includes('testChain === 143') && s2236.includes("GAME_VERSION === 'v22.47'"));
ok('smoke_v2235 的 GAME_VERSION 字面量 pin 已更新为 v22.38', s2235.includes("const GAME_VERSION = 'v22.47';"));
ok('smoke_v2234 的 GAME_VERSION 恒等 pin 已更新为 === v22.38', s2234.includes("GAME_VERSION === 'v22.47'"));
...
ok('smoke_v2232 的 package.json 件套计数 pin 已更新为 === 134', s2232.includes('testChain === 143'));
ok('smoke_v2230 的 GAME_VERSION 恒等 pin 已更新为 === v22.38', s2230.includes("GAME_VERSION === 'v22.47'"));
