...
ok('GAME_VERSION 字面量已为 v22.38（旧 v22.37 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.89';") && !dSrc.includes("const GAME_VERSION = 'v22." + "37';"));
...
ok('静默档零星砂浮光（#cfeaff 仅出现在 hum 分支内；v22.39 星砂车再增注释+绘制各 1 处 → 全文件 4 处；v22.47 村井再增注释+绘制各 1 处 → 全文件 6 处；v22.59 星砂堆再增注释+绘制各 1 处 → 全文件 9 处；v22.62 终焉水晶再增注释+绘制各 1 处 → 全文件 11 处）', wSrc.split('#cfeaff').length === 12);
...
function captureCave(flags) {
  ...
  const spark = () => rects.filter((r) => r.w === 2 && r.h === 2 && r.fs === '#cfeaff' && r.x >= px - 1 && r.x < px + 33).length;
...
ok('契约：NPC 总数保持 30（零 NPC 变更，v22.37 pin 续守）', Object.keys(NPC_SPOTS).length === 35,
...
ok('地图指南页 8 行（v22.43 起加机制行）', page.length === 8, `实际 ${page.length}`);
...
ok('通关之路行逐字零回归（金色收尾行内容不变）', page[7][1] === '讨回灯芯 → 击败洞窟领主 → 双徽记开门 → 回廊尽头面对终焉之神');
...
ok('页长派生：末行基线 414 ≤440（行数 8 不变、r[2] 数 3→6（v22.53 3→5、v22.78 5→6、v22.87 6→7 但末行自带不推基线），页脚 452 之上留白）',
  80 + (page.length - 1) * 34 + (page.slice(0, -1).reduce((a, r) => a + (r[2] ? 1 : 0), 0)) * 16 === 414);

// —— 运行期实证：drawHelp 第 2 页渲染捕获（新 r[2] 落位 y=282，既有行随 v22.53/v22.78 让位下移）——
...
ok('运行期：状态标次行含星井落位（x=100 y=282 12px 次级灰，v22.78 后 +16）',
  calls.some((c) => c.t === '遇敌槽红条 · 大灯 熄冷灰/亮暖金（与画布大灯同档） · 星井 低鸣星蓝/静默灰 · 无委托NPC 米色' &&
    c.y === 282 && c.font === '12px sans-serif' && c.fill === '#7d93a3'));
ok('运行期：状态标主行/色标两行/通关之路/页脚基线 v22.78 后落位（264/314/332/414/452）',
  calls.some((c) => c.t.startsWith('小地图·状态标') && c.y === 264) &&
  calls.some((c) => c.t.startsWith('小地图·色标') && c.y === 314) &&
  calls.some((c) => c.t.startsWith('设施：商店·金') && c.y === 332) &&
  calls.some((c) => c.t.startsWith('通关之路') && c.y === 414) &&
...
ok('package.json test 串共 134 件套', testChain === 185, String(testChain));
...
ok('smoke_v2237 的 GAME_VERSION 字面量 pin 已更新为 v22.38', s2237.includes("const GAME_VERSION = 'v22.89';"));
ok('smoke_v2237 的 GAME_VERSION 恒等 pin 已更新为 === v22.38', s2237.includes("GAME_VERSION === 'v22.89'"));
ok('smoke_v2237 的 README 串尾 pin 已更新为 + smoke_v2238_starwell',
  s2237.includes('smoke_v2236_villagelamp + smoke_v2237_minimaplegend + smoke_v2238_starwell + smoke_v2240_tallgrass + smoke_v2241_gatearch + smoke_v2242_mushfield + smoke_v2243_encguide + smoke_v2244_fulldanger + smoke_v2245_watcher + smoke_v2246_fountgauge + smoke_v2247_villagewell + smoke_v2248_mapguide + smoke_v2249_shopkeep + smoke_v2250_brewer + smoke_v2251_oathkeep + smoke_v2252_rail + smoke_v2253_supplypoint + smoke_v2254_grainfield + smoke_v2255_steleglow + smoke_v2256_campfire + smoke_v2257_pondglow + smoke_v2258_potionhelp + smoke_v2259_sandpile + smoke_v2260_fountripple + smoke_v2261_rich3 + smoke_v2262_crystal + smoke_v2263_ptime3 + smoke_v2264_hunt3 + smoke_v2265_lucky3 + smoke_v2266_stock3 + smoke_v2267_elixir3 + smoke_v2268_brew3 + smoke_v2269_mush3 + smoke_v2270_outstep + smoke_v2271_outstep2 + smoke_v2272_scholar2 + smoke_v2273_seen5 + smoke_v2274_seen2 + smoke_v2275_codexempty + smoke_v2276_lampkid + smoke_v2277_pondhint + smoke_v2278_mushguide + smoke_v2279_lampwell + smoke_v2280_grainfield + smoke_v2281_starwell + smoke_v2282_archgate + smoke_v2283_menuekey + smoke_v2284_skillekey + smoke_v2285_winekey + smoke_v2286_titleekey + smoke_v2287_trueroute + smoke_v2288_scrollhint + smoke_v2289_winprog（npm test 串跑）'));
ok('smoke_v2237 的 package.json 件套计数 pin 已更新为 === 134', s2237.includes('testChain === 185'));
ok('smoke_v2237 的 README 件套口径 pin 已更新为一百三十四件套', s2237.includes('冒烟一百八十五件套（一百八十四件套清除）'));
ok('smoke_v2237 的图例正文 pin 已随星井更新（状态标 r[2] 含星井）',
  s2237.includes('星井 低鸣星蓝/静默灰'));
ok('smoke_v2236 的 GAME_VERSION 字面量 pin 已更新为 v22.38', s2236.includes("const GAME_VERSION = 'v22.89';"));
ok('smoke_v2236 的 README 串尾 pin 已更新为 + smoke_v2238_starwell',
  s2236.includes('smoke_v2235_minimapquest + smoke_v2236_villagelamp + smoke_v2237_minimaplegend + smoke_v2238_starwell + smoke_v2240_tallgrass + smoke_v2241_gatearch + smoke_v2242_mushfield + smoke_v2243_encguide + smoke_v2244_fulldanger + smoke_v2245_watcher + smoke_v2246_fountgauge + smoke_v2247_villagewell + smoke_v2248_mapguide + smoke_v2249_shopkeep + smoke_v2250_brewer + smoke_v2251_oathkeep + smoke_v2252_rail + smoke_v2253_supplypoint + smoke_v2254_grainfield + smoke_v2255_steleglow + smoke_v2256_campfire + smoke_v2257_pondglow + smoke_v2258_potionhelp + smoke_v2259_sandpile + smoke_v2260_fountripple + smoke_v2261_rich3 + smoke_v2262_crystal + smoke_v2263_ptime3 + smoke_v2264_hunt3 + smoke_v2265_lucky3 + smoke_v2266_stock3 + smoke_v2267_elixir3 + smoke_v2268_brew3 + smoke_v2269_mush3 + smoke_v2270_outstep + smoke_v2271_outstep2 + smoke_v2272_scholar2 + smoke_v2273_seen5 + smoke_v2274_seen2 + smoke_v2275_codexempty + smoke_v2276_lampkid + smoke_v2277_pondhint + smoke_v2278_mushguide + smoke_v2279_lampwell + smoke_v2280_grainfield + smoke_v2281_starwell + smoke_v2282_archgate + smoke_v2283_menuekey + smoke_v2284_skillekey + smoke_v2285_winekey + smoke_v2286_titleekey + smoke_v2287_trueroute + smoke_v2288_scrollhint + smoke_v2289_winprog（npm test 串跑）'));
ok('smoke_v2236 的件套/恒等 pin 已更新（===134/===v22.38）',
  s2236.includes('testChain === 185') && s2236.includes("GAME_VERSION === 'v22.89'"));
ok('smoke_v2235 的 GAME_VERSION 字面量 pin 已更新为 v22.38', s2235.includes("const GAME_VERSION = 'v22.89';"));
ok('smoke_v2234 的 GAME_VERSION 恒等 pin 已更新为 === v22.38', s2234.includes("GAME_VERSION === 'v22.89'"));
ok('smoke_v2233 的 README 串尾 pin 已更新为 + smoke_v2238_starwell',
  s2233.includes('smoke_v2234_innkeeper + smoke_v2235_minimapquest + smoke_v2236_villagelamp + smoke_v2237_minimaplegend + smoke_v2238_starwell + smoke_v2240_tallgrass + smoke_v2241_gatearch + smoke_v2242_mushfield + smoke_v2243_encguide + smoke_v2244_fulldanger + smoke_v2245_watcher + smoke_v2246_fountgauge + smoke_v2247_villagewell + smoke_v2248_mapguide + smoke_v2249_shopkeep + smoke_v2250_brewer + smoke_v2251_oathkeep + smoke_v2252_rail + smoke_v2253_supplypoint + smoke_v2254_grainfield + smoke_v2255_steleglow + smoke_v2256_campfire + smoke_v2257_pondglow + smoke_v2258_potionhelp + smoke_v2259_sandpile + smoke_v2260_fountripple + smoke_v2261_rich3 + smoke_v2262_crystal + smoke_v2263_ptime3 + smoke_v2264_hunt3 + smoke_v2265_lucky3 + smoke_v2266_stock3 + smoke_v2267_elixir3 + smoke_v2268_brew3 + smoke_v2269_mush3 + smoke_v2270_outstep + smoke_v2271_outstep2 + smoke_v2272_scholar2 + smoke_v2273_seen5 + smoke_v2274_seen2 + smoke_v2275_codexempty + smoke_v2276_lampkid + smoke_v2277_pondhint + smoke_v2278_mushguide + smoke_v2279_lampwell + smoke_v2280_grainfield + smoke_v2281_starwell + smoke_v2282_archgate + smoke_v2283_menuekey + smoke_v2284_skillekey + smoke_v2285_winekey + smoke_v2286_titleekey + smoke_v2287_trueroute + smoke_v2288_scrollhint + smoke_v2289_winprog（npm test 串跑）'));
ok('smoke_v2232 的 package.json 件套计数 pin 已更新为 === 134', s2232.includes('testChain === 185'));
ok('smoke_v2230 的 GAME_VERSION 恒等 pin 已更新为 === v22.38', s2230.includes("GAME_VERSION === 'v22.89'"));
ok('smoke_v2235 的 NPC 总数 pin 保持 30（零 NPC 变更）', s2235.includes('NPC_SPOTS).length === 35'));
