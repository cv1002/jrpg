...
ok('GAME_VERSION 字面量已为 v21.95', GAME_VERSION === 'v22.66', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v21.95', dataSrc.includes("const GAME_VERSION = 'v22.66';"));
ok('data.js 仍保留 v21.94 历史注释（累积注释块，姊妹 pin 不失效）', dataSrc.includes('v21.94 状态页资源行补图鉴进度'));
ok('data.js 导出 PLAY_TIME2_GOAL（export 块落位，POTIONS_GOAL/POTIONS2_GOAL 随其后——v22.6 起 v2206 冒烟守护；v22.63 起 PLAY_TIME3_GOAL 并入同族）', dataSrc.includes(', PLAY_TIME2_GOAL, PLAY_TIME3_GOAL, POTIONS_GOAL, POTIONS2_GOAL, POTIONS3_GOAL, ELIXIR_STOCK_GOAL, ELIXIR_STOCK2_GOAL, PERFECTION_GOLD'));
...
ok('README 件套口径为一百六十二件套（一百六十一件套清除）',
  readme.includes('冒烟一百六十二件套（一百六十一件套清除）') && !readme.includes('冒烟九十件套（八十九件套清除）'));
...
for (const nm of achFiles) {
  ...
  ok(`${nm} 的 README 成就 pin 已随新现实更新（34 项正向 pin 零残留，35 项双处落位）`,
    src.includes("readme.includes('成就一览（全部 50 项进度'") &&
