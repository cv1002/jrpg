...
ok('GAME_VERSION 字面量已为 v21.95', GAME_VERSION === 'v22.0', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v21.95', dataSrc.includes("const GAME_VERSION = 'v22.0';"));
...
ok('README tests 树收录 smoke_v2195_ptime2 且位于串尾（v2195 随新现实由当版冒烟守护）', readme.includes('smoke_v2195_ptime2 + smoke_v2196_deadrecap + smoke_v2197_lucky2 + smoke_v2198_winsave + smoke_v2199_rich2 + smoke_v2200_stock（npm test 串跑）'));
ok('README 件套口径为九十六件套（九十五件套清除）',
  readme.includes('冒烟九十六件套（九十五件套清除）') && !readme.includes('冒烟九十件套（八十九件套清除）'));
