...
ok('GAME_VERSION 字面量已为 v21.95', GAME_VERSION === 'v21.95', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v21.95', dataSrc.includes("const GAME_VERSION = 'v21.95';"));
ok('data.js 仍保留 v21.94 历史注释（累积注释块，姊妹 pin 不失效）', dataSrc.includes('v21.94 状态页资源行补图鉴进度'));
ok('data.js 导出 PLAY_TIME2_GOAL（export 块落位）', dataSrc.includes(', PLAY_TIME2_GOAL, PERFECTION_GOLD'));
...
ok('README tests 树收录 smoke_v2195_ptime2 且位于串尾（v2195 随新现实由当版冒烟守护）', readme.includes('smoke_v2194_statuscodex + smoke_v2195_ptime2（npm test 串跑）'));
ok('README 件套口径为九十一件套（九十件套清除）',
  readme.includes('冒烟九十一件套（九十件套清除）') && !readme.includes('冒烟九十件套（八十九件套清除）'));
ok('README 含 v21.95 守护描述', readme.includes('v21.95 起含新成就「彻夜长明」'));
ok('README 成就口径「35 项」双处同步（快速上手表 C 键行 + 图鉴&成就行）',
  readme.includes('成就一览（全部 35 项进度') && readme.includes('**35 项成就**') &&
...
for (const nm of suite91) {
  ...
  ok(`${nm} 的 README 件套 pin 已随新现实更新为九十一件套（九十件套清除）`,
    src.includes('九十一件套（九十件套清除）'));
}
...
for (const nm of vers) {
  ...
  ok(`${nm} 的 GAME_VERSION 字面量 pin 已随新现实更新为 v21.95`,
    src.includes("const GAME_VERSION = 'v21.95';"));
}
...
for (const nm of achFiles) {
  ...
  ok(`${nm} 的 README 成就 pin 已随新现实更新（34 项正向 pin 零残留，35 项双处落位）`,
    src.includes("readme.includes('成就一览（全部 35 项进度'") &&
