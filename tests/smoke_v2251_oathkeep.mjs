...
ok('GAME_VERSION 字面量已为 v22.51（旧 v22.50 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.74';") && !dSrc.includes("const GAME_VERSION = 'v22." + "50';"));
...
ok('README 件套口径为一百七十件套（一百六十九件套清除）',
  readme.includes('冒烟一百七十件套（一百六十九件套清除）') && !readme.includes('冒烟一百四十六件套（一百四十五件套清' + '除）'));
...
ok('package.json test 串共 147 件套', testChain === 170, String(testChain));
ok('CHANGELOG 顶部已追加 v22.51 条目', changelog.startsWith('## v22.74'));
...
ok('smoke_v2250 的 GAME_VERSION 字面量 pin 已更新为 v22.51（旧 v22.50 零残留）',
  s2250.includes("const GAME_VERSION = 'v22.74';") && !s2250.includes("const GAME_VERSION = 'v22." + "50';"));
ok('smoke_v2250 的 README 件套 pin 已随新现实更新为一百七十件套（一百六十九件套清除）',
  s2250.includes('一百七十件套（一百六十九件套清除）'));
ok('smoke_v2250 的 package.json 件套计数 pin 已更新为 === 147', s2250.includes('testChain === 170'));
...
ok('v2143-45「件套守护领先一位」哨兵链已推进至 148（一百七十一件套（一百七十件套清除））',
