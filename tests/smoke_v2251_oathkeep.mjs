...
ok('GAME_VERSION 字面量已为 v22.51（旧 v22.50 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.61';") && !dSrc.includes("const GAME_VERSION = 'v22." + "50';"));
...
ok('README 件套口径为一百五十七件套（一百五十六件套清除）',
  readme.includes('冒烟一百五十七件套（一百五十六件套清除）') && !readme.includes('冒烟一百四十六件套（一百四十五件套清' + '除）'));
...
ok('package.json test 串共 147 件套', testChain === 157, String(testChain));
ok('CHANGELOG 顶部已追加 v22.51 条目', changelog.startsWith('## v22.61'));
...
ok('smoke_v2250 的 GAME_VERSION 字面量 pin 已更新为 v22.51（旧 v22.50 零残留）',
  s2250.includes("const GAME_VERSION = 'v22.61';") && !s2250.includes("const GAME_VERSION = 'v22." + "50';"));
ok('smoke_v2250 的 README 件套 pin 已随新现实更新为一百五十七件套（一百五十六件套清除）',
  s2250.includes('一百五十七件套（一百五十六件套清除）'));
ok('smoke_v2250 的 package.json 件套计数 pin 已更新为 === 147', s2250.includes('testChain === 157'));
ok('smoke_v2250 的 README 串尾 pin 已随新现实延伸至 smoke_v2251_oathkeep',
  s2250.includes('smoke_v2250_brewer + smoke_v2251_oathkeep + smoke_v2252_rail + smoke_v2253_supplypoint + smoke_v2254_grainfield + smoke_v2255_steleglow + smoke_v2256_campfire + smoke_v2257_pondglow + smoke_v2258_potionhelp + smoke_v2259_sandpile + smoke_v2260_fountripple + smoke_v2261_rich3（npm test 串跑）'));
...
ok('v2143-45「件套守护领先一位」哨兵链已推进至 148（一百五十八件套（一百五十七件套清除））',
