...
ok('GAME_VERSION 字面量已为 v22.51（旧 v22.50 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.99';") && !dSrc.includes("const GAME_VERSION = 'v22." + "50';"));
...
ok('README 件套口径为二百二十三件套（二百二十二件套清除）',
  readme.includes('冒烟二百二十三件套（二百二十二件套清除）') && !readme.includes('冒烟一百四十六件套（一百四十五件套清' + '除）'));
...
ok('CHANGELOG 顶部已追加 v22.51 条目', changelog.startsWith('## v23.99'));
...
ok('smoke_v2250 的 GAME_VERSION 字面量 pin 已更新为 v22.51（旧 v22.50 零残留）',
  s2250.includes("const GAME_VERSION = 'v23.99';") && !s2250.includes("const GAME_VERSION = 'v22." + "50';"));
ok('smoke_v2250 的 README 件套 pin 已随新现实更新为二百二十三件套（二百二十二件套清除）',
  s2250.includes('二百二十三件套（二百二十二件套清除）'));
ok('smoke_v2250 的 package.json 件套计数 pin 已更新为 === 147', s2250.includes('testChain === 223'));
