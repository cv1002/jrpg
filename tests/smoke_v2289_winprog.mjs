...
ok('GAME_VERSION 格式合法且已越过 v22.88', !!_gv && (_gv[0] > 22 || (_gv[0] === 22 && _gv[1] >= 89)), GAME_VERSION);
ok('GAME_VERSION 恒等于 v22.89', GAME_VERSION === 'v22.89', GAME_VERSION);
...
// 2. 源级落位
ok('data.js GAME_VERSION 字面量为 v22.89', dataSrc.includes("const GAME_VERSION = 'v22.89';"));
...
ok('README tests 树串尾已延伸至 smoke_v2289_winprog（v2288 后接 v2289）',
  readme.includes('smoke_v2287_trueroute + smoke_v2288_scrollhint + smoke_v2289_winprog（npm test 串跑）'));
ok('README 件套口径为一百八十五件套（一百八十四件套清除）且旧 184 口径零残留',
  readme.includes('冒烟一百八十五件套（一百八十四件套清除）') && !readme.includes('一百八十四件套（一百八十三件套清' + '除）'));
...
ok('package.json test 串含 smoke_v2289_winprog.mjs 且位于串尾',
  pkg.includes('node tests/smoke_v2288_scrollhint.mjs && node tests/smoke_v2289_winprog.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 185 件套', testChain === 185, String(testChain));
ok('CHANGELOG 含 v22.89 条目（顶 pin）', changelog.startsWith('## v22.89 '));
...
ok('smoke_v2288 的 GAME_VERSION 字面量 pin 已更新为 v22.89', s2288.includes("const GAME_VERSION = 'v22.89';"));
ok('smoke_v2288 的 CHANGELOG 顶 pin 已更新为 ## v22.89', s2288.includes("startsWith('## v22.89 '"));
ok('smoke_v2288 的件套 pin 已更新为一百八十五件套（一百八十四件套清除）', s2288.includes('一百八十五件套（一百八十四件套清除）'));
ok('smoke_v2288 的 README 串尾 pin 已延伸至 smoke_v2289_winprog',
  s2288.includes('smoke_v2287_trueroute + smoke_v2288_scrollhint + smoke_v2289_winprog（npm test 串跑）'));
ok('smoke_v2288 的 package 串尾 pin 已延伸至 smoke_v2289_winprog',
  s2288.includes('node tests/smoke_v2288_scrollhint.mjs && node tests/smoke_v2289_winprog.mjs"'));
ok('smoke_v2288 的 testChain pin 已更新为 185', s2288.includes('testChain === 185'));
ok('smoke_v2287 的 GAME_VERSION 字面量 pin 已更新为 v22.89', s2287.includes("const GAME_VERSION = 'v22.89';"));
ok('smoke_v2287 的 testChain pin 已更新为 185', s2287.includes('testChain === 185'));
ok('smoke_v2287 的哨兵 pin 已更新为一百八十六件套（一百八十五件套清除）', s2287.includes('一百八十六件套（一百八十五件套清除）'));
ok('smoke_v2286 的 GAME_VERSION 字面量 pin 已更新为 v22.89', s2286.includes("const GAME_VERSION = 'v22.89';"));
ok('smoke_v2286 的 哨兵 pin 已更新为一百八十六件套（一百八十五件套清除）', s2286.includes('一百八十六件套（一百八十五件套清除）'));
ok('smoke_v2286 的 README 串尾 pin 已延伸至 smoke_v2289_winprog',
  s2286.includes('smoke_v2287_trueroute + smoke_v2288_scrollhint + smoke_v2289_winprog（npm test 串跑）'));
...
ok('哨兵链 v2143 已含下一版件套口径（一百八十六件套（一百八十五件套清除））',
  s2143.includes('一百八十六件套（一百八十五件套清除）') &&
  s2143.includes("!readme.includes('一百八十六件套（一百八十五件套清除）')"));
