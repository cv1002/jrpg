...
ok('data.js GAME_VERSION 字面量为 v22.81', dataSrc.includes("const GAME_VERSION = 'v22.96'"));
...
ok('README 件套口径为一百九十二件套（一百九十一件套清除）且旧 176 口径零残留',
  readme.includes('冒烟一百九十二件套（一百九十一件套清除）') && !readme.includes('冒烟一百七十六件套（一百七十五件套清' + '除）'));
...
ok('package.json test 串共 177 件套', testChain === 192, String(testChain));
ok('CHANGELOG 含 v22.81 条目（顶 pin）', changelog.startsWith('## v22.96 '));
...
ok('smoke_v2280 的 GAME_VERSION 字面量 pin 已更新为 v22.81', s2280.includes("const GAME_VERSION = 'v22.96';"));
ok('smoke_v2280 的 CHANGELOG 顶 pin 已更新为 ## v22.81', s2280.includes("startsWith('## v22.96'"));
ok('smoke_v2280 的件套 pin 已更新为一百九十二件套（一百九十一件套清除）', s2280.includes('一百九十二件套（一百九十一件套清除）'));
...
ok('哨兵链 v2143 已含下一版件套口径（一百九十二件套（一百九十一件套清除））', s2143.includes('一百九十三件套（一百九十二件套清除）'));
