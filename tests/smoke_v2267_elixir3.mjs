...
ok('data.js GAME_VERSION 字面量已为 v22.67（旧 v22.66 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.80';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "66';"));
...
ok('ACH_LIST 精确总数 51 项（v2229 精确计数 pin 随新现实更新 50→51）', ACH_LIST.length === 73, String(ACH_LIST.length));
...
ok('README 件套口径为二百一十五件套（二百一十四件套清除）且旧 162 口径零残留',
  readme.includes('冒烟二百一十五件套（二百一十四件套清除）') && !readme.includes('冒烟一百六十二件套（一百六十一件套清' + '除）'));
...
ok('package.json test 串共 163 件套', testChain === 215, String(testChain));
ok('CHANGELOG 顶为 v22.67 条目', changelog.startsWith('## v23.80 '));
...
ok('smoke_v2266 的 GAME_VERSION 字面量 pin 已更新为 v22.67', s2266.includes("const GAME_VERSION = 'v23.80';"));
ok('smoke_v2266 的 README 件套 pin 已随新现实更新为二百一十五件套（二百一十四件套清除）',
  s2266.includes('二百一十五件套（二百一十四件套清除）'));
ok('smoke_v2266 的 package.json 件套计数 pin 已更新为 === 163', s2266.includes('testChain === 215'));
ok('smoke_v2266 的 CHANGELOG 顶 pin 已更新为 ## v22.67', s2266.includes("startsWith('## v23.80'") || s2266.includes("startsWith('## v22.78 ')"));
...
ok('smoke_v2266 的 README 成就 pin 已随新现实更新为 51 项双处', s2266.includes('成就一览（全部 73 项进度') && s2266.includes('**73 项成就**'));
ok('smoke_v2266 的 ACH_LIST 精确计数 pin 已更新为 === 51', s2266.includes('ACH_LIST.length === 73'));
ok('smoke_v2265 的 GAME_VERSION 字面量 pin 已更新为 v22.67', s2265.includes("const GAME_VERSION = 'v23.80';"));
ok('smoke_v2265 的 README 件套 pin 已随新现实更新为二百一十五件套（二百一十四件套清除）',
  s2265.includes('二百一十五件套（二百一十四件套清除）'));
ok('smoke_v2265 的 package.json 件套计数 pin 已更新为 === 163', s2265.includes('testChain === 215'));
...
ok('smoke_v2264 的 GAME_VERSION 字面量 pin 已更新为 v22.67', s2264.includes("const GAME_VERSION = 'v23.80';"));
...
ok('smoke_v2263 的 GAME_VERSION 字面量 pin 已更新为 v22.67', s2263.includes("const GAME_VERSION = 'v23.80';"));
ok('smoke_v2263 的 README 件套 pin 已随新现实更新为二百一十五件套（二百一十四件套清除）',
  s2263.includes('二百一十五件套（二百一十四件套清除）'));
ok('smoke_v2262 的 GAME_VERSION 字面量 pin 已更新为 v22.67', s2262.includes("const GAME_VERSION = 'v23.80';"));
ok('smoke_v2261 的 GAME_VERSION 字面量 pin 已更新为 v22.67', s2261.includes("const GAME_VERSION = 'v23.80';"));
...
ok('smoke_v2259 的 GAME_VERSION 字面量 pin 已更新为 v22.67', s2259.includes("const GAME_VERSION = 'v23.80';"));
ok('smoke_v2258 的 GAME_VERSION 字面量 pin 已更新为 v22.67', s2258.includes("const GAME_VERSION = 'v23.80';"));
ok('smoke_v2229 的 ACH_LIST 精确总数 pin 已更新为 === 51（50→51）',
  s2229.includes('ACH_LIST.length === 73') && !s2229.includes('ACH_LIST.length === 5' + '0'));
...
ok('v2143-45「件套守护领先一位」哨兵链已推进至 164（二百一十五件套（二百一十四件套清除））',
