...
ok('data.js GAME_VERSION 字面量为 v22.79', dataSrc.includes("const GAME_VERSION = 'v23.11'"));
...
ok('README 件套口径为二百零七件套（二百零六件套清除）且旧 174 口径零残留',
  readme.includes('冒烟二百零七件套（二百零六件套清除）') && !readme.includes('冒烟一百七十四件套（一百七十三件套清' + '除）'));
...
ok('package.json test 串共 175 件套', testChain === 207, String(testChain));
ok('CHANGELOG 含 v22.79 条目（顶 pin）', changelog.startsWith('## v23.11 '));
...
ok('smoke_v2278 的 GAME_VERSION 字面量 pin 已更新为 v22.79', s2278.includes("const GAME_VERSION = 'v23.11';"));
ok('smoke_v2278 的 CHANGELOG 顶 pin 已更新为 ## v22.79', s2278.includes("startsWith('## v23.11'"));
ok('smoke_v2278 的件套 pin 已更新为二百零七件套（二百零六件套清除）', s2278.includes('二百零七件套（二百零六件套清除）'));
