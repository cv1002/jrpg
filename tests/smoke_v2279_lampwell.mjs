...
ok('data.js 潮灯镇行 r[2] 含「广场大灯·村井」指针（v22.80 后接粮田派生）',
  dataSrc.includes("'水塘灯影 · ' + NPCS.granny.name + '/' + NPCS.lampboat.name + ' · 广场大灯·村井 · 粮田（'"));
...
ok('README 件套口径为二百二十一件套（二百二十件套清除）且旧 174 口径零残留',
...
ok('package.json test 串共 175 件套', testChain === 221, String(testChain));
ok('CHANGELOG 含 v22.79 条目（顶 pin）', changelog.startsWith('## v23.97 '));
...
ok('smoke_v2278 的 GAME_VERSION 字面量 pin 已更新为 v22.79', s2278.includes("const GAME_VERSION = 'v23.97';"));
ok('smoke_v2278 的 CHANGELOG 顶 pin 已更新为 ## v22.79', s2278.includes("startsWith('## v23.97'"));
ok('smoke_v2278 的件套 pin 已更新为二百二十一件套（二百二十件套清除）', s2278.includes('二百二十一件套（二百二十件套清除）'));
