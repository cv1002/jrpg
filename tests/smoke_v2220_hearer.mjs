...
ok('GAME_VERSION 字面量已为 v22.20（旧 v22.19 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.20';") && !dSrc.includes("const GAME_VERSION = 'v22." + "19';"));
...
ok('NPC_SPOTS 总数 22（既有 21 键 + 听矿人 1 键，v22.20 随新现实更新）', Object.keys(NPC_SPOTS).length === 22, Object.keys(NPC_SPOTS).length);
...
ok('README tests 树收录 smoke_v2220_hearer 且位于串尾', readme.includes('smoke_v2219_footkeys + smoke_v2220_hearer（npm test 串跑）'));
ok('README tests 树尾链完整（v2218_mush2 未被新尾吞并，全链连到 smoke_v2220_hearer）',
  readme.includes('smoke_v2217_elixir2 + smoke_v2218_mush2 + smoke_v2219_footkeys + smoke_v2220_hearer（npm test 串跑）'));
ok('README 件套口径为一百一十六件套（一百一十五件套清除）',
  readme.includes('冒烟一百一十六件套（一百一十五件套清除）') && !readme.includes('冒烟一百一十五件套（一百一十四件套清' + '除）'));
...
ok('package.json test 串共 116 件套', testChain === 116, String(testChain));
...
ok('smoke_v2219 的 GAME_VERSION 字面量 pin 已更新为 v22.20', s2219.includes("const GAME_VERSION = 'v22.20';"));
ok('smoke_v2219 的 GAME_VERSION 恒等 pin 已更新为 === v22.20', s2219.includes("GAME_VERSION === 'v22.20'"));
ok('smoke_v2219 的 README 件套 pin 已随新现实更新为一百一十六件套（一百一十五件套清除）',
  s2219.includes('一百一十六件套（一百一十五件套清除）'));
ok('smoke_v2219 的 README 树尾 pin 已更新为 + smoke_v2220_hearer',
  s2219.includes('smoke_v2218_mush2 + smoke_v2219_footkeys + smoke_v2220_hearer（npm test 串跑）'));
ok('smoke_v2219 的 package.json 件套计数 pin 已更新为 === 116', s2219.includes('testChain === 116'));
ok('smoke_v2218 的 GAME_VERSION 字面量 pin 已更新为 v22.20', s2218.includes("const GAME_VERSION = 'v22.20';"));
ok('smoke_v2218 的 README 件套 pin 已随新现实更新为一百一十六件套（一百一十五件套清除）',
  s2218.includes('一百一十六件套（一百一十五件套清除）'));
ok('smoke_v2218 的 README 树尾 pin 已更新为 + smoke_v2220_hearer',
  s2218.includes('smoke_v2218_mush2 + smoke_v2219_footkeys + smoke_v2220_hearer（npm test 串跑）'));
ok('smoke_v2217 的 GAME_VERSION 字面量 pin 已更新为 v22.20', s2217.includes("const GAME_VERSION = 'v22.20';"));
ok('smoke_v2217 的 package.json 件套计数 pin 已更新为 === 116', s2217.includes('testChain === 116'));
ok('smoke_v2216 的 README 件套 pin 已随新现实更新为一百一十六件套（一百一十五件套清除）',
  s2216.includes('一百一十六件套（一百一十五件套清除）'));
ok('smoke_v2216 的 README 树尾 pin 已更新为 + smoke_v2220_hearer',
  s2216.includes('smoke_v2215_tutorvol + smoke_v2216_picker + smoke_v2217_elixir2 + smoke_v2218_mush2 + smoke_v2219_footkeys + smoke_v2220_hearer（npm test 串跑）'));
ok('smoke_v2216 的 GAME_VERSION 字面量 pin 已更新为 v22.20', s2216.includes("const GAME_VERSION = 'v22.20';"));
ok('smoke_v2216 的 NPC 总数 pin 已随新现实更新为 22（听矿人落位）', s2216.includes('总数 22'));
ok('smoke_v2213 的 NPC 总数 pin 已随新现实更新为 22（听矿人落位）', s2213.includes('总数 22'));
ok('smoke_v2211 的 NPC 总数 pin 已随新现实更新为 22（听矿人落位）', s2211.includes('总数 22'));
ok('smoke_v2215 的 package.json 串尾 pin 已更新为 + smoke_v2220_hearer',
  s2215.includes('smoke_v2219_footkeys.mjs && node tests/smoke_v2220_hearer.mjs"'));
...
for (const f of fs.readdirSync(new URL('../tests', import.meta.url))) {
  if (!/^smoke_.*\.mjs$/.test(f)) continue;
  const src = fs.readFileSync(path.join(ROOT, 'tests', f), 'utf8');
  if (src.includes('smoke_v2218_mush2 + smoke_v2220_hearer（npm test 串' + '跑）')) brokenTail.push(f);
}
if (readme.includes('smoke_v2218_mush2 + smoke_v2220_hearer（npm test 串' + '跑）')) brokenTail.push('README.md');
