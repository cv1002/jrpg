...
ok('GAME_VERSION 字面量已为 v22.20（旧 v22.19 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.26';") && !dSrc.includes("const GAME_VERSION = 'v22." + "19';"));
...
ok('NPC_SPOTS 总数 27（既有 22 键 + 失名的旅人 1 键，v22.21 随新现实更新）', Object.keys(NPC_SPOTS).length === 27, Object.keys(NPC_SPOTS).length);
...
ok('README tests 树收录 smoke_v2220_hearer 且位于串尾', readme.includes('smoke_v2219_footkeys + smoke_v2220_hearer + smoke_v2221_wanderer + smoke_v2222_peddler + smoke_v2223_lampman + smoke_v2224_sifter + smoke_v2225_stonecarver + smoke_v2226_chestprogress（npm test 串跑）'));
ok('README tests 树尾链完整（v2218_mush2 未被新尾吞并，全链连到 smoke_v2220_hearer）',
  readme.includes('smoke_v2217_elixir2 + smoke_v2218_mush2 + smoke_v2219_footkeys + smoke_v2220_hearer + smoke_v2221_wanderer + smoke_v2222_peddler + smoke_v2223_lampman + smoke_v2224_sifter + smoke_v2225_stonecarver + smoke_v2226_chestprogress（npm test 串跑）'));
ok('README 件套口径为一百二十二件套（一百二十一件套清除）',
  readme.includes('冒烟一百二十二件套（一百二十一件套清除）') && !readme.includes('冒烟一百一十五件套（一百一十四件套清' + '除）'));
...
ok('package.json test 串共 116 件套', testChain === 122, String(testChain));
...
ok('smoke_v2219 的 GAME_VERSION 字面量 pin 已更新为 v22.20', s2219.includes("const GAME_VERSION = 'v22.26';"));
ok('smoke_v2219 的 GAME_VERSION 恒等 pin 已更新为 === v22.20', s2219.includes("GAME_VERSION === 'v22.26'"));
...
ok('smoke_v2219 的 README 树尾 pin 已更新为 + smoke_v2220_hearer',
  s2219.includes('smoke_v2218_mush2 + smoke_v2219_footkeys + smoke_v2220_hearer + smoke_v2221_wanderer + smoke_v2222_peddler + smoke_v2223_lampman + smoke_v2224_sifter + smoke_v2225_stonecarver + smoke_v2226_chestprogress（npm test 串跑）'));
ok('smoke_v2219 的 package.json 件套计数 pin 已更新为 === 116', s2219.includes('testChain === 122'));
ok('smoke_v2218 的 GAME_VERSION 字面量 pin 已更新为 v22.20', s2218.includes("const GAME_VERSION = 'v22.26';"));
ok('smoke_v2218 的 README 件套 pin 已随新现实更新为一百二十二件套（一百二十一件套清除）',
  s2218.includes('一百二十二件套（一百二十一件套清除）'));
ok('smoke_v2218 的 README 树尾 pin 已更新为 + smoke_v2220_hearer',
  s2218.includes('smoke_v2218_mush2 + smoke_v2219_footkeys + smoke_v2220_hearer + smoke_v2221_wanderer + smoke_v2222_peddler + smoke_v2223_lampman + smoke_v2224_sifter + smoke_v2225_stonecarver + smoke_v2226_chestprogress（npm test 串跑）'));
ok('smoke_v2217 的 GAME_VERSION 字面量 pin 已更新为 v22.20', s2217.includes("const GAME_VERSION = 'v22.26';"));
ok('smoke_v2217 的 package.json 件套计数 pin 已更新为 === 116', s2217.includes('testChain === 122'));
ok('smoke_v2216 的 README 件套 pin 已随新现实更新为一百二十二件套（一百二十一件套清除）',
  s2216.includes('一百二十二件套（一百二十一件套清除）'));
ok('smoke_v2216 的 README 树尾 pin 已更新为 + smoke_v2220_hearer',
  s2216.includes('smoke_v2215_tutorvol + smoke_v2216_picker + smoke_v2217_elixir2 + smoke_v2218_mush2 + smoke_v2219_footkeys + smoke_v2220_hearer + smoke_v2221_wanderer + smoke_v2222_peddler + smoke_v2223_lampman + smoke_v2224_sifter + smoke_v2225_stonecarver + smoke_v2226_chestprogress（npm test 串跑）'));
ok('smoke_v2216 的 GAME_VERSION 字面量 pin 已更新为 v22.20', s2216.includes("const GAME_VERSION = 'v22.26';"));
ok('smoke_v2216 的 NPC 总数 pin 已随新现实更新为 22（听矿人落位）', s2216.includes('总数 27'));
ok('smoke_v2213 的 NPC 总数 pin 已随新现实更新为 22（听矿人落位）', s2213.includes('总数 27'));
ok('smoke_v2211 的 NPC 总数 pin 已随新现实更新为 22（听矿人落位）', s2211.includes('总数 27'));
ok('smoke_v2215 的 package.json 串尾 pin 已更新为 + smoke_v2220_hearer',
  s2215.includes('smoke_v2221_wanderer.mjs && node tests/smoke_v2222_peddler.mjs && node tests/smoke_v2223_lampman.mjs && node tests/smoke_v2224_sifter.mjs && node tests/smoke_v2225_stonecarver.mjs && node tests/smoke_v2226_chestprogress.mjs"'));
