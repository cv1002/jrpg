...
// README/package.json/CHANGELOG 同步（冒烟一百九十九件套（一百九十八件套清除）/串尾/入库 197 份/顶 pin）+
...
ok('data.js GAME_VERSION 字面量已为 v23.01（旧 v23.00 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.03';") && !dataSrc.includes("const GAME_VERSION = 'v23." + "00';"));
...
ok('package.json 串尾为 ... smoke_v2299_crosslink.mjs && node tests/smoke_v2300_sidemore.mjs && node tests/smoke_v2301_eco.mjs && node tests/smoke_v2302_cmdprev.mjs && node tests/smoke_v2303_rushnum.mjs"',
...
ok('package.json test 串共 197 件套', testChain === 199, String(testChain));
ok('CHANGELOG 顶部已追加 v23.01 条目', changelog.startsWith('## v23.03 '));
...
ok('smoke_v2300 的 GAME_VERSION 字面量 pin 已更新为 v23.01', s2300.includes("const GAME_VERSION = 'v23.03';"));
ok('smoke_v2300 的 CHANGELOG 顶 pin 已更新为 ## v23.01',
  s2300.includes("startsWith('## v23.03 '"));
ok('smoke_v2300 的件套 pin 已更新为一百九十九件套（一百九十八件套清除）',
  s2300.includes('一百九十九件套（一百九十八件套清除）'));
ok('smoke_v2300 的 README 串尾 pin 已延伸至 smoke_v2301_eco',
  s2300.includes('smoke_v2299_crosslink + smoke_v2300_sidemore + smoke_v2301_eco + smoke_v2302_cmdprev + smoke_v2303_rushnum（npm test 串跑）'));
ok('smoke_v2300 的 package 串尾 pin 已延伸至 smoke_v2301_eco',
  s2300.includes('node tests/smoke_v2299_crosslink.mjs && node tests/smoke_v2300_sidemore.mjs && node tests/smoke_v2301_eco.mjs && node tests/smoke_v2302_cmdprev.mjs && node tests/smoke_v2303_rushnum.mjs"'));
ok('smoke_v2300 的 testChain pin 已更新为 197', s2300.includes('testChain === 199'));
