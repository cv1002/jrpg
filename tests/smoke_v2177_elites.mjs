...
ok('GAME_VERSION 格式合法且已越过 v21.76', !!_gv && (_gv[0] > 21 || (_gv[0] === 21 && _gv[1] >= 99)), GAME_VERSION);
...
ok('灯火同心（allquests）支线分母 v23.57 随新现实更新为 0/11（side_grain/side_tree/side_wolf/side_snake 是支线，elites 不是）',
  achAllQuests && achAllQuests.prog({ quests: {} }) === '0/11', achAllQuests && achAllQuests.prog({ quests: {} }));
ok('QUESTS side 支线 11 条（v21.80 side_grain 入列、v23.32 side_tree 入列、v23.42 side_wolf 入列、v23.57 side_snake 入列，随新现实更新）',
  Object.values(QUESTS).filter((q) => q.kind === 'side').length === 11);
...
ok('README 件套口径为二百一十二件套（二百一十一件套清除）', readme.includes('二百一十二件套（二百一十一件套清除）'));
...
ok('smoke_v2176 的 README 件套口径断言已随新现实更新（七十五件套 pin 零残留，七十六件套落位）',
  s2176.includes('二百一十二件套（二百一十一件套清除）') && !s2176.includes('七十五件套（七十四件套清除）'));
ok('smoke_v2176 的 README 成就 pin 已随新现实更新（28 项 pin 零残留，31 项双处落位）',
  s2176.includes("readme.includes('成就一览（全部 64 项进度'") && !s2176.includes('全部 28 项进度'));
...
ok('smoke_v2168 的 README 成就 pin 已随新现实更新（28 项 pin 零残留，31 项双处落位）',
  s2168.includes("readme.includes('成就一览（全部 64 项进度'") && !s2168.includes('全部 28 项进度'));
ok('smoke_v2168 对 smoke_v2159 的 README pin 复查已随新现实更新（31 项双处落位）',
  s2168.includes("s2159.includes(\"readme.includes('成就一览（全部 64 项进度')\")"));
...
ok('smoke_v2159 的 README 成就 pin 已随新现实更新（28 项 pin 零残留，31 项双处落位）',
  s2159.includes("readme.includes('成就一览（全部 64 项进度'") && !s2159.includes('全部 28 项进度'));
