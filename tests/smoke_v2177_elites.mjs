...
ok('GAME_VERSION 格式合法且已越过 v21.76', !!_gv && (_gv[0] > 21 || (_gv[0] === 21 && _gv[1] >= 90)), GAME_VERSION);
...
ok('smoke_v2176 的 README 件套口径断言已随新现实更新（七十五件套 pin 零残留，七十六件套落位）',
  s2176.includes('一百八十六件套（一百八十五件套清除）') && !s2176.includes('七十五件套（七十四件套清除）'));
...
ok('smoke_v2168 对 smoke_v2159 的 README pin 复查已随新现实更新（31 项双处落位）',
  s2168.includes("s2159.includes(\"readme.includes('成就一览（全部 58 项进度')\")"));
