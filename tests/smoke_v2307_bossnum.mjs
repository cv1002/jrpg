...
ok('data.js GAME_VERSION 字面量已为 v23.07（旧 v23.06 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.07';") && !dataSrc.includes("const GAME_VERSION = 'v23." + "06';"));
...
ok('README 件套口径为二百零三件套（二百零二件套清除）且旧 202 口径零残留',
  readme.includes('冒烟二百零三件套（二百零二件套清除）') && !readme.includes('冒烟二百零二件套（二百零一件套清' + '除）'));
...
ok('package.json test 串共 203 件套', testChain === 203, String(testChain));
ok('CHANGELOG 顶部已追加 v23.07 条目', changelog.startsWith('## v23.07 '));
...
ok('smoke_v2306 的 GAME_VERSION 字面量 pin 已更新为 v23.07', s2306.includes("const GAME_VERSION = 'v23.07';"));
ok('smoke_v2306 的 CHANGELOG 顶 pin 已更新为 ## v23.07',
  s2306.includes("startsWith('## v23.07 "));
ok('smoke_v2306 的件套 pin 已更新为二百零三件套（二百零二件套清除）',
  s2306.includes('二百零三件套（二百零二件套清除）'));
ok('smoke_v2306 的 README 串尾 pin 已延伸至 smoke_v2307_bossnum',
  s2306.includes('smoke_v2305_monnum + smoke_v2306_skillnum + smoke_v2307_bossnum（npm test 串跑）'));
ok('smoke_v2306 的 package 串尾 pin 已延伸至 smoke_v2307_bossnum',
  s2306.includes('node tests/smoke_v2305_monnum.mjs && node tests/smoke_v2306_skillnum.mjs && node tests/smoke_v2307_bossnum.mjs"'));
ok('smoke_v2306 的 testChain pin 已更新为 203', s2306.includes('testChain === 203'));
