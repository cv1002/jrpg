...
ok('GAME_VERSION 字面量已为 v22.51（旧 v22.50 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.57';") && !dSrc.includes("const GAME_VERSION = 'v22." + "49';"));
...
ok('README tests 树已收录 smoke_v2250_brewer 且位于串尾', readme.includes('smoke_v2249_shopkeep + smoke_v2250_brewer + smoke_v2251_oathkeep + smoke_v2252_rail + smoke_v2253_supplypoint + smoke_v2254_grainfield + smoke_v2255_steleglow + smoke_v2256_campfire + smoke_v2257_pondglow（npm test 串跑）'));
ok('README 件套口径为一百五十三件套（一百五十二件套清除）',
  readme.includes('冒烟一百五十三件套（一百五十二件套清除）') && !readme.includes('冒烟一百四十五件套（一百四十四件套清' + '除）'));
...
ok('package.json test 串共 147 件套', testChain === 153, String(testChain));
ok('CHANGELOG 顶部已追加 v22.50 条目', changelog.startsWith('## v22.57'));
...
ok('smoke_v2249 的 GAME_VERSION 字面量 pin 已更新为 v22.50（旧 v22.49 零残留）',
  s2249.includes("const GAME_VERSION = 'v22.57';") && !s2249.includes("const GAME_VERSION = 'v22." + "49';"));
ok('smoke_v2249 的 README 件套 pin 已随新现实更新为一百五十三件套（一百五十二件套清除）',
  s2249.includes('一百五十三件套（一百五十二件套清除）'));
ok('smoke_v2249 的 package.json 件套计数 pin 已更新为 === 146', s2249.includes('testChain === 153'));
ok('smoke_v2249 的 README 串尾 pin 已随新现实延伸至 smoke_v2251_oathkeep',
  s2249.includes('smoke_v2249_shopkeep + smoke_v2250_brewer + smoke_v2251_oathkeep + smoke_v2252_rail + smoke_v2253_supplypoint + smoke_v2254_grainfield + smoke_v2255_steleglow + smoke_v2256_campfire + smoke_v2257_pondglow（npm test 串跑）'));
...
ok('v2143-45「件套守护领先一位」哨兵链已推进至 148（一百五十三件套（一百五十二件套清除））',
