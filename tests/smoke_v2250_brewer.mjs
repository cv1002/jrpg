...
ok('GAME_VERSION 字面量已为 v22.51（旧 v22.50 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.92';") && !dSrc.includes("const GAME_VERSION = 'v22." + "49';"));
...
ok('README 件套口径为二百一十六件套（二百一十五件套清除）',
  readme.includes('冒烟二百一十六件套（二百一十五件套清除）') && !readme.includes('冒烟一百四十五件套（一百四十四件套清' + '除）'));
...
ok('package.json test 串共 147 件套', testChain === 216, String(testChain));
ok('CHANGELOG 顶部已追加 v22.50 条目', changelog.startsWith('## v23.92'));
...
ok('smoke_v2249 的 GAME_VERSION 字面量 pin 已更新为 v22.50（旧 v22.49 零残留）',
  s2249.includes("const GAME_VERSION = 'v23.92';") && !s2249.includes("const GAME_VERSION = 'v22." + "49';"));
ok('smoke_v2249 的 README 件套 pin 已随新现实更新为二百一十六件套（二百一十五件套清除）',
  s2249.includes('二百一十六件套（二百一十五件套清除）'));
ok('smoke_v2249 的 package.json 件套计数 pin 已更新为 === 146', s2249.includes('testChain === 216'));
...
ok('v2143-45「件套守护领先一位」哨兵链已推进至 148（二百一十六件套（二百一十五件套清除））',
