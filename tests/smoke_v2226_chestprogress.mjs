...
ok('GAME_VERSION 字面量已为 v22.26（旧 v22.25 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.78';") && !dSrc.includes("const GAME_VERSION = 'v22." + "25';"));
...
ok('README 件套口径为二百一十四件套（二百一十三件套清除）',
  readme.includes('冒烟二百一十四件套（二百一十三件套清除）') && !readme.includes('冒烟一百二十一件套（一百二十件套清' + '除）'));
...
ok('package.json test 串共 122 件套', testChain === 214, String(testChain));
...
ok('姊妹件套（v2225..v2215）GAME_VERSION 字面量 pin 已更新为 v22.26',
  sibNames.every((k) => sib[k].includes("const GAME_VERSION = 'v23.78';")));
ok('姊妹件套（v2225..v2215）GAME_VERSION 恒等 pin 已更新为 === v22.26',
  sibNames.every((k) => sib[k].includes("GAME_VERSION === 'v23.78'")));
ok('姊妹件套（v2225..v2215）README 件套 pin 已更新为二百一十四件套（二百一十三件套清除）',
  sibNames.every((k) => sib[k].includes('二百一十四件套（二百一十三件套清除）')));
...
ok('姊妹件套（v2225..v2215）package.json 件套计数 pin 已更新为 === 122',
  sibNames.every((k) => sib[k].includes('testChain === 214')));
