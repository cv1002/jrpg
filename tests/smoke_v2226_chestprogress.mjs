...
ok('GAME_VERSION 字面量已为 v22.26（旧 v22.25 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.30';") && !dSrc.includes("const GAME_VERSION = 'v22." + "25';"));
...
ok('README tests 树收录 smoke_v2226_chestprogress 且位于串尾', readme.includes('smoke_v2225_stonecarver + smoke_v2226_chestprogress + smoke_v2227_minstrel + smoke_v2228_titlesave + smoke_v2229_metall + smoke_v2230_pausewarn（npm test 串跑）'));
ok('README 件套口径为一百二十六件套（一百二十五件套清除）',
  readme.includes('冒烟一百二十六件套（一百二十五件套清除）') && !readme.includes('冒烟一百二十一件套（一百二十件套清' + '除）'));
...
ok('package.json test 串共 122 件套', testChain === 126, String(testChain));
...
ok('姊妹件套（v2225..v2215）GAME_VERSION 字面量 pin 已更新为 v22.26',
  sibNames.every((k) => sib[k].includes("const GAME_VERSION = 'v22.30';")));
ok('姊妹件套（v2225..v2215）GAME_VERSION 恒等 pin 已更新为 === v22.26',
  sibNames.every((k) => sib[k].includes("GAME_VERSION === 'v22.30'")));
ok('姊妹件套（v2225..v2215）README 件套 pin 已更新为一百二十六件套（一百二十五件套清除）',
  sibNames.every((k) => sib[k].includes('一百二十六件套（一百二十五件套清除）')));
ok('姊妹件套（v2225..v2215）README 树尾 pin 已更新为 + smoke_v2226_chestprogress',
  sibNames.every((k) => sib[k].includes('smoke_v2225_stonecarver + smoke_v2226_chestprogress + smoke_v2227_minstrel + smoke_v2228_titlesave + smoke_v2229_metall + smoke_v2230_pausewarn（npm test 串跑）')));
ok('姊妹件套（v2225..v2215）package.json 件套计数 pin 已更新为 === 122',
  sibNames.every((k) => sib[k].includes('testChain === 126')));
ok('串尾 pin（v2215/v2220..v2225）已更新为 + smoke_v2226_chestprogress',
  ['v2215_tutorvol', 'v2220_hearer', 'v2221_wanderer', 'v2222_peddler', 'v2223_lampman', 'v2224_sifter', 'v2225_stonecarver']
    .every((k) => sib[k].includes('smoke_v2225_stonecarver.mjs && node tests/smoke_v2226_chestprogress.mjs && node tests/smoke_v2227_minstrel.mjs && node tests/smoke_v2228_titlesave.mjs && node tests/smoke_v2229_metall.mjs && node tests/smoke_v2230_pausewarn.mjs"')));
