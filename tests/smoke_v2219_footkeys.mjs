...
// GAME_VERSION v22.19 / v2218..v2192 恒等 v22.19 / v2218..v2192 树尾 pin / package testChain === 216）、
...
ok('GAME_VERSION 字面量已为 v22.19（本版独占精确锚点）', GAME_VERSION === 'v23.92', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.19（旧 v22.18 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.92';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "18';"));
...
ok('页脚既有项逐字保留（Esc/P/F/I/J/B/C/T/M/H/1-6/结局句）',
  ['<kbd>Esc</kbd>菜单', '<kbd>P</kbd>存档', '<kbd>F</kbd>喝药（优先灵药·普通 50%HP+8 · 灵药 80%HP+20并回40%MP）', '<kbd>I</kbd>状态', '<kbd>J</kbd>任务',
   '<kbd>B</kbd>图鉴', '<kbd>C</kbd>成就', '<kbd>T</kbd>旅行（已到访图瞬移 · 推荐等级/补给标注）', '<kbd>M</kbd>静音', '<kbd>H</kbd>操作说明',
...
ok('README 件套口径为二百一十六件套（二百一十五件套清除）',
  readme.includes('冒烟二百一十六件套（二百一十五件套清除）') && !readme.includes('冒烟一百一十四件套（一百一十三件套清' + '除）'));
...
ok('package.json test 串共 115 件套', testChain === 216, String(testChain));
...
ok('smoke_v2218 的 GAME_VERSION 字面量 pin 已更新为 v22.19', s2218.includes("const GAME_VERSION = 'v23.92';"));
ok('smoke_v2218 的 GAME_VERSION 恒等 pin 已更新为 === v22.19', s2218.includes("GAME_VERSION === 'v23.92'"));
ok('smoke_v2218 的 package.json 件套计数 pin 已更新为 === 115', s2218.includes('testChain === 216'));
ok('smoke_v2217 的 GAME_VERSION 字面量 pin 已更新为 v22.19', s2217.includes("const GAME_VERSION = 'v23.92';"));
ok('smoke_v2217 的 package.json 件套计数 pin 已更新为 === 115', s2217.includes('testChain === 216'));
