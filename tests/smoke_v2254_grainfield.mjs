...
ok('GAME_VERSION 字面量已为 v22.54（旧 v22.53 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.55';") && !dSrc.includes("const GAME_VERSION = 'v22." + "53';"));
...
ok('README tests 树收录 smoke_v2254_grainfield 且位于串尾', readme.includes('smoke_v2253_supplypoint + smoke_v2254_grainfield + smoke_v2255_steleglow（npm test 串跑）'));
ok('README 件套口径为一百五十一件套（一百五十件套清除）',
  readme.includes('冒烟一百五十一件套（一百五十件套清除）') && !readme.includes('冒烟一百四十三件套（一百四十二件套清' + '除）'));
...
ok('package.json test 串共 150 件套', testChain === 151, String(testChain));
...
ok('smoke_v2253 的 GAME_VERSION 字面量 pin 已更新为 v22.54', s2253.includes("const GAME_VERSION = 'v22.55';"));
...
ok('smoke_v2253 的 README 件套 pin 已随新现实更新为一百五十一件套（一百五十件套清除）',
  s2253.includes('一百五十一件套（一百五十件套清除）'));
ok('smoke_v2253 的 package.json 件套计数 pin 已更新为 === 150', s2253.includes('testChain === 151'));
ok('smoke_v2253 的 CHANGELOG 顶 pin 已更新为 ## v22.54', s2253.includes("startsWith('## v22.55')"));
ok('smoke_v2252 的 GAME_VERSION 字面量 pin 已更新为 v22.54', s2252.includes("const GAME_VERSION = 'v22.55';"));
...
const archChain = (s) => /smoke_v2253_supplypoint[.\\]{0,4}mjs\s*&&\s*node tests[\\/]{0,4}smoke_v2254_grainfield[.\\]{0,4}mjs[\s\S]*?node tests[\\/]{0,4}smoke_v2255_steleglow[.\\]{0,4}mjs/.test(s);
...
const looseChain = (s) => /smoke_v2253_supplypoint[.\\]{0,6}mjs[\s\S]*?node tests[\\/]{0,6}smoke_v2254_grainfield[.\\]{0,6}mjs[\s\S]*?node tests[\\/]{0,6}smoke_v2255_steleglow[.\\]{0,6}mjs/.test(s);
...
// 坏链防回归：README 不得出现「smoke_v2253_supplypoint + smoke_v2254_grainfield + smoke_v2255_steleglow（npm test 串跑）」孤尾
