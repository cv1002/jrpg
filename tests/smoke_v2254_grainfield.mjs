...
ok('GAME_VERSION 字面量已为 v22.54（旧 v22.53 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.54';") && !dSrc.includes("const GAME_VERSION = 'v22." + "53';"));
...
ok('README tests 树收录 smoke_v2254_grainfield 且位于串尾', readme.includes('smoke_v2253_supplypoint + smoke_v2254_grainfield（npm test 串跑）'));
...
ok('package.json test 串共 150 件套', testChain === 150, String(testChain));
ok('CHANGELOG 含 v22.54 条目', changelog.includes('## v22.54 '));
...
ok('smoke_v2253 的 GAME_VERSION 字面量 pin 已更新为 v22.54', s2253.includes("const GAME_VERSION = 'v22.54';"));
ok('smoke_v2253 的 README 串尾 pin 已随新现实延伸至 smoke_v2254_grainfield',
  s2253.includes('smoke_v2252_rail + smoke_v2253_supplypoint + smoke_v2254_grainfield（npm test 串跑）'));
ok('smoke_v2253 的 README 件套 pin 已随新现实更新为一百五十件套（一百四十九件套清除）',
  s2253.includes('一百五十件套（一百四十九件套清除）'));
ok('smoke_v2253 的 package.json 件套计数 pin 已更新为 === 150', s2253.includes('testChain === 150'));
ok('smoke_v2253 的 CHANGELOG 顶 pin 已更新为 ## v22.54', s2253.includes("startsWith('## v22.54')"));
ok('smoke_v2252 的 GAME_VERSION 字面量 pin 已更新为 v22.54', s2252.includes("const GAME_VERSION = 'v22.54';"));
ok('smoke_v2252 的 README 串尾 pin 已延伸至 smoke_v2254_grainfield',
  s2252.includes('smoke_v2251_oathkeep + smoke_v2252_rail + smoke_v2253_supplypoint + smoke_v2254_grainfield（npm test 串跑）'));
ok('smoke_v2242 的 README 串尾 pin 已延伸至 smoke_v2254_grainfield',
  s2242.includes('smoke_v2252_rail + smoke_v2253_supplypoint + smoke_v2254_grainfield（npm test 串跑）'));
// 串尾锚延伸（任意转义风格：plain `/` / regex `\/` / 双转义 `\\/`）
const archChain = (s) => /smoke_v2253_supplypoint[.\\]{0,4}mjs\s*&&\s*node tests[\\/]{0,4}smoke_v2254_grainfield[.\\]{0,4}mjs/.test(s);
...
const looseChain = (s) => /smoke_v2253_supplypoint[.\\]{0,6}mjs[\s\S]*?node tests[\\/]{0,6}smoke_v2254_grainfield/.test(s);
ok('smoke_v2239/smoke_v2240 的容错正则尾锚已延伸至 smoke_v2254_grainfield',
  looseChain(s2239) && looseChain(s2240));
ok('v2143-45「件套守护领先一位」哨兵链已推进至 151（一百五十一件套（一百五十件套清除））',
...
ok('smoke_v2228 的 GAME_VERSION 字面量 pin 已更新为 v22.54 且旧代零残留复查仍在（v22.27 风格）',
  s2228.includes("const GAME_VERSION = 'v22.54';") && s2228.includes("'v22." ) && s2228.includes("27';"));
...
// 坏链防回归：README 不得出现「smoke_v2253_supplypoint + smoke_v2254_grainfield（npm test 串跑）」孤尾
