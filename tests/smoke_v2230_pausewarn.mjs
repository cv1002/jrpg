...
ok('GAME_VERSION 字面量已为 v22.30（旧 v22.29 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.97';") && !dSrc.includes("const GAME_VERSION = 'v22." + "29';"));
...
// —— 运行期：drawPause 真实渲染捕获（unsaved 真 → 橙行含槽位占位；unsaved 假 → 零「未存档」）——
{
  ...
  try {
    ...
    ok('菜单头部槽号行零回归（守灯人 · 槽 1）', cap.some((t) => t.includes('守灯人  ·  槽 1')));
    cap.length = 0;
    S.unsaved = false;
    drawPause();
...
ok('README 件套口径为二百二十一件套（二百二十件套清除）',
  readme.includes('冒烟二百二十一件套（二百二十件套清除）') && !readme.includes('冒烟一百二十五件套（一百二十四件套清' + '除）'));
...
ok('package.json test 串共 126 件套', testChain === 221, String(testChain));
...
ok('smoke_v2229 的 GAME_VERSION 字面量 pin 已更新为 v22.30', s2229.includes("const GAME_VERSION = 'v23.97';"));
ok('smoke_v2229 的 GAME_VERSION 恒等 pin 已更新为 === v22.30', s2229.includes("GAME_VERSION === 'v23.97'"));
ok('smoke_v2229 的 README 件套 pin 已更新为二百二十一件套（二百二十件套清除）',
...
ok('smoke_v2229 的 package.json 件套计数 pin 已更新为 === 126', s2229.includes('testChain === 221'));
