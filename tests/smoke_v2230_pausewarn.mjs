...
ok('GAME_VERSION 字面量已为 v22.30（旧 v22.29 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.31';") && !dSrc.includes("const GAME_VERSION = 'v22." + "29';"));
...
ok('README tests 树收录 smoke_v2230_pausewarn 且位于串尾',
  readme.includes('smoke_v2229_metall + smoke_v2230_pausewarn + smoke_v2231_smith（npm test 串跑）'));
ok('README 件套口径为一百二十七件套（一百二十六件套清除）',
  readme.includes('冒烟一百二十七件套（一百二十六件套清除）') && !readme.includes('冒烟一百二十五件套（一百二十四件套清' + '除）'));
...
ok('package.json 已收录 smoke_v2230_pausewarn（npm test 串跑第 126 份）',
  pkg.includes('smoke_v2230_pausewarn.mjs') && /smoke_v2229_metall\.mjs && node tests\/smoke_v2230_pausewarn\.mjs && node tests\/smoke_v2231_smith\.mjs"/.test(pkg));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 126 件套', testChain === 127, String(testChain));
...
ok('smoke_v2229 的 GAME_VERSION 字面量 pin 已更新为 v22.30', s2229.includes("const GAME_VERSION = 'v22.31';"));
ok('smoke_v2229 的 GAME_VERSION 恒等 pin 已更新为 === v22.30', s2229.includes("GAME_VERSION === 'v22.31'"));
ok('smoke_v2229 的 README 件套 pin 已更新为一百二十七件套（一百二十六件套清除）',
...
ok('smoke_v2229 的 package.json 件套计数 pin 已更新为 === 126', s2229.includes('testChain === 127'));
ok('smoke_v2229 的 package.json 串尾 pin 已更新为 + smoke_v2231_smith',
  s2229.includes('node tests\\/smoke_v2230_pausewarn\\.mjs && node tests\\/smoke_v2231_smith\\.mjs"'));
ok('smoke_v2228 的 GAME_VERSION 字面量 pin 已更新为 v22.30', s2228.includes("const GAME_VERSION = 'v22.31';"));
...
ok('smoke_v2226 的 README 件套 pin 已更新为一百二十七件套（一百二十六件套清除）',
