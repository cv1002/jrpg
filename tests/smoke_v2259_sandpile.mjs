...
ok('GAME_VERSION 字面量已为 v22.60（旧 v22.58 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.60';") && !dSrc.includes("const GAME_VERSION = 'v22." + "58';"));
...
ok('README tests 树收录 smoke_v2259_sandpile 且位于串尾',
  readme.includes('smoke_v2258_potionhelp + smoke_v2259_sandpile + smoke_v2260_fountripple（npm test 串跑）'));
ok('README 件套口径为一百五十六件套（一百五十五件套清除）且旧 154 口径零残留',
  readme.includes('冒烟一百五十六件套（一百五十五件套清除）') && !readme.includes('冒烟一百五十五件套（一百五十四件套清' + '除）'));
...
ok('package.json 串尾为 smoke_v2259_sandpile.mjs && node tests/smoke_v2260_fountripple.mjs"',
  pkg.includes('smoke_v2259_sandpile.mjs && node tests/smoke_v2260_fountripple.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 155 件套', testChain === 156, String(testChain));
...
ok('smoke_v2258 的 GAME_VERSION 字面量 pin 已更新为 v22.59', s2258.includes("const GAME_VERSION = 'v22.60';"));
ok('smoke_v2258 的 README 串尾 pin 已随新现实延伸至 smoke_v2259_sandpile',
  s2258.includes('smoke_v2258_potionhelp + smoke_v2259_sandpile + smoke_v2260_fountripple（npm test 串跑）'));
ok('smoke_v2258 的 README 件套 pin 已随新现实更新为一百五十六件套（一百五十五件套清除）',
  s2258.includes('一百五十六件套（一百五十五件套清除）'));
ok('smoke_v2258 的 package.json 件套计数 pin 已更新为 === 155', s2258.includes('testChain === 156'));
ok('smoke_v2258 的 CHANGELOG 顶 pin 已更新为 ## v22.59', s2258.includes("startsWith('## v22.60')"));
ok('smoke_v2258 的 package.json 串尾 plain pin 已延伸至 smoke_v2259_sandpile',
  s2258.includes('smoke_v2259_sandpile.mjs && node tests/smoke_v2260_fountripple.mjs"'));
ok('smoke_v2257 的 GAME_VERSION 字面量 pin 已更新为 v22.59', s2257.includes("const GAME_VERSION = 'v22.60';"));
ok('smoke_v2257 的 README 串尾 pin 已延伸至 smoke_v2259_sandpile',
  s2257.includes('smoke_v2258_potionhelp + smoke_v2259_sandpile + smoke_v2260_fountripple（npm test 串跑）'));
ok('smoke_v2256 的 GAME_VERSION 字面量 pin 已更新为 v22.59', s2256.includes("const GAME_VERSION = 'v22.60';"));
ok('smoke_v2255/v2254/v2253/v2252 的 GAME_VERSION 字面量 pin 已更新为 v22.59',
  s2255.includes("const GAME_VERSION = 'v22.60';") && s2254.includes("const GAME_VERSION = 'v22.60';") &&
  s2253.includes("const GAME_VERSION = 'v22.60';") && s2252.includes("const GAME_VERSION = 'v22.60';"));
ok('smoke_v2230 的 package.json 串尾 plain pin 已延伸至 smoke_v2259_sandpile',
  s2230.includes('smoke_v2259_sandpile.mjs && node tests\\\\/smoke_v2260_fountripple.mjs"'));
// 串尾锚延伸（任意转义风格：plain `/` / regex `\/` / 双转义 `\\/`）
const archChain = (s) => /smoke_v2259_sandpile[.\\/]{0,4}mjs[\s\S]*?node tests[\\/]{0,4}smoke_v2260_fountripple[.\\/]{0,4}mjs/.test(s);
...
const looseChain = (s) => /smoke_v2259_sandpile[.\\/]{0,6}mjs[\s\S]*?node tests[\\/]{0,6}smoke_v2260_fountripple[.\\/]{0,6}mjs/.test(s);
ok('smoke_v2239/smoke_v2240 的容错正则尾锚已延伸至 smoke_v2259_sandpile',
  looseChain(s2239) && looseChain(s2240));
ok('v2143-45「件套守护领先一位」哨兵链已推进至 156（一百五十七件套（一百五十六件套清除））',
