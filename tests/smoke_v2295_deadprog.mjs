...
ok('data.js GAME_VERSION 字面量为 v22.95', dataSrc.includes("const GAME_VERSION = 'v22.98';"));
...
ok('GAME_VERSION 格式合法且已越过 v22.94（本版守 v22.95）', !!_gv && (_gv[0] > 22 || (_gv[0] === 22 && _gv[1] >= 98)), GAME_VERSION);
...
ok('README tests 树串尾已延伸至 smoke_v2295_deadprog（v2294 后接 v2295）',
  readme.includes('+ smoke_v2293_deadsave + smoke_v2294_crystalwatch + smoke_v2295_deadprog + smoke_v2296_endingprog + smoke_v2297_chestmid + smoke_v2298_encnum（npm test 串跑）'));
ok('README 旧串尾零残留（smoke_v2294_crystalwatch（npm test 串跑）不在树尾）',
  !readme.includes('smoke_v2294_crystalwatch（npm test 串跑）'));
ok('README 件套口径为一百九十四件套（一百九十三件套清除）且旧 190 口径零残留',
  readme.includes('冒烟一百九十四件套（一百九十三件套清除）') && !readme.includes('冒烟一百九十件套（一百八十九件套清' + '除）'));
...
ok('package.json 串尾为 ... smoke_v2294_crystalwatch.mjs && node tests/smoke_v2295_deadprog.mjs && node tests/smoke_v2296_endingprog.mjs && node tests/smoke_v2297_chestmid.mjs && node tests/smoke_v2298_encnum.mjs"',
  pkg.includes('node tests/smoke_v2294_crystalwatch.mjs && node tests/smoke_v2295_deadprog.mjs && node tests/smoke_v2296_endingprog.mjs && node tests/smoke_v2297_chestmid.mjs && node tests/smoke_v2298_encnum.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 191 件套', testChain === 194, String(testChain));
ok('CHANGELOG 顶部已追加 v22.95 条目', changelog.startsWith('## v22.98 '));
...
ok('smoke_v2294 的 GAME_VERSION 字面量 pin 已更新为 v22.95', s2294.includes("const GAME_VERSION = 'v22.98';"));
ok('smoke_v2294 的 CHANGELOG 顶 pin 已更新为 ## v22.95', s2294.includes("startsWith('## v22.98 '"));
ok('smoke_v2294 的件套 pin 已更新为一百九十四件套（一百九十三件套清除）', s2294.includes('一百九十四件套（一百九十三件套清除）'));
ok('smoke_v2294 的 README 串尾 pin 已延伸至 smoke_v2295_deadprog',
  s2294.includes('smoke_v2293_deadsave + smoke_v2294_crystalwatch + smoke_v2295_deadprog + smoke_v2296_endingprog + smoke_v2297_chestmid + smoke_v2298_encnum（npm test 串跑）'));
ok('smoke_v2294 的 package 串尾 pin 已延伸至 smoke_v2295_deadprog',
  s2294.includes('node tests/smoke_v2294_crystalwatch.mjs && node tests/smoke_v2295_deadprog.mjs && node tests/smoke_v2296_endingprog.mjs && node tests/smoke_v2297_chestmid.mjs && node tests/smoke_v2298_encnum.mjs"'));
ok('smoke_v2294 的 testChain pin 已更新为 191', s2294.includes('testChain === 194'));
ok('smoke_v2294 的版本锚已推进至 >= 95', s2294.includes('_gv[1] >= 98'));
ok('smoke_v2293 的 GAME_VERSION 字面量 pin 已更新为 v22.95', s2293.includes("const GAME_VERSION = 'v22.98';"));
ok('smoke_v2293 的 testChain pin 已更新为 191', s2293.includes('testChain === 194'));
