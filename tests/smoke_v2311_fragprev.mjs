...
// CHANGELOG 同步（冒烟二百零八件套（二百零七件套清除）/串尾/入库 207 份/顶 pin）+ 姊妹件套 pin
...
ok('data.js GAME_VERSION 字面量已为 v23.11（旧 v23.10 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.12';") && !dataSrc.includes("const GAME_VERSION = 'v23." + "10';"));
...
ok('README 件套口径为二百零八件套（二百零七件套清除）且旧 206 口径零残留',
  readme.includes('冒烟二百零八件套（二百零七件套清除）') && !readme.includes('冒烟二百零六件套（二百零五件套清' + '除）'));
...
ok('package.json 串尾为 ... node tests/smoke_v2310_diffsum.mjs && node tests/smoke_v2311_fragprev.mjs && node tests/smoke_v2312_voltitle.mjs"',
  pkg.includes('node tests/smoke_v2310_diffsum.mjs && node tests/smoke_v2311_fragprev.mjs && node tests/smoke_v2312_voltitle.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 207 件套', testChain === 208, String(testChain));
ok('CHANGELOG 顶部已追加 v23.11 条目', changelog.startsWith('## v23.12 '));
...
ok('smoke_v2310 的 GAME_VERSION 字面量 pin 已更新为 v23.11', s2310.includes("const GAME_VERSION = 'v23.12';"));
ok('smoke_v2310 的 CHANGELOG 顶 pin 已更新为 ## v23.12',
  s2310.includes("startsWith('## v23.12 "));
ok('smoke_v2310 的件套 pin 已更新为二百零八件套（二百零七件套清除）',
  s2310.includes('二百零八件套（二百零七件套清除）'));
ok('smoke_v2310 的 README 串尾 pin 已延伸至 smoke_v2311_fragprev',
  s2310.includes('smoke_v2309_questnum + smoke_v2310_diffsum + smoke_v2311_fragprev + smoke_v2312_voltitle（npm test 串跑）'));
ok('smoke_v2310 的 package 串尾 pin 已延伸至 smoke_v2311_fragprev',
  s2310.includes('node tests/smoke_v2310_diffsum.mjs && node tests/smoke_v2311_fragprev.mjs && node tests/smoke_v2312_voltitle.mjs"'));
ok('smoke_v2310 的 testChain pin 已更新为 207', s2310.includes('testChain === 208'));
