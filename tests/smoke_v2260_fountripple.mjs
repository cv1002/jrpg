...
ok('GAME_VERSION 字面量已为 v22.60（旧 v22.59 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.60';") && !dSrc.includes("const GAME_VERSION = 'v22." + "59';"));
...
ok('README tests 树收录 smoke_v2260_fountripple 且位于串尾',
  readme.includes('smoke_v2259_sandpile + smoke_v2260_fountripple（npm test 串跑）'));
ok('README 件套口径为一百五十六件套（一百五十五件套清除）且旧 155 口径零残留',
  readme.includes('冒烟一百五十六件套（一百五十五件套清除）') && !readme.includes('一百五十五件套（一百五十四件套清' + '除）'));
...
ok('package.json test 串共 156 件套', testChain === 156, String(testChain));
...
ok('smoke_v2259 的 GAME_VERSION 字面量 pin 已更新为 v22.60', s2259.includes("const GAME_VERSION = 'v22.60';"));
ok('smoke_v2259 的 README 串尾 pin 已随新现实延伸至 smoke_v2260_fountripple',
  s2259.includes('smoke_v2258_potionhelp + smoke_v2259_sandpile + smoke_v2260_fountripple（npm test 串跑）'));
ok('smoke_v2259 的 README 件套 pin 已随新现实更新为一百五十六件套（一百五十五件套清除）',
  s2259.includes('一百五十六件套（一百五十五件套清除）'));
ok('smoke_v2259 的 package.json 件套计数 pin 已更新为 === 156', s2259.includes('testChain === 156'));
ok('smoke_v2259 的 CHANGELOG 顶 pin 已更新为 ## v22.60', s2259.includes("startsWith('## v22.60')"));
ok('smoke_v2259 的 package.json 串尾 plain pin 已延伸至 smoke_v2260_fountripple',
  s2259.includes('smoke_v2259_sandpile.mjs && node tests/smoke_v2260_fountripple.mjs"'));
ok('smoke_v2258 的 GAME_VERSION 字面量 pin 已更新为 v22.60', s2258.includes("const GAME_VERSION = 'v22.60';"));
ok('smoke_v2257 的 GAME_VERSION 字面量 pin 已更新为 v22.60', s2257.includes("const GAME_VERSION = 'v22.60';"));
ok('smoke_v2256 的 GAME_VERSION 字面量 pin 已更新为 v22.60', s2256.includes("const GAME_VERSION = 'v22.60';"));
...
ok('v2143-45「件套守护领先一位」哨兵链已推进至 157（一百五十七件套（一百五十六件套清除））',
