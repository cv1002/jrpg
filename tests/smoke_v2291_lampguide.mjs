...
ok('GAME_VERSION 字面量已为 v22.91（旧 v22.90 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.77';") && !dSrc.includes("const GAME_VERSION = 'v22." + "90';"));
...
ok('README 件套口径为二百一十三件套（二百一十二件套清除）且旧 186 口径零残留',
  readme.includes('冒烟二百一十三件套（二百一十二件套清除）') && !readme.includes('冒烟一百八十六件套（一百八十五件套清' + '除）'));
...
ok('package.json test 串共 187 件套', testChain === 213, String(testChain));
ok('CHANGELOG 顶部已追加 v22.91 条目', changelog.startsWith('## v23.77 '));
...
ok('smoke_v2290 的 GAME_VERSION 字面量 pin 已更新为 v22.91（旧 v22.90 零残留）',
  s2290.includes("const GAME_VERSION = 'v23.77';") && !s2290.includes("const GAME_VERSION = 'v22." + "90';"));
ok('smoke_v2290 的 README 件套 pin 已随新现实更新为二百一十三件套（二百一十二件套清除）',
  s2290.includes('二百一十三件套（二百一十二件套清除）'));
ok('smoke_v2290 的 package.json 件套计数 pin 已更新为 === 187', s2290.includes('testChain === 213'));
...
ok('smoke_v2290 的 CHANGELOG 顶 pin 已更新为 ## v22.91', s2290.includes("startsWith('## v23.77 '"));
ok('smoke_v2290 的哨兵 pin 已更新为二百一十三件套（二百一十二件套清除）', s2290.includes('二百一十三件套（二百一十二件套清除）'));
