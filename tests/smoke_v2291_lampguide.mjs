...
ok('GAME_VERSION 格式合法且已越过 v22.90（本版守 v22.91）', !!_gv && (_gv[0] > 22 || (_gv[0] === 22 && _gv[1] >= 95)), GAME_VERSION);
ok('data.js 含 v22.91 注释（引灯人说明）', dSrc.includes('v22.91 新内容·纯风味'));
ok('GAME_VERSION 字面量已为 v22.91（旧 v22.90 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.95';") && !dSrc.includes("const GAME_VERSION = 'v22." + "90';"));
...
ok('README tests 树含 smoke_v2291_lampguide 串尾且旧串尾零残留（树为历史清单，总数不守恒）',
  treeLine && treeLine.includes('+ smoke_v2291_lampguide + smoke_v2292_cavewatch + smoke_v2293_deadsave + smoke_v2294_crystalwatch + smoke_v2295_deadprog（npm test 串跑）') &&
  !treeLine.includes('smoke_v2290_statlink（npm test 串跑）'));
ok('README 件套口径为一百九十一件套（一百九十件套清除）且旧 186 口径零残留',
  readme.includes('冒烟一百九十一件套（一百九十件套清除）') && !readme.includes('冒烟一百八十六件套（一百八十五件套清' + '除）'));
...
ok('package.json 串尾为 ... smoke_v2290_statlink.mjs && node tests/smoke_v2291_lampguide.mjs && node tests/smoke_v2292_cavewatch.mjs && node tests/smoke_v2293_deadsave.mjs && node tests/smoke_v2294_crystalwatch.mjs && node tests/smoke_v2295_deadprog.mjs"',
  pkg.includes('node tests/smoke_v2290_statlink.mjs && node tests/smoke_v2291_lampguide.mjs && node tests/smoke_v2292_cavewatch.mjs && node tests/smoke_v2293_deadsave.mjs && node tests/smoke_v2294_crystalwatch.mjs && node tests/smoke_v2295_deadprog.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 187 件套', testChain === 191, String(testChain));
ok('CHANGELOG 顶部已追加 v22.91 条目', changelog.startsWith('## v22.95 '));
...
ok('smoke_v2290 的 GAME_VERSION 字面量 pin 已更新为 v22.91（旧 v22.90 零残留）',
  s2290.includes("const GAME_VERSION = 'v22.95';") && !s2290.includes("const GAME_VERSION = 'v22." + "90';"));
ok('smoke_v2290 的 README 件套 pin 已随新现实更新为一百九十一件套（一百九十件套清除）',
  s2290.includes('一百九十二件套（一百九十一件套清除）'));
ok('smoke_v2290 的 package.json 件套计数 pin 已更新为 === 187', s2290.includes('testChain === 191'));
ok('smoke_v2290 的 README 串尾 pin 已随新现实延伸至 smoke_v2291_lampguide（v2287 起尾部）',
  s2290.includes('smoke_v2287_trueroute + smoke_v2288_scrollhint + smoke_v2289_winprog + smoke_v2290_statlink + smoke_v2291_lampguide + smoke_v2292_cavewatch + smoke_v2293_deadsave + smoke_v2294_crystalwatch + smoke_v2295_deadprog（npm test 串跑）'));
ok('smoke_v2290 的 package 串尾 pin 已延伸至 smoke_v2291_lampguide',
  s2290.includes('node tests/smoke_v2290_statlink.mjs && node tests/smoke_v2291_lampguide.mjs && node tests/smoke_v2292_cavewatch.mjs && node tests/smoke_v2293_deadsave.mjs && node tests/smoke_v2294_crystalwatch.mjs && node tests/smoke_v2295_deadprog.mjs"'));
ok('smoke_v2290 的 CHANGELOG 顶 pin 已更新为 ## v22.91', s2290.includes("startsWith('## v22.95 '"));
ok('smoke_v2290 的哨兵 pin 已更新为一百九十一件套（一百九十件套清除）', s2290.includes('一百九十二件套（一百九十一件套清除）'));
