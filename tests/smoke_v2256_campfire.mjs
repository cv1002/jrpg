...
ok('GAME_VERSION 字面量已为 v22.56（旧 v22.55 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.56';") && !dSrc.includes("const GAME_VERSION = 'v22." + "55';"));
...
ok('README tests 树收录 smoke_v2256_campfire 且位于串尾',
  readme.includes('smoke_v2255_steleglow + smoke_v2256_campfire（npm test 串跑）'));
ok('README 件套口径为一百五十二件套（一百五十一件套清除）',
  readme.includes('冒烟一百五十二件套（一百五十一件套清除）') && !readme.includes('冒烟一百五十一件套（一百五十件套清' + '除）'));
...
ok('package.json test 串共 152 件套', testChain === 152, String(testChain));
...
ok('smoke_v2255 的 GAME_VERSION 字面量 pin 已更新为 v22.56', s2255.includes("const GAME_VERSION = 'v22.56';"));
ok('smoke_v2255 的 README 串尾 pin 已随新现实延伸至 smoke_v2256_campfire',
  s2255.includes('smoke_v2254_grainfield + smoke_v2255_steleglow + smoke_v2256_campfire（npm test 串跑）'));
ok('smoke_v2255 的 README 件套 pin 已随新现实更新为一百五十二件套（一百五十一件套清除）',
  s2255.includes('一百五十二件套（一百五十一件套清除）'));
ok('smoke_v2255 的 package.json 件套计数 pin 已更新为 === 152', s2255.includes('testChain === 152'));
ok('smoke_v2255 的 CHANGELOG 顶 pin 已更新为 ## v22.56', s2255.includes("startsWith('## v22.56')"));
...
ok('smoke_v2254 的 GAME_VERSION 字面量 pin 已更新为 v22.56', s2254.includes("const GAME_VERSION = 'v22.56';"));
...
ok('smoke_v2253 的 GAME_VERSION 字面量 pin 已更新为 v22.56', s2253.includes("const GAME_VERSION = 'v22.56';"));
...
ok('smoke_v2252 的 GAME_VERSION 字面量 pin 已更新为 v22.56', s2252.includes("const GAME_VERSION = 'v22.56';"));
...
ok('v2143-45「件套守护领先一位」哨兵链已推进至 153（一百五十三件套（一百五十二件套清除））',
...
ok('smoke_v2255 的旧代零残留扫描已指向 v22.55（字面量/恒等/件套/串尾/testChain）',
  s2255.includes("src.includes(\"const GAME_VERSION = 'v22.\" + \"55';\")") &&
  s2255.includes("src.includes('一百五十一件套（一百五十件套清' + '除）')") &&
  s2255.includes("src.includes('smoke_v2255_steleglow（npm test 串' + '跑）')) stale.push(f);"));
