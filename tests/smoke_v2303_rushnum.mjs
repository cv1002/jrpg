...
// README/package.json/CHANGELOG 同步（冒烟二百件套（一百九十九件套清除）/串尾/入库 199 份/顶 pin）+
...
ok('data.js GAME_VERSION 字面量已为 v23.03（旧 v23.02 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.04';") && !dataSrc.includes("const GAME_VERSION = 'v23." + "02';"));
...
ok('package.json 串尾为 ... smoke_v2301_eco.mjs && node tests/smoke_v2302_cmdprev.mjs && node tests/smoke_v2303_rushnum.mjs && node tests/smoke_v2304_achgoal.mjs"',
  pkg.includes('node tests/smoke_v2301_eco.mjs && node tests/smoke_v2302_cmdprev.mjs && node tests/smoke_v2303_rushnum.mjs && node tests/smoke_v2304_achgoal.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 199 件套', testChain === 200, String(testChain));
ok('CHANGELOG 顶部已追加 v23.03 条目', changelog.startsWith('## v23.04 '));
...
ok('smoke_v2302 的 GAME_VERSION 字面量 pin 已更新为 v23.03', s2302.includes("const GAME_VERSION = 'v23.04';"));
ok('smoke_v2302 的 CHANGELOG 顶 pin 已更新为 ## v23.03',
  s2302.includes("startsWith('## v23.04 '"));
ok('smoke_v2302 的件套 pin 已更新为二百件套（一百九十九件套清除）',
  s2302.includes('二百件套（一百九十九件套清除）'));
ok('smoke_v2302 的 README 串尾 pin 已延伸至 smoke_v2303_rushnum',
  s2302.includes('smoke_v2301_eco + smoke_v2302_cmdprev + smoke_v2303_rushnum + smoke_v2304_achgoal（npm test 串跑）'));
ok('smoke_v2302 的 package 串尾 pin 已延伸至 smoke_v2303_rushnum',
  s2302.includes('node tests/smoke_v2301_eco.mjs && node tests/smoke_v2302_cmdprev.mjs && node tests/smoke_v2303_rushnum.mjs && node tests/smoke_v2304_achgoal.mjs"'));
ok('smoke_v2302 的 testChain pin 已更新为 199', s2302.includes('testChain === 200'));
