...
// README/package.json/CHANGELOG 同步（冒烟二百二十二件套（二百二十一件套清除）/串尾/入库 199 份/顶 pin）+
...
ok('data.js GAME_VERSION 字面量已为 v23.03（旧 v23.02 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.98';") && !dataSrc.includes("const GAME_VERSION = 'v23." + "02';"));
...
ok('README 件套口径为二百二十二件套（二百二十一件套清除）且旧 196 口径零残留',
  readme.includes('冒烟二百二十二件套（二百二十一件套清除）') && !readme.includes('冒烟一百九十六件套（一百九十五件套清' + '除）'));
...
ok('package.json test 串共 199 件套', testChain === 222, String(testChain));
ok('CHANGELOG 顶部已追加 v23.03 条目', changelog.startsWith('## v23.98 '));
...
ok('smoke_v2302 的 GAME_VERSION 字面量 pin 已更新为 v23.03', s2302.includes("const GAME_VERSION = 'v23.98';"));
ok('smoke_v2302 的 CHANGELOG 顶 pin 已更新为 ## v23.03',
  s2302.includes("startsWith('## v23.98 '"));
ok('smoke_v2302 的件套 pin 已更新为二百二十二件套（二百二十一件套清除）',
  s2302.includes('二百二十二件套（二百二十一件套清除）'));
...
ok('smoke_v2302 的 testChain pin 已更新为 199', s2302.includes('testChain === 222'));
