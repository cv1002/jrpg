// v23.77 专项冒烟：Esc 暂停菜单头部「当前所在地」——体验打磨·信息透明·纯显示，
...
// README/package.json/CHANGELOG 同步（tests 树串尾 + 件套口径 + v23.77 守护描述 + Esc 行口径 + 入库 213 份）、
...
console.log('— v23.77 暂停菜单「当前所在地」 冒烟 —');
...
ok('data.js 含 v23.77 注释（暂停菜单当前所在地说明）', dSrc.includes('v23.77 体验打磨·信息透明·纯显示'));
ok('GAME_VERSION 字面量已为 v23.77（旧 v23.76 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v23.77';") && !dSrc.includes("const GAME_VERSION = 'v23.7" + "6';"));
ok('data.js 仍保留 v23.76 历史注释（累积注释块，姊妹 pin 不失效）', dSrc.includes('v23.76 新内容·步行累计维度里程碑'));

// —— menus.js 源级落位：v23.77 注释 + drawPause 头部同数据源派生 + 既有子串保留 ——
ok('menus.js 含 v23.77 注释（drawPause 头部当前所在地说明）', menusSrc.includes('v23.77 体验打磨·信息透明·纯显示'));
...
ok('package.json test 串共 213 件套', testChain === 213, String(testChain));
ok('package.json 已收录 smoke_v2317_pausemap（npm test 串跑第 213 份）',
  JSON.parse(pkg).scripts.test.includes('smoke_v2317_pausemap.mjs'));
ok('package.json 串尾为 ... smoke_v2316_voiceshead.mjs && node tests/smoke_v2317_pausemap.mjs"',
  pkg.includes('node tests/smoke_v2316_voiceshead.mjs && node tests/smoke_v2317_pausemap.mjs"'));
ok('README tests 树串尾已延伸至 smoke_v2317_pausemap',
  readme.includes('+ smoke_v2316_voiceshead + smoke_v2317_pausemap（npm test 串跑）'));
ok('README 件套口径为二百一十三件套（二百一十二件套清除）',
  readme.includes('冒烟二百一十三件套（二百一十二件套清除）') && !readme.includes('冒烟二百一十二件套（二百一十一件套清' + '除）'));
ok('README 含 v23.77 守护描述（暂停菜单「当前所在地」守护）', readme.includes('v23.77 起含 暂停菜单「当前所在地」守护'));
ok('README 含 smoke_v2317_pausemap 入库（213 份）', readme.includes('smoke_v2317_pausemap 入库（213 份）'));
ok('README Esc 行含 v23.77 当前所在地口径', readme.includes('v23.77 起菜单头部常显「📍 当前地图名」'));
ok('README 仍保留 v23.76 历史守护描述（累积描述，姊妹 pin 不失效）', readme.includes('千里之行 X/1000 步（步行累计，v23.76）'));
ok('CHANGELOG 顶部已追加 v23.77 条目（暂停菜单当前所在地）', changelog.startsWith('## v23.77 '));
ok('CHANGELOG 顶部条目含 v23.77 守护说明', changelog.includes('## v23.77 暂停菜单（Esc）头部补「当前所在地」'));
...
console.log(`— v23.77 冒烟结束：${n} 项，失败 ${failed} —`);
