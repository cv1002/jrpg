...
ok('data.js GAME_VERSION 字面量已更新为 v21.89', dataSrc.includes("const GAME_VERSION = 'v24.00';"));
...
ok('README 件套口径为二百二十四件套（二百二十三件套清除）',
  readme.includes('冒烟二百二十四件套（二百二十三件套清除）') && !readme.includes('冒烟八十三件套（八十二件套清除）'));
...
for (const nm of achFiles) {
  ...
  ok(`${nm} 的 README 成就 pin 已随新现实更新（31 项 pin 零残留，32 项双处落位）`,
    src.includes("readme.includes('成就一览（全部 76 项进度'") && !src.includes('**30 项成就**'));
}
