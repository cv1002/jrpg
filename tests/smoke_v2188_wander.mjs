...
ok('data.js GAME_VERSION 字面量已更新为 v21.88', dataSrc.includes("const GAME_VERSION = 'v21.88';"));
...
ok('ACH_LIST 由 31 → 32 项（新增一项，精确总数由本版守护）', ACH_LIST.length === 32, String(ACH_LIST.length));
...
ok('README 件套口径为八十四件套（八十三件套清除）',
  readme.includes('冒烟八十四件套（八十三件套清除）') && !readme.includes('冒烟八十三件套（八十二件套清除）'));
ok('README 含 v21.88 守护描述', readme.includes('v21.88 起含'));
ok('README 成就口径「32 项」双处同步（快速上手表 C 键行 + 图鉴&成就行）',
...
for (const nm of suite84) {
  ...
  ok(`${nm} 的 README 件套 pin 已随新现实更新为八十四件套（八十三件套清除）`,
    src.includes('八十四件套（八十三件套清除）'));
}
for (const nm of ['smoke_v2187_endingrecap.mjs', 'smoke_v2186_brew.mjs', 'smoke_v2185_steleclear.mjs', 'smoke_v2184_lvl12.mjs', 'smoke_v2183_mpsip.mjs', 'smoke_v2182_winrecap.mjs', 'smoke_v2181_helpquickcast.mjs', 'smoke_v2179_titlerecap.mjs']) {
  const src = read(`../tests/${nm}`);
  ok(`${nm} 的 GAME_VERSION 字面量 pin 已随新现实更新为 v21.88`,
    src.includes("const GAME_VERSION = 'v21.88';"));
}
...
for (const nm of achFiles) {
  ...
  ok(`${nm} 的 README 成就 pin 已随新现实更新（31 项 pin 零残留，32 项双处落位）`,
    src.includes("readme.includes('成就一览（全部 32 项进度'") && !src.includes('**30 项成就**'));
}
