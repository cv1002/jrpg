...
ok('data.js GAME_VERSION 字面量已为 v22.69（旧 v22.68 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.08';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "68';"));
...
ok('ACH_LIST 精确总数 53 项（v2229 精确计数 pin 随新现实更新 52→53）', ACH_LIST.length === 59, String(ACH_LIST.length));
...
ok('README 件套口径为二百零四件套（二百零三件套清除）且旧 164 口径零残留',
  readme.includes('冒烟二百零四件套（二百零三件套清除）') && !readme.includes('冒烟一百六十四件套（一百六十三件套清' + '除）'));
...
ok('package.json test 串共 165 件套', testChain === 204, String(testChain));
ok('CHANGELOG 顶为 v22.69 条目', changelog.startsWith('## v23.08 '));
...
ok('smoke_v2268 的 GAME_VERSION 字面量 pin 已更新为 v22.69', s2268.includes("const GAME_VERSION = 'v23.08';"));
ok('smoke_v2268 的 README 件套 pin 已随新现实更新为二百零四件套（二百零三件套清除）',
  s2268.includes('二百零四件套（二百零三件套清除）'));
ok('smoke_v2268 的 package.json 件套计数 pin 已更新为 === 165', s2268.includes('testChain === 204'));
ok('smoke_v2268 的 CHANGELOG 顶 pin 已更新为 ## v22.69', s2268.includes("startsWith('## v23.08'") || s2268.includes("startsWith('## v22.78 '"));
...
ok('smoke_v2268 的 README 成就 pin 已随新现实更新为 53 项双处', s2268.includes('成就一览（全部 59 项进度') && s2268.includes('**59 项成就**'));
ok('smoke_v2268 的 ACH_LIST 精确计数 pin 已更新为 === 53', s2268.includes('ACH_LIST.length === 59'));
...
ok('smoke_v2267 的 README 成就 pin 已随新现实更新为 53 项双处', s2267.includes('成就一览（全部 59 项进度') && s2267.includes('**59 项成就**'));
ok('smoke_v2267 的 ACH_LIST 精确计数 pin 已更新为 === 53', s2267.includes('ACH_LIST.length === 59'));
...
ok('smoke_v2266 的 README 成就 pin 已随新现实更新为 53 项双处', s2266.includes('成就一览（全部 59 项进度') && s2266.includes('**59 项成就**'));
ok('smoke_v2266 的 ACH_LIST 精确计数 pin 已更新为 === 53', s2266.includes('ACH_LIST.length === 59'));
...
ok('smoke_v2229 的 ACH_LIST 精确总数 pin 已更新为 === 53（52→53）',
  s2229.includes('ACH_LIST.length === 59') && !s2229.includes('ACH_LIST.length === 5' + '2'));
