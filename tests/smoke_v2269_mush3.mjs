...
ok('data.js GAME_VERSION 字面量已为 v22.69（旧 v22.68 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.00';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "68';"));
...
ok('README 件套口径为一百九十六件套（一百九十五件套清除）且旧 164 口径零残留',
  readme.includes('冒烟一百九十六件套（一百九十五件套清除）') && !readme.includes('冒烟一百六十四件套（一百六十三件套清' + '除）'));
...
ok('package.json test 串共 165 件套', testChain === 196, String(testChain));
ok('CHANGELOG 顶为 v22.69 条目', changelog.startsWith('## v23.00 '));
...
ok('smoke_v2268 的 GAME_VERSION 字面量 pin 已更新为 v22.69', s2268.includes("const GAME_VERSION = 'v23.00';"));
ok('smoke_v2268 的 README 件套 pin 已随新现实更新为一百九十六件套（一百九十五件套清除）',
  s2268.includes('一百九十七件套（一百九十六件套清除）'));
ok('smoke_v2268 的 package.json 件套计数 pin 已更新为 === 165', s2268.includes('testChain === 196'));
ok('smoke_v2268 的 CHANGELOG 顶 pin 已更新为 ## v22.69', s2268.includes("startsWith('## v23.00'") || s2268.includes("startsWith('## v22.78 '"));
