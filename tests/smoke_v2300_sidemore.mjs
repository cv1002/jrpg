...
ok('data.js GAME_VERSION 字面量已为 v23.00（旧 v22.99 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.04';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "99';"));
...
// —— README 同步守护 ——
ok('README 件套口径为二百件套（一百九十九件套清除）且旧 195 口径零残留',
  readme.includes('冒烟二百件套（一百九十九件套清除）') && !readme.includes('冒烟一百九十五件套（一百九十四件套清' + '除）'));
...
ok('README tests 树串尾已延伸至 smoke_v2300_sidemore（v2299 后接 v2300）',
  readme.includes('smoke_v2299_crosslink + smoke_v2300_sidemore + smoke_v2301_eco + smoke_v2302_cmdprev + smoke_v2303_rushnum + smoke_v2304_achgoal（npm test 串跑）'));
...
ok('package.json 串尾为 ... smoke_v2299_crosslink.mjs && node tests/smoke_v2300_sidemore.mjs"',
  pkg.includes('node tests/smoke_v2299_crosslink.mjs && node tests/smoke_v2300_sidemore.mjs && node tests/smoke_v2301_eco.mjs && node tests/smoke_v2302_cmdprev.mjs && node tests/smoke_v2303_rushnum.mjs && node tests/smoke_v2304_achgoal.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 196 件套', testChain === 200, String(testChain));
ok('CHANGELOG 顶部已追加 v23.00 条目', changelog.startsWith('## v23.04 '));
...
ok('smoke_v2299 的 GAME_VERSION 字面量 pin 已更新为 v23.00', s2299.includes("const GAME_VERSION = 'v23.04';"));
ok('smoke_v2299 的 CHANGELOG 顶 pin 已更新为 ## v23.00',
  s2299.includes("startsWith('## v23.04 '"));
ok('smoke_v2299 的件套 pin 已更新为二百件套（一百九十九件套清除）',
  s2299.includes('二百件套（一百九十九件套清除）'));
ok('smoke_v2299 的 README 串尾 pin 已延伸至 smoke_v2300_sidemore',
  s2299.includes('smoke_v2298_encnum + smoke_v2299_crosslink + smoke_v2300_sidemore + smoke_v2301_eco + smoke_v2302_cmdprev + smoke_v2303_rushnum + smoke_v2304_achgoal（npm test 串跑）'));
ok('smoke_v2299 的 package 串尾 pin 已延伸至 smoke_v2300_sidemore',
  s2299.includes('node tests/smoke_v2298_encnum.mjs && node tests/smoke_v2299_crosslink.mjs && node tests/smoke_v2300_sidemore.mjs && node tests/smoke_v2301_eco.mjs && node tests/smoke_v2302_cmdprev.mjs && node tests/smoke_v2303_rushnum.mjs && node tests/smoke_v2304_achgoal.mjs"'));
ok('smoke_v2299 的 testChain pin 已更新为 196', s2299.includes('testChain === 200'));
