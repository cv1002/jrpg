...
ok('data.js GAME_VERSION 字面量已为 v23.00（旧 v22.99 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.40';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "99';"));
...
ok('CHANGELOG 顶部已追加 v23.00 条目', changelog.startsWith('## v23.40 '));
...
ok('smoke_v2299 的 GAME_VERSION 字面量 pin 已更新为 v23.00', s2299.includes("const GAME_VERSION = 'v23.40';"));
ok('smoke_v2299 的 CHANGELOG 顶 pin 已更新为 ## v23.00',
  s2299.includes("startsWith('## v23.40 '"));
...
// 双侧旗标 + 三张卡 active 时其余支线均为 offer——全部支线在列（9 条）
ok('运行期：sideObjectives 全部支线在列（9 条：蘑菇/星砂/名字 + 6 条 offer）', sides.length === 9, JSON.stringify(sides));
ok('运行期：questLines 9 条支线 + statusSideSuffix → （还有 8 条）', statusSideSuffix(sides) === '（还有 8 条）', statusSideSuffix(sides));
...
ok('运行期：支线行落画含「支线：」且带「（还有 8 条）」计数',
  (capMulti || []).some((c) => c.includes('支线：') && c.includes('（还有 8 条）') && c.includes('J 任务日志')),
