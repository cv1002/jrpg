...
ok('data.js GAME_VERSION 字面量已为 v23.00（旧 v22.99 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.08';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "99';"));
...
ok('package.json test 串共 196 件套', testChain === 204, String(testChain));
ok('CHANGELOG 顶部已追加 v23.00 条目', changelog.startsWith('## v23.08 '));
...
ok('smoke_v2299 的 GAME_VERSION 字面量 pin 已更新为 v23.00', s2299.includes("const GAME_VERSION = 'v23.08';"));
ok('smoke_v2299 的 CHANGELOG 顶 pin 已更新为 ## v23.00',
  s2299.includes("startsWith('## v23.08 '"));
ok('smoke_v2299 的件套 pin 已更新为二百零四件套（二百零三件套清除）',
  s2299.includes('二百零四件套（二百零三件套清除）'));
ok('smoke_v2299 的 README 串尾 pin 已延伸至 smoke_v2300_sidemore',
  s2299.includes('smoke_v2298_encnum + smoke_v2299_crosslink + smoke_v2300_sidemore + smoke_v2301_eco + smoke_v2302_cmdprev + smoke_v2303_rushnum + smoke_v2304_achgoal + smoke_v2305_monnum + smoke_v2306_skillnum + smoke_v2307_bossnum + smoke_v2308_diffnum（npm test 串跑）'));
ok('smoke_v2299 的 package 串尾 pin 已延伸至 smoke_v2300_sidemore',
  s2299.includes('node tests/smoke_v2298_encnum.mjs && node tests/smoke_v2299_crosslink.mjs && node tests/smoke_v2300_sidemore.mjs && node tests/smoke_v2301_eco.mjs && node tests/smoke_v2302_cmdprev.mjs && node tests/smoke_v2303_rushnum.mjs && node tests/smoke_v2304_achgoal.mjs && node tests/smoke_v2305_monnum.mjs && node tests/smoke_v2306_skillnum.mjs && node tests/smoke_v2307_bossnum.mjs && node tests/smoke_v2308_diffnum.mjs"'));
ok('smoke_v2299 的 testChain pin 已更新为 196', s2299.includes('testChain === 204'));
...
// 双侧旗标 + 三张卡 active 时其余支线均为 offer——全部支线在列（8 条）
ok('运行期：sideObjectives 全部支线在列（8 条：蘑菇/星砂/名字 + 5 条 offer）', sides.length === 8, JSON.stringify(sides));
ok('运行期：questLines 8 条支线 + statusSideSuffix → （还有 7 条）', statusSideSuffix(sides) === '（还有 7 条）', statusSideSuffix(sides));
...
ok('运行期：支线行落画含「支线：」且带「（还有 7 条）」计数',
  (capMulti || []).some((c) => c.includes('支线：') && c.includes('（还有 7 条）') && c.includes('J 任务日志')),
