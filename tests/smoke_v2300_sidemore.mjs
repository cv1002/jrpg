...
ok('data.js GAME_VERSION 字面量已为 v23.00（旧 v22.99 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.12';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "99';"));
...
// —— README 同步守护 ——
ok('README 件套口径为二百零八件套（二百零七件套清除）且旧 195 口径零残留',
  readme.includes('冒烟二百零八件套（二百零七件套清除）') && !readme.includes('冒烟一百九十五件套（一百九十四件套清' + '除）'));
...
ok('package.json test 串共 196 件套', testChain === 208, String(testChain));
ok('CHANGELOG 顶部已追加 v23.00 条目', changelog.startsWith('## v23.12 '));
...
ok('smoke_v2299 的 GAME_VERSION 字面量 pin 已更新为 v23.00', s2299.includes("const GAME_VERSION = 'v23.12';"));
ok('smoke_v2299 的 CHANGELOG 顶 pin 已更新为 ## v23.00',
  s2299.includes("startsWith('## v23.12 '"));
ok('smoke_v2299 的件套 pin 已更新为二百零八件套（二百零七件套清除）',
  s2299.includes('二百零八件套（二百零七件套清除）'));
...
ok('smoke_v2299 的 testChain pin 已更新为 196', s2299.includes('testChain === 208'));
