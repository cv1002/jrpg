...
ok('data.js GAME_VERSION 字面量为 v22.83', dataSrc.includes("const GAME_VERSION = 'v22.83'"));
...
ok('README tests 树串尾已延伸至 smoke_v2283_menuekey（v2282 后接 v2283）',
  readme.includes('smoke_v2282_archgate + smoke_v2283_menuekey（npm test 串跑）'));
ok('README 件套口径为一百七十九件套（一百七十八件套清除）且旧 178 口径零残留',
  readme.includes('冒烟一百七十九件套（一百七十八件套清除）') && !readme.includes('冒烟一百七十八件套（一百七十七件套清' + '除）'));
...
ok('package.json test 串含 smoke_v2283_menuekey.mjs 且位于串尾',
  pkg.includes('node tests/smoke_v2282_archgate.mjs && node tests/smoke_v2283_menuekey.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 179 件套', testChain === 179, String(testChain));
ok('CHANGELOG 含 v22.83 条目（顶 pin）', changelog.startsWith('## v22.83 '));
...
ok('smoke_v2282 的 GAME_VERSION 字面量 pin 已更新为 v22.83', s2282.includes("const GAME_VERSION = 'v22.83';"));
ok('smoke_v2282 的 CHANGELOG 顶 pin 已更新为 ## v22.83', s2282.includes("startsWith('## v22.83 '"));
ok('smoke_v2282 的件套 pin 已更新为一百七十九件套（一百七十八件套清除）', s2282.includes('一百七十九件套（一百七十八件套清除）'));
ok('smoke_v2282 的 README 串尾 pin 已延伸至 smoke_v2283_menuekey', s2282.includes('smoke_v2282_archgate + smoke_v2283_menuekey（npm test 串跑）'));
ok('smoke_v2282 的 package 串尾 pin 已延伸至 smoke_v2283_menuekey', s2282.includes('node tests/smoke_v2282_archgate.mjs && node tests/smoke_v2283_menuekey.mjs"'));
ok('smoke_v2281 的 GAME_VERSION 字面量 pin 已更新为 v22.83', s2281.includes("const GAME_VERSION = 'v22.83';"));
ok('smoke_v2281 的 CHANGELOG 顶 pin 已更新为 ## v22.83', s2281.includes("startsWith('## v22.83 '"));
ok('smoke_v2281 的 testChain pin 已更新为 179', s2281.includes('testChain === 179'));
ok('smoke_v2281 的 README 串尾 pin 已延伸至 smoke_v2283_menuekey', s2281.includes('smoke_v2282_archgate + smoke_v2283_menuekey（npm test 串跑）'));
ok('smoke_v2260 的 package.json 串尾 pin 已延伸至 smoke_v2283_menuekey', s2260.includes('node tests/smoke_v2283_menuekey.mjs'));
...
for (const f of allTests) {
  ...
  if (src.includes("const GAME_VERSION = 'v22." + "82';") || src.includes("GAME_VERSION === 'v22." + "82'") ||
      src.includes('一百七十八件套（一百七十七件套清' + '除）') || src.includes('testChain === ' + '178') ||
      src.includes('smoke_v2282_archgate（npm test ' + '串跑）') || src.includes('_gv[1] >= ' + '82') ||
      src.includes("startsWith('## v22." + "82 '")) stale.push(f);
}
ok('旧代 v22.82 字面量/恒等/件套/testChain/串尾/版本锚 pin 全库零残留（' + allTests.length + ' 件扫描）', stale.length === 0, stale.join(','));

// 9. 哨兵链（件套守护领先一位）已指向下一版 180 口径
