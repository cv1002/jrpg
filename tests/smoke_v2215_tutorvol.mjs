...
// 冒烟一百一十九件套（一百一十八件套清除）、v22.15 守护描述、README 上手表 [ / ] 行）、姊妹件套
...
ok('GAME_VERSION 字面量已为 v22.15（本版独占精确锚点）', GAME_VERSION === 'v22.23', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.15', dataSrc.includes("const GAME_VERSION = 'v22.23';"));
...
ok('README 件套口径为一百一十九件套（一百一十八件套清除）', readme.includes('冒烟一百一十九件套（一百一十八件套清除）') && !readme.includes('冒烟一百一十件套（一百零九件套清' + '除）'));
...
ok('package.json test 串收录 smoke_v2219_footkeys（第 115 份·串尾随新现实更新）', pkg.includes('&& node tests/smoke_v2219_footkeys.mjs && node tests/smoke_v2220_hearer.mjs && node tests/smoke_v2221_wanderer.mjs && node tests/smoke_v2222_peddler.mjs && node tests/smoke_v2223_lampman.mjs"'));
