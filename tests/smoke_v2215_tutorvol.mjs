...
// 冒烟一百一十二件套（一百一十一件套清除）、v22.15 守护描述、README 上手表 [ / ] 行）、姊妹件套
...
ok('GAME_VERSION 字面量已为 v22.15（本版独占精确锚点）', GAME_VERSION === 'v22.16', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.15', dataSrc.includes("const GAME_VERSION = 'v22.16';"));
...
// —— README 同步 ——
ok('README tests 树尾含 + smoke_v2215_tutorvol + smoke_v2216_picker（npm test 串跑）', readme.includes('smoke_v2214_mush + smoke_v2215_tutorvol + smoke_v2216_picker（npm test 串跑）'));
ok('README 件套口径为一百一十二件套（一百一十一件套清除）', readme.includes('冒烟一百一十二件套（一百一十一件套清除）') && !readme.includes('冒烟一百一十件套（一百零九件套清' + '除）'));
...
ok('package.json test 串收录 smoke_v2216_picker（第 112 份·串尾随新现实更新）', pkg.includes('&& node tests/smoke_v2216_picker.mjs"'));
