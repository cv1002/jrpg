...
ok('GAME_VERSION 格式合法且已越过 v22.76（本版守 v22.79）', !!_gv && (_gv[0] > 22 || (_gv[0] === 22 && _gv[1] >= 96)), GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已为 v22.78（旧 v22.75 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v22.96';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "75';"));
...
ok('README 件套口径为一百九十二件套（一百九十一件套清除）且旧 171 口径零残留',
  readme.includes('冒烟一百九十二件套（一百九十一件套清除）') && !readme.includes('冒烟一百七十一件套（一百七十件套清' + '除）'));
...
ok('package.json test 串共 172 件套', testChain === 192, String(testChain));
ok('CHANGELOG 含 v22.78 条目（顶 pin）', changelog.startsWith('## v22.96 '));

// —— 姊妹件套 pin 随新现实更新 + v22.75 遗留 stale 串尾锚修复落位 ——
const s2275 = read('smoke_v2275_codexempty.mjs');
ok('smoke_v2275 的 GAME_VERSION 字面量 pin 已更新为 v22.79', s2275.includes("const GAME_VERSION = 'v22.96';"));
ok('smoke_v2275 的 CHANGELOG 顶 pin 已更新为 ## v22.78', s2275.includes("startsWith('## v22.96'"));
ok('smoke_v2275 的件套 pin 已更新为一百九十二件套（一百九十一件套清除）', s2275.includes('一百九十三件套（一百九十二件套清除）'));
...
ok('smoke_v2143 哨兵链已推进至一百九十二件套（一百九十一件套清除）', s2143.includes('一百九十三件套（一百九十二件套清除）') && s2143.includes("!readme.includes('一百九十三件套（一百九十二件套清除）')"));
