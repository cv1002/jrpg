...
ok('GAME_VERSION 格式合法且已越过 v22.76（本版守 v22.79）', !!_gv && (_gv[0] > 22 || (_gv[0] === 22 && _gv[1] >= 79)), GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已为 v22.78（旧 v22.75 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v22.79';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "75';"));
...
ok('README tests 树串尾已延伸至 smoke_v2276_lampkid（v2275 后接 v2276）',
  readme.includes('smoke_v2275_codexempty + smoke_v2276_lampkid + smoke_v2277_pondhint + smoke_v2278_mushguide + smoke_v2279_lampwell（npm test 串跑）'));
ok('README 件套口径为一百七十五件套（一百七十四件套清除）且旧 171 口径零残留',
  readme.includes('冒烟一百七十五件套（一百七十四件套清除）') && !readme.includes('冒烟一百七十一件套（一百七十件套清' + '除）'));
...
ok('package.json test 串共 172 件套', testChain === 175, String(testChain));
ok('CHANGELOG 含 v22.78 条目（顶 pin）', changelog.startsWith('## v22.79 '));

// —— 姊妹件套 pin 随新现实更新 + v22.75 遗留 stale 串尾锚修复落位 ——
const s2275 = read('smoke_v2275_codexempty.mjs');
ok('smoke_v2275 的 GAME_VERSION 字面量 pin 已更新为 v22.79', s2275.includes("const GAME_VERSION = 'v22.79';"));
ok('smoke_v2275 的 CHANGELOG 顶 pin 已更新为 ## v22.78', s2275.includes("startsWith('## v22.79'"));
ok('smoke_v2275 的件套 pin 已更新为一百七十五件套（一百七十四件套清除）', s2275.includes('一百七十五件套（一百七十四件套清除）'));
ok('smoke_v2275 的 README 串尾 pin 已延伸至 smoke_v2276_lampkid', s2275.includes('smoke_v2275_codexempty + smoke_v2276_lampkid + smoke_v2277_pondhint + smoke_v2278_mushguide + smoke_v2279_lampwell（npm test 串跑）'));
...
ok('smoke_v2228 的「已收录」regex 串尾锚已延伸至 smoke_v2276_lampkid（stale 修复落位）',
  s2228.includes('node tests\\/smoke_v2276_lampkid\\.mjs && node tests\\/smoke_v2277_pondhint\\.mjs && node tests\\/smoke_v2278_mushguide\\.mjs && node tests\\/smoke_v2279_lampwell\\.mjs"'));
