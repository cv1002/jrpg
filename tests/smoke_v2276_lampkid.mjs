...
ok('data.js GAME_VERSION 字面量已为 v22.78（旧 v22.75 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.93';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "75';"));
...
ok('README 件套口径为二百一十七件套（二百一十六件套清除）且旧 171 口径零残留',
  readme.includes('冒烟二百一十七件套（二百一十六件套清除）') && !readme.includes('冒烟一百七十一件套（一百七十件套清' + '除）'));
...
ok('package.json test 串共 172 件套', testChain === 217, String(testChain));
ok('CHANGELOG 含 v22.78 条目（顶 pin）', changelog.startsWith('## v23.93 '));

// —— 姊妹件套 pin 随新现实更新 + v22.75 遗留 stale 串尾锚修复落位 ——
const s2275 = read('smoke_v2275_codexempty.mjs');
ok('smoke_v2275 的 GAME_VERSION 字面量 pin 已更新为 v22.79', s2275.includes("const GAME_VERSION = 'v23.93';"));
ok('smoke_v2275 的 CHANGELOG 顶 pin 已更新为 ## v22.78', s2275.includes("startsWith('## v23.93'"));
ok('smoke_v2275 的件套 pin 已更新为二百一十七件套（二百一十六件套清除）', s2275.includes('二百一十七件套（二百一十六件套清除）'));
