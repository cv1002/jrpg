// v22.76 专项冒烟：潮灯镇「放灯童」新 NPC（新内容·纯风味·数据层三件套，承 v22.11 掌灯童 /
...
console.log('— v22.76 潮灯镇「放灯童」新 NPC 冒烟 —');

// —— 版本锚点（v21.7 去硬化惯例）：格式合法 + 已越过 v22.75 ——
const _vm = (s) => { const m = /^v(\d+)\.(\d+)$/.exec(String(s || '')); return m ? [Number(m[1]), Number(m[2])] : null; };
const _gv = _vm(GAME_VERSION);
ok('GAME_VERSION 格式合法且已越过 v22.75（本版守 v22.76）', !!_gv && (_gv[0] > 22 || (_gv[0] === 22 && _gv[1] >= 76)), GAME_VERSION);
...
// —— 源级落位：data.js v22.76 注释 + 三件套 + GAME_VERSION ——
ok('data.js 含 v22.76 版本注释（放灯童三件套说明）', dataSrc.includes('v22.76 新 NPC·纯风味「放灯童」'));
ok('data.js GAME_VERSION 字面量已为 v22.76（旧 v22.75 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v22.76';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "75';"));
...
ok('NPC_SPOTS 总数 35（既有 34 键 + 放灯童 1 键，v22.76 随新现实更新）', Object.keys(NPC_SPOTS).length === 35, String(Object.keys(NPC_SPOTS).length));
...
ok('README tests 树串尾已延伸至 smoke_v2276_lampkid（v2275 后接 v2276）',
  readme.includes('smoke_v2275_codexempty + smoke_v2276_lampkid（npm test 串跑）'));
ok('README 件套口径为一百七十二件套（一百七十一件套清除）且旧 171 口径零残留',
  readme.includes('冒烟一百七十二件套（一百七十一件套清除）') && !readme.includes('冒烟一百七十一件套（一百七十件套清' + '除）'));
ok('README 含 v22.76 守护描述（潮灯镇放灯童新 NPC 守护）', readme.includes('v22.76 起含潮灯镇「放灯童」新 NPC 守护'));
...
ok('package.json 已收录 smoke_v2276_lampkid（npm test 串跑第 172 份）',
  pkg.includes('smoke_v2275_codexempty.mjs && node tests/smoke_v2276_lampkid.mjs'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 172 件套', testChain === 172, String(testChain));
ok('CHANGELOG 含 v22.76 条目（顶 pin）', changelog.startsWith('## v22.76 '));

// —— 姊妹件套 pin 随新现实更新 + v22.75 遗留 stale 串尾锚修复落位 ——
const s2275 = read('smoke_v2275_codexempty.mjs');
ok('smoke_v2275 的 GAME_VERSION 字面量 pin 已更新为 v22.76', s2275.includes("const GAME_VERSION = 'v22.76';"));
ok('smoke_v2275 的 CHANGELOG 顶 pin 已更新为 ## v22.76', s2275.includes("startsWith('## v22.76'"));
ok('smoke_v2275 的件套 pin 已更新为一百七十二件套（一百七十一件套清除）', s2275.includes('一百七十二件套（一百七十一件套清除）'));
ok('smoke_v2275 的 README 串尾 pin 已延伸至 smoke_v2276_lampkid', s2275.includes('smoke_v2275_codexempty + smoke_v2276_lampkid（npm test 串跑）'));
ok('smoke_v2275 的 package.json 串尾 pin 已延伸至 smoke_v2276_lampkid', s2275.includes('smoke_v2275_codexempty.mjs && node tests/smoke_v2276_lampkid.mjs'));
// 注：v2275 为图鉴空态版冒烟，本就无 NPC_SPOTS 计数 pin（v22.75 未立、v22.76 无需级联），此处不赘姊妹断言。
const s2143 = read('smoke_v2143_talkekey.mjs');
ok('smoke_v2143 哨兵链已推进至一百七十三件套（一百七十二件套清除）', s2143.includes('一百七十三件套（一百七十二件套清除）') && s2143.includes("!readme.includes('一百七十三件套')"));
...
ok('smoke_v2228 的「已收录」regex 串尾锚已延伸至 smoke_v2276_lampkid（stale 修复落位）',
  s2228.includes('node tests\\/smoke_v2276_lampkid\\.mjs"'));
...
console.log(`\n— v22.76 潮灯镇「放灯童」新 NPC 冒烟：${n - failed}/${n} 通过 —`);
