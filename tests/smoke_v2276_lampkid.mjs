...
ok('GAME_VERSION 格式合法且已越过 v22.76（本版守 v22.79）', !!_gv && (_gv[0] > 22 || (_gv[0] === 22 && _gv[1] >= 94)), GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已为 v22.78（旧 v22.75 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v22.94';") && !dataSrc.includes("const GAME_VERSION = 'v22." + "75';"));
...
ok('README tests 树串尾已延伸至 smoke_v2276_lampkid（v2275 后接 v2276）',
  readme.includes('smoke_v2275_codexempty + smoke_v2276_lampkid + smoke_v2277_pondhint + smoke_v2278_mushguide + smoke_v2279_lampwell + smoke_v2280_grainfield + smoke_v2281_starwell + smoke_v2282_archgate + smoke_v2283_menuekey + smoke_v2284_skillekey + smoke_v2285_winekey + smoke_v2286_titleekey + smoke_v2287_trueroute + smoke_v2288_scrollhint + smoke_v2289_winprog + smoke_v2290_statlink + smoke_v2291_lampguide + smoke_v2292_cavewatch + smoke_v2293_deadsave + smoke_v2294_crystalwatch（npm test 串跑）'));
ok('README 件套口径为一百九十件套（一百八十九件套清除）且旧 171 口径零残留',
  readme.includes('冒烟一百九十件套（一百八十九件套清除）') && !readme.includes('冒烟一百七十一件套（一百七十件套清' + '除）'));
...
ok('package.json test 串共 172 件套', testChain === 190, String(testChain));
...
ok('smoke_v2275 的 GAME_VERSION 字面量 pin 已更新为 v22.79', s2275.includes("const GAME_VERSION = 'v22.94';"));
ok('smoke_v2275 的 CHANGELOG 顶 pin 已更新为 ## v22.78', s2275.includes("startsWith('## v22.94'"));
ok('smoke_v2275 的件套 pin 已更新为一百九十件套（一百八十九件套清除）', s2275.includes('一百九十一件套（一百九十件套清除）'));
