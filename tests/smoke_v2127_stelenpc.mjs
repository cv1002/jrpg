...
ok('试炼进阶页仍 9 行（追加不增行，不触发 v19.59 页长自适应）', page3.length === 9, `实际 ${page3.length}`);
...
ok('试炼进阶页全部 9 行估算宽 ≤470（含追加指针的试炼碑位置行）', allOk3);

// —— 页长派生预算：9 行 + 2 个 r[2] → 末行基线 418 不触页脚 452 ——
ok('试炼进阶页 r[2] 仍 2 个（试炼三连战/终焉之神）', page3.filter((r) => r[2]).length === 2);
ok('末行基线 80+9*34+2*16=418 ≤440（页脚 452 之前）', 80 + 9 * 34 + 2 * 16 === 418 && 418 <= 440);
