...
ok('GAME_VERSION 字面量已为 v22.3', GAME_VERSION === 'v22.11', GAME_VERSION);
...
ok('data.js GAME_VERSION 字面量已更新为 v22.3', dataSrc.includes("const GAME_VERSION = 'v22.11';"));
...
ok('README 已同步（tests 树收录 smoke_v2203_fragdead + 九十九件套口径）',
  readme.includes('smoke_v2202_fragwin + smoke_v2204_brew2 + smoke_v2209_achstatus + smoke_v2210_unsaved + smoke_v2211_lampkid（npm test 串跑）') && readme.includes('冒烟一百零七件套（一百零六件套清除）'));
...
for (const nm of ['smoke_v2202_fragwin.mjs', 'smoke_v2201_fragstatus.mjs', 'smoke_v2200_stock.mjs',
  ...
  ok(`${nm} 的 README tests 树尾 pin 已随新现实更新（+ smoke_v2203_fragdead）`,
    src.includes('smoke_v2202_fragwin + smoke_v2204_brew2 + smoke_v2209_achstatus + smoke_v2210_unsaved + smoke_v2211_lampkid（npm test 串跑）'));
}
