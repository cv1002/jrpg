...
ok('GAME_VERSION 字面量已为 v22.49（旧 v22.48 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.95';") && !dSrc.includes("const GAME_VERSION = 'v22." + "48';"));
...
ok('README 件套口径为一百九十一件套（一百九十件套清除）',
  readme.includes('冒烟一百九十一件套（一百九十件套清除）') && !readme.includes('冒烟一百四十四件套（一百四十三件套清' + '除）'));
...
ok('package.json test 串共 145 件套', testChain === 191, String(testChain));
ok('CHANGELOG 顶部已追加 v22.49 条目', changelog.startsWith('## v22.95'));
...
ok('smoke_v2248 的 GAME_VERSION 字面量 pin 已更新为 v22.49（旧 v22.48 零残留）',
  s2248.includes("const GAME_VERSION = 'v22.95';") && !s2248.includes("const GAME_VERSION = 'v22." + "48';"));
ok('smoke_v2248 的 README 件套 pin 已随新现实更新为一百九十一件套（一百九十件套清除）',
  s2248.includes('一百九十二件套（一百九十一件套清除）'));
ok('smoke_v2248 的 package.json 件套计数 pin 已更新为 === 145', s2248.includes('testChain === 191'));
...
ok('v2143-45「件套守护领先一位」哨兵链已推进至 146（一百九十一件套（一百九十件套清除））',
