...
// README/package.json/CHANGELOG 同步（冒烟二百二十三件套（二百二十二件套清除）/串尾/入库 201 份/顶 pin）+
...
ok('data.js GAME_VERSION 字面量已为 v23.05（旧 v23.04 字面量零残留）',
  dataSrc.includes("const GAME_VERSION = 'v23.99';") && !dataSrc.includes("const GAME_VERSION = 'v23." + "04';"));
...
ok('README 件套口径为二百二十三件套（二百二十二件套清除）且旧 200 口径零残留',
  readme.includes('冒烟二百二十三件套（二百二十二件套清除）') && !readme.includes('冒烟二百件套（一百九十九件套清' + '除）'));
...
ok('package.json test 串共 201 件套', testChain === 223, String(testChain));
ok('CHANGELOG 顶部已追加 v23.05 条目', changelog.startsWith('## v23.99 '));
...
ok('smoke_v2304 的 GAME_VERSION 字面量 pin 已更新为 v23.05', s2304.includes("const GAME_VERSION = 'v23.99';"));
ok('smoke_v2304 的 CHANGELOG 顶 pin 已更新为 ## v23.05',
  s2304.includes("startsWith('## v23.99 "));
ok('smoke_v2304 的件套 pin 已更新为二百二十三件套（二百二十二件套清除）',
  s2304.includes('二百二十三件套（二百二十二件套清除）'));
...
ok('smoke_v2304 的 testChain pin 已更新为 201', s2304.includes('testChain === 223'));
