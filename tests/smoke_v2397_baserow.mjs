...
ok('data.js GAME_VERSION 字面量已为 v23.97（旧 v23.96 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v24.01';") && !dSrc.includes("const GAME_VERSION = 'v23.9" + "6';"));
...
ok('package.json test 串共 221 件套', testChain === 225, String(testChain));
...
ok('package.json 串尾为 ... smoke_v2396_steelarmor.mjs && node tests/smoke_v2397_baserow.mjs && node tests/smoke_v2398_econrow.mjs && node tests/smoke_v2399_dmgformula.mjs && node tests/smoke_v2400_trialwarn.mjs && node tests/smoke_v2401_winbgm.mjs"',
  pkg.includes('node tests/smoke_v2396_steelarmor.mjs && node tests/smoke_v2397_baserow.mjs && node tests/smoke_v2398_econrow.mjs && node tests/smoke_v2399_dmgformula.mjs && node tests/smoke_v2400_trialwarn.mjs && node tests/smoke_v2401_winbgm.mjs"'));
ok('README tests 树串尾已延伸至 smoke_v2397_baserow',
  readme.includes('+ smoke_v2396_steelarmor + smoke_v2397_baserow + smoke_v2398_econrow + smoke_v2399_dmgformula + smoke_v2400_trialwarn + smoke_v2401_winbgm（npm test 串跑）'));
...
ok('CHANGELOG 顶部已追加 v23.97 条目（基础属性速查行）', changelog.startsWith('## v24.01 '));
