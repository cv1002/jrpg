...
ok('GAME_VERSION 字面量已为 v22.58（旧 v22.57 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.77';") && !dSrc.includes("const GAME_VERSION = 'v22." + "57';"));
...
ok('地图指南行数不变仍 8 且 r[2] 数 6（v22.77 潮灯镇行拆 r[1]+r[2]，5→6）', HELP_PAGES[1].length === 8 && HELP_PAGES[1].filter((r) => r.length > 2).length === 6,
...
ok('README 件套口径已为一百七十三件套（一百七十二件套清除）且旧 153 口径零残留',
  readme.includes('一百七十三件套（一百七十二件套清除）') && !readme.includes('一百五十三件套（一百五十二' + '件套清除）'));
...
ok('package.json test 串共 154 件套', testChain === 173, String(testChain));
ok('CHANGELOG 顶为 v22.60 条目', changelog.startsWith('## v22.77 '));
...
ok('smoke_v2257 的 GAME_VERSION 字面量 pin 已更新为 v22.58', s2257.includes("const GAME_VERSION = 'v22.77';"));
...
ok('smoke_v2257 的 README 件套 pin 已随新现实更新为一百七十三件套（一百七十二件套清除）',
  s2257.includes("readme.includes('冒烟一百七十三件套（一百七十二件套清除）')"));
ok('smoke_v2257 的 package.json 件套计数 pin 已更新为 === 154', s2257.includes('testChain === 173'));
ok('smoke_v2257 的 CHANGELOG 顶 pin 已更新为 ## v22.58', s2257.includes("startsWith('## v22.77')"));
ok('smoke_v2256 的 GAME_VERSION 字面量 pin 已更新为 v22.58', s2256.includes("const GAME_VERSION = 'v22.77';"));
...
ok('smoke_v2255 的 GAME_VERSION 字面量 pin 已更新为 v22.58', s2255.includes("const GAME_VERSION = 'v22.77';"));
...
ok('smoke_v2254 的 GAME_VERSION 字面量 pin 已更新为 v22.58', s2254.includes("const GAME_VERSION = 'v22.77';"));
ok('smoke_v2253 的 GAME_VERSION 字面量 pin 已更新为 v22.58', s2253.includes("const GAME_VERSION = 'v22.77';"));
ok('smoke_v2252 的 GAME_VERSION 字面量 pin 已更新为 v22.58', s2252.includes("const GAME_VERSION = 'v22.77';"));
