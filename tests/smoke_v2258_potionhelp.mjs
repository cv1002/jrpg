...
//（tests 树尾/件套口径 154/v22.58 守护描述/入库 154 份/testChain===154/## v22.58 条目）、姊妹件套 pin
...
ok('GAME_VERSION 字面量已为 v22.58（旧 v22.57 字面量零残留）',
  dSrc.includes("const GAME_VERSION = 'v22.58';") && !dSrc.includes("const GAME_VERSION = 'v22." + "57';"));
...
ok('README tests 树尾收录 smoke_v2258_potionhelp（npm test 串跑）',
  readme.includes('smoke_v2257_pondglow + smoke_v2258_potionhelp（npm test 串跑）'));
ok('README 含 smoke_v2258_potionhelp 入库（154 份）', readme.includes('smoke_v2258_potionhelp 入库（154 份）'));
ok('README 件套口径已为一百五十四件套（一百五十三件套清除）且旧 153 口径零残留',
  readme.includes('一百五十四件套（一百五十三件套清除）') && !readme.includes('一百五十三件套（一百五十二' + '件套清除）'));
...
ok('package.json 串尾为 smoke_v2257_pondglow.mjs && node tests/smoke_v2258_potionhelp.mjs"',
  pkg.includes('smoke_v2257_pondglow.mjs && node tests/smoke_v2258_potionhelp.mjs"'));
const testChain = (pkg.match(/node tests\/smoke/g) || []).length;
ok('package.json test 串共 154 件套', testChain === 154, String(testChain));
ok('CHANGELOG 顶为 v22.58 条目', changelog.startsWith('## v22.58 '));
...
ok('smoke_v2257 的 GAME_VERSION 字面量 pin 已更新为 v22.58', s2257.includes("const GAME_VERSION = 'v22.58';"));
ok('smoke_v2257 的 README 串尾 pin 已随新现实延伸至 smoke_v2258_potionhelp',
  s2257.includes('smoke_v2256_campfire + smoke_v2257_pondglow + smoke_v2258_potionhelp（npm test 串跑）'));
ok('smoke_v2257 的 README 件套 pin 已随新现实更新为一百五十四件套（一百五十三件套清除）',
  s2257.includes("readme.includes('冒烟一百五十四件套（一百五十三件套清除）')"));
ok('smoke_v2257 的 package.json 件套计数 pin 已更新为 === 154', s2257.includes('testChain === 154'));
ok('smoke_v2257 的 CHANGELOG 顶 pin 已更新为 ## v22.58', s2257.includes("startsWith('## v22.58')"));
ok('smoke_v2256 的 GAME_VERSION 字面量 pin 已更新为 v22.58', s2256.includes("const GAME_VERSION = 'v22.58';"));
...
ok('smoke_v2255 的 GAME_VERSION 字面量 pin 已更新为 v22.58', s2255.includes("const GAME_VERSION = 'v22.58';"));
...
ok('smoke_v2254 的 GAME_VERSION 字面量 pin 已更新为 v22.58', s2254.includes("const GAME_VERSION = 'v22.58';"));
ok('smoke_v2253 的 GAME_VERSION 字面量 pin 已更新为 v22.58', s2253.includes("const GAME_VERSION = 'v22.58';"));
ok('smoke_v2252 的 GAME_VERSION 字面量 pin 已更新为 v22.58', s2252.includes("const GAME_VERSION = 'v22.58';"));
ok('smoke_v2230 的 package.json 串尾 pin 已延伸至 smoke_v2258_potionhelp（plain/单转义/双转义三风格）',
  /smoke_v2256_campfire[.\\/]{0,6}mjs && node tests[\\\\/]{0,6}smoke_v2257_pondglow[.\\/]{0,6}mjs && node tests[\\\\/]{0,6}smoke_v2258_potionhelp[.\\/]{0,6}mjs"/.test(s2230));
...
ok('v2143-45「件套守护领先一位」哨兵链已推进至 155（一百五十五件套（一百五十四件套清除））',
